...
//   "보수·청구·매장과 직접" 문구 전부 교체. 영입 2% 행 삭제(09-16 폐지). ✅ 코드는 같은 날 PR #1499 로 main 에(승계 코드 · 협업 코드 · 딜 % 조정 · 성과 화면).
//   귀사 몫 직접 송금(broker-share.ts)만 게이트 broker_share_enabled OFF — §14 가 그 사실을 밝힌다. 개별 소개비 = 협업 코드 기본 % + 딜별 조정(등록 때 정한 상한 안).
...
(async () => {
  ...
  // ───────── 01 표지 ─────────
  {
    ...
    T(s, '나머지 95%는 매장 몫입니다. 귀사 몫 %와 인플루언서 상한 %는 매장을 등록할 때 귀사가 정하고, 팔린 만큼 유어딜이 각자에게 직접 보냅니다. 귀사 몫에 유어딜이 정한 요율은 없습니다.', {
  ...
  // ───────── 02 한 장 요약 ─────────
  {
    ...
    const cells = [
      ...
      ['FiPercent', '누가 얼마를 내나', '매장이 팔린 금액의 5%를 유어딜에 냅니다(직접 입점은 10%). 나머지 95% 안에서 귀사 몫과 인플루언서 소개비가 나갑니다. 귀사 몫과 인플루언서 상한은 등록 때, 개별 소개비는 협업 코드로 정합니다. 송금은 유어딜이 각자에게 직접 합니다. 대행사가 유어딜에 내는 돈은 0입니다.'],
  ...
  // ───────── 04 돈의 흐름 ─────────
  {
    ...
    drop(2, '5%', '중개 경유 매장\n(직접 입점은 10%)');
    {
      ...
      T(s, '협업 코드에 싣는 %(등록 때 정한 상한 안). 귀속과 지급은 유어딜이 자동 처리합니다.', { x: x + 0.25, y: cy + 1.22, w: cw - 0.5, h: 0.4, fontSize: 9.5, color: C.inkSoft, lineSpacingMultiple: 1.35, valign: 'top' });
    }
    // 흔한 오해 / 실제
  ...
  // ───────── 04-2 직접 입점 vs 대행사 경유 (강조 열 표) ─────────
  {
    ...
    table(s, {
      ...
      body: [
        ...
        ['인플루언서 소개비', '협업 코드의 %', '협업 코드의 %. 상한은 등록 때, 딜마다 조정'],
  ...
  // ───────── 05 매장 1곳 단위 경제 + 규모별 계산 ─────────
  {
    ...
    const yEnd = kv(s, [
      ...
      ['   인플루언서 소개비 (협업 코드 10% 가정)', '97,233원', 1], ['   귀사 몫 (3% 예시)', '29,170원', 2], ['   매장 순수취', '797,311원', 1],
    ], x, 2.7, w, { rowH: 0.41 });
  ...
  // ───────── 06 사장님 셈법 ─────────
  {
    ...
    const yEnd = kv(s, [
      ...
      ['유어딜 5%', '1,250원', 0], ['인플루언서 소개비 10% (협업 코드 예시)', '2,500원', 0], ['귀사 몫 3% (등록 때 정한 예시)', '750원', 0, true],
  ...
  // ───────── 08 매장 등록 10분 (+ 매장 관리 화면) ─────────
  {
    ...
    const steps = [
      ...
      ['FiLayers', '"누가 운영하나요"에서 중개 선택, 두 % 입력', '"중개·대행사에요"를 고르면 그 매장은 5%가 적용됩니다. 이때 사장님께 드릴 코드가 자동으로 생기고, 귀사 몫 %와 인플루언서 상한 %를 함께 정합니다. 정산 계좌는 사장님이 주인이 된 뒤에만.', '1분'],
    ...
    s.addNotes('시간은 현장 추정. 필드는 seller-stores.routes.ts + seller-broker-terms.routes.ts 실재(채널·국세청·등록증·중개사 몫 0~50·인플루언서 상한 0~50·owner_claim 코드 — PR #1499). 오른쪽은 /seller/stores 실제 UI 를 예시 데이터로 렌더한 캡처(코드 열은 캡처 시점 전이라 안 보인다).');
  }
  ...
  // ───────── 10 인플루언서 붙이기 (+ 소개 파트너 찾기 화면) ─────────
  {
    ...
    const steps = [
      ...
      ['코드 링크를 보낸다', '협업 코드가 담긴 링크 하나를 보냅니다. 인플루언서가 탭하면 가입이 되고 그 순간 매장과 매칭됩니다. 승인이 필요한 코드로 만들면 귀사가 한 번 더 확인합니다.'],
      ['소개비 %는 코드에 싣고, 딜마다 조정', '협업 코드를 만들 때 기본 %를 정합니다(등록 때 정한 상한 안). 인플루언서마다 다른 코드를 쓰거나, 딜에서 바로 조정하면 이후 판매분부터 적용됩니다.'],
  ...
  // ───────── 14 한 가지는 미리 말씀드립니다 (운영자별 매출 귀속) ─────────
  {
    ...
    T(s, '5%의 경계, 권한, 코드는 라이브. 귀사 몫 직접 송금은 켜기 전입니다', { x: lx + 0.35, y: 4.72, w: lw - 0.7, h: 0.3, fontSize: 12.5, bold: true, color: C.ink, charSpacing: -0.3 });
    T(s, '5%와 95%의 경계, 정산계좌 차단, 매장 코드와 협업 코드, 성과 화면은 지금 작동하는 기능입니다. 귀사 몫을 유어딜이 직접 보내는 송금은 코드가 준비된 채 실결제 검증 뒤에 켭니다. 켜기 전까지는 종전대로 귀사 몫을 매장과 직접 정산합니다.',
    ...
    s.addNotes('09-19 v8: 09-04 인용을 09-16·09-19 결정으로 교체. 귀사 몫은 매장 단위 %(seller_meta.broker_share_pct). PR #1499: 코드·협업 코드·성과 화면은 라이브, broker-share.ts 는 게이트 broker_share_enabled OFF(STAGING S-BROKER 5건 뒤 대표가 켠다) — 문장이 그 사실을 밝힌다. 오른쪽은 /seller/operating 실제 UI + 예시 데이터(인플루언서별 행은 캡처 시점 전).');
  }
  ...
  // ───────── 15 8주 파일럿 제안 ─────────
  {
    ...
    const cols = [
      ['대행사가 준비할 것', false, ['담당자 1명과 카카오 계정 (셀러 계정은 사업자 인증으로 10분)', '후보 매장 리스트 10~20곳 (7장 체크리스트 기준)', '매장별 귀사 몫 %와 인플루언서 상한 % 초안 (등록할 때 입력합니다)', '첫 방문 일정 2주치']],
  ...
  // ───────── 16-2 세 당사자의 절차 (3열) ─────────
  {
    ...
    procedureColumns(s, [
      ...
      { title: '인플루언서', steps: ['귀사가 보낸 코드 링크를 탭 → 가입, 즉시 매칭', '(직접 왔다면 마이페이지에서 코드 입력)', '마이페이지에 매장 단위 링크 (복사·카톡 공유)', '콘텐츠와 유어샵에 링크', '이용권 사용 후 확정, ' + FACTS.influencerPayoutDay + ' 유어딜이 지급', '원천징수는 유어딜이 계산'], note: '소개비 %는 협업 코드의 값, 딜마다 조정' },
      { title: '귀사 (대행사)', hi: true, steps: ['셀러 계정 (사업자 인증 10분)', '매장을 "중개"로 등록 → 5%, 코드 자동 생성', '귀사 몫 %와 인플루언서 상한 % 입력, 사장님께 코드', '이용권 2~3개 열기 (사진·정가·판매가·기간)', 'DB에서 인플루언서 골라 코드 링크 발송', '대시보드에서 매장별·인플루언서별 성과와 내 몫'], note: '유어딜에 내는 돈 0. 귀사 몫은 유어딜이 송금' },
    ], { y: 2.55, h: 4.2 });
    s.addNotes('09-15 참고 덱 "광고 집행 절차" 장치. 09-19 v8: 세 열을 대표 확정 플로우 11단계로 재배열 — PR #1499 구현(승계 코드 /store/find?code= · 협업 코드 /i/join/:code · 매장 링크 · /seller/operating). 직접 송금은 게이트 OFF.');
  }
  ...
  // ───────── 17 FAQ + 시작하는 방법 + 연락처 ─────────
  {
    ...
    const steps = [
      ...
      ['매장을 중개로 등록하고, 두 %를 정하십시오', '"누가 운영하나요"에서 중개를 고르면 5%. 그 자리에서 사장님께 드릴 코드가 생기고, 귀사 몫 %와 인플루언서 상한 %를 정합니다. 개별 소개비는 협업 코드로.'],
    ...
    s.addNotes('FAQ 출처: 사업계획서 C-3, 셀러 가이드, auto-settlement.ts, 09-16·09-19 결재. 채널 변경은 POST /api/seller/stores/:id/channel (소유자만) + 어드민. 코드로 주인 되기: /store/find?code= (PR #1499) — 등록증·어드민 승인은 그대로.');
  }

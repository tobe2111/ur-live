// 유어딜 인플루언서 제휴 소개서 (.pptx) 생성기 — v3 (2026-09-19). 16:9 · Pretendard · 라이브 캡처 · 예시 데이터 화면
...
//   ⚠️ 07 경로 A(13페이지)는 대표가 "실제 유어딜 플로우를 다시 봐야 한다 · 마지막에 다시 작업" — 자격(인플루언서/대행사/중개사) 선택 가입은
//      아직 라이브에 없어(셀러 가입은 사업자 정보 폼뿐) 문구만 중개사 가입으로 바꾸고 URL 을 뺐다. 재작업 예정.
...
(async () => {
  const d = await createDeck({
    ...
    shotKeys: ['home', 'detail', 'voucher-wallet', 'voucher-qr', 'ushop', 'shorts', 'influencer-offer', 'influencer-settlement'],
  ...
  // ───────── 01 표지 ─────────
  {
    ...
    chip(s, px - 0.55, py + ph * 0.77, '영상 아래에서 바로 결제, 딜이 있으면 소개비');
  ...
  // ───────── 03-2 이런 채널에 맞습니다 (페르소나 + 말풍선) ─────────
  {
    ...
    personas(s, [
      ...
      ['FiMapPin', '아는 사장님이 있는 사람', '"단골 가게 사장님을 소개해 드릴 수 있어요."', ['중개사 계정으로 매장을 등록하면 유어딜 수수료는 ' + FACTS.feeBrokered + ' 입니다', '소개비 %는 매장과 정하고, 내 링크로 팔린 건마다 쌓입니다']],
  ...
  // ───────── 04 돈의 흐름 ─────────
  {
    ...
    drop(2, '95%', '사장님 몫.\n소개비는 이 안에서 냅니다.\n미리 나가는 광고비가 없습니다');
    drop(3, '소개비 %', '매장이 딜마다 제안한 비율.\n내 링크로 팔린 건마다,\n유어딜이 대신 정산', { hi: true });
    const cy2 = 5.35, cw2 = (W - 2 * M - 0.3) / 2, ch2 = 1.15;
    [['소개비는 매장이 냅니다', '플랫폼이 정한 고정 요율이 아닙니다. 매장이 딜마다 제안한 비율이 그대로 적립되고, 유어딜이 중간에서 깎지 않습니다.'],
    ...
    s.addNotes('직접 10% / 중개 5%: fee-resolver.ts. 카드 수수료(약 2.75%)는 이 덱에서 말하지 않는다(09-19 대표). 딜 소개비: 매장 부담(store-operator-model §7). 영입 2% 제거(09-19).');
  }
  ...
  // ───────── 05 수익 두 갈래 ─────────
  {
    ...
    const cols = [
      ...
      ['FiBriefcase', '내가 섭외한 매장의 이용권', '그 매장 판매가 × 매장과 정한 %',
        '중개사 계정으로 내가 아는 가게를 등록하면, 그 매장은 유어딜 수수료 ' + FACTS.feeBrokered + '로 시작합니다. 소개비 %는 사장님과 정해 딜로 걸고, 내 링크와 유어샵으로 팔린 건마다 그 %가 쌓입니다. 매장 섭외를 이어갈수록 딜과 이용권이 늘어납니다.',
        [['유어딜 수수료', FACTS.feeBrokered + ' (직접 등록은 ' + FACTS.feeDirect + ')', 0], ['소개비', '사장님과 정한 %', 1], ['등록', '셀러 대시보드 중개사 계정', 2]]],
    ];
    ...
    T(s, '나의 팔로워들이 매장 사장님들에게 수익을 가져다줄 수 있도록 힘써주세요. 팔린 만큼 사장님도 나도 버는 구조라, 실적이 보이는 사람에게 매장은 다시 제안합니다.',
      { x: M + 0.3, y: 5.92, w: W - 2 * M - 0.6, h: 0.65, fontSize: 10.5, color: C.ink, lineSpacingMultiple: 1.4, valign: 'top' });
    s.addNotes('딜 소개비: influencer-deals(매장 제안 %, 상한 없음). 중개 매장 등록: /store/new "중개·대행사에요"(store_channel=brokered → 5%). 영입 2% 열은 09-19 폐지로 삭제하고 "내가 섭외한 매장" 열로 교체.');
  }
  ...
  // ───────── 06-2 쌓이는 플라이휠 ─────────
  {
    ...
    const why = [
      ...
      ['FiTrendingUp', '매장이 늘수록 커집니다', '중개사 계정으로 섭외한 매장이 늘수록 딜과 이용권이 늘고, 유어샵 진열대가 넓어집니다. 매장 하나가 바퀴 하나를 더 만듭니다.'],
    ];
    let wy = 2.75;
    why.forEach(([i, h, p]) => {
  ...
  {
    const s = pres.addSlide();
    section(s, { n: 3, name: '유어딜 정산 프로세스', sub: '제안을 받는 길과 매장을 섭외하는 길, 링크를 두는 자리, 정산 시점, 자주 묻는 질문.', items: ['경로 A · 제안을 받는다', '경로 B · 매장을 섭외한다', '유어샵과 유어쇼츠, SNS 별 홍보', '정산', '시작하기'] });
  }
  ...
  // ───────── 07 경로 A ─────────
  {
    ...
    lead(s, '매장 섭외가 필요 없습니다. 가입해 두면 조건을 담은 제안이 오고, 수락하면 그 자리에서 팔 수 있습니다. 수락하기 전까지 아무 의무도 생기지 않습니다.', { y: 2.0, h: 0.65, w: 8.2 });
    const steps = [
      ['셀러 대시보드에 가입하고, 프로필을 공개한다', '유어딜 셀러 대시보드에 인플루언서(중개사) 자격으로 가입합니다. 카카오 로그인 뒤 내 정산 화면에서 소개자 프로필을 "공개"로 켜면, 매장이 그 프로필을 보고 제안을 보냅니다. 팔로워 수 하한은 없습니다.'],
    ...
    s.addNotes('⚠️ 대표 09-19: "실제 유어딜 플로우를 다시 봐야 한다 · 이 부분은 마지막에 다시 작업" — 자격(인플루언서/대행사/중개사) 선택 가입은 라이브에 없다(/seller/register/supplier 는 사업자 정보 폼). 지금은 URL 만 빼고 문구를 중개사 가입으로 바꿔 둔 상태. 제안 수락: /i/offer/:token → tracking_url. 정산 화면: /influencer/settlement.');
  }
  ...
  // ───────── 08 경로 B ─────────
  {
    ...
    lead(s, '사장님을 설득하는 일이 아니라, 열어 두면 손해 볼 게 없다는 것을 알려 드리는 일에 가깝습니다. 소개비 %를 사장님과 정하고, 내 계정에서 매장을 등록하면 됩니다.', { y: 2.0, h: 0.6 });
    const flow = ['매장을 고른다', '사장님과 소개비 %를 정한다', '중개사 계정으로 매장을 등록한다', '이용권을 올리고 내 링크로 판다'];
    ...
    T(s, '등록은 셀러 대시보드의 매장 등록에서 "중개·대행사에요" 를 고르면 됩니다. 사업자 확인은 사장님 정보로 하고, 이용권 등록과 딜 관리는 내 계정에서 합니다.', { x: M, y: by + 1.15, w: 8.2, h: 0.5, fontSize: 10.5, color: C.inkSoft, lineSpacingMultiple: 1.4, valign: 'top' });
    // 사장님께 하는 말
    ...
    const quotes = [
      ...
      ['소개비는 사장님과 제가 정해요.', '미리 나가는 광고비가 없습니다.'],
    ...
    const conds = [
      ...
      ['소개비는 그 안에서', '95% 안에서 사장님과 정한 %가 내 몫. 유어딜이 대신 정산합니다.'],
      ['운영은 내 계정에서', '이용권 등록과 딜 관리는 중개사 계정에서. 사장님은 매장에서 QR 확인만 합니다.'],
    ...
    s.addNotes('중개 등록: /store/new 마법사 "중개·대행사에요"(store_channel=brokered → 5%, fee-resolver.ts). 운영: seller_operators. 영입 2%·1년·직접 입점 조건은 09-19 폐지로 제거.');
  }
  ...
  // ───────── 09 유어샵과 유어쇼츠 (두 화면 + 인출선) ─────────
  {
    ...
    callouts(s, [
      ...
      { label: '딜이 있으면 여기서도 소개비', sub: '영상 자체에 대한 몫은 없고, 그 이용권에 매장과 맺은 딜이 있을 때 붙습니다', tx: 0.86, ty: 0.93 },
  ...
  // ───────── 09-2 각 SNS 별 이용권 홍보 방법 ─────────
  {
    ...
    const chans = [
      ...
      ['FiVideo', C.brandSoft, '유어쇼츠', '이용권 페이지 안의 세로 영상', ['유어딜 홈과 이용권 상세에 영상이 실립니다', '영상 아래 구매 버튼으로 바로 결제합니다', '외부 채널 없이도 딜이 있으면 소개비가 붙습니다']],
    ];
  ...
  // ───────── 11 이젠 체험단이 아닌 유어딜인 이유 + FAQ ─────────
  {
    ...
    lead(s, '체험단은 한 번 받고 끝나지만, 유어딜은 내 유어샵과 링크가 남고 매장과 맺은 딜이 계속 쌓입니다. 자주 묻는 질문에 먼저 답해 두었습니다.', { y: 2.0, h: 0.6 });
    const faq = [
      ...
      ['제안은 어떻게 오나요?', '매장은 내가 공개해 둔 소개자 프로필을 보고 직접 보냅니다. 아는 가게가 있으면 중개사 계정으로 직접 등록해도 됩니다. 둘 다 해 두는 것이 빠릅니다.'],
    ];
  ...
  // ───────── 11-2 두 경로와 정산, 한 장에 (절차 3열) ─────────
  {
    ...
    procedureColumns(s, [
      { title: '경로 A · 제안을 받는다', hi: true, steps: ['셀러 대시보드에 인플루언서(중개사) 자격으로 가입', '카카오 로그인 뒤 소개자 프로필 공개', '매장 제안 수신 (소개비 %, 촬영 지원)', '수락하면 전용 링크 발급', '콘텐츠와 유어샵에 링크', '정산 화면에서 주문·확정 확인'], note: '수락 전까지 아무 의무도 없습니다' },
      { title: '경로 B · 매장을 섭외한다', steps: ['아는 사장님 가게를 고른다', '사장님과 소개비 %를 정한다', '중개사 계정으로 매장을 등록한다 (유어딜 ' + FACTS.feeBrokered + ')', '이용권을 올리고 딜을 건다', '내 링크와 유어샵으로 판다', '정산 화면에서 확인'], note: '유어딜 수수료 ' + FACTS.feeBrokered + '. 사장님 몫 95% 안에서 소개비' },
      { title: '정산 · 유어딜이 한다', steps: ['링크로 들어온 주문에 자동 귀속', '이용권 사용 후 확정 (환불 건은 그 건만 정리)', FACTS.influencerPayoutDay + ' 지급 가능 금액 집계', '현금은 ' + FACTS.influencerPayoutMin + '부터 등록 계좌로', '딜로 받으면 하한 없이 잔액으로', '못 미치면 다음 달로 이월'], note: '매장에 돈을 달라고 할 일이 없습니다' },
    ], { y: 2.55, h: 4.2 });
    s.addNotes('09-15 참고 덱 "광고 집행 절차" 장치. 09-19: 경로 B 를 영입 2% → 중개사 등록(5%) 으로, URL 문자열 제거. 사실: /i/offer/:token 수락 · /store/new 중개 등록 · influencer-payout.ts(매월 1일 · 10만원 · 딜 하한 없음 · 이월).');
  }
  ...
  // ───────── 12 시작하기 + 연락처 ─────────
  {
    ...
    const x = M, w = 7.6;
    const steps = [['가입', '셀러 대시보드에 인플루언서(중개사) 자격으로 가입하고, 내 정산 화면에서 소개자 프로필을 공개합니다.'], ['제안 수신 · 매장 섭외', '조건이 맞는 매장의 제안을 받거나, 아는 가게를 중개사 계정으로 직접 등록합니다.'], ['공유와 정산', '링크를 걸고, 성과와 정산은 내 정산 화면에서 확인합니다.']];
    steps.forEach(([h, p], i) => {
      const y = 2.75 + i * 0.72;

...
//   소개비 %는 "매장이 딜마다 제안" → "매장 등록 때 정한 값". 07 경로 A 재작업 완료(대표 "마지막에" 항목). 🚧 코드 미착수 — 문서·덱만.
...
(async () => {
  ...
  // ───────── 03-2 이런 채널에 맞습니다 (페르소나 + 말풍선) ─────────
  {
    ...
    personas(s, [
      ...
      ['FiMapPin', '아는 사장님이 있는 사람', '"단골 가게 사장님을 소개해 드릴 수 있어요."', ['중개사 계정으로 매장을 등록하면 유어딜 수수료는 ' + FACTS.feeBrokered + ' 입니다', '중개사 몫 %와 소개비 %는 등록 때 정하고, 사장님은 매장 코드로 가입해 주인이 됩니다']],
  ...
  // ───────── 04 돈의 흐름 ─────────
  {
    ...
    drop(2, '95%', '사장님 몫.\n소개비는 이 안에서 냅니다.\n미리 나가는 광고비가 없습니다');
    drop(3, '소개비 %', '매장 등록 때 정한 비율.\n내 매장 링크로 팔린 건마다,\n유어딜이 대신 정산', { hi: true });
    const cy2 = 5.35, cw2 = (W - 2 * M - 0.3) / 2, ch2 = 1.15;
    [['소개비는 매장이 냅니다', '플랫폼이 정한 고정 요율이 아닙니다. 매장을 등록할 때 정한 비율이 그대로 적립되고, 유어딜이 중간에서 깎지 않습니다.'],
    ...
    s.addNotes('직접 10% / 중개 5%: fee-resolver.ts. 카드 수수료(약 2.75%)는 이 덱에서 말하지 않는다(09-19 대표). 소개비 %: 매장 등록 때 정함(09-19 확정 플로우, 코드 미착수). 영입 2% 제거(09-19).');
  }
  ...
  // ───────── 05 수익 두 갈래 ─────────
  {
    ...
    const cols = [
      ['FiPercent', '매칭된 매장의 이용권 소개비', '결제 금액 × 매장 등록 때 정한 %',
        '대행사(중개사)가 보낸 링크를 탭하면 가입과 동시에 매장과 매칭됩니다. 마이페이지에 생기는 매장 링크로 팔린 건마다 등록 때 정한 소개비 %가 쌓입니다. 상한이 없고 유어딜이 떼지 않습니다. 이용권이 바뀌어도 링크는 그대로입니다.',
        [['재원', '사장님 몫 안에서', 0], ['비율', '매장 등록 때 정한 %', 1], ['매칭', '링크 한 탭, 또는 코드 입력', 2]]],
      ['FiBriefcase', '내가 섭외한 매장의 이용권', '그 매장 판매가 × 등록 때 정한 중개사 몫 %',
        '중개사 계정으로 아는 가게를 등록하면 매장 코드가 생기고, 그때 중개사 몫 %와 소개비 %를 정합니다. 사장님은 그 코드로 가입해 주인이 되고 나는 운영자로 남습니다. 팔린 건마다 중개사 몫이 쌓여 유어딜이 직접 보냅니다.',
    ...
    s.addNotes('09-19 확정 플로우(docs/decisions/2026-09-19-broker-matching-flow.md): 코드 링크 한 탭 매칭 · 매장 단위 링크 · 두 요율은 매장 등록 때 · 중개사 몫은 유어딜 직접 송금(09-16). 🚧 코드 미착수 — 라이브는 아직 influencer-deals(제안·수락) + /store/new 중개 등록. 영입 2% 열은 09-19 폐지.');
  }
  ...
  // ───────── 07 경로 A (v4: 코드 링크 한 탭) ─────────
  {
    ...
    const steps = [
      ...
      ['적립을 확인한다', '마이페이지에서 매장별 주문과 대기, 확정 금액을 봅니다. 소개비 %는 매장 등록 때 정해진 값이라 물어볼 일이 없습니다.'],
    ];
    let y = 2.95;
    steps.forEach(([h, p], i) => {
    ...
    const three = [
      ...
      ['매칭', '링크 안의 매장 코드로 그 매장과 바로 연결됩니다. 신청도 승인 대기도 없습니다.'],
    ...
    s.addNotes('09-19 대표 확정 플로우 7·8·9 단계(docs/decisions/2026-09-19-broker-matching-flow.md): 코드 링크 탭 → 가입 즉시 매칭 · 마이페이지 코드 입력은 직접 온 사람용 · 매장 단위 고유 링크(복사·카톡 공유, 이용권 바뀌어도 그대로). 🚧 코드 미착수 — 그래서 이 장은 화면 대신 카드. 열리면 마이페이지 캡처로 교체.');
  }
  ...
  // ───────── 08 경로 B ─────────
  {
    ...
    T(s, '셀러 대시보드의 매장 등록에서 "중개·대행사에요" 를 고르면 매장 코드가 자동으로 생기고, 중개사 몫 %와 인플루언서 소개비 %를 함께 정합니다. 사장님이 가입하며 그 코드를 넣으면 매장 주인이 되고, 나는 운영자로 남습니다. 등록증 확인은 그대로입니다.', { x: M, y: by + 1.15, w: 8.2, h: 0.5, fontSize: 10.5, color: C.inkSoft, lineSpacingMultiple: 1.4, valign: 'top' });
    // 사장님께 하는 말
    ...
    const conds = [
      ...
      ['두 %는 등록할 때', '중개사 몫 %와 소개비 %를 등록하며 정합니다. 둘 다 유어딜이 직접 보냅니다.'],
    ...
    s.addNotes('09-19 대표 확정 플로우 2·4·5 단계: 매장 등록 때 코드 자동 생성 + 두 요율 · 사장님이 가입하며 코드 입력 → owner(등록증 확인 그대로, 첫 정산 전까지만, 판매 안 막음) · 대행사 = operator. 라이브: /store/new "중개·대행사에요"(brokered → 5%) + seller_operators. 🚧 코드·요율 입력은 미착수.');
  }
  ...
  // ───────── 11-2 두 경로와 정산, 한 장에 (절차 3열) ─────────
  {
    ...
    procedureColumns(s, [
      { title: '경로 A · 링크로 시작한다', hi: true, steps: ['대행사(중개사)가 보낸 코드 링크를 탭', '카카오 로그인으로 가입, 그 순간 매칭', '(직접 왔다면 마이페이지에서 코드 입력)', '마이페이지에 매장 단위 링크 생성', '콘텐츠와 유어샵에 링크 (복사·카톡 공유)', '마이페이지에서 주문·확정 확인'], note: '소개비 %는 매장 등록 때 정해진 값' },
      { title: '경로 B · 매장을 섭외한다', steps: ['아는 사장님 가게를 고른다', '중개사 계정으로 등록 (유어딜 ' + FACTS.feeBrokered + ') → 매장 코드 자동 생성', '중개사 몫 %와 소개비 %를 그때 정한다', '사장님이 가입하며 코드 입력 → 매장 주인', '이용권을 올리고 내 링크로 판다', '마이페이지·대시보드에서 확인'], note: '중개사 몫도 유어딜이 직접 보냅니다' },
    ...
    s.addNotes('09-15 참고 덱 "광고 집행 절차" 장치. 09-19 v4: 두 경로를 대표 확정 플로우(코드 링크 · 매장 코드 · 두 요율)로. 사실: influencer-payout.ts(매월 1일 · 10만원 · 딜 하한 없음 · 이월). 🚧 코드 링크·매장 코드는 미착수.');
  }

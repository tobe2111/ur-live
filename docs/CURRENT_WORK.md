...
```
- **판정 ①(시트)**: `sheets_gate` 가 `false` 면 **고장이 아니라 설정** — env 만 켜면 된다.
  `true` 인데 `sheets_cron` 이 `null` 이면 cron 이 그 잡에 도달조차 못 하는 것(다른 원인).
  이 두 필드가 이번에 신설됐다 — **그전엔 스탬프 한 줄로 두 경우가 구분되지 않아 41시간을 오진했다.**
- **판정 ②(발송)**: `st_contacted`/`reached` 가 0에서 움직였는가. 안 움직였으면 도구가 아니라
  **운영 시간** 문제 — 코드로 더 밀어붙일 것이 없다(아래 "틀렸던 판단" 참조).
...
### 🔴 최대 미개통 레버 — **RESEND_API_KEY 가 없어서 이메일을 한 통도 못 보낸다**
```
GET /api/version → "secrets": { ..., "RESEND_API_KEY": false }   ← 유일한 false
```
- 이메일 보유 리드 **2,284명**, 발송 경로(`send-cold`·`send-consented`)와 법적 장치
  (야간금지·수신거부·전송자정보·1일 상한·반송 회로차단)는 **전부 완비**돼 있다.
- 그런데 두 라우트 모두 진입부에서 `if (!c.env.RESEND_API_KEY) return 400` 으로 즉시 반환한다.
  `reached: 0` + `bounced: 0`(시도 자체가 0)이 그 증거다.
- 규모 대비: 수동 쪽지는 사람이 눌러서 **하루 20명**, 이메일은 **2,284명**이 대기 중 — 자릿수가 다르다.
  발송 큐(B-1)를 만든 것보다 **이 키 하나가 더 큰 레버**일 수 있다.
- ⚠️ `RESEND_FROM` 도 함께 필요하다(도메인 인증된 발신 주소). 키만 넣고 from 이 없으면 발송이 실패한다.

...
## 🔴 2026-07-28 — 📧 파트너 풀 **이메일 크롤 전멸 근본원인 확정 + 수리** (적중률 0% → 구조 복구)
**원인(추측 아님 — 실측 분포가 특정)**: 대표 실측 `크롤 59 · 적중 0% · 페이지 못가져옴 45 · 제외 호스트 14 · **이메일 미게시 0**`.
`no_contact`(사이트에 이메일이 없음)가 **0건**이라는 건 HTML 을 **한 장도 못 받았다**는 뜻이고, 정상 집계된 유일한 사유
`blocked_host` 는 **네트워크가 필요 없는** 사유다 → 사이트별 봇차단이 아니라 **네트워크가 필요한 모든 경로의 전역 실패**.
범인은 **Cloudflare 서브리퀘스트 한도**: `enrichHeldLeads` 예산이 `ADS_ENRICH_BUDGET=800`(1 인보케이션)인데
실제 한도는 그보다 훨씬 낮아, 한도 초과 지점부터 **이후 모든 fetch 가 throw** → `.catch(() => '')` 가 전부 삼켜
`fetch_fail` 로 뭉뚱그려졌다. 같은 사고를 이미 겪고 만든 **관측 학습 상한(`collect-budget.ts`, 2026-07-28)** 이
**인플루언서 레인에만 배선**돼 있었고 이 레인은 무방비였음(예산 800 vs 학습 하한 25).
**② 복리 피해(정체의 진짜 원인)**: 한도 뒤 무의미하게 실패한 행들이 `stamp()` 로 **7일 쿨다운 도장**까지 받아
매 라운드 수백 행이 재시도 풀에서 이탈 → 백로그가 흐르지 않고 이메일 수확이 0 에 고착.
**수리**: ⓐ `safeFetch` 공용 래퍼(contact-enrich) — 모든 외부 fetch 가 에러를 **삼키지 않고** 한도 신호를 `FetchBudget.limitHit`
에 남김(`FetchBudget` 에 optional 필드 추가, 기존 호출부 전부 호환) ⓑ 새 사유 `subreq_limit` 분리 — "사이트 문제"와
"우리 인프라 한도"를 통계에서 혼동 불가 ⓒ 보강 레인에 학습 상한 배선(`resolveSubreqBudget`/`nextSubreqCap`, 인플루언서
레인과 동일 SSOT·동일 키 `ads_subreq_cap` — 같은 워커라 학습 공유가 정확) ⓓ **한도 도달 시 도장 없이 즉시 중단**
(Phase 1·2·3 전부) → 인프라 실패가 데이터를 오염시키지 못함 ⓔ `CRAWL_RULES_VERSION` 4→5 로 **기존 오염분 전량 즉시
재시도 대상 복구**(마이그레이션 불필요 — 기존 버전 스탬프 체계 그대로) ⓕ 어드민 상태줄에 `예산 n/N 사용 · 한도 도달 여부 ·
다음 실행 상한` 노출(`/stats` 는 통째 전달이라 자동).
**허위 0 불변**: 연락처를 만들어내지 않음 — 실패를 실패로 정직하게 분류하고 재시도 가능하게 되돌릴 뿐. 머니/게이트 무접촉.
**검증**: sql bind/column/table · theme · file-size · pagination 가드 GREEN. ⚠️ 이 환경 npm 403 → tsc/build 는 CI.
**🔬 남은 판별 1건(정직한 한계)**: 실패가 전부 `network`(HTTP 상태가 아니라 **예외 발생**)인데, 예외 후보는 둘이다 —
`TypeError: Too many subrequests`(워커 한도 = 위 수리가 정답) vs `AbortError`(8초 타임아웃 = 처방이 다름: 대기시간·동시성).
전자가 압도적으로 유력하지만(84/84 전멸 + 과거 같은 워커의 한도 사고 이력 + 예산 800), **표본 4건으로 단정하지 않는다.**
→ 사유 버킷을 `subreq_limit` / `timeout` / `network`(DNS·TLS)로 **쪼개 분포 자체가 답하게** 했다(양 트랙 동일).
배포 후 상태줄의 사유 분포 한 줄이면 판별 끝. **어느 쪽이든 이번 수리는 안전** — 타임아웃이면 `limitHit` 이 안 서서
학습 상한이 발동하지 않고 `⏱ 응답 시간초과`가 분포로 드러난다.
**⏳ 배포 후 확인(대표)**: `/admin/partner-pool` 「보강」 1회 → 상태줄에 ⓐ `⛔ 플랫폼 요청한도` 사유가 뜨면 원인 확증
(그리고 다음 실행부터 학습 상한으로 자동 축소) ⓑ 한도 여유인데도 실패면 그때는 **진짜 사이트측 원인**(403/404/타임아웃)이
사유별로 분해되어 보임 → 다음 개선을 데이터가 고름.

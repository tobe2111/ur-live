...
## 🔴 2026-07-28 — 📧 이메일 크롤 적중 0% — **원인 규명 완료 + 수리(PR #784)** · 라이브 확증 1스텝 남음
...
**🔬 남은 1스텝(정직한 한계)**: 위 4근거로 사실상 확정이지만 **예외 문자열 원문**을 아직 못 봤다
(`TypeError: Too many subrequests` vs DNS/TLS 계열). 계측은 이미 배포됨(`70d44f6`) + PR #784 가 사유 버킷을
`subreq_limit` / `timeout` / `network`(DNS·TLS)로 **쪼개 분포 자체가 답하게** 했다. 배포 후 「보강」 1회면 판별 끝.
**어느 쪽이든 #784 수리는 안전** — 한도가 아니면 `limitHit` 이 안 서서 학습 상한이 발동하지 않는다.

...
## 🟢 2026-07-29 (9차) — **유어애즈: 수집 레인 2개가 0건이던 진짜 이유 = 코드가 아니라 호출 방식**
...
1. **시트 미러 원인 확정** — 어드민 인플루언서 풀 화면의 시트 배너를 볼 것. 이번 배포 후엔 배너가
   `⛔ ADS_SHEETS_SYNC_ENABLED 가 꺼져 있어요` 를 **직접 말해준다**. 그러면 대표가 켜면 끝.
   그 문구가 **안 뜨는데도** 멈춰 있으면 게이트는 켜져 있다는 뜻 → 예산 굶음 쪽이고, 이번 kick 전환
   (백필 192 fetch 제거)이 그걸 풀었을 가능성이 크므로 **스탬프에 `subreq` 필드가 생겼는지** 확인.
   (지금 스탬프엔 `subreq` 가 없다 = #814 수리 코드가 **한 번도 실행된 적 없다**는 증거다.)
...
- **CF API 토큰 여전히 죽어 있다** — `platform_settings.cf_api_token` 으로 `verify` 하면 `Invalid API Token`
  (7차가 보고한 상태 그대로). 워커 env 직접 조회·빌드로그 진단은 이 세션에서도 불가했다. 대표 갱신 대기.

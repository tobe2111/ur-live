...
- **업체형 블로그 판별**(부동산·한의원·중고주방 등, 보수적 정규식으로 표본 ~7%)은 여전히 후보. 단
  현 브랜드 규칙(`looksLikeBrandChannel`)이 잡을 건 이미 다 잡았고(표본 16건 전부 태깅 완료) 상호형은
  못 잡는다. ⚠️ **보강 레인 배포 후** 활동성 데이터가 채워진 뒤 판단할 것 — 지금은 진짜 블로거도 지표가
  비어 있어 무엇으로 갈라야 하는지 알 수 없다.

...
## 🔴 2026-07-28 — 📧 대행사 이메일 크롤 **적중 0% 원인 규명 중** (계측 배포 완료, 판정 대기) ← **다음 세션 첫 액션**

**미해결 문제**: 대행사 이메일이 16→17 에서 사실상 정지. 사이트는 620개나 발견했는데 이메일이 안 나온다.

**라이브 실측(07-28 14:01 상태줄)**:
```
...
**여기까지 확정된 사실**:
- `no_contact` 가 **0건** → "사이트에 이메일이 없음"이 아니라 **HTML 을 한 번도 못 받았다**. 원인은 추출이 아니라 fetch.
- 8초 타임아웃이 84회 순차면 672초 → 실행시간에 안 맞음. 즉 **타임아웃이 아니라 fetch 가 즉시 throw**.
- 즉시 실패의 가능한 원인은 둘뿐: ① 워커 **서브요청 한도(호출당 1,000) 소진** ② 사이트가 실제로 안 열림(DNS/TLS/연결거부).
- 84/84 전부 실패는 개별 사이트 문제보다 계통적 원인(①) 쪽이 자연스럽다 — **단 확정 아님. 추측으로 고치지 말 것.**

**배포된 계측(PR #783, `70d44f6`)** — 이걸로 ①/②가 갈린다:
- `contact-enrich.ts` fetch catch 에서 **예외 이름·메시지 보존**(`failErr`) → 상태줄 실패 샘플에 동반 표기.
- `company-collect.ts` **실사용 서브요청 수**(`fetches`) 결과에 기록 → 상태줄 표기. 1,000 근접이면 ① 확정.
- 같이 고친 실제 결함: **`robots.txt` 를 예산에 안 세고 있었음**(사이트당 실사용이 회계보다 +1 → 예산이 남았다고 믿는 채로 실제 한도를 먼저 침). `spendBudget` 추가 — ①이 맞다면 이 자체가 원인의 일부.

**다음 세션이 할 일 (순서대로)**:
1. 대표에게 `/admin/partner-pool` 상태줄의 `📧 이메일 보강 레인` 줄을 받는다(15:01 이후 사이클부터 예외 메시지가 찍힘).
2. 실패 샘플의 예외로 판정:
   | 찍힌 값 | 원인 | 수리 방향 |
   |---|---|---|
   | `TypeError: Too many subrequests` | 서브요청 한도 소진 | 수집/보강 레인을 **별도 워커 호출**로 분리 + 예산 재산정(한 invocation 에 몰지 말 것) |
   | `AbortError` | 상대 서버 무응답 | 타임아웃 상향 + 느린 호스트 후순위 |
   | `fetch failed`/DNS 계열 | 대상 URL 품질 | 네이버 웹문서 기반 홈페이지 **발견 경로** 정확도 수리 |
3. 원인 확정 **후에야** 수리. 이 문제는 이미 짐작 수리로 한 사이클(경로 6→12 확장 → 사이트당 서브요청 2배 → 크롤 가능 수 반토막) 날린 전력이 있다.

**⚠️ 넘기지 말 것**: 제조사 트랙(`supply/api/maker-collect.ts`)에는 이메일 크롤을 **일부러 안 붙였다** — 같은 결함 복제 방지. 원인 규명 후 이식.

**⚠️ 이 환경의 한계**: 에이전트 프록시가 허용목록 밖 외부 호스트를 차단(`x-deny-reason: host_not_allowed`)해 실패 사이트를 **직접 검증할 수 없다**. 어드민 stats 도 401. → 판정은 대표가 붙여주는 상태줄로만 가능.

**같은 날 배포된 것(완료)**: PR #781 🏭 **제조사·판매사 후보 풀** — `/admin/maker-pool`. 대표 확정 "제조사=브랜드사"(자사 상품 보유 공급 주체). 격리 테이블 `supply_maker_leads`, 코드 `features/supply/**`, 라우트 `/api/admin/maker-pool/*`(유어애즈 테이블은 SELECT 만 — 서비스 분리 준수). 레인 2개: 🏭 카카오 로컬 그리드(품목×제조·브랜드 어휘 19종 × 제조밀집 16지역, 매장성 업종 제외 → 회사명·전화·주소) / 📥 통신판매 원부 2.5만 → 판매사 후보 임포트(커서, `rprsvEmladr` 대표자 이메일 보유). 게이트 `SUPPLY_MAKER_COLLECT_ENABLED`(기본 OFF — 수동 버튼은 무관, 수집 품질 확인 후 켤 것).

**대표 액션 대기 중(코드로 못 뚫음)**: 고용24 = 현재 키가 **개인회원**이라 API 거부 → 기업회원 전환 후 `WORK24_API_KEY` 교체 / 프랜차이즈 = HTTP 404, data.go.kr 15125467 활용신청 승인 + 요청 URL → `ADS_FRANCHISE_ENDPOINT`·`ADS_FRANCHISE_OP`.

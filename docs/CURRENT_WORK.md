...
- **3차 증분 — MOQ(박스 단위)**: `products.min_order_qty`(lazy ensure 기본1) + 공급자 등록폼 + API 4종 반환 + 카드/상세/카트 박스·개당 + 서버 `qty<moq` 차단. 기본1이라 기존 무영향. verify:sql 9/9.
- **남은 갭**: 수량구간단가(등급 단일가 모델 → 확장 필요, 대형) · 자료(거래명세서/세금계산서) 유통사 다운로드 뷰 · OEM 토큰 미세정렬.

...
## ✅ 2026-07-01 — 어드민 대시보드 라이브 전수조사 (대표 "어드민 대시보드 라이브 접근 전수조사")
라이브(live.ur-team.com) 기준 어드민 대시보드 ~80 페이지 + 38개 어드민 라우트 파일(~11.4k줄) 전수조사. 라이브 프로빙 결과 모든 보호 엔드포인트가 `requireAdmin` 401 로 깔끔히 막힘(라우팅 건강). 감사게이트 GREEN 도메인(테이블/컬럼/bind/NOT NULL/인증/테마/서비스분리)은 재감사 생략, **가드 미보유 "런타임 500 크래시" 클래스**만 6개 병렬 에이전트로 수동 감사. 백엔드는 전반적으로 매우 방어적(핸들러 try-catch→safeError, D1 `.catch()` 폴백, `?? []` 가드) — 결함 2 클래스만 확정·수정:
- **백엔드 (uncaught `c.req.json()` → 빈/malformed body 시 global onError 500)**: 파일 전체 try-catch 없는 3개 라우트만 진짜 결함(형제 파일들은 이미 `.catch(() => ({}))`). `admin-agency.routes`(4핸들러: POST/PATCH/reset-password/sellers)·`admin-tools.routes`(6: banners·notices·settlements/process·reports/resolve·settings)·`admin-flags.routes`(2: emergency-mode·:name)에 `.catch(() => ({}))` 추가 → 500 대신 기존 검증이 400 반환. flags 2개는 상태변경이라 `typeof !== 'boolean'` 400 명시 검증 추가(빈 body 무음 default 방지).
- **프론트엔드 (5xx 를 ₩0/빈목록으로 위장 — 재무 오독 위험, wholesale `check-query-iserror` 가드와 동일 클래스인데 어드민 미적용)**: `AdminInfluencerPayoutsPage`(송금 대기 0명·₩0)·`AdminPayoutsPage`(정산 대기 잔액 없음)·`AdminVoucherTransactionsPage`(오늘 거래 ₩0)에 `isError` 분기 + 공용 `DashboardLoadError`(상태별 안내+재시도/재로그인) 배선.
- **불변**: 잠금 파일(payment/webhook/toss·SSR·로딩) 무수정. 서비스 분리 준수(모두 유어딜 어드민 — 에이전시/정산/배너/플래그/교환권; 도매 어드민 무관). 어드민 라이트 테마 준수(dark: 0).
- 검증: tsc 0 · build(client+ssr+prerender+worker+prepare) 0 · theme(strict)·query-iserror·sql-bind 가드 0 · audit-gate 37 GREEN(사전존재 file-size RED 는 타 세션 god-file, 내 변경과 무관). ⚠️ 자격증명 제공 시 라이브 인증 상태에서 재검증 예정(현재 인증 없이는 핸들러 런타임 재현 불가).

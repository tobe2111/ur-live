...
## 🟢 2026-07-28 (7차) — **인플루언서 풀: 보강이 한 건도 안 돌던 것 수리 + 발송 차단 원인 규명**

### ➡️ 다음 세션 첫 액션 (숫자 2개만 보면 판정된다)
...
curl -sS "https://live.ur-team.com/api/admin/ads/influencer-pool/stats" -H "Authorization: Bearer $TOK" -H "User-Agent: $UA" \
 | python3 -c "import sys,json;d=json.load(sys.stdin);print(d['stats']['nb_unmeasured'],'/',d['stats']['naver_blog']);print(d.get('enrich_lane'))"
```
- **판정 ①**: `nb_unmeasured`(활동성 미측정 블로거)가 **27,864 보다 줄었나**. 줄었으면 레인이 도는 것.
- **판정 ②**: `enrich_lane.naver` 의 `tried` 대비 `measured`. **`tried>0` 인데 `measured=0` 이면
  네이버가 서버 요청을 차단**하는 것 → 그때는 RSS/홈 크롤을 접고 검색 API `postdate`(이미 저장 중인
  `last_post_at`)만으로 활동성을 판단하도록 방향을 틀 것. `crash` 필드가 있으면 원문이 곧 사인이다.
- 처리량을 올리려면 `ADS_INFLUENCER_ENRICH_ROUNDS`(기본 6, 상한 20) — 라운드=독립 인보케이션이라
  이게 유일한 정직한 레버(파트너풀 `ADS_ENRICH_ROUNDS` 와 같은 구조).

### 🔎 이번 세션이 라이브에서 실측한 것 (추측 아님)
| 지표 | 값 | 뜻 |
|---|---|---|
| 풀 전체 / 연락처 보유 | 37,414 / **3,308 (8.8%)** | 재고는 큰데 **쓸 수 있는 리드가 1/11** |
| 네이버 블로거 | 27,864 (풀의 74%) · 이메일 ≈185 (0.6%) | 이메일 2,262 중 2,077이 유튜브 |
| 마지막 수집 스냅샷 | `naver_enrich.tried:0 · bio_enriched:0 · perf_enriched:0` | **보강 4종이 한 건도 못 돎** |
| 블로거 표본 1,000행 | `perf_checked_at` 채워진 행 **0** | 활동성이 한 번도 측정된 적 없음 |
| 아웃리치 | `st_contacted:0 · reached:0 · recruited:0` · `ch_note:1` | 퍼널 오른쪽이 통째로 비어 있음 |

### ✅ 이번에 고친 것 (PR — 커밋 `aec567b`)
1. **보강 전용 레인 분리** `influencer-enrich-lane.ts` + `POST /__ads/enrich-influencer` +
   cron 시간당 N라운드(기본 6, 킬스위치 `ADS_INFLUENCER_ENRICH_DISABLED`). 파트너풀 이메일 보강에서
   이미 검증된 "라운드 = 독립 인보케이션 = 새 서브리퀘스트 예산" 패턴 그대로.
   - **원인**: 보강이 수집과 같은 인보케이션이라, 무료 플랜 한도(≈50, D1 포함)를 발굴 루프가 먼저 다 씀.
   - 학습 상한 레인 분리(`ads_subreq_cap_influencer_enrich`) · 벽시계 가드 · **crash 원문 스냅샷**.
2. **예약분 누수 수리** — `enrichReserve` 가 *키워드 경계에서만* 검사돼 한 키워드(최대 16)가 예약분을
   뚫고 0 까지 파고들 수 있었다(= 예약이 사실상 무효). 발굴에게 **별도 FetchBudget** 을 줘 구조적으로 차단.
3. 수집에 남긴 보강은 **YT 성과 2종뿐** — 그쪽만 발굴 검색과 YouTube 쿼터(search=100units)를 공유하므로
   라운드로 증속하면 발굴이 굶는다. 쿼터를 안 쓰는 블로거/링크인바이오만 새 레인으로 옮긴 이유.
4. `ensurePerfExtraColumns` → `runDdlOnce`(인보케이션당 ALTER 5 → 체크섬 1 조회). 유닛테스트로 체크섬 고정.
5. 어드민 상태줄: 보강 라운드 결과 · **남은 블로거 수** · 📨 **RESEND_API_KEY 미설정 경고**.

### ⏳ 대표 조치 대기 — **발송이 0인 진짜 원인**
`POST /api/admin/ads/influencer-pool/send-cold` 실측 응답: `RESEND_API_KEY 설정 후 사용할 수 있습니다`.
**메일 키가 없어서 한 통도 못 나간다**(콜드/동의 발송 둘 다). 발송 가능 모수는 이미 있다 —
유튜브 이메일 2,077(스위트스팟+노이즈숨김 후 1,249). Cloudflare → **ur-live** → Variables 에
`RESEND_API_KEY`(+`RESEND_FROM`) 추가하면 열린다. 코드는 준비 완료(야간금지·(광고)표기·수신거부·
1일 상한·반송 회로차단 전부 구현돼 있음).

### ⚠️ 이번에 조심할 점 / 남은 것
- **6차가 찾은 `nextSubreqCap` 회복 조건 결함은 새 레인에도 그대로 적용된다** — 다만 새 레인은
  학습값 0 에서 시작(=env 45 사용)하고 한도 관측 시에만 낮추므로 **당장은 무해**. 6차의 근본수리가
  들어가면 자동으로 이득을 본다(별도 대응 불필요).
- 네이버 블로거의 **이메일 수확률은 원래 낮다**(discovery 단계 크롤도 0.6%). 이 레인의 1차 가치는
  연락처보다 **활동성·최신 글 제목**(죽은 블로그 배제 + 카테고리 재분류 신호 + `lead_score` 정상화)이다.
  "연락처가 확 늘 것"으로 기대하면 오판.
- 이 환경에서 **naver.com 은 프록시 차단**(CONNECT 403) → 블로거 크롤 수확률은 로컬에서 검증 불가.
  라이브 스냅샷(`enrich_lane.naver`)이 유일한 증거다.
- 풀 품질: 네이버 블로거에 **업체 블로그가 섞여 있다**(표본에서 부동산·한의원·중고주방 등 ~7%).
  현재는 `lead_score` 가 낮게 줘서 정렬로 밀리므로 급하지 않음. 정리한다면 "업체형 상호 판별 →
  파트너풀(광고주 리드)로 라우팅"이 자연스러운 다음 수(둘 다 유어애즈 안이라 서비스 분리 위반 아님).

# 파트너 약관-as-계약 (벤더사·셀러 온보딩) — 설계 스펙

> 출처: 2026-07-04 대표 지시(구두 스펙). "계정 발급 = 계약 체결. 종이계약 없이 시스템(약관 동의)이
> 계약을 대신한다. 셀러 입점도 같은 원리." **기능 요건** — 벤더사(그리고 셀러) 가입 시 약관 동의가 필요.
> 채팅은 휘발되므로 즉시 박제(디자인 룰).

## 0. 위치 — 벤더사/셀러 온보딩의 기능 요건 (타이밍 중립)

이 문서는 **약관 동의를 가입 플로우에 넣는 기능 스펙**이다. 특정 계약/파트너 건과 무관하게, 벤더사·셀러
온보딩을 구현할 때 적용한다.

- 현재 벤더사는 `AGENCY_HIDDEN=true` 로 **셸브** 상태 → **지금 당장 재개/구현할 필요 없음**. 이 스펙은
  벤더사 온보딩을 (린하게) 구현/재개하는 시점에 그 가입 플로우에 반영한다.
- **셀러 온보딩은 지금도 라이브** → 약관 동의(clickwrap)는 **셀러 입점에 먼저 적용해도 됨**(벤더사와 공용 컴포넌트).
...
### ④ 가입 시 사업자번호 검증
벤더사는 세금계산서 발행·원천징수 면제 판단이 걸려 **사업자 필수**. 셀러 입점처럼 사업자번호 검증을
가입 필수 단계로. (✅ 셀러는 이미 `business_number`·`business_registration_status/image/verified_*` 보유 → 벤더사에 동형 이식)
...
| 사업자번호 검증 | ✅ 셀러 보유(`sellers.business_number` 등) | 벤더사에 **동형 이식** + 셀러 온보딩에도 약관동의 연결 |
| 벤더사 가입 페이지 | 얇은 "카카오로 권한 신청"(`AgencyRegisterBusinessPage`) | 사업자번호 + 약관 4체크 단계 **추가** |
...
- **`PartnerTermsConsent`** — 중요조항 4개를 각각 요약 박스 + 개별 체크박스로 렌더, 전문 링크, 4개 전부
  체크해야 '동의하고 계속' 활성. **벤더사·셀러 공용**(props: `termsType`, `keyClauses`).
...
## 5. 서비스 분리 주의
이건 **소비자 유어딜 축**(벤더사=매장영입 파트너 / 셀러=사업자 유저). 도매몰(판매사·제조사)과 무관 —
도매 온보딩 약관은 별도. 명칭 SSOT 준수(벤더사/사업자 유저).

## 6. 미결 결정 (대표 확인 필요 — 구현 착수 전)
1. **적용 대상·순서**: 셀러 온보딩(라이브)에 먼저 적용 vs 벤더사 재개 시점에 맞춰? (공용 컴포넌트라 어느 쪽이든 무방)
2. **약관 실제 문언**: 4개 중요조항 + 전문의 **법적 텍스트는 누가 제공**(대표/법무)? — 시스템은 구현 가능하나
   구속력 있는 약관 문안은 개발자가 창작 불가. 문안 확정 = version 1. (착수의 실질 블로커)
3. **커미션율 표기**: 스펙의 **1%** vs 현행 코드 default **2%** (SSOT §13 "벤더사 2→1% cutover gated"). 약관 ① 문구에 들어갈 값 확정.
4. **벤더사 재개 여부**: 벤더사 온보딩 구현은 `AGENCY_HIDDEN` 해제를 전제. 지금은 셸브 유지(급하지 않음).

## 구현 로그
- 2026-07-05 **법적 문안 5종 공식본 반영** — `src/shared/legal/*`(이용/판매자/벤더사/개인정보/위치기반), 렌더 `/terms`·`/terms/seller`·`/terms/agency`·`/privacy`·`/terms/location`. 개인정보 §5 위탁 실vendor 반영(교환권=KT알파(주), 알림톡·SMS=알리고·솔라피).
...
- **벤더사측**: PARTNER_TERMS.agency 시드·약관 준비됨. AGENCY_HIDDEN 재개 시 동일 공용 컴포넌트로 배선(§0).

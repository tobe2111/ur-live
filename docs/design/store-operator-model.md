...
## 9. ✅ 확정 플로우 — 대행사(중개사) → 매장 → 인플루언서 (2026-09-19 대표 확정)

결재 파일 `docs/decisions/2026-09-19-broker-matching-flow.md` 가 SSOT 다(대표 말 그대로 11단계 + 불변식 9개 + 구현 순서).
여기엔 **이 모델(§4·§7·§8)에서 무엇이 달라지는가**만 적는다.

| 자리 | §8 까지 | 09-19 확정 |
|---|---|---|
| 주인이 되는 입구 | ① 어드민 지정 · ② `/store/find` 신청(사업자번호로 찾기) | **③ 대행사가 준 매장 코드를 사장님이 가입하며 입력** — 셋 다 `transferStoreOwnership` 하나만 부른다 |
| 코드의 방향 | (원안) 대행사가 사장님 매장 코드를 입력 | **거꾸로.** 권한을 내주는 쪽(사장님)이 입력해야 동의가 되고, 코드가 새도 남의 매장을 못 가져간다 |
| 등록증 확인 | 사람이 본다 | **그대로.** 코드는 손잡이지 증명이 아니다 |
| 사장님 가입 시점 | (미정) | **첫 정산 전까지만.** 이용권 판매를 막지 않는다(막는 건 정산 — 주인 없는 매장은 계좌가 없다) |
| 요율 | 유어딜 5% 만(§7.2 — 중개사 몫은 장부 밖) | 매장 등록 때 **중개사 몫 % · 인플루언서 커미션 %** 를 함께 정한다. 중개사 몫은 **유어딜이 직접 송금**(09-16 결재) — §7.2 의 "장부 밖" 은 **폐기** |
| 인플루언서 매칭 | 매장 제안 → 수락(`/i/offer/:token`) | **코드가 담긴 링크 한 탭** = 가입 + 매칭. 직접 온 사람은 마이페이지 코드 입력 |
| 인플루언서 링크 | 딜(이용권)마다 발급 | **매장 단위 고유 링크** 하나. 이용권이 바뀌어도 그대로. 복사·카톡 공유 버튼 |
| 성과 | 운영 매장 요약(매장 총액, 운영자 귀속 미추적) | 대행사: 매장별·인플루언서별 성과 + **자기 몫** · 인플루언서: 마이페이지 적립 |

⚠️ §7.2 *"유어딜 장부·정산 화면에 중개사 지급은 한 줄도 등장하지 않는다"* 는 **2026-09-16 결재로 뒤집혔다**(안 1 — 각자에게 직접 송금).
그 문단은 작성 당시 기록으로 두고 소급 수정하지 않는다. 지금 살아 있는 값은 `actor-benefit-map.md` §1.

🚧 **코드는 미착수**(2026-09-19 기준). 라이브에 있는 것은 §4·§8 까지다. 구현 순서와 등급은 결재 파일 표를 따른다.

...
- **2026-06-21 (8차 — 추가 감사: 버그 + 톤통일 + 정리)** 대표 "추가로 봐야 할 건?" — 표면 너머 정합/엣지 감사:
  - 🐛 **[FIX] 히어로 '아낀 돈' 항상 0** — `/api/vouchers/my` 가 `product_price` 미반환인데 `(원가-액면)` 으로 계산해 영구 미표시였음. `applied_discount_pct`(반환됨)로 `액면 * pct/(100-pct)` 계산. 타입에 `applied_discount_pct?` 추가.
  - **매장 스캔 화면(`VoucherVerifyPage`, `/v/:code`) 톤 통일** (대표 선택) — 핑크 액센트(`pink-400/500/100`) + 회색그라데 버튼 → 지갑 잉크 패스 톤: 잉크 로고 블록 · 블랙 필 버튼(조회/사장님사용/확인) · 잉크 코드입력 focus · 잉크 코드 표시 · '다른 코드' 잉크 링크. **사장님 '사용 처리' 패널의 emerald(성공 기능색)·결과 green/red 는 보존**(기능 상태색). lookupVoucher/useVoucher/useVoucherAsSeller/PIN/CAS 로직 무수정.
  - 🐛 **[정리] 지도 `mapVouchers` 탭 스코프** — gb+gift 전체 → 현재 탭 스코프(교환권 탭에서 공구권 핀/지도버튼 새던 것 차단).
  - 🐛 **[정리] clipboard 토스트** — 미지원/거부 컨텍스트에서도 '복사됨' 뜨던 것 → 실제 성공(`writeText().then`) 시에만.

...
| **🥕 당근 비즈프로필 온보딩 — 참고 시안**(대표 "당근 참고해봐. **메인서비스랑 셀러대시보드 간극이 크다** — 바로 연결이 당근처럼 보여야") | 2026-08-26 | 🔴 **미구현 · 대표 확정 대기** — 시안 10장 아카이브 + 원칙 6가지 추출. 핵심 판정: **갭은 등록 폼이 아니라 "발견"** — 소비자 지도에 카카오맵 매장이 이미 떠 있는데 그 화면에 "사장님이신가요?"가 없다. 제안 A~D(A=클레임 진입점·B=한 화면 한 질문·C=등록증 OCR로 개업일 제거·D=미션 체크리스트). ⚠️ C는 `AI` 바인딩 부재로 현재 무동작 | [danggeun-bizprofile-onboarding.md](./danggeun-bizprofile-onboarding.md) |

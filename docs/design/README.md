...
| 페이지 | 시안 받은 날 | 상태 | 파일 |
|---|---|---|---|
| **🎬 홈 헤더에 유어쇼츠 진입점 — 시안 5안**(대표 *"이 부분에 유어쇼츠 페이지로 가는 버튼이 있으면 좋을까? 시안들 받아보고 싶네?"*) | 2026-09-09 | ⏳ **대표 선택 대기**(코드 0) — 진입점이 레일 안 「전체 보기」 하나뿐이라 스크롤한 사람만 본다. 헤더 세 줄이 각각 계약을 지고 있어 그 안에서 5안. 추천 **안 4**(위치 줄 글자 링크 — 세로 0 · 말로 알림 · 색 안 씀). 안 2(토글 3분할)는 비추천(그 컨트롤은 "같은 목록의 다른 보기"). ⚠️ 자리보다 먼저: 영상 3편에 이용권이 안 붙어 있어 도착해도 살 것이 없다 | [home-header-shorts-entry-2026-09.md](./home-header-shorts-entry-2026-09.md) |

...
| 페이지 | 시안 받은 날 | 상태 | 파일 |
|---|---|---|---|
| **🎨 디자인 외주 브리프 — 파일럿 표면**(AI 디자이너 전달용. 우선순위 6화면 + **CI 가 차단하는 제약** + 손대면 안 되는 것) | 2026-08-02 | 🟢 전달 준비 완료(시안 대기) — 기능은 완성이고 **디자인만 미적용**(신규 5화면 전부 브랜드 컬러 **0**, 원시 gray 유틸). Tier1 = 몰 홈 `/{슬러그}`(소비자 유일 착지점) · 3분 등록(P0 판정 ①의 무대). ⚠️ 대시보드 `dark:` **금지** / 소비자 다크 **필수** — 어기면 CI 차단 | [design-outsourcing-brief.md](./design-outsourcing-brief.md) |

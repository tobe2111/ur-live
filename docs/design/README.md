...
| 페이지 | 시안 받은 날 | 상태 | 파일 |
|---|---|---|---|
| **📱 셀러 대시보드 모바일 우선 재설계 — 시안 M1~M4 (+PC A~D)**(대표 *"모바일 버전이 더욱 중요해. 기존 UI 전체를 뒤엎어도 되니까 시안 더 줘봐. 카드·섹션 자체가 마음에 들지 않아. 이용권 등록·관리·매출이 가장 중요"*) | 2026-09-14 | ⏳ **대표 선택 대기**(코드 0) — 실측: 메인 블록 14개 세로 나열, 모바일은 메뉴 14개가 햄버거 뒤. 공통 뼈대 = 하단 탭 5개(홈·주문·이용권·정산·더보기) + 홈 1화면 + 파란 등록 버튼 하나. 추천 **홈 M2 + 이용권 탭 M4** | [seller-dashboard-mobile-first-2026-09.md](./seller-dashboard-mobile-first-2026-09.md) |

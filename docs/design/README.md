...
| **🤝 유어애즈 B2B 파트너(업체) 수집 트랙 — 3레인(자동/레지스트리/수동)** | 2026-07-21 | 🟡 설계 승인 ✅ / 착수는 우선순위 뒤(지역 키워드 시딩·8문항 → 그다음) · 순서=테이블·어드민→레인A→B→C · 결정 2건(크롤 방식·전화vs이메일) 표본검증 후 확정 | [partner-company-collection.md](./partner-company-collection.md) |

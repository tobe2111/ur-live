...
> 🔴 **2026-08-25 정정 — 아래 "5%" 는 이제 "채널별 요율"로 읽어야 한다.**
> 대표 최종(**2026-08-20**): **직접 입점 10% / 중개 5%**(`fee-resolver.ts` `platformPctDirect: 10`).
> 이 문서의 §확정 원칙은 그 이전(7월 8일)에 쓰여 "유어딜 5%" 로 못박혀 있다 —
> **불가침 원칙 자체(커미션이 플랫폼 take 를 잠식하지 않는다)는 그대로 유효하고, 그 take 의 *값*만
> 채널별로 갈린다.** 2026-08-25 세션이 이 문서를 믿고 하루 종일 5% 를 전제로 작업하다가
> 대표 질문으로 알아챘다. 게이트 전수·순서: [money-gates-inventory.md](./money-gates-inventory.md).

## ⭐ 확정 원칙 (2026-07-08 대표 확정 — "커미션 구조 확정, 8월 promo flip 때 이대로 켠다")

> 🛑 **2026-09-07 폐기 — 결재 `docs/decisions/2026-09-07-actor-benefit-conflicts.md` Q4-2(대표 *"기본안대로 모두 승인"*).** 아래 §확정 원칙("유어딜 5% 는 어떤 커미션에도 안 쓴다")과 그에 딸린 8월 flip 체크리스트·불변식 #44 는 **더 이상 추진하지 않는다.** 현행 원칙은 §0~§6 의 원래 프레임이다 — **커미션은 플랫폼 수수료 안에서 부담하되 주문당 총합 ≤ `수수료 − PG 준비금` 을 [INV-CB] 예산 아비터가 강제**(`commission_budget_enabled`, 기본 OFF · 켜기는 staging S1 뒤 대표). 이유: 07-08 계산은 플랫폼 수수료가 5% 하나이던 시절 것이고, 채널 요율(직접 10% / 중개 5%)과 08-31 결정(영입 2% 는 직접 입점만)으로 적자 경로가 사라졌다. 08-25 에 구현된 owner 경로는 게이트 OFF 인 채 남긴다(되살리려면 새 결재). 아래 본문은 **역사 기록**으로 보존한다.
...
> 🛑 2026-09-07 결재 Q4-2 로 **신설하지 않는다** — 유효한 불변식은 [INV-CB](`check-commission-budget.mjs`) 의 "주문당 커미션 총합 ≤ 수수료 − PG 준비금" 이다.

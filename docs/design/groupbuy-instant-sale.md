...
### 소비자 UI 카피 정합 (2026-06-17 — "가장 이상적인 형태" 점검)
가격 코어는 단일가지만 **소비자 UI 카피가 여전히 '인원 모으면 싸진다/목표 달성 시 특가'**(§8 표시광고 메모)로
남아 있던 것을 정직한 단일가 + 소셜 증거로 전환:
- `GroupBuyListPage` 배너: "함께 모일수록 더 싸져요 / 목표 달성 시 특가" → "함께라서 더 좋은 가격 / 지금 바로 공동구매가로" (6개 언어 locale 동시).
- `GroupBuyListPage` 큐레이션 strip: '🔥 1명만 더 모이면 성공 / ✨ 거의 성공 70%'(임계 게이트) + 진행률 바 + 'N명 남음' → '🔥 인기(함께 구매 중) / ⏰ 오늘 마감 / 🎉 목표 달성' + 소셜 증거 'N명 함께'. 선택 로직도 `group_buy_current` 정렬·`current>=target` 으로 전환(데이터 없으면 graceful hide).
- `GroupBuyDetailPage`: 공유 문구 'N/M명 참여 중'(분모=목표=게이트 연상) → 'N명 함께 구매 중'. 사다리 제거 후 미사용 `progress` 변수 정리.
- 커뮤니티 탭(같이 모으기)의 보증금-임계 모델 진행률/'목표 달성'은 **실제 임계라 그대로 유지**(셀러 즉시판매와 구분).

#### 잔여(점검에서 발견, 미수정 — 별도 판단 필요)
- `introduce`(라이브 온보딩) `step2Desc`/`faq3Q·A`: "공구 인원이 모일수록 할인폭이 커져요" + "목표 미달 시 자동 취소·전액 환불" → **둘 다 사실과 다름**(단일가 + 자동환불 cron 제거됨). 단 이 페이지는 '라이브 방송' 서사와 얽혀 있어(라이브 영구중단) 전면 재작성 필요.
- `referral`(친구초대) `heroDesc`/`step3Desc`: "인원 모일수록 할인율 커져요/정해진 인원 모이면 추가 할인 자동 적용" — 동적 tier 서술.
- `TimeDealPopup`(라이브 타임딜, 현재 LIVE_COMMERCE_SUSPENDED): "N명 더 모이면 -X% 추가 할인".

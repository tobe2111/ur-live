...
3. **[완료]** ~~멀티테넌트 고객사 전환~~ ✅ · ~~모바일 정밀(하단 탭바)~~ ✅ · 대시보드 라이트/다크 토글 ✅ · ~~견고성 패스~~ ✅(패널 에러/재시도 배너 `PanelError` 전 패널 + `/ads`·`/ads/dashboard` ErrorBoundary + 랜딩 CTA→loginHref 통일 + 부정클릭 픽셀 일일 남용 캡). 남은 디자인: 랜딩 더미요소(후기/로고) 실데이터 교체(의도된 자리표시자).

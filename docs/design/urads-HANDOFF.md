...
### B. 코드로 가능 — 미구현(선택)
- **비밀번호 재설정(찾기)** — 독립 계정에 복구 플로우 없음(현재 로그인 후 변경만). 이메일 토큰 재설정 필요. 🔴 실질 갭.
- **순위 하락 알림** — 키워드 순위 시계열 저장 필요(현재 알림 2종=예산·가격만). `alerts.ts` 에 dimension 추가.
- **약관·개인정보 페이지(유어애즈 전용)** — 현재 푸터가 유어딜 것 링크.
- **데이터 CSV 내보내기** — 실적·키워드·연관키워드.
- **랜딩 더미요소** — 수치(ROAS 412%)·후기·로고 실데이터 교체(의도된 자리표시자).

### C. 완료 (재작업 금지)
~~독립 계정+계정관리(가입/로그인/프로필/비번/로그아웃)~~ ✅ · ~~라이트 통일(인증페이지+대시보드 기본 화이트)~~ ✅ · ~~KPI 요약 홈~~ ✅ · ~~소싱 인구통계 세분화~~ ✅ · ~~임계값 알림(예산·가격)~~ ✅ · ~~멀티테넌트 고객사 전환~~ ✅ · ~~견고성(PanelError·ErrorBoundary)~~ ✅ · ~~헤더 계정 드롭다운/직접 로그아웃~~ ✅ · ~~하단 탭바 제거~~ ✅.

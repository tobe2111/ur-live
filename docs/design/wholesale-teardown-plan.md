...
## 🛑 2026-08-03 대표 확정 — **삭제하지 않는다(존치)**

> 대표: *"C는 남의 돈은 걸려있지 않아. **아예 사업을 안했거든.** 대신 **존치하자**."*

두 가지가 동시에 정해졌다:

1. **§4 머니 게이트는 무의미하다** — 도매몰은 실사업을 시작한 적이 없어 예치금·미확인 충전요청·
   미지급 정산금·plus 구독이 **애초에 발생하지 않았다.** 게이트가 막고 있던 위험이 존재하지 않는다.
2. **그렇다고 지우지 않는다.** §2(라우트 마운트 해제 + 화면 삭제)·§5-3 는 **보류**다.
   삭제로 얻는 것은 코드 줄 수뿐이고, 잃는 것은 P2(공동 소싱)에서 되살릴 때의 재작성 비용이다
   (§8 🟡 판단 보류가 이미 같은 결론을 냈다 — *"제거가 아니라 비노출"*).

**⇒ 지금 유효한 상태**: 소비자 도메인에서 **안 보이고**(어드민 도매 밴드 숨김 · 소비자 표면 유입
링크 제거) 도매 도메인에서는 **그대로 산다.** 코드·테이블·라우트 전부 잔류.

⚠️ **다음 세션에게**: 이 문서의 §2·§5 를 보고 삭제 작업을 시작하지 말 것. 위 확정이 우선이다.
되돌리려면(=삭제로 방향 전환) **대표의 새 지시**가 있어야 한다.
...
| 6 | ~~번들 cron no-op 게이트 제거~~ → **제거하지 않는다** | — |

> 🔴 **대표 확정(2026-07-29)**: **정산 엔진이 소비자 worker 에 잔류하는 한 cron no-op 게이트도 잔류**한다.
> 도매 번들이 사라져도 게이트는 남긴다 — 비용 0 이고, 되살아났을 때의 이중성숙을 막는 마지막 방어선이다.
>
> 🔴 **대표 확정: 삭제는 기능 구현과 섞지 말고 별도 PR 로 고립**한다.
> 삭제 PR 에 기능이 섞이면 revert 가 기능까지 되돌리고, 리뷰에서 "뭐가 지워졌는지"가 안 보인다.
...
> ✅ **1·2번은 2026-07-29 에 선제 구현됨** — `src/tests/unit/supply-teardown-safety.test.ts`.
> 머니 게이트와 무관하고 코드 삭제도 아니라 **미리 박아뒀다**(삭제 시점엔 이미 늦다).
> 되돌려-검증 3종: 정산 엔진 삭제 → 빨강 / buyer-pool 삭제 → 빨강 / 소비자 번들 마운트 제거 → 빨강.
> ⚠️ 못 막는 것(주석 명시): 파일은 남기고 **export 만 지우는 경우**(tsc 가 잡는다) · 라이브 데이터·배포 상태.

...
**제안 날짜**: 2026-05-25
**현 상태**: ✅ **Phase 1–4 구현 완료** (링크샵·핀·어필리에이트 정산·공구 호스팅·셀러 승급) — Phase 5(셀러 흡수, 대부분 호환)+폴리시만 잔여. 아래 §4 "정책 결정" 표는 구현으로 대부분 해소됨(0.5% 양방향 유지·voucher 공구 A채택 등).
> ⚠️ **라이브커머스는 영구 중단**(`LIVE_COMMERCE_SUSPENDED=true`, 2026-06-17 확정). 본 문서의 "라이브를 큐레이터 탭으로 강등"·"라이브권" 등 라이브 관련 서술은 **모두 무효** — 라이브는 큐레이터 탭이 아니라 *제거*됨. 신규 작업 시 라이브 언급 무시.

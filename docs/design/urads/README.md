...
- 카피는 실제 구현 기능에 맞게 손질(미구현 기능 과장 제거):
  - 발주 수집 = "준비 중" 배지(`order-collection.ts` 보류 — 커머스API·고정IP·권한 필요).
  - 자동입찰 불릿에서 "시간대·요일 전략 / CSV 대량등록"(미구현) 제거 → 실제 기능(예상입찰가·원클릭 등록·안전상한)으로 교체.
  - 부정클릭 "자동 차단" → "차단 목록 자동 생성"(차단은 반자동 — 네이버 공식 API 에 노출제한IP 없음).
  - AI "주간 리포트 자동 발송" → "읽기 진단부터"(자동 cron 없음).
  - 통합실적 "채널별" → "캠페인별"(멀티채널 미구현).

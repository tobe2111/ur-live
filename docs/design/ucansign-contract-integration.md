# 🗑️ [폐기] 유캔싸인(UCanSign) 전자계약 자동발송 연동

> 🔴 **2026-08-10 폐기 — 대표 지시 "유캔사인은 안 써" + "더이상 도매몰은 안 해".**
> 코드(worker 5개 파일 446줄 + 호출부)를 **전부 제거**했다. 라이브 `contract_signatures` 는
> **0건**이었다 — 템플릿이 등록된 적이 없어 계약서가 한 번도 나가지 않았으므로 **동작 변화 0**.
> 판매사 발주를 막던 미서명 게이트도 (행이 없어) 한 번도 발동한 적이 없어 함께 제거했다.
>
> **전자계약이 다시 필요하면 자체 시스템으로 만든다** — 유어딜에는 이미 **약관 승낙형 전자계약**이
> 가동 중이다(`terms_consents` / `terms_agreements`: 버전 스탬프·IP·핵심조항 개별동의·재동의 게이트).
> 셀러·에이전시·유저 가입이 그걸로 체결된다. 외부 의존 0, 비용 0.
>
> ⚠️ 아래 원문은 **역사 기록**이다. 이걸 보고 유캔사인을 다시 붙이지 말 것.

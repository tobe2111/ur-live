...
| **에이전시** | `agencies` (B2B 조직) | 여러 매장 관리·영입·성장 | 관리 매장 GMV rollup, 영입 커미션 | 영입 가게 GMV **1%**(플랫폼분에서·**한시 마중물**, 영구 아님), **24개월** 한도, 실판매 시만 |
...
| **매장영입(에이전시)** | 영입 에이전시 | 1% (플랫폼 5%에서·**한시 마중물**) | 실판매 시만, 24개월 한도 | 대칭 역전 |
...
>
> ⭐ **확정 원칙 (2026-07-08 대표 확정 — 8월 promo flip 방향)**: **유어딜 5% 는 판매 커미션에 일절 안 쓴다.** 위 stack 중 **판매 커미션(어필리에이트·멀티티어·인플루언서 매장영입 1.5%)은 매장 promo(5% 밖, `promo_funding_source=owner`) 재원**으로 이전 — 인플/벤더에게 얼마를 주든 **유어딜 5% 는 불변**(깨지면 버그). **에이전시 매장영입 1%(플랫폼 5%에서)는 promo 생태계 없던 시기의 한시 마중물** — 영구 지분 아니며 자립 후 축소·폐지. **A(직접)/B(벤더 pass-through) 모두 promo 재원**이라 5% 무영향. 오늘 owner 스위치는 어필리에이트만 커버 → 8월 flip 이 멀티티어·인플영입·이용권 인플 20% share 를 owner 로 확장 + 불변식 #44("platform net == 5%") 신설. 상세·flip 체크리스트: `commission-funding-restructure.md` §확정 원칙. **지금은 문서 박제만 — 코드 무변경.**

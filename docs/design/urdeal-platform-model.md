...
| **매장 영입(소개)** | 매장을 영입해 온 사람 | **2%** (2026-08-27 대표 확정, 1.5%→2%) · **promo 재원(5% 밖, flip)** | 매 결제, **기간 제한 없음(무기한)**, T+7 성숙, 원천징수 | `reverseInfluencerStoreIntroOnRefund` |
...
2. **영입 루프**: 누군가 매장을 유어딜에 **영입** → 그 매장 매 결제마다 영입자에게 **2%**
   (2026-08-27 대표 확정, 1.5%→2%). **기간 제한 없음** — 에이전시 영입(1%)은 매장별
   `referral_bonus_until` 로 만료를 걸 수 있지만 이 축엔 그 검사가 없다. 공급 확장의 주 엔진.

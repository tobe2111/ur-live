...
| **에이전시** 🌇 | `agencies` (B2B 조직) — **신규 가입 종료(2026-08-19)** | 매장-인플 **조율**(= 쇼핑 벤더의 오프라인판) | 관리 매장 GMV rollup, 조율 수수료 | 영입 가게 GMV **1%** (오늘 5%재원 → **8월 flip 시 매장 promo(조율 마진), 유어딜 5% 무관**), **24개월** 한도, 실판매 시만 |
...
| **매장 영입(소개)** | 매장을 영입해 온 사람 | **2%** (2026-08-27 대표 확정, 1.5%→2%) | 🔴 **직접 입점(`store_channel='direct'`) 매장만 — 미지정도 미지급**(2026-08-31 대표: 중개 5% 매장은 `5−PG 2.75−2 = +0.25%` 로 적자 구간). 매 결제, **유효기간 1년**(기산 `introduced_at`, 어드민 `referral_bonus_until` 우선), T+7 성숙, 원천징수 | `reverseInfluencerStoreIntroOnRefund` |
...
> **플랫폼 부담 비례(%) 커미션**(어필리에이트·멀티티어 추천트리·매장 영입(소개)·에이전시 영입)의 총합은

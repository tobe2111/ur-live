...
| **에이전시** 🌇 | `agencies` (B2B 조직) — **신규 가입 종료(2026-08-19)** | 매장-인플 **조율**(= 쇼핑 벤더의 오프라인판) | 관리 매장 GMV rollup, 조율 수수료 | **채널 요율 차액**(대행 경유 매장은 유어딜이 5%만 떼므로 10%−5% 가 대행사 몫). 🛑 **영입 1%·24개월은 2026-08-31 폐지** — 같은 행위(매장 영입)에 신분별 이중 보상이라 경계가 모호했고, 차액으로 이미 보상된다 |
...
> 🌇 **에이전시 대시보드 일몰 (2026-08-19 대표 확정)**: 에이전시는 **행위자로는 남지만 전용 대시보드는
> 축소**됐다(라우트 39→16 · 신규 가입 종료 · `AGENCY_DASHBOARD_SUNSET`). 근거는 실측 — 관계가 0건이었다
> (`agency_sellers` 0행 · `store_agency_delegation` 0행 · `introduced_by_agency_id` 0명).
> **"누가 이 매장을 운영하는가"는 별도 대시보드가 아니라 관계**(`store_agency_delegation` → 향후
> `seller_operators`)로 두고, 운영 화면은 **셀러 대시보드로 수렴**한다. 그래야 중개자가 올린 매장을
> 사장님이 직접 이어받을 때 계정 양도가 아니라 권한 변경이 된다. 설계 SSOT: `store-operator-model.md`.
> ⚠️ 이 일몰은 **"새로 안 받는다"이지 "쓰던 사람을 끊는다"가 아니다** — 기존 계정의 로그인·정산·위임은 유지.
...
### 에이전시 / 운영 / 도매
- `/agency/*` 🌇 **일몰 축소(2026-08-19, 39→16)** — 남은 축은 관계·정산·승계뿐: `introduced-stores`(영입) · `delegations`(위임) · `transfers`(승계) · `settlements`/`ledger`(정산) · `sellers`/`profile`/`guide`. 운영 화면은 셀러 대시보드로 수렴(`store-operator-model.md`)

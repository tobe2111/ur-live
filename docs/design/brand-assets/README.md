# 유어딜(Ur Deal) 확정 브랜드 자산 — 2026-07 Final → **2026-09-02 브랜드 블루 전환**

> 🔵 2026-09-02 대표 지시(*"우리 색상도 저 로즈 색 말고 이미지 색으로"* · *"로고도 이 색으로"*): 로즈 `#E0526B` → 블루 `#1C69EF`, 밝힌 로즈 `#EF6E85` → `#4D8DF5`, 바탕 크림 `#FAF7F5` → `#F8F7FC`.
> `public/` 의 favicon·앱 아이콘·OG PNG 는 납품 렌더를 **픽셀 재채색**(글리프·안티앨리어싱 그대로)했고, 원본 SVG 는 `*-blue.svg` 로 추가(로즈 판은 기록으로 보존). 아래 본문의 로즈 표기는 당시 기록.

...
**진행 — 코드로 막음 (대표 "나머지는 다")**
- 1️⃣ **차지백 클로백** — PG(카드) 차지백 시 이미 정산된 매장 돈을 회수. *유일하게 실제 돈이 새는 구멍.* 기존 역전 인프라(`order-refund.ts`/`restaurant-settlement`/`clawbackVoucherCommission`) 위에 얹음.
- 2️⃣ **양방향 분쟁** — 손님 항변권. 사장님이 "안 왔어요" 신고하면 **손님에게 알림 → 이용했어요(contest)/취소할게요(concede)**. 어드민이 양쪽 보고 판단(또는 concede 시 자동 재사용 복원).
- 3️⃣ **사용 위치 증거(소프트, 게이트 X)** + 가게측 선택 확인 — self-redeem 시 GPS *기록만* → 분쟁 시 "사용 위치↔가게 거리" 증거. 하드 게이트 금지(스푸핑/실내오차).

...
- **상태**: ✅ v1 구현 완료 + 유어딜 연계 보강 (2026-07-02) — 결정 4건 모두 추천안 확정. 아래 §5·구현완료 참조. v2(홍보 자격·발견 피드)만 잔여.
...
- [x] 사용 완료 이용권 제출 모달에 매장 카카오맵 원탭 딥링크 (`map.kakao.com/link/search/{매장명+주소}` — place_url 불필요, 수동 검색 제거) — 2026-07-02 연계 보강
- [x] 마이페이지 동네 리뷰어 레벨 카드 (`ReviewLevelCard` — 레벨/진행도 상시 노출, 동기부여 루프) — 2026-07-02 연계 보강
- [ ] (v2) 홍보 자격 — 레벨 배지 노출·핀 부스트 (대표 결정 3 범위 밖)
- [ ] (v2) 레벨 전용 이용권 **발견 경로** — 현재는 상세 배지+구매 게이트(전체 노출 후 차단, 잠금 표시로 상향 동기부여). "내 레벨 전용" 피드 섹션/필터(`?review_level=` 서버 쿼리 + 홈/동네딜 큐레이션)는 별도 규모 → 대표 확인 후

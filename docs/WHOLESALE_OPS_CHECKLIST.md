...
## 4. 🧩 의도적으로 미적용한 코드 꼬리 (가치/위험 판단상 보류)

### ✅ P1 — 클레임 승인 환불 전액·전 라인 고정 (해결 — commit `804e3a4`, 2026-06-26)
- 라인 스코프 환불 로직을 공유 헬퍼 `wholesale-refund.ts` 로 추출해 제조사 환불 라우트 + 클레임 승인 양쪽에서
  호출. 클레임 승인 시 `claim.supplier_id`(+항목) 스코프로 **서버에서 즉시 환불 집행**(2차 클릭 의존 제거,
  실패 시 클레임 재시도 가능). 다제조사 order-level 클레임만 어드민 수동 힌트 유지. tsc 0·build 0·단위 21 pass.

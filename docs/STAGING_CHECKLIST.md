...
## 📮 S-OWNERNOTICE — 사장님 "매장이 등록되었습니다" 알림톡 (2026-09-21)

**S-OWNERNOTICE** — 대표 확정 *"매장 사장님의 전화번호. 010으로 말이야. 되면 보내주는걸로"* ·
*"알림톡은 셀러대시보드 링크를 주는게 맞지 않을까?"* (2026-09-21).

발송 조건이 **둘 다** 필요하다 — `ALIGO_TPL_STORE_NOTICE`(카카오 검수 통과한 템플릿 코드)와
게이트 `platform_settings.store_owner_notice_enabled`(기본 `false`). 하나만 있으면 어드민 버튼이
비활성이고 이유가 화면에 뜬다. **자동 발송은 없다 — 어드민이 누를 때만 나간다.**
큐 SSOT `src/worker/utils/store-owner-notice.ts` · 화면 `/admin/store-owner` ·
설정 `/admin/platform-settings`.

| # | 무엇 | 통과 기준 |
|---|---|---|
| S-OWNERNOTICE-1 | 매장 승인 1건(사장님 번호가 **010**) | `store_owner_notices` 에 `queued` **1행** — 승인 순간엔 아무것도 안 나간다 |
| S-OWNERNOTICE-2 | 같은 매장을 다시 승인 | 행이 **안 늘어난다**(부분 UNIQUE 인덱스가 1회로 고정) |
| S-OWNERNOTICE-3 | 번호가 02·070·빈값인 매장 승인 | 행 **0** · 어드민 큐에 "직접 통화" 로 뜬다 — 알림톡은 010 에만 간다 |
| S-OWNERNOTICE-4 | 게이트 OFF 또는 템플릿 코드 없음에서 발송 시도 | 버튼 **비활성** · API 도 거절 — 아무것도 안 나간다 |
| S-OWNERNOTICE-5 | 둘 다 갖춘 뒤 **1건만** 발송 | 사장님 폰에 실제 수신 · 행이 `sent` · 버튼 2개(**내 매장 관리하기** / **내가 등록한 게 아니에요**)가 보인다 |
| S-OWNERNOTICE-6 | 🔗 사장님 폰에서 **내 매장 관리하기** 탭 | `/seller/waiting` 이 그 세션에서 셀러 토큰을 받아 `/seller` 로 들어간다(이메일·비번 안 묻는다) |
| S-OWNERNOTICE-7 | 로그인 안 된 폰에서 같은 버튼 | `/seller/login` 으로 간다 — 막히지만 **왜 왔는지 알 수 없다**(알려진 갭, 여기서 실제 체감을 판정) |
| S-OWNERNOTICE-8 | 🔒 발송 도중 같은 행을 두 번 긁기 | 실제 전송은 **1회**(CAS 선점) · 실패해도 `queued` 로 안 돌아간다(무한 재발송 없음) |

⚠️ **S-OWNERNOTICE-5 가 핵심이다** — 문구·버튼은 카카오 검수를 통과한 템플릿이 정하므로
코드가 무엇을 보내든 실제로 도착한 화면이 진실이다. 유닛은 "버튼 2개를 이 순서로 넘긴다"까지만 본다.

상태: ⬜ 미검증 (2026-09-21 배선, 템플릿 검수 대기). 통과 전 프로덕션 ON 금지.

...
| **S8** | `store_intro_payout_in_deal='true'` | platform_settings | 🔴 **머니 경로 · 보상의 종류가 바뀐다.** 영입 매장 주문 1건 결제 → T+7 성숙 대기(또는 available_at 조작) → cron 실행 | ① 영입자 **딜 잔액**이 커미션만큼 증가 ② `influencer_attributions` 가 `paid` 로 종결(현금 잔액 미유입) ③ cron 재실행 시 **이중 적립 0**(선점 CAS) ④ 적립 실패 시 `pending` 복귀 ⑤ `store_intro` 아닌 축(매칭 등)은 **현금 경로 유지** | ⬜ 미검증 (2026-08-30 신설 — 대표 "매장 영입도 딜로 쌓아줘"). ⚠️ **세무 확인 필요**: 현금은 3.3%/8.8% 원천징수하는데 딜은 안 한다. 딜은 교환권으로 실물 교환이 되므로 소득으로 볼 여지가 있다 — 켜기 전 대표/세무 판단. |

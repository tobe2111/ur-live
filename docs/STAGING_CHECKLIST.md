...
## 💸 S-MYSELL — 마이 안 판매 도구 (환불 · 출금) (2026-09-25)

대표 확정 *"등록, 환불, 분석, 출금도 마이에서 돼야해"*(설계 §19). 사장님이 **셀러 대시보드에
안 들어가고** 마이에서 환불·출금을 한다. 둘 다 **머니 경로**라 유닛으로는 끝이 안 난다 —
유닛은 "가드가 있고 이 API 를 부른다"까지만 본다.

⚠️ **이 표의 핵심은 S-MYSELL-3·7 이다** — 화면이 말한 가게·계좌와 실제로 움직인 돈이
갈리면 사장님은 **송금이 안 된 뒤에야** 안다(어드민 지급 행이 조용히 비어 있다).

| # | 무엇 | 통과 기준 |
|---|---|---|
| S-MYSELL-1 | 가게 **둘 이상**인 계정으로 마이 진입 → A 선택 | 오늘 카드·주문 목록이 **A 것**만 · 도구 넷이 보인다 |
| S-MYSELL-2 | 🔴 A 를 띄워 둔 채 **다른 탭에서** B 로 전환 → 마이에서 [환불] 탭 | 좌석 불일치를 잡고 **요청을 안 보낸다** · 다시 맞춘 뒤 열린다(남의 주문이 뜨지 않는다) |
| S-MYSELL-3 | 환불 1건 **실제 실행**(소액 주문) | 결제가 **실제로 취소**되고 손님에게 환불 · 주문 상태·커미션 역전이 대시보드 환불과 **같다** |
| S-MYSELL-4 | 환불 후보 목록 | 결제가 캡처된 주문(`PAID/DONE/PREPARING/SHIPPING/DELIVERED`)만 · 이미 취소·환불된 건 **안 뜬다** |
| S-MYSELL-5 | 🔒 환불 시트에서 같은 주문 두 번 탭 | 실제 환불은 **1회**(서버 멱등) · 두 번째는 사람 말로 거절된다 |
| S-MYSELL-6 | 출금 시트 열기(사업자 미검증 매장) | 금액 입력이 **안 뜨고** "사업자등록증 확인 후" 안내 — 400/412 를 본 뒤 알게 되지 않는다 |
| S-MYSELL-7 | 🏦 **계좌 표시** — 출금 시트를 연다 | 매장에 등록된 **그 계좌**의 은행·뒤 4자리·예금주가 뜬다(직전에 보던 다른 가게 계좌가 아니다) |
| S-MYSELL-8 | 🏦 계좌 **미등록** 매장에서 출금 시도 | 신청 버튼이 **안 눌린다** · 어디서 등록하는지 알려준다(빈 지급 행이 안 생긴다) |
| S-MYSELL-9 | 🔴 출금 **1건 실제 신청**(최소 10,000딜) | 잔액이 그만큼 줄고 · `settlements` 새 행의 **bank_name/account_number/account_holder 가 채워져 있다** · 어드민 지급 센터에 그대로 보인다 |
| S-MYSELL-10 | S-MYSELL-9 직후 어드민에서 **실제 송금 처리** | 그 계좌로 입금된다 — 원천징수 차감 후 `net_amount` 와 일치 |
| S-MYSELL-11 | 🔒 잔액보다 큰 금액 · 10,000 미만 | 각각 이유가 다른 문장으로 뜬다("출금 실패" 한 마디 금지) |
| S-MYSELL-12 | 🔒 출금 버튼 연타 | 차감은 **1회**(CAS) · 두 번째는 409 를 사람 말로 |
| S-MYSELL-13 | PIN 미설정 계정에서 출금 | PIN 을 **어디서** 거는지 알려준다 · 건 뒤 성공한다 |
| S-MYSELL-14 | 이용권 **등록** 탭 → 폼 → 저장 | 좌석 가게로 등록된다(직전 가게 아님) · 마이 판매 목록에 바로 보인다 |
| S-MYSELL-15 | 📷 계산대(사용처리) — 가게 **둘**인 계정 | 화면이 **어느 가게** 것을 처리하는지 말하고, 실제로 그 가게 이용권만 소각된다 |

상태: ⬜ 미검증 (2026-09-25 배선). **S-MYSELL-3·9·10 통과 전 대표에게 "출금 된다"고 말하지 않는다.**

### S-MYSELL 보강 — 온보딩 셋 (2026-09-26, 설계 §20-5)

| # | 무엇 | 통과 기준 |
|---|---|---|
| S-MYSELL-16 | 🔑 PIN 미설정 계정으로 출금 시도 | 대시보드로 **안 보내고** 그 자리에서 PIN 시트가 열린다 · 걸고 나면 **출금 시트로 되돌아온다** |
| S-MYSELL-17 | 🔑 `/store/new` 로 만든 매장(소비자 계정과 안 묶임)에서 PIN 설정 | 비밀번호 칸이 **서버가 요구한 뒤에** 열린다 · 넣으면 설정된다 |
| S-MYSELL-18 | 🔑 PIN 을 건 직후 바로 출금 | **또 412 를 받지 않는다**(set 뒤 verify 까지 갔다는 뜻) |
| S-MYSELL-19 | 🏦 계좌 미등록 매장에서 출금 → [계좌 등록하기] | 그 자리에서 시트 · 저장 후 출금 시트로 복귀 · 뒤 4자리가 방금 넣은 계좌다 |
| S-MYSELL-20 | 🏦 가게를 **옮긴 뒤** 출금 시트 열기 | 계좌가 **새 가게 것**이다(직전 가게 계좌가 남지 않는다) |
| S-MYSELL-21 | 🧰 전체 도구에서 아무 화면이나 진입 | 그 화면 맨 위에 **마이로 돌아가기** 띠 · 누르면 마이로 · 그 뒤 대시보드를 직접 열면 띠가 **없다** |
| S-MYSELL-22 | 🪑 종류가 다른 가게 둘을 번갈아 전환 | 전체 도구 목록이 **그 가게의 것**으로 바뀐다(A 의 메뉴가 B 에 안 남는다) |
| S-MYSELL-23 | 📱 안드로이드에서 시트 열고 **시스템 뒤로가기** | 시트만 닫힌다(마이가 통째로 닫히지 않는다) · X 로 닫은 뒤 뒤로가기도 한 번 먹히지 않는다 |
| S-MYSELL-24 | 🏠 알림톡 **내 매장 관리하기** 를 폰에서 탭 | **마이**로 착륙하고 맨 위에 내 가게가 보인다(대시보드가 아니다) |
| S-MYSELL-25 | 🏠 지도에서 매장 클레임 완료 | 방금 얻은 가게가 **마이**에 보인다 |
| S-MYSELL-26 | 🏠 공구 상세 "공구 관리 →" · 유어샵 "전체 관리 →" | 그 화면 위에 **마이로 돌아가기** 띠가 있다 |
| S-MYSELL-27 | 🧾 **주문 묶음** — 마이 → 주문 → 기간 칩(전체/확인 대기/준비 중/취소·환불) | 목록이 바뀐다 · 총 건수가 맞다 · **더 보기**로 옛 주문까지 내려간다 |
| S-MYSELL-28 | 🧾 주문 한 줄 펼치기 → 주문번호·연락처·품목 | 값이 실제 주문과 같다 · 확인 대기면 **주문 확인** 버튼이 있고 누르면 준비 중으로 바뀐다 |
| S-MYSELL-29 | 🪑 주문 시트를 열어 둔 채 **다른 가게로 전환** | 목록이 새 가게 것으로 바뀌거나 안내가 뜬다(A 의 주문이 B 좌석에 남지 않는다) |
| S-MYSELL-30 | 🎟️ **이용권 묶음** — 목록·스위치로 중지/재개 | 카드의 "판매 중 N개" 와 숫자가 같다 · 끈 것은 손님 화면에서 사라진다(삭제가 아니다) |
| S-MYSELL-31 | 💵 이용권 이름 탭 → 가격 바꾸고 저장 | **"얼마 → 얼마" 확인 문장이 한 번 뜬다** · 다시 눌러야 저장된다 · 손님 화면 가격이 바뀐다 |
| S-MYSELL-32 | 💵 가격은 그대로 두고 **안내 문구만** 고쳐 저장 | 확인 문장이 **안 뜬다**(바로 저장) · 상세의 이용 안내가 바뀐다 |
| S-MYSELL-33 | 💵 가격을 바꿨다가 **원래 값으로 되돌린** 뒤 저장 | 확인 단계가 사라진다(옛 금액이 저장되지 않는다) |
| S-MYSELL-34 | 🏪 **가게 묶음** — 이름·연락처·주소·소개 저장 | 저장된다 · **PIN 을 묻지 않는다**(계좌 필드를 안 보내므로) · 손님 화면 가게 정보가 바뀐다 |
| S-MYSELL-35 | 🏪 승인 대기·반려 매장에서 가게 묶음 열기 | 마이 카드와 **같은 문장**의 상태 안내가 보인다 |
| S-MYSELL-36 | 🔑 정산 계좌를 **PIN 없이** 바꿔 보기(15분 지난 상태) | 막다른 길이 아니다 — PIN 시트가 열리고, 확인하면 **계좌 시트로 돌아온다**(출금이 아니다) |
| S-MYSELL-37 | 🔑 위임 운영자 계정으로 계좌 변경 시도 | 403 — "정산 계좌는 소유자만" 문장이 그대로 보인다(PIN 으로 못 푸는 벽이다) |
| S-MYSELL-38 | 🎟️ **등록** — 마이 → 이용권 → [새 이용권 등록] | 시트가 열리고 **3단계 위저드가 그대로** 나온다(대시보드와 같은 화면) · 마이를 안 벗어난다 |
| S-MYSELL-39 | 🎟️ 그 시트에서 1~3단계를 끝까지 채워 등록 | 상품이 실제로 만들어진다 · 닫으면 **이용권 목록에 바로 보인다** · 손님 화면에도 뜬다 |
| S-MYSELL-40 | 🎟️ 등록 시트에서 **카카오맵 검색 · 사진 업로드** | 둘 다 동작한다(지도 선택창이 시트 뒤로 숨지 않는다 · 사진 고르는 동안 시트가 안 닫힌다) |
| S-MYSELL-41 | 💾 시트에서 1단계만 쓰고 닫았다가 다시 열기 | **임시저장 복원 배너**가 뜬다(대시보드에서 쓰던 드래프트와 같은 것) |
| S-MYSELL-42 | 🌙 **다크 모드**로 등록 시트 열기 | 폼 글자가 보인다(흰 배경 위 흰 글자 0) · 입력 칸 글자·placeholder 정상 |
| S-MYSELL-43 | 🎟️ 등록 시트에서 [취소] | 시트만 닫히고 **마이가 그대로 있다**(대시보드로 안 나간다) |
| S-MYSELL-44 | 🏨 **숙소** — 이용권 시트 → 숙소 | 목록(없으면 안내 문구) · [숙소 등록]·목록 항목은 전체화면으로 나가고 맨 위에 **마이로 돌아가기** 띠 |
| S-MYSELL-45 | 🏭 **도매 겸업 계정**으로 등록 시트 열기 | `/wholesale` 로 안 튕긴다(마이가 사라지지 않는다) |
| S-MYSELL-46 | 🎫 셀러 대시보드 사이드바·⌘K 확인 | **쿠폰 메뉴가 없다** · 직링크 `/seller/coupons` 는 **여전히 열린다**(되돌릴 수 있다) |
| S-MYSELL-47 | 🤝 마이 → **소개 파트너** | 받은 제안 목록 · 상태 배지 · 좌석이 맞는 가게의 제안만 뜬다 |
| S-MYSELL-48 | 🤝 **상대가 보낸** 대기 제안에서 [수락] | 성공 토스트 · 목록이 '진행 중' 으로 바뀐다 · 404 안 남 |
| S-MYSELL-49 | 🤝 **내가 보낸** 제안 · **콘텐츠 인증** 제안 | 수락 버튼이 **없고** 왜 없는지 문장이 뜬다(빈 카드 금지) |
| S-MYSELL-50 | 🤝 제안 수락 중 **다른 가게로 전환** | "가게가 바뀌었어요" 안내 후 시트가 닫힌다(남의 가게에 계약 0) |
| S-MYSELL-51 | 💬 마이 → **브랜드메시지** (미연결 가게) | "아직 보낸 메시지가 없어요" 안내 한 장 + [브랜드메시지 시작하기] · **발송 버튼 없음** |
| S-MYSELL-52 | 💬 **충전된** 가게로 열기 | 남은 건수 + 최근 발송 목록 · 버튼 라벨이 [충전 · 보내기] 로 바뀐다 |
| S-MYSELL-53 | 💬 네트워크를 끊고 열기 | "잔액을 불러오지 못했어요" — **"0건" 으로 안 뜬다** |
| S-MYSELL-54 | 🧾 마이 → 정산 → **[지난 정산 보기]** | 빈 계정은 "아직 정산 내역이 없어요 / 신청하면 쌓입니다" · 닫으면 **출금 시트로 복귀** |
| S-MYSELL-55 | 🧾 출금 신청 후 다시 [지난 정산 보기] | 방금 신청한 행이 보이고 금액이 신청액과 **같다**(화면이 다시 계산하지 않는다) |
| S-MYSELL-56 | 🛏️ 셀러 사이드바·⌘K·마이 이용권 시트 | **숙소가 세 곳 모두에서 안 보인다** · 직링크 `/seller/stays` 는 여전히 열린다 |
| S-MYSELL-57 | 🛏️ 직링크 `/seller/review-verifications` 로 진입 | 하단 탭 **이용권**이 켜진다(위치를 잃지 않는다) |
| S-MYSELL-58 | 🧹 셀러 사이드바 전체 훑기 | 체험 캠페인·후기 인증이 없다 · **리뷰는 그대로 있다**(내리지 않았다) |
| S-MYSELL-59 | 📖 `/seller/guide` 운영백서 숫자표 | 꺼진 기능 **14** 로 갱신돼 있다(시드 v37 반영) |

### S-MYSELL-60~64 — 마이에서 전부 (2026-09-26, PR 대기)

셀러 계정 로그인이 필요해 세션이 못 한 것. 시안 `my-seller-all-in-my-2026-09-26.md`.

- **S-MYSELL-60** 비셀러 계정으로 `/user/profile` 하드로드 → 네트워크에 `app-seller-components-*.js`
  와 `SellerSection-*.js` 가 **없다**. (있으면 게이트가 새는 것 — 다이어트가 통째로 무효다.)
- **S-MYSELL-61** 셀러로 마이 → 전체 도구 → 아무 도구 → **시트 안에서** 열린다(전체화면 이동 아님).
- **S-MYSELL-62** `ExternalLink` 표시가 붙은 다섯(사용처리·이용권 등록·광고 슬롯·유망 매장·대행 승인)만
  전체화면으로 나가고, 그 화면 맨 위에 "마이로 돌아가기" 띠가 뜬다.

...
| **S7** | `fee_channel_rates_enabled='true'` | platform_settings | 🔴 **머니 경로 · 플랫폼 take 율 자체를 바꾼다.** 직판 상품 주문 1건 + 중개(벤더) 상품 주문 1건 각각 결제 → 환불 | ① 직판 주문의 원장 fee 가 **10%** ② 중개 주문이 **5%** ③ 채널을 못 가리면 종전 `sellers.commission_rate` 로 폴백(모르면 바꾸지 않는다) ④ 환불 시 fee 역전 대칭 ⑤ **유어딜 5% 불변식**([INV-#44]) 위반 0 | ⬜ 미검증 (2026-08-25 손잡이 신설 — 그전엔 켤 화면이 없었다) |

...
|---|---|---|---|---|---|
| **S1** | `commission_budget_enabled='true'` | platform_settings | 영입자 커미션 + 추천트리 커미션이 **겹치는** 3P 주문 결제 → 환불 | ① Σ(모든 커미션 적립) ≤ 주문당 예산(수수료−`pg_reserve_pct`) ② 환불 시 전 커미션 역전 대칭 ③ OFF 복귀 시 기존 동작과 동일 | ⬜ 미검증 (2026-07-04 배선) — **절차: 아래 §S1 절차** |
...
### 📐 S1 절차 — 예산 아비터를 켜도 되는지 판정하는 법 (2026-09-01 신설)

> 🩸 **이 게이트는 2026-07-04 에 배선되고 두 달 넘게 미검증으로 남았다.** 이유가 분명하다 —
...
> ⇒ 판정을 서버가 내놓게 했다(read-only): **`GET /api/admin/promo-ledger/order/{주문번호}`**

**무엇을 켜는 것인가**: 성장 커미션 4축(어필리에이트 · 추천트리 · 영입 · 에이전시)이 각자
GMV % 로 얹히면서 서로의 존재를 모른다. 아비터는 **주문당 예산**(플랫폼 수수료 − PG 준비금) 안에서
비례 배분한다. **꺼져 있으면 배분 없이 각자 전액** — 겹친 주문에서 플랫폼이 손실을 볼 수 있다.

**준비 (staging)**
| | 무엇 | 왜 |
|---|---|---|
| A | 소개자 링크가 붙은 매장 상품 1개 | 영입/어필리에이트 축을 태운다 |
| B | 그 구매자가 **추천트리 아래**에 있을 것 | 멀티티어 축을 겹친다 — **겹치지 않으면 이 검증은 무의미하다** |
| C | `pg_reserve_pct` 확인(기본 2.75) | 예산 계산의 입력 |

**한 방문에 되는 것**
1. 게이트 **OFF 인 채로** A·B 조건으로 결제 1건 → 주문번호 기록 → 판정 패널 조회.
   → 이때 `within_budget: false` 가 나오면 **그게 이 게이트가 필요한 증거**다(현행이 예산을 넘고 있다).
   `false` 가 안 나오면 겹침이 부족한 것이니 B 를 다시 확인한다.
2. `commission_budget_enabled='true'` 저장 → 같은 조건으로 결제 1건 → 판정 패널 조회.
   → **`within_budget: true` · `over_by_krw: 0`** 이어야 한다.
3. 2번 주문을 **전액 환불** → 판정 패널 재조회 → 4축 `amount` 가 전부 0 으로 역전됐는지.
4. 게이트를 `false` 로 되돌리고 결제 1건 → 1번과 같은 숫자가 나오는지(**즉시 현행 복귀**).

**판정은 응답의 `verdict` 세 줄이 전부다** — 손으로 더하지 말 것.
`within_budget`(Σ적립 ≤ 예산) · `over_by_krw`(넘은 금액) · `platform_revenue_untouched`([INV-#44]).

⚠️ **`platform_revenue_untouched: false` 는 지금은 정상이다** — flip(promo 재원 전환) 전에는
에이전시·영입 커미션이 플랫폼 부담이라 `platform:revenue` 를 debit 한다. 그 줄이 `true` 가 되는 것은
8월 promo flip 의 통과 기준이지 S1 의 기준이 아니다. 섞지 말 것.

**프로덕션 점등은 대표가 어드민에서** (`/admin/platform-settings`). 세션은 판정만 보고한다.

...
✅ **2026-08-30 대표 확인으로 종결**: *"홍대돈까스는 내가 가상으로 AB테스트 용이야."*
⇒ **실제 사업자 매장이 아니다.** 채널을 `direct` 로 바꿀 이유가 없고, 5% 로 두어도 실제 돈이 안 움직인다.

🔑 **그래서 지금 라이브에 실제 사업자 매장은 0곳이다.** 활성 매장 1곳(홍대돈까스)은 대표의 AB테스트용
가상 매장이다. 이걸 모르면 다음 세션이 *"라이브 매장이 5% 를 내고 있다"* 고 오판한다.

💡 **역으로 이게 S7 검증의 기회다**: 대표가 통제하는 매장이므로 **실제 매장 돈을 안 건드리고**
채널 요율을 실결제로 검증할 수 있다.
  - 지금 상태(brokered)로 결제 1건 → 원장 `platform_fee` 가 **5%** 인지
  - `PATCH /api/admin/sellers/14/channel {"channel":"direct"}` 후 결제 1건 → **10%** 인지
  둘 다 맞으면 S7 이 닫힌다.

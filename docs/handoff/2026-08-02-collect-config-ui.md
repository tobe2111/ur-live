...
## ⑮ ⚖️ **공정위 가맹 주소가 지금 존재하지 않는다** (#982 배포 후) — ⚠️ 아래 ⑯ 에서 이 절의 결론을 **정정**했다
...
| **franchise(공정위 가맹)** | 🔴 **주소 불일치**(폐기인지 오타인지 불명 — ⑯) | 오퍼레이션 4종 전부 code 12 |
| nara(나라장터) | 🔴 **주소 불일치**(⑯ 과 같은 이유로 폐기 단정 불가) | 프로브·레인 동일 주소에서 code 12 |
| localdata(인허가) | 🔴 주소 불명 + **원천 호스트 도달 불가** | 포털 경로 code 12 · 원천은 robots.txt 조차 timeout |

## ⑯ 🚫 **정정 — `NO_OPENAPI_SERVICE_ERROR` 로는 "폐기"를 단정할 수 없다** (대조군 실측)

⑮ 에서 나는 *"공정위 가맹 서비스 폐기 확정"* 이라고 적었다. **그 확신은 틀렸다.**
근거로 삼은 논리는 *"같은 키로 같은 기관(1130000)의 commerce 는 200 이니, 권한이 아니라 서비스가 없는 것"*
이었는데 — **앞부분(권한 아님)은 맞고 뒷부분(그러므로 폐기)이 틀렸다.**
...
**셋이 구분되지 않는다.** 이 코드가 말하는 것은 **"이 주소가 지금 안 맞는다"** 까지이고,
그 이유가 폐기인지 경로 오타인지는 **원리적으로 알 수 없다.**

⇒ 정확한 진술: **`FftcBrandRlsInfo2_Service/getBrandList` 라는 주소가 지금 존재하지 않는다.**
nara·localdata(포털 경로)도 같은 이유로 **"폐기"가 아니라 "주소 불일치"** 로 낮춰 적어야 한다.

> 🟢 **이건 오히려 좋은 소식이다** — 서비스가 통째로 사라진 게 아니라 *이름만 바뀐 것*일 수 있다.
>   그렇다면 대표님의 "요청주소" 한 줄로 세 레인이 한꺼번에 살아난다.

### 왜 놓쳤나 — 그리고 무엇을 박았나

힌트 문구는 원래도 **두 가능성을 다 적고 있었다**(`"URL/오퍼레이션명이 틀렸거나 폐기됨"`).
문구는 맞았고 **내가 그중 내 가설에 맞는 쪽을 골랐다.** 나열만 하는 안내는 이 오추론을 못 막는다.

⇒ `public-data-diag.ts` 의 code 12 힌트를 **오추론을 명시적으로 막는 문장**으로 강화하고
(`"구분할 수 없는 코드다"` + 대조군 근거 + `"단정하지 말고 요청주소와 대조할 것"`),
유닛 3건으로 고정 + 되돌려-검증 RED + 매니페스트 등재(CI 가 대신 깨뜨린다).
...
### 🩸 `quality` 는 더 나빴다 — **전진 0** 이었을 가능성

같은 방식으로 열어 보니 `runQualityPass` 의 상한이 **행 수(MAX=8,000)뿐**이었다. 한 인보케이션이
16페이지 × 500행을 통째로 채점한다. 그리고 **커서 저장이 루프 뒤**에 있다 —
CPU 로 죽으면 **그 줄에 도달하지 못하고** 다음 회차가 같은 지점을 또 훑고 또 죽는다 ⇒ **전진 0**.
(`ads-cpu-deadline` 이 통신판매에서 확정한 그 실패 모양 그대로.)

**수리**: 루프에 벽시계 마감선(무료 1,700ms / 유료 12,000ms — 호출부가 `envPlanValue` 로 넘김,
안 넘겨도 무료 기본값으로 폴백). ⚠️ **마감선 중단은 `done` 을 false 로 남긴다** — true 로 하면
커서가 0 으로 리셋돼 매 회차가 처음부터 돌고 **풀 뒷부분은 영영 채점되지 않는다**(가드로 고정).
`stopped_by`·`elapsed_ms` 반환.

⚠️ **`enrich-company` 는 손대지 않았다.** 열어 보니 **이미 마감선·예산·건별 저장을 갖췄다**
(`envEnrichDeadlineMs` 무료 7s, `save()` 가 리드마다 커밋). CPU 사망은 크롤/파싱이 무거워
벽시계 마감선 전에 CPU 가 먼저 닿은 것이라, 처방은 "상한 추가"가 아니라 **값 조정**이고
그건 처리량 트레이드오프다 — 근거 없이 만지지 않았다. 무배포 노브가 이미 있다(`ADS_ENRICH_DEADLINE_MS`).

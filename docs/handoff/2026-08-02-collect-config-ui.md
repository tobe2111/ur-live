...
## ⑳ 🛡️ **"조용한 전진 0" 을 가드로 환원** — 세 번째를 막는다

같은 실패가 **두 번** 났다(`commerce` 08-02 · `quality` 08-03). 그리고 `ads-cpu-deadline.test.ts` 가
그 실패 모양을 **문서로 확정해 뒀는데도** 다음 레인에서 그대로 재발했다 — **문서로는 못 막는다는 증거**다.

**`check-cursor-after-loop.mjs`** — 커서 저장 지점 앞 창(120줄)에 루프가 있는데 **시간 상한이 없으면** 신고.
audit-gate(84번째 불변식) + `verify.yml` strict. 래칫이라 **신규만** 막는다(기준선 7건).

### 🐛 시험이 가드의 구멍을 즉시 잡았다

가드를 쓰자마자 자기-시험을 붙였더니 **첫 판정이 `outOfBudget(budget)` 를 못 알아봤다** —
마감선이 헬퍼에 감싸인 형태다. 그대로 뒀으면 **이미 마감선을 가진 레인 3개**를 신고해서
사람들이 중복 검사를 넣게 만들었을 것이다. **오경보는 가드를 죽인다.**
패턴에 추가 → 기준선 10 → **7건**.

> 🧭 교훈: 새 가드는 **자기-시험을 같은 커밋에** 붙일 것. 그 시험이 "이 레포가 실제로 쓰는 표현"을
> 나열해 대조하게 하면, 오경보 원인이 작성 시점에 드러난다.

### 남은 7건은 왜 안 고쳤나

지금 CPU 로 **죽고 있지 않다**(하트비트 확인). 추측으로 마감선을 넣으면 처리량을 근거 없이 깎는다 —
CLAUDE.md 개발 룰 #1. 죽기 시작하면 하트비트가 먼저 말해 주고, 그때 이 목록이 후보가 된다.

...
2. **상태 색 래칫 줄이기** — `scripts/status-tone-baseline.json` 에 **59파일**이 남아 있다.
   ```
   node scripts/check-status-tone.mjs            # 남은 파일 보기
   node scripts/check-status-tone.mjs --rebaseline
   ```
   기계적으로 바꿀 수 있다(아래 §3 의 코드모드와 같은 방식). 다만 **tone 선택은 의미로** —
   `ok(완료·승인)` · `warn(대기·검수)` · `bad(반려·실패·연체)` · `info(진행중·발송)`.

...
## 6. 2026-09-04 — 딜 결제를 켰고, 켜자마자 환불이 막혀 있던 걸 찾았다

대표: *"머지하고, 너가 직접 켜줘"* · *"2번으로 할게 어차피 지금 고객 없어"* · *"남은 작업도 마저 해"*

### 켠 것 (라이브 실측 확인)
```
voucher_deal_payment_enabled  = true     (전부-딜)
voucher_partial_deal_enabled  = true     (딜 일부 + 카드)
influencer_deal_bonus_pct     = 0        ← 선행 조건. 이게 20 이면 팔릴수록 적자다
```
클라 플래그 `VOUCHER_DEAL_PAYMENT_ENABLED` 도 `true` 로. ⇒ **유일한 스위치는 서버 키**이고
끄면 배포 없이 즉시 돌아간다. 주입 매니페스트도 *"기본 ON 이 된다"* → *"서버 키가 느슨해져
스위치가 사라진다"* 로 재조준했다(위험의 무게가 옮겨간 자리에 가드를 다시 세운 것).

### 🩸 켜자마자 드러난 환불 결함 2건 (같은 클래스)

    주문:  총액 10,000 = 카드 8,000 + 딜 2,000
    환불:  카드에 10,000 취소 요청 → EXCEED_CANCEL_AMOUNT → 402 로 즉시 return
           → 상태 전이도 딜 복원도 도달 못 함 ⇒ **카드도 딜도 안 돌아온다**

`refundOrderFully`(전액)와 `returns.routes`(반품) **둘 다** 그랬다. `orders.total_amount` 에는
총액이 들어가는데 카드 승인액은 `deal_used` 만큼 적다는 걸 취소 요청이 몰랐다.

**같은 파일이 같은 사고를 이미 겪고 주석에 적어 뒀다** — *"Toss 에 전액 재요청 →
EXCEED_CANCEL_AMOUNT → 잔여분 취소 영구 불능"*. 그때 부분취소 누적은 뺐는데 **딜 사용분은
안 뺐다.** 쇼핑탭이 숨겨져 노출이 0이었을 뿐이고, 부분결제를 켜는 순간 열렸다.

수리: 환불액을 **카드 몫과 딜 몫으로 나눈다.** `deal_used` 를 **한 번만** 읽어 둘이 같은 값을
쓰게 했고(두 번 읽으면 갈리고 그 차액이 조용한 미수가 된다), 딜 복원은 잔여 환불액으로 클램프했다
(부분반품 뒤 과다 환불 차단).

⚠️ **실측으로 피해 0 확인**: `SELECT COUNT(*) FROM orders WHERE COALESCE(deal_used,0)>0` → **0건**.
9/1 이후 결제 주문도 0건이라 고칠 과거 주문이 없다.

### 검색 화면
- *"함께 검색된 키워드"* 6개가 **하드코딩**이라 검색어와 무관하고 **누르면 0건**이었다
  (`무료배송` 은 이용권 서비스에 개념 자체가 없다). 진짜 인기 검색어 API 는 **이미 있었고**
  빈 검색 화면에서만 쓰이고 있어서 → 공유 훅 `usePopularSearches` 로 묶고 두 화면이 같은 값을 본다.
  라벨도 '인기 검색어' 로 — 연관검색어가 아니므로 그렇게 부르면 그 자체가 거짓이다.
- 결과 개수의 `text-red-500` → 잉크(빨강은 기능 신호로 예약).
- ⚠️ **필터 칩 5개 무동작은 다른 세션이 09-03 에 이미 제거**했다 — 내 이전 메모가 낡았었다.

### 이번에 틀렸거나 배운 것
1. **플래그를 켜고 생성기를 안 돌렸다** — `generate-ops-handbook --check` 가 CI 를 막았다.
   운영백서의 기능 켜짐/꺼짐 표가 코드와 어긋난 채였다. 그 가드 문구 그대로 *"매장 사장님이
   틀린 요율을 읽는다"*. **`feature-flags.ts`/`policy.ts` 를 만졌으면 생성기를 함께 돌릴 것.**
2. **내가 만든 가드가 헛돌았다** — `toContain('summary.dealUsed')` 는 그 이름이 파일 곳곳에 있어
   블록을 통째로 꺼도 통과했다. 되돌려-검증이 잡았다. **이름이 아니라 렌더 조건을 앵커할 것.**
3. **기존 가드가 내 정상 변경을 잡았다** — "요약 값이 금액 계산에 새어들지 않는다" 가 표시용
   덧셈까지 빨간불. 가드를 푸는 대신 **규약으로 재조준**(요약 파생값은 `display*` 로 이름 짓는다).
4. **테마 가드는 `dark:` 가 0개인 파일을 통째로 건너뛴다** — `SortFilterBar` 에 dark 하나를 넣자
   기존 누락이 드러났다. 결제 화면에서 본 것과 같은 사각지대다.
5. **주석 낱말이 테스트를 깨뜨렸다** — 왜 그 여섯이 문제였는지 설명에 적었더니 "하드코딩이
   되살아났다" 고 빨간불. `codeOnly` 로 주석을 걷어내고 검사할 것.
6. **`audit-gate` 를 타임아웃으로 끊지 말 것** — 자식 `check-guard-mutations` 가 살아남아
   소스를 주입한 채 남고, `git add` 가 그걸 집는다. 실제로 `StoreStep.tsx` 의 `if (false)` 가
   index 에 들어갔다. 커밋 전 `ps aux | grep [g]uard-mutations` + `git diff` 대조.

### 다음 세션의 첫 액션
🔴 **실결제 1건이 유일하게 남은 검증이다**(소스 검사로는 못 본다):
1. 잔액 충분 계정 → 이용권 **전부-딜** 구매 → Toss 호출 0 · 이용권 발급 · 매장 원장 크레딧
2. 잔액 부족 계정 → **딜 + 카드** → 결제창에 `상품 10,000 / 딜 −2,000 / 카드 8,000` 표시
3. 그 주문을 **환불** → 카드 몫 취소 + 딜 몫 복원, **합계 = 총액** ← 오늘 고친 자리
문제가 보이면 `/admin/platform-settings` 에서 두 게이트를 `OFF` (배포 불필요, 즉시 복귀).

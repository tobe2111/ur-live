...
소비자 주문은 입금 웹훅이 확정을 완결하지만, **웹훅에는 공구 교환권 발급 코드가 한 줄도 없다.**
그래서 "입금을 기다리게 두는" 처리를 하면 **입금은 됐는데 교환권은 영원히 안 나오는 주문**이 생긴다.
⇒ 지금 정직한 처리는 **가상계좌를 취소하고 카드로 안내**하는 것뿐이다(발급 금지 + 자동 취소 + 흔적 주문).

> 📌 **가상계좌를 정식 지원하려면** 웹훅에 공구 교환권 발급을 배선하는 **별도 작업**이 선행돼야 한다.
...
| 파일 | 변경 |
|---|---|
| `src/features/group-buy/api/gb-purchase-guards.ts` | **신규** — `isSelfOwnedGroupBuy` · `resolveGbOrderNumber` · `guardAwaitingDeposit` |
| `src/features/group-buy/api/group-buy.routes.ts` | 자기참여 판정 2곳 교체 · 주문번호 = 토스 orderId · VA 가드 배선 |
| `src/features/group-buy/api/group-buy-seller.routes.ts` | 환불 조회 범위 = 판매 범위(`voucherCategoriesSqlClause()` + `deal_only`) |
| `src/tests/unit/gb-purchase-guards.test.ts` | **신규** 17건 |
...
2. **가상계좌 결제 1회** → 교환권 **미발급** + 사용자에게 `VIRTUAL_ACCOUNT_UNSUPPORTED` 안내 +
...
- **가상계좌 정식 지원 여부**(대표 판단). 지원하려면 웹훅 공구 교환권 발급 배선이 선행.
...
- 처음엔 ②를 "소비자 `/confirm` 처럼 `AWAITING_PAYMENT` 로 두고 입금 웹훅에 맡기면 된다"고 계획했다.
  **웹훅에 공구 교환권 발급이 0건**이라는 실측을 하고서야 그 처방이 *더 나쁜 상태*(입금됐는데 미발급)를

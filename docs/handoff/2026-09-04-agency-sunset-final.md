...
⇒ **대표가 승인한 범위는 "7곳"이므로 그 7곳만 지운다.** 5·12·13 은 승인 목록에 없었고, 특히
**id 5 는 대표 본인 계정의 매장이고 활성 상품 6개를 안고 있다** — 문구("홍대돈가스 말고는 다")를
글자대로 밀면 그걸 지우게 된다. **대표 확인 필요**(아래 "남은 결정").
...
- 상품·주문·운영자·정산·원장이 **하나라도** 있으면 **409 거부**(연결된 유저 계정도 거부)
...
## 다음 세션 첫 액션
1. **배포 후** 매장 7곳(3·6·7·8·9·10·11) purge 실행 — 위 §② 절차. 판정: `SELECT COUNT(*) FROM sellers` 가
   11 → 4 (5·12·13·14 만 남음).
2. **대표 확인 대기**: 매장 5(UR Team, 활성 상품 6) · 12(Lister) · 13(셀메이커스) 를 어떻게 할지.
   ⚠️ 5 는 대표 본인 계정 매장이라 문구대로 밀면 안 된다.
3. **③은 설계 승인 전 구현 금지.** 결정할 것 3가지는 `store-operator-model.md` §7.7 에 적어 뒀다.

## 남은 결정 (대표 판단)
| # | 무엇 | 왜 지금 못 정하나 |
|---|---|---|
| 1 | 매장 5·12·13 삭제 여부 | 승인 목록(7곳)에 없었고, 5는 활성 상품 6개를 안고 있다 |
| 2 | operator 권한 범위 | 정산계좌·사업자정보를 운영자가 보게 할지 — 대표만 정할 수 있다 |
| 3 | 중개사 실적 화면 | 매장에 청구할 근거를 어디서 보여줄지(지금은 매장 전환뿐) |

...
## 🗑️ 2026-09-06 — 머지·배포·매장 정리 **실행** (대표 *"머지하고 매장 정리 및 내가 해야할 거 너가 다 해줘 판정까지도"*)

PR #1352 squash 머지(`8d3f5a3`) → 배포 확인 → 매장 purge 실행.

**배포 판정 신호를 배포 전에 떠 놨다**(번들 해시는 남의 PR 배포로도 바뀌어 부정확하다):
없는 매장 id 로 purge 를 찔러 `{"error":"Not found"}`(라우터 generic 404) → 배포 후
`{"error":"매장을 찾을 수 없습니다"}`(핸들러 자신의 404) 로 바뀌는지. 정확히 그렇게 바뀌었다.

### 결과 — 11곳 → 3곳 (목표는 1곳이었다)
```
삭제 성공 9곳: 6 7 8 9 10 11 12 13  (+ 아래 5번은 부분)
남음: 3(테스트 상점) · 5(UR Team) · 14(홍대돈까스 — 남길 매장)
```

### 🩸 실행이 드러낸 내 코드의 구멍 둘 (둘 다 500 으로 나타났다)

**① 머니 blocker 가 두 테이블을 빠뜨렸다** — 매장 3
```
donations       2행 (₩600)   FK: REFERENCES sellers(id)                  ← ON DELETE 절 없음 = RESTRICT
voucher_orders  3행           FK: REFERENCES sellers(id) ON DELETE CASCADE ← 조용히 함께 사라진다
```
`donations` 가 RESTRICT 라 DB 가 막아 500 이 났다. **그건 설계가 아니라 운이다.**
`voucher_orders` 쪽이 더 나쁘다 — CASCADE 라 매장을 지우면 KT 교환권 발송 기록이 **에러도 로그도 없이**
없어진다. 내가 대표에게 보고한 *"돈은 안 걸려 있습니다"* 는 **주문·주문항목·이용권·정산·원장** 다섯 개만
센 것이었고, 이 둘은 목록에 없었다.
> ✅ 이미 지운 8곳은 무사하다 — **추정이 아니라 측정**: `voucher_orders`·`donations` 를 seller 별로
> 집계하니 **오직 매장 3번에만** 행이 있다(다른 seller_id 는 한 줄도 없다).

**② `seller_business_info` 를 안 지워 매장이 안 지워졌다** — 매장 5
그 FK 도 RESTRICT 다. cascade 가 상품 9건을 지운 뒤 `DELETE FROM sellers` 가 그대로 던졌고,
`safeAdminError` 가 "Internal server error" 로 덮어 원인이 안 보였다.
⇒ **매장 5는 상품만 지워진 채 남았다(부분 적용).** 500 은 "우리가 모른다"는 뜻이라 다시 누르면 안 된다.

### 수리 (별도 PR)
- `donations`·`voucher_orders` 를 머니 blocker 에 추가 — **`if (!cascade)` 분기 밖**(cascade 로도 못 지운다)
- `seller_business_info` 를 함께 삭제(매장 없이는 의미 없는 그 매장의 사업자등록 정보)
- 마지막 `DELETE FROM sellers` 를 **삼키지 않는다** — 남은 참조가 있으면 500 대신 409 + 무엇이 막았는지
- 🩸 **주입 검증이 내 가드가 헛돌고 있다고 잡았다**: cascade 순서 검사가 `indexOf(money) < gate` 만 봐서
  **문자열이 없으면 -1 이라 그 조건이 참**이었다 — push 줄을 통째로 지워도 초록. 기존 4항목
  (주문·이용권·정산·원장)도 같은 구멍이었다. `toBeGreaterThan(0)` 을 먼저 보도록 수리.

### 🧭 다음 세션이 가져갈 것
- **"돈이 안 걸렸다"를 다섯 테이블로 판정하지 마라.** `sqlite_master` 에서 `REFERENCES sellers` 를
  먼저 뽑으면 16개가 나온다. 그 목록으로 세라 — 나는 안 그래서 두 개를 놓쳤다.
- **FK 의 `ON DELETE` 절을 같이 봐라.** RESTRICT 는 시끄럽게 막고(500), CASCADE 는 **조용히 지운다**.
  위험한 쪽은 후자다.
- **매장 3은 지우지 않았다** — 후원 2건(₩600) + 교환권 발송 3건이 붙어 있다. 대표 지시(*"홍대돈까스만
  남기고 전부 삭제"*)는 내가 보고한 *"돈 0"* 전제 위에서 나온 것이고 그 전제가 틀렸으므로,
  **되돌릴 수 없는 삭제를 그 상태로 진행하지 않고 대표 판단을 남겼다.** 이미 `suspended` 라 노출은 0.

---

## 🔴 2026-09-06 후속 — 매장 3도 삭제 (대표 재지시 *"응 매장 3도 지워줘"*)

내가 *"후원 2건(₩600)+교환권 발송 3건이 붙어 있다"* 고 보고한 **뒤에** 나온 지시다.
전제를 정정해 알린 상태에서의 재확인이므로 진행한다.

### 무엇을 지우는가 (실측)
```
donations 2건 · ₩600  — 둘 다 payment_key NULL(딜 후원) · donor_user_id 3 → seller 3 ·
                        live_stream_id 20. 대표 본인이 본인 테스트 매장에 라이브 방송에서 보낸 것.
                        (라이브커머스는 영구 중단된 기능이다.)
voucher_orders 3건    — KT-Alpha 아메리카노 1,800원 ×3 (sent 2 · failed 1), 2026-05-26.
                        이건 KT 로 실제로 나간 돈이다.
```

### 어떻게 지우는가 — `?cascade=1&purge_ancillary=1`
프로덕션 raw SQL 은 레포 룰상 금지다(재현·리뷰가 안 된다). **코드 경로**로 만들었다:

- 덮는 것은 **후원·교환권 발송 둘뿐**. 주문·주문항목·이용권·정산·원장은 **이 플래그로도 못 지운다**
  (그 다섯의 `blockers.push` 는 `if (!purgeAncillary)` 블록 **밖**에 있다).
- `cascade` 없이는 안 먹는다 — 문을 둘 다 열어야 한다.
- 🔴 **지우기 전에 행 전문을 감사 로그에 박제한다**(개수가 아니라 내용 — 금액·상대·외부 주문번호까지).
  박제에 실패하면 **삭제를 중단**한다("사본 없이 지웠다"가 되면 안 된다).
- `voucher_orders` 는 FK 가 CASCADE 라 어차피 함께 사라지지만 **명시적으로** 지운다 —
  *조용히 사라지는 것*과 *지우기로 하고 지운 것*은 다른 일이고, 그 차이가 이 플래그의 존재 이유다.

### 🩸 주입 검증이 또 잡았다 (오늘 두 번째)
"주문·정산이 이 플래그로 함께 풀린다" 주입에 **초록불**이 떴다. 내 단언이 blocker 의 **위치**만 봐서,
같은 줄에 `&& !purgeAncillary` 를 끼워 넣으면 자리는 그대로인데 조건은 풀렸다.
⇒ **줄 자체**를 읽어 `purgeAncillary` 가 안 끼어들었는지까지 본다.
> 교훈: *"블록 밖에 있다"* 는 **위치**의 성질이고, *"그 조건으로 안 풀린다"* 는 **논리**의 성질이다.
> 위치로 논리를 대신 검사하면 한 줄 안에서 뚫린다.

### ✅ 실행 결과 (2026-09-06 12:46 KST)

```
매장 11곳 → 1곳 (14 홍대돈까스)          고아 상품 0 · 남은 부수 머니 행 0
```

세 게이트가 전부 설계대로 동작하는 것을 **실측으로** 확인했다:
| 요청 | 결과 |
|---|---|
| `?cascade=1` (플래그 없이) | **409** — "후원 2건 · 교환권 발송 3건" |
| `?purge_ancillary=1` (cascade 없이) | **409** — 안 먹는다 |
| `?cascade=1&purge_ancillary=1` | **200** — `ancillary_purged: {donations:2, voucher_orders:3}` |

**박제 확인** — `admin_audit_logs` id 2762 의 `before_value.ancillary_money` 에 행 전문이 남았다:
후원 2건(₩500 `DEAL-DON-3-20-…` / ₩100 `TEAM-DON-3-20-…`, 시각 포함) ·
KT 교환권 3건(1,800원×3, `external_order_id` `20260527669410`/`20260527669407`/`ur-cons-85-118-1-…`, sent 2·failed 1).
⇒ **행은 사라졌지만 기록은 남았다.** 이 삭제에서 감사 로그가 유일한 사본이므로 개수가 아니라 내용을 남긴 것이 핵심이다.

### ⚠️ 아직 안 끝난 것 하나
**중개사 계정 403 확인** — 실제 카카오 로그인이 필요해 세션이 못 한다. user 3(대표)이 홍대돈까스의
운영자로 등록돼 있으므로 3단계면 된다: 카카오 로그인 → 매장 전환에서 홍대돈까스 → 셀러 설정에서
정산계좌 변경 시도 → *"매장 소유자만 변경할 수 있습니다"* 가 뜨면 통과.

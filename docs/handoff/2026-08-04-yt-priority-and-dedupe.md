...
## ✅ 라이브 판정 완료 (PR #1069 → `b7840ee`, KST 08-05 00:42)

**두 회차 유형을 배포 전후로 대조했다.** 회차 유형(`led`)마다 동작이 다르므로 **한 유형만 보면 오판한다** —
실제로 배포 직후 첫 확인이 `front` 회차라 "8% 개선"으로 끝날 뻔했다.

| 회차 유형 | 배포 전 | 배포 후 | 회차당 기대 이메일 |
|---|---|---|---|
| `led: naver` | **yt 0** · naver 22 · spent 44 | **yt 20** · naver 11 · spent 44 | 4.66 → **7.67 (+65%)** |
| `led: front` | yt 14 · naver 14 · spent 44 | yt 20 · naver 9 · spent 43 | 6.71 → **7.25 (+8%)** |

*(수율은 같은 날 코호트 YT 26.7% / 블로거 21.2% 적용)*

⇒ **두 유형 평균 5.69 → 7.46, 약 +31%.** 예산은 여전히 44/45 로 안 남긴다.

🔑 **효과의 대부분은 `naver` 회차에서 나왔다** — 거기가 `yt: 0` 이었기 때문이다.
즉 **비율(0.35→0.55)이 아니라 예약분이 진짜 처방이었다.** 비율만 올렸으면 회차의 절반에서
아무 일도 안 일어나 전체 개선은 +8% 수준에 그쳤을 것이다.

...
### ✅ 판정 완료 — 시간당 5xx 1건의 정체 (2026-08-03, 계측 1시간 만에)

경로별 계수를 붙이자마자 24시간 유일한 경로가 나왔다: **`/api/_healthcheck/cron`**.

**고장이 아니다.** 그건 cron 침묵을 알리는 **dead-man's switch** 라 침묵이 있으면
설계대로 503 을 낸다(`health.ok ? 200 : 503`). 외부 프로브(`uptime.yml`)가 10분마다 두드리니
세는 순간 **5xx 채널이 영구 점유**되고, 그러면 진짜 5xx 가 와도 구분이 안 된다 —
이 시리즈가 고친 거짓 경보와 **정확히 같은 클래스**다.

⇒ 그 503 만 5xx 계수에서 면제했다(경로+503 한정. 같은 경로의 500 과 다른 헬스체크의 5xx 는 그대로).
침묵 자체는 `uptime.yml` + 자가진단 cron 침묵 항목이 **각자 채널로** 이미 보고하므로 잃는 정보가 없다.

### 🔑 그 503 이 가리키던 사실은 진짜였다 — **레인 침묵의 원인 사슬**

라이브 실측(읽기 전용):

```
ads_lanes_learned = {"cap":2, "clean":0, "pinned":1}      ← AIMD 학습기의 바닥값
레인 34개 ÷ 회차당 2개 = 한 바퀴 17시간  ⇒ 뒤쪽 도메인은 며칠씩 밀린다
```

`cap:2` 는 `MIN_LANES_PER_TICK`. 부모 회차가 CPU 로 계속 해를 입어 바닥까지 물러난 상태이고
(`clean:0`), 그 회차를 죽인 게 바로 **67s·60s·31s 짜리 레인들** — 이 시리즈가 마감선을 넣은 그것들이다.

> 🎯 **그래서 다음 세션의 판정선은 하트비트 `ms` 가 아니라 이 숫자다.**
> 학습기는 자가치유형이라(깨끗한 회차 2번 → cap +1) 마감선이 먹히면 **자동으로 올라간다**:
> ```sql
> SELECT value FROM platform_settings WHERE key='ads_lanes_learned';   -- cap 이 2에서 오르는가
> ```
> `stale` 목록(`/api/_healthcheck/cron`)이 줄어드는지도 같이 본다. cap 이 안 오르면 마감선이
> 부족한 것이거나 원인이 부하가 아니다(`pinned` 이 6에 닿으면 학습기가 스스로 한 칸 올려 탐색한다).

### 🕳️ 같이 발견 — **판정 대상에서 빠져 있던 레인** (같은 세션에 수리)

`missing = ["ads:collect-maker", "ads:sweep-kakao-chain"]` 을 "안 돌았다"로 읽으면 틀린다.
`missing` 은 **"주기를 몰라 판정할 수 없다"** 는 뜻이고, `ok` 를 **깨지 않는다.**

원인: 하트비트를 두 번 쓴다 — 부모(`adsBeat`, `cron` 실음)와 레인 자신(`writeSelfBeat`).
자기 하트비트는 **설계상 cron 을 일부러 안 싣고**(레인은 자기 주기를 모른다) 부모가 넘긴 `gap` 만 믿는데,
**자식 쓰기가 나중이라 부모의 `cron` 을 덮는다.** `kick` 이 `gap` 없이 부른 매시간 레인은
`{"at":…,"ok":true,"ms":17004}` — 둘 다 없어 **stale 검사에서 통째로 빠진다.**

`ads:sweep-kakao-chain` 은 **멀쩡히 돌고 있었다**(17초, 매시간). *"안 도는 것"* 이 아니라
*"판정 대상이 아닌 것"* 이었고, 둘은 화면에서 똑같이 생겼다.
⇒ `kick` 의 `gapMin` 기본값을 `hourlyGapMinutes()`(150분 = `expectedMaxAgeMinutes('0 * * * *')`)로.
게이트 레인은 자기 주기를 명시로 넘기므로 이 기본값에 안 닿는다(일 1회 오경보 위험 없음).

⚠️ `ads:collect-maker` 는 게이트 `SUPPLY_MAKER_COLLECT_ENABLED` 가 OFF 라 **원래 안 뜬다** —
결함이 아니라 스위치다. 대표가 켜기 전까지 이 이름이 목록에 있는 것은 정상.

### 🔴 판정 실패 — **cap 은 안 올랐고, 이유가 예상과 달랐다** (2026-08-03 14:20 KST)

마감선 4PR + `#1007`(제어 반전) 배포 후 회차 3번(12·13·14시)이 지났다. 결과:

| | |
|---|---|
| `ads_lanes_learned` | `{"cap":2,"clean":0,"pinned":1}` — **한 글자도 안 변함** |
| 배포 이후 `cron_failures` | **0건** (직전 24시간은 25건 전부 CPU 한도) |
| `ads_dispatch_last` | 14:00 까지 **정상 기록** |
| `ads_tick_history` | **09:00 KST 에서 멈춤** — 그 뒤 한 줄도 안 늘었다 |

**"회차가 여전히 해로워서 cap 이 안 오른 것"이 아니다. 판정기 자체가 안 돈다.**

`writeTickSummary`(`tick-history-write.ts:18`)가 이력과 **학습기 상태를 같은 배치에서** 쓴다.
그 함수는 부모 꼬리에 있다:

```ts
ctx.waitUntil(Promise.allSettled(kicked).then(async () => {
  await beats.flush()
  await writeTickSummary(...)      // ← cap/clean/pinned 이 갱신되는 유일한 자리
}))
```

**띄운 레인이 전부 끝나기를 기다린다.** 레인 하나가 53~67초를 쓰면 부모가 그때까지 살아 있어야 하는데,
못 버티면 isolate 가 죽고 `flush()` 도 `writeTickSummary` 도 **실행되지 않는다.**
`cron_failures` 가 0인 건 고쳐져서가 아니라 **예외 없이 잘려서**다 — 이 레포가 반복해 만난 "침묵" 그 자체다.
증거: 14:00 회차에 레인 자기 하트비트 4건은 남았는데 **부모가 쓰는 `ads:scheduled` 는 없다**(13:00 에는 있다).

> 🔒 **자가치유가 구조적으로 막혀 있다.** 다음에 그 꼬리가 어쩌다 실행돼도
> `missedTicks(prevAt=09:00, now)` 가 커서 **그 회차를 '해로웠다'로 판정**한다(#983). ⇒ 물러나거나 바닥 유지.
> 즉 **한 번 뒤처지면 스스로는 절대 못 올라온다.** 마감선이 아무리 잘 들어도 이 자리를 안 고치면 cap 은 2에 남는다.

**⇒ 다음 세션의 첫 액션은 cap 재측정이 아니라 이 자리다.** 후보 두 가지(어느 쪽도 아직 검증 안 됨):
1. 학습기 갱신을 `Promise.allSettled(kicked)` **뒤에서 떼어낸다** — 디스패치 직후(레인 완료와 무관)에
   쓰면 부모 수명에 안 묶인다. 단, 그 시점엔 레인 성패를 모르니 `tick` 요약의 의미가 달라진다.
2. `missedTicks` 를 **회복 판정에서만** 무해하게 본다 — 빈자리는 '피해'가 아니라 '모름'이다.
   지금은 모름을 피해로 읽어 하강 방향으로만 작동한다.

⚠️ **`tick-history-write.ts`·`lane-aimd.ts` 는 오늘 다른 세션(`bbbe46c35`)이 만지는 중이다.**
그래서 이 세션은 **고치지 않고 기록만 했다.** 착수 전 `git log -1 --date=short -- <file>` 로 재확인할 것.

⚠️ **내가 세워 둔 판정표가 이 경우를 이미 예고했다** — *"cap=2, clean=0 + 실패 0 → 학습기가 clean 을
못 세는 것이다. 그건 코드 문제."* 실제로 그 가지였다. **실패 0건을 '좋아졌다'로 읽지 말 것** —
기록이 안 남는 것과 실패가 없는 것은 화면에서 똑같이 생긴다.

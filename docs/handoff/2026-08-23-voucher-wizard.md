...
## 14차 (2026-08-26 — 매장 등록 상시 진입점 + 당근식 등록 폼)

대표: *"회색 핀이 의미가 있나? 어차피 카카오맵으로 검색하는 거잖아"* · *"OCR은 돈이 들어?
내가 보내줬던 당근마켓 플로우 정도로 하자. 나머지는 다 해보자."*

### 🩸 내가 틀렸던 것 — 회색 핀은 본체가 아니다
13차에서 클레임 진입점을 **지도 회색 핀 하나**에만 달았다. 대표 지적이 맞았다:
- 회색 핀이든 컬러 핀이든 **어차피 카카오맵 데이터**다. 사장님이 자기 가게를 찾으려고 우리 지도를
  뒤질 이유가 없다 — 우연히 자기 핀을 눌러야만 발견된다.
- 이미 등록된(컬러 핀) 가게의 진짜 사장님에겐 여전히 길이 없다.
- 당근도 **핀을 누르게 하지 않는다** — 이름을 입력하면 찾아 주고 "이 업체가 맞나요?"(시안 04).
  그 검색은 우리도 이미 갖고 있다(`StoreRegisterModal` 의 카카오맵 검색). **없던 건 그 검색으로
  가는 상시 문**이었다.

### 한 것
- **`/store/new`(`StoreClaimPage`) 신설 = 단일 목적지.** 마이페이지 알약 · 푸터 "내 가게 등록하기" ·
  지도 회색 핀이 전부 여기로 온다(문구가 갈려도 목적지는 하나).
  ⚠️ 5탭 IA 는 대표 확정이라 **BottomNav ➕ 탭은 되살리지 않았다**.
- **셀러 가입(`/seller/register/supplier`)을 먼저 안 거친다.** `POST /stores` 가 매장 행 + 운영 권한을
  함께 만들므로 소비자 계정이 바로 매장 주인이 된다 — 대표 확정 "매장 등록이 무조건 선행"과 같은 순서.
  마이페이지 알약 목적지를 가입 폼 → `/store/new` 로 바꿨다.
- **`enterStoreSeat()` SSOT**(`utils/enter-store.ts`) — 좌석 전환이 세 곳에서 필요해졌다(대시보드 패널·
  지도 클레임·등록 페이지). 손으로 세 번 쓰면 갈리고, 갈리면 **한 경로에서만 좌석이 안 잡히는**
  막다른 길이 생긴다(13차의 지도 경로가 실제로 그랬다 — 토스트만 띄우고 지도에 떨궈 놨다).
- **등록 폼 = 당근 플로우**: 대표자명·개업일 타이핑 제거 → **사업자등록증 사진 1장 필수**
  (`POST /upload/business-cert` 는 이미 있었다). OCR·AI 바인딩 **불필요** — 당근도 사람이 심사한다.

### ⚠️ 계약 변경 — 자동 승인이 없어졌다
종전: 사업자번호가 '계속사업자'면 **즉시 approved**. → 인터넷에 공개된 아무 번호나 통과했고
실제로 그렇게 승인된 매장이 라이브에 있다(id=14 홍대돈까스, `nts_checked=1`). 번호와 그 매장의
연결은 아무도 확인하지 않았다.
지금: **항상 `pending`** → 어드민 승인(`AdminPage.approveSeller`). 국세청 조회는 계속 돌지만
**승인 근거가 아니라 심사 재료**(meta 스탬프)다.
⇒ **AB 테스트 순서가 바뀐다**: 등록 → (대표가 어드민에서 승인) → 이용권 등록.
되돌리려면 `seller-stores.routes.ts` 의 `const status = 'pending'` 을 옛 `autoApprove` 분기로 환원.

### (14차 후속) 대표 재지적 2건 — 회색 핀 제거 · "쇼핑몰 열기" 폐기

- **회색 핀 클레임 버튼 제거.** 대표가 두 번 물었고, 다시 보니 뺴는 게 맞다: 그 모달을 여는 사람은
  사실상 **전부 손님**이라 사장님 CTA 는 노이즈고, 사장님이 자기 가게를 찾으려고 이 지도를 뒤질
  이유가 없다(우연히 자기 핀을 눌러야만 발견). `/store/new` 에 카카오맵 검색이 있으니 프리필 이득도 작다.
  ⚠️ **왜 뺐는지 그 파일 주석에 남겼다** — 안 남기면 다음 세션이 "여기 클레임이 없네" 하고 되살린다.

- 🏷️ **"내 쇼핑몰 열기" 는 거짓말이다 — 7곳 전부 교체.** 유어샵은 **가입하면 이미 있다**
  (모든 유저에게 `/u/{handle}` 자동 생성). "쇼핑몰을 연다"고 하면 이미 가진 걸 또 만들라는 소리가 된다.
  여기서 새로 만드는 건 **매장**이다. 마이 알약 · 마이 역할카드(목적지도 `/store/new` 로) ·
  유어샵 오너뷰 CTA · 결제완료 넛지 · 방문자 레일 · 가입페이지 제목/SEO · 하단바 죽은 시트.
  6개 언어 값 포함.

⇒ **다음 세션 주의**: 이 클래스(유어샵이 이미 있는데 "만든다"고 말하는 문구)는 새 CTA 를 쓸 때마다
  다시 생긴다. 새 문구를 쓰기 전에 "이 사람에게 이게 없나?"를 먼저 물을 것.

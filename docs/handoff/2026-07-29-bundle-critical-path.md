...
## 3-4. 4단계 — 남은 후보 2건 조사 결론 (대표 "남은 거 다" 지시)

같은 덤프 기법으로 둘 다 열어봤고, **둘 다 "지금 하지 않는다"가 결론**이다. 근거를 남긴다.

### ② raw 총량 8.75 / 8.8 MB — **임계값을 올리지도, 무리해서 줄이지도 않았다**

실측 상위: `sentry` 1,375 KB · `charts` 1,353 KB · `firebase` 837 KB (rendered raw) —
**상위 3개만 3.5MB(40%)이고 전부 lazy 청크**다. 즉 **사용자는 이 8.75MB 를 받지 않는다.**

⇒ 이 예산은 **UX 지표가 아니라 성장 지표**다. 실제로 사용자 체감은 `critical path` 예산이,
실제 플랫폼 한도는 `_worker.js` gzip 1MB(main.yml)과 `dist/client` 50MB(**실측 36MB**)가
각각 따로 지킨다. `.map` 파일은 이 합계에 안 들어간다(`.js` 만 필터).

**`firebase` 837 KB 를 뜯지 않은 이유**: 2026-07-28 에 제거된 건 **워커의 Firebase 토큰 수용**이고,
클라이언트는 `useAuthKR`/`useAuthWorld`/`RouteGuards` 에 아직 얽혀 있다(카카오 흐름 안에서
`updateProfile` 등). 인증 코어 + CLAUDE.md 의 **"dual-mode 제거 금지"** 정면 대상이라
번들 PR 끝자락에 끼워 넣을 변경이 아니다. **하려면 단독 PR + GLOBAL 런칭 여부 확인 + staging.**

**대신 한 것**: 이 예산이 터졌을 때 상위 5개를 **`lazy` / `⚠️ CRITICAL` 태그와 함께** 출력하도록
진단을 개선했다. 지금까지 5번 모두 "상향"으로 끝난 이유 중 하나가 메시지가 `8.81 > 8.8` 뿐이라
**판단 재료가 없었기 때문**이다. 이제 "lazy 만 커졌으면 체감은 그대로"라는 사실이 현장에서 보인다.

### ③ `index` 청크 (App.tsx) — **비용 대비 이득 미달**

크리티컬 안의 `app-routes`(셀러/어드민/에이전시/공급자 라우트 트리)가 92.4 KB raw 로 보였지만
**gzip 은 9.4 KB — 크리티컬 216 KB 의 4.3%뿐**이다.

lazy 화하려면 `<Route path="/seller/*" element={<Lazy…/>}>` + 중첩 `<Routes>` 로 **재구조화**해야 한다.
현재 `{SellerRoutes()}` **함수 호출** 형태는 우연이 아니라 **의도된 것**이고 소스에 NOTE 가 있다
(RR6 `createRoutesFromChildren` 가 Fragment+Route 트리를 직접 보게 하려고).
→ 100+ 대시보드 라우트의 경로 매칭을 건드리는 변경인데 이득이 9.4 KB. **하지 않는다.**
정말 필요해지면 단독 PR + 4개 역할 대시보드 라우팅 회귀 테스트가 전제다.

## 4. 남은 결정 / 대기
- **대표 판단 대기**: PR #878 머지 여부(draft).
- **대표 결정이 필요한 것(세션이 임의로 하지 않음)**: `firebase` 837 KB 를 클라이언트에서
  걷어낼지 — GLOBAL 런칭 계획과 묶인 판단이다(§3-4 ② 참조). 그 전까지 raw 예산은 8.8 에서
  언젠가 터지고, 그때 **진단이 lazy/CRITICAL 을 구분해 보여주므로** 반사적 상향 대신 판단이 가능하다.

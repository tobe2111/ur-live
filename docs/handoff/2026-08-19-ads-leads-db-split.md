...
### ⑤ 이사 도구가 두 번 죽었다 — **행 수로 자르면 안 된다**

`--copy` 첫 실행이 `SQLITE_TOOBIG` 로 죽었다. 400행씩 묶었는데 `ad_influencer_leads` 는
행당 약 1 KB 라 한 INSERT 가 **430 KB** 가 됐다. 바이트 기준(80 KB)으로 바꿔도 **또 죽었다.**

실측해 보니 **바이트만의 문제가 아니다**:
```
2컬럼 테이블   50,000자 → OK
약 50컬럼      80,000자 → SQLITE_TOOBIG      ← VALUES × 컬럼 수가 파스 트리를 키운다
              100,000자 → TOOBIG 이 아니라 rate-limit(code 971) 이 먼저 온다
```
⇒ 기본 예산 **30 KB**(`--sql-budget`). 40 KB 도 통과하지만 행 폭 변동을 감당할 여유를 뒀다.

🟢 **실패가 조용하지 않았던 게 컸다** — 첫 배치에서 터져 **아무것도 안 들어갔고**, `--verify` 가
전 테이블 "원본 N / 사본 0" 으로 정확히 보고했다. 반쯤 들어간 채 넘어갔으면 훨씬 나빴다.

### ⑥ 600줄 래칫에 **import 한 줄** 때문에 걸렸다

코드모드가 파일마다 import 를 더했는데 4개 파일이 **정확히 600줄**이었다 → 601줄 → 차단.
`[SKIP_SIZE]` 나 baseline 상향은 그 래칫이 지키려던 선을 무너뜨린다. 대신 각 파일에서
**내용 없는 주석 줄** 하나씩 지웠다(설명 손실 0).
⚠️ 그중 하나는 제목과 `##` 사이 여백이라 문단이 붙어 버려 되돌리고 다른 줄로 바꿨다 —
**줄 수만 맞추면 되는 게 아니라 읽히는 게 남아야 한다.**

### ⑦ 주입 매니페스트가 **낡은 지도**가 됐다 (CI 가 잡음)

`check-guard-mutations` 가 `sweepCompanyHygiene(env.DB…` 를 못 찾는다고 실패했다 —
코드모드가 그 핸들을 바꿨기 때문이다. **가드가 설계대로 작동한 것**이다.
find/replace 두 쌍을 갱신하고 `--only` 로 둘 다 빨간불 확인했다.
> 💡 `--only <부분문자열>` 을 쓰면 332건 전체(30분+) 대신 한 건만 30초에 돌릴 수 있다.
> ⚠️ 전체 실행은 소스를 훼손·복원하므로 그동안 **`git add -A` 금지**(파일 명시 staging).

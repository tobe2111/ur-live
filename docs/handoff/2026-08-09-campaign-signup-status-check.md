...
### 다음 세션 첫 액션
1. ALIGO 키 설정 여부 확인 → 설정됐으면 실발송 1건 + `alimtalk_logs` 행 확인
2. 어드민 `/admin` 알림톡 단가 화면: 원가 저장 → 마진율 표시 · 일괄 지급 1회(같은 참조 재실행 시 no-op)
...
- **worker gzip**: 실한도까지 ~16KB(+유캔사인 제거분). Paid 전환 vs 시드 산문 외부화.

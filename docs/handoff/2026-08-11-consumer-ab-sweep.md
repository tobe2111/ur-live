...
## 4-d. 🔴 모바일 홈 LCP 8.4초 — 홈이 지도가 된 뒤의 사각지대

Web Vitals 를 실제 렌더로 재 봤다. **홈만 압도적으로 느리다**:

```
/                LCP 8,372ms   159req 1,412KB   ← LCP 요소 = 카카오맵 **타일 이미지**
/vouchers        LCP   364ms
/u/jiwon1228     LCP   676ms
/blog            LCP   728ms
/map             LCP 1,412ms
/group-buy/25    LCP 1,448ms
PC 홈            LCP 3,256ms   ← LCP = /api/image/resize (워커 프록시 = 리사이즈 불가, 원본 크기)
```
체인(모바일):
```
FCP                                 2,532ms
dapi.kakao.com/v2/maps/sdk.js   2,843 → 4,440   (1.6초, 전송 0KB)
t1.daumcdn.net/…/kakao.js       4,442 → 5,686   (1.2초)  ← SDK 2단 직렬
지도 타일 첫 장                  5,863 → 7,480
```
**이번 커밋의 조치**: `dapi.kakao.com`·`t1.daumcdn.net`·`mts.daumcdn.net` **preconnect 추가**.
`index.html:340` 의 *"dapi.kakao preconnect 제거 (메인 사용 X)"* 는 2026-05-27 기준이고
**2026-07-29 에 홈이 지도가 되면서 전제가 뒤집혔다**(낡은 지도 클래스). 실제 쓰는 두 호스트는
목록에 아예 없었고, 있던 건 안 쓰는 `map.daumcdn.net` 이었다.

⚠️ **이건 연결 비용만 줄인다. 2단 직렬 자체는 그대로다** — 남은 레버(다음 세션 후보):
ⓐ SDK 로드 시작을 마운트 이후(2.8초)보다 앞당기기 ⓑ 지도 대신 가벼운 정적 프리뷰를 먼저 그리기
ⓒ PC 홈은 `img1.kakaocdn.net` 이 `CDN_CGI_VERIFIED` 에 없어 워커 프록시(리사이즈 불가)로 가는데,
   2026-06-11 에 cdn-cgi 직결이 깨져 되돌린 이력이 있으니 **재실측 후** 판단.

## 4-e. 모바일 홈에 상세 링크 `<a>` 가 0개

PC 홈은 53개인데 모바일 홈(지도)은 **0개**다. 구글은 모바일 우선 색인이라 내부 링크가 0이면
상세 발견을 sitemap 에만 의존하게 된다. 사용자에게도 '새 탭으로 열기 / 링크 복사'가 안 된다.
(치명적이진 않다 — sitemap 이 상세를 제출한다. 하지만 약점으로 남긴다.)

...
4. **🔴 내 ④ 수정 자체에 회귀가 있었다 — 배포 전에 측정해서 잡았다.**
   처음엔 URL **전체**의 `%` 를 `%25` 로 바꿨는데, 라이브 표본 5개로 재보니 카카오 썸네일은
   **쿼리에 서명**을 달고 있어(`…&signature=TKCd…%3D&ts=…`) 그걸 바꾸면 서명이 깨진다:
   ```
   ...
   ```
   ⇒ **경로만** 이스케이프하도록 좁혔다. 교훈: "그 URL 들이 다 같은 모양일 것"이라고 가정하지 말 것 —
   표본 5개를 실제로 두드려 본 것이 회귀를 막았다.
5. **`git add -A` 타이밍** — `audit-gate.sh` 는 `check-guard-mutations` 를 **포함**한다(결함을 실제로
   주입한다). 이 세션에서 그게 도는 동안 커밋할 뻔했다. ⇒ 커밋 전 `pgrep -f check-guard-mutations`.

## 4. 대표 판단 4건 — **전부 승인·반영됨** (2026-08-11 "모두 진행해")

| # | 결정 | 반영 |
|---|---|---|
| ① 데모 결제버튼 오인 유도 | 결제 CTA 를 `결제하기` 로 | 무료 응모(`FcfsApplyBlock`)와 라벨이 갈림. `구매하기` 는 데모에 안 쓰는 대표 뜻 유지 |
| ② 주간 cron 7개 | **살렸다** — 월 09:45 KST(`*/5` + 분 게이트) | `payouts-generate` 는 **지급 목록 생성**일 뿐 송금은 어드민 수동 + 멱등. 죽은 슬롯 **0** |
| ③ `demo-linkshop` 8개 | **내린다**(`is_active=0`, 가역) | 사진 소스가 지도뿐이라 포장식품에 매장 사진이 붙는다 — 규칙이 못 잡는 축 |
| ④ 비한국어 | **언어 전환 UI 를 닫는다** + ko 고정 | `CONSUMER_LANGUAGE_SWITCH_HIDDEN`. 숨김만 하면 이미 en 인 사용자가 **갇히므로** 고정과 한 쌍 |

⚠️ ①에서 **2026-08-08 의 전제가 틀렸다는 것**이 드러났다. 그때 주석은 *"데모는 추첨이라 결제가
아니라 응모다"* 라고 적었는데, `onBuy` → `handleJoin` 은 **실제 결제**로 간다. 그래서 같은 화면에
`응모하기` 가 둘(무료/유료) 생겼다. 8/8 테스트도 옛 문구를 고정하고 있어 함께 갱신했다.

### 남은 대기
- **이용권 실결제 1회** (대표 몫) — baseline: order 88 / product 2847 / ledger 2행.
- **비한국어 289개 실제 번역** — 지금은 문을 닫아 급하지 않다. 글로벌을 다시 열 때 판단.
- `html.lang` 이 항상 `ko` — ④로 ko 고정이 됐으니 현재는 **정합**. 언어를 다시 열면 같이 고칠 것.
...
## 4-f. 🔴 다른 언어로 보면 `[TODO:en]` 표식이 화면에 뜬다

언어 전환은 마이 페이지(`LanguageSection`)에 **실제로 노출**돼 있다. 영어로 바꾸고 `/faq` 를 열면:

```
[TODO:en] 자주 묻는 질문
[TODO:en] 무엇을 도와드릴까요?
```
출처는 `scripts/check-i18n-sync.mjs --fix` 다 — 6개 언어의 **키 동수**를 맞추려고 빠진 키를
ko 값 + 마커로 채운다. 번역자용 신호인데 i18next 가 그대로 렌더한다.
실측 **언어당 289개**(ko 0): `introduce` 65 · `register` 37 · `faq` 36 · `orderDetail` 35 ·
`groupbuy` 28 · `referral` 18 — 대부분 소비자 표면이다.

**이번 조치**: `src/i18n.ts` 에 `stripTodoMarker` postProcessor — 렌더 직전에 **마커만** 벗긴다.
키를 지우면 `check-i18n-sync` 가 깨지므로 키는 그대로 둔다. 결과는 ko 폴백과 같은 화면이고 마커만 사라진다.

⚠️ **이건 번역이 아니다.** 289개를 실제로 번역할지는 대표 판단 — 이 서비스는 한국 전용
(`urdeal.kr` · GLOBAL 폐기 #804)이라 우선순위가 낮을 수 있다. 대안은 **언어 전환 UI 자체를 숨기는 것**.
어느 쪽을 택하든 이 안전판은 유지한다(`--fix` 를 다시 돌리면 마커가 언제든 되살아난다).

## 4-g. 접근성·이미지 — 실측했고 **결함 아님**으로 판정

- **`alt` 없는 이미지 63개(홈)** → 오탐. 21개는 **카카오맵 타일**(SDK 소관), 나머지는 소스에서
  `alt=""` 를 **의도적으로** 준 장식 이미지다(카드에 매장명이 텍스트로 바로 옆에 있다).
  여기에 alt 를 채웠으면 스크린리더가 이름을 두 번 읽는 **개악**이 됐다.
- **과대 이미지** → 오탐. `96px 원본 → 32px 표시` 는 3× DPR 대응이고 `cfImage(width:96)` 로
  의도적으로 요청한 것이다(2026-07-29 브랜드 칩 수정과 동일).
- ⚠️ 다만 `html.lang` 은 언어를 바꿔도 **항상 `ko`** 다. 영어 본문에 `lang="ko"` 는 스크린리더·번역기·
  검색엔진에 잘못된 신호다(이번엔 손대지 않음 — 위 번역 판단과 함께 결정할 사항).

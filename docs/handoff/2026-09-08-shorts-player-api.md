...
## 🌑 추가 — 유어쇼츠 레일 "안 2"(잉크 패널) 구현 (2026-09-08, 같은 PR)

대표 *"지금 유어쇼츠 섹션 부분 바탕도 다른 이용권들 섹션이랑 똑같잖아 … 시안들을 보여줘봐"*
→ 시안 5안×2테마 렌더 → 대표 **"안 2가 좋겠는데?"**.

- 시안·결과·되돌린 판단: `docs/design/urshorts-rail-surface-2026-09.md`
- 가드: `src/tests/unit/urshorts-rail-ink-panel.test.ts` 6건 + 주입 3건(되돌려-검증 빨간불 확인)
- 하네스 보강: `scripts/visual-preview.mjs` 에 `/api/urshorts` 시드 추가.
  **이게 없으면 레일이 `items.length === 0` 으로 통째로 안 그려져** 디자인을 화면으로 볼 방법이
  없었다(2026-08-31 브랜드 스트립과 같은 함정). 라이브의 "이용권 안 붙은 영상"도 시드에 섞어 뒀다.

### 다음 세션 첫 액션
1. **다크에서 이 안이 약한 것은 결함이 아니라 선택**이다(실측 `#11141C` vs `#16181C`).
   대표가 더 세게 원하면 안 1(패널 제거) 또는 안 4 — 코드 변경 전에 확답을 받을 것.
2. `/videos` 가 "시안처럼 안 뜬다"는 신고의 원인 둘(이미 규명):
   ① 구매 바는 배포돼 있으나 `cur.product_id ? … : null` 이라 **상품이 붙은 영상에만** 뜬다.
      라이브 3편 전부 `product_id` null → 어드민에서 이용권을 붙이면 그대로 뜬다.
   ② 자막 끄기·탭 일시정지는 이 PR(#1412)에 있고 아직 미배포.

### ⚠️ 하지 말 것
- `.dark-island` 를 tailwind `darkMode` 배열에 넣지 말 것 — 동작은 하지만 앱 전체 CSS 가
  **+3.9KB gzip** 늘어난다(실측). 레일 한 칸이 낼 값이 아니다.
- 잉크 패널에 `light-island` 을 같이 붙이지 말 것 — 라이트에서 글자가 사라진다(주입이 지킨다).

...
| **섹션 수정 화면** | `home-sections/SectionForm.tsx` — 생성·수정 **같은 폼**(두 벌이면 반드시 갈라진다) |
...
- ~~섹션 수정 화면~~ → **완료**(대표 "없는 것도 다 해줘"). 생성·수정이 `SectionForm` 하나를 공유하고
  payload 빌더도 하나다 — 갈라지면 테스트가 빨강.
- **`layout` 컬럼(grid3/grid2/scroll)은 죽어 있다.** DB·CHECK 제약은 있는데 `HomeSections` 가
  안 읽어서 항상 같은 그리드다. 그래서 **폼에 일부러 노출하지 않았다**(고를 수는 있는데 아무 일도
  안 일어나는 스위치는 없는 것보다 나쁘다). 홈이 실제로 쓰게 만들 때 폼에 함께 추가할 것.

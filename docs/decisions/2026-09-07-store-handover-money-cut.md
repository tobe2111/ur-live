...
**2026-09-14 19:1x KST 대표: *"승계 시점으로 끊는다 (전체 구현)"*** ⇒ **선택지 1, 전체.**
기본안이 권한 "지금은 잠금만"을 **넘어서** 승계 시점 절단을 실제로 구현하라는 뜻이다.

### ⚠️ 이 결정이 실제로 요구하는 것 — 착수 전에 읽을 것

**승계 기능 자체가 아직 없다**(`owner_verified` 레포 0건 · 손바뀜 0회). 그래서 "전체 구현"은
절단 한 줄이 아니라 **둘**이다:
  (a) 승계 흐름을 만든다 — 누가 언제 무슨 근거로 주인을 바꾸는가(사업자번호 대조 · 어드민 승인)
  (b) 그 시점으로 돈을 자른다 — 이전 주인 정산 마감 → 잔액 0 → 새 주인은 그 시점부터

🔴 **머니 경로다.** 그래서 **단독 세션 + staging 실결제**가 붙는다(CLAUDE.md). 다른 작업과
같은 PR 에 섞지 말 것 — 이 결정을 기록한 2026-09-14 세션도 UI 작업 중이라 **구현은 하지 않았다.**

### 착수하는 세션을 위한 좌표 (2026-09-07 실측 그대로)

| 고칠 자리 | 지금 상태 |
|---|---|
| `payouts-generate.ts:44-55` | 집계에 **기간이 없다**(전기간 누적 — 2026-06-26 의 의도된 변경) |
| `ledger.ts:428-434` `getLedgerReceivable` | 무기간 |
| `payouts-generate.ts:85` | 송금 목적지 = `sellers.bank_account` (사람이 바꾸면 그대로 따라간다) |
| `ledger.ts:317-319` | 이용권 **사용** 레일의 소개 커미션이 **소급**된다 — 여기도 같은 절단이 필요 |
| `seller_operators` `granted_at`/`revoked_at` | **이미 기록되고 있다.** 돈 경로가 안 볼 뿐 |

⇒ 시점 값은 새로 만들 필요가 없다. **정산이 그걸 보게 하는 것**이 이 작업의 핵심이다.

### 함께 가야 하는 선행 수리 3건 (위 "함께 발견한 것")
1. `credit_account='seller:null'` 원장 1건 — `group-buy.routes.ts:513` null 가드 부재
2. 정산 조회에 owner 게이트 없음 — `seller-settlements.routes.ts:317`(운영자가 과거 전부 열람)

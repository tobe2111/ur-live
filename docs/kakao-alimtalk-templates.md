...
## ⚠️ 아직 **코드에 없는** 알림톡 (요청하신 "이용권 구매완료·사용")

대표님이 예시로 든 것 중 **현재 코드에 배선이 없는** 항목:
- **이용권/공구 구매완료(구매자에게)** — 구매자 대상 구매완료 알림톡 트리거 없음(구매자는 인앱+웹푸시로만 통보,
  `new_order`는 셀러에게 감). 필요하면 신규 배선 필요.
- **이용권 사용(매장 사용처리 시)** — 사용 완료 알림톡 없음.

→ 이 두 개를 원하시면 별도로 **문안 확정 + 코드 배선**을 진행해야 합니다(말씀 주시면 설계·구현).

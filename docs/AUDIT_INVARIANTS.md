...
| **커미션 재원=promo·5%불가침 [#44 — flip 시 신설, 미구현]** | (2026-07-08 확정 원칙) 판매 커미션(어필리에이트·멀티티어·인플 영입)은 매장 promo(5% 밖, `promo_funding_source=owner`) 재원 → **platform net == 5% 불변**(커미션이 5% 를 안 건드림). 에이전시 1%/24mo 만 한시 마중물로 5% 잔류(예외). 오늘 owner 스위치는 어필리에이트만 커버 → flip 이 나머지 확장. 가드: `commission-budget.ts` flip 플래그로 플랫폼 예산 0 강제 + `commission-budget.test.ts` `platformNet==fee` 단언 + `check-commission-budget` R4(성장 커미션의 `platform:revenue` debit 차단, 에이전시 예외) | `check-commission-budget`(R4 예정) | **미구현** — 8월 promo flip. 설계: `commission-funding-restructure.md` §확정 원칙 |

...
### 3-3. 스크린샷 캡처 (4종 필수 첨부)

KT Alpha 가 요구하는 운영 화면 캡처:

1. **어드민 페이지 전체**: 로그인 후 `https://live.ur-team.com/admin/kt-alpha` 접속 → 전체 페이지 캡처
   - 비즈머니 잔액 카드 + KPI 4종 + 설정 + 카탈로그 미리보기 모두 나오게
2. **셀러 발송 모달**: 셀러 계정으로 `/seller/settlements` → '🎁 상품권으로 받기' 클릭 → 모달 캡처
3. **셀러 발송 이력**: `/seller/voucher-orders` 페이지 캡처 (테스트 발송 1건 이상 있는 상태)
4. **테스트 발송 성공 화면**: 본인 폰으로 dev_flag='Y' 테스트 발송 받은 MMS 캡처

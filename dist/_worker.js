...
`))}catch(o){console.error("[SSE Stock] Polling failed:",o)}},6e4)},cancel(){console.log(`[SSE Stock] Seller ${e} disconnected`),t&&clearInterval(t)}});return new Response(a,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive","X-Accel-Buffering":"no"}})}async function sa(e,s,r,t){await e.prepare(`
  ...
  `;const o=[];s&&(n+=" AND ls.status = ?",o.push(s)),r&&(n+=" AND ls.seller_id = ?",o.push(r)),n+=' ORDER BY CASE ls.status WHEN "active" THEN 1 WHEN "scheduled" THEN 2 ELSE 3 END, ls.created_at DESC',n+=" LIMIT ? OFFSET ?",o.push(parseInt(t),parseInt(a));const{results:i}=await e.prepare(n).bind(...o).all();return i}d.get("/api/live-streams/:id",async e=>{const{DB:s}=e.env,r=e.req.param("id");try{const t=`live_stream:${r}`,a=30,n=ye(t);if(n)return console.log("[LiveStream] ⚡ 메모리 캐시 히트:",t),e.executionCtx.waitUntil((async()=>{try{console.log("[LiveStream] 🔄 백그라운드 갱신 시작:",t);const i=await Us(s,r);i&&(z(t,i,a),console.log("[LiveStream] ✅ 백그라운드 갱신 완료:",t))}catch(i){console.error("[LiveStream] ❌ 백그라운드 갱신 실패:",i)}})()),e.json({success:!0,data:n});console.log("[LiveStream] 💾 DB 조회:",t);const o=await Us(s,r);return o?(z(t,o,a),e.json({success:!0,data:o})):e.json({success:!1,error:"Stream not found"},404)}catch(t){return e.json({success:!1,error:t.message},500)}});async function Us(e,s){return await e.prepare(`
    ...
    LEFT JOIN products p ON ls.current_product_id = p.id
    WHERE ls.id = ?
  `).bind(s).first()}d.get("/api/products",async e=>{const{DB:s,CACHE_KV:r}=e.env;try{const t=e.req.query("featured"),a=parseInt(e.req.query("limit")||"20"),n=parseInt(e.req.query("offset")||"0"),o=`products:list:${t||"all"}:${a}:${n}`,i=ye(o);if(i)return e.executionCtx.waitUntil((async()=>{try{const u=await Ps(s,t,a,n);z(o,u,3600),await Ge(r,o,u,300)}catch(u){console.error("[Cache Revalidate] Products error:",u)}})()),e.json({success:!0,data:i,cached:!0});const c=await Ps(s,t,a,n);return z(o,c,3600),await Ge(r,o,c,300),e.json({success:!0,data:c,cached:!1})}catch(t){return console.error("Products list error:",t),e.json({success:!1,error:t.message},500)}});async function Ps(e,s,r,t){let a;return s==="true"?a=`
      SELECT 
        p.id,
        p.name,
        p.description,
    ...
    `:a=`
      ...
      ORDER BY sold_count DESC, p.created_at DESC
      LIMIT ? OFFSET ?
    `,(await e.prepare(a).bind(r,t).all()).results||[]}d.get("/api/products/popular",async e=>{const{DB:s,CACHE_KV:r}=e.env;try{const t="products:popular",a=ye(t);if(a)return e.executionCtx.waitUntil((async()=>{try{const o=await $s(s);z(t,o,3600),await Ge(r,t,o,600)}catch(o){console.error("[Cache Revalidate] Popular products error:",o)}})()),e.json({success:!0,data:a,cached:!0});const n=await $s(s);return z(t,n,3600),await Ge(r,t,n,600),e.json({success:!0,data:n,cached:!1})}catch(t){return console.error("Popular products error:",t),e.json({success:!1,error:t.message},500)}});async function $s(e){return(await e.prepare(`
    SELECT 
      p.id,
      p.name,
      p.description,
  ...
  `).bind(s).first();if(!r)return null;const t=await e.prepare("SELECT * FROM product_options WHERE product_id = ?").bind(s).all();return{product:r,options:t.results}}d.get("/api/products/:id/stock",async e=>{const{DB:s}=e.env,r=e.req.param("id");try{const t=await s.prepare("SELECT id, name, stock FROM products WHERE id = ? AND is_active = 1").bind(r).first();return t?e.json({success:!0,data:{productId:t.id,productName:t.name,stock:t.stock,available:t.stock>0}}):e.json({success:!1,error:"Product not found"},404)}catch(t){return e.json({success:!1,error:t.message},500)}});d.get("/api/streams/:streamId/products",async e=>{const{DB:s}=e.env,r=e.req.param("streamId");try{const t=await s.prepare(`
    ...
    `).bind(m,n,o||null,o||null).first();let h;if(f){const g=f.quantity+i;await s.prepare(`
      ...
      `).bind(g,c,f.id).run(),h=f.id}else h=(await s.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).bind(m,n,o||null,i,c,u||null).run()).meta.last_row_id;return e.json({success:!0,data:{id:h,isUpdate:!!f}})}catch(r){return console.error("[API /api/cart POST] Error:",r),console.error("[API /api/cart POST] Error message:",r.message),console.error("[API /api/cart POST] Error stack:",r.stack),e.json({success:!1,error:"Failed to add to cart: "+(r.message||"Unknown error")},500)}});d.delete("/api/cart/:cartItemId",k,async e=>{const{DB:s}=e.env,r=e.req.param("cartItemId");try{return await s.prepare("DELETE FROM cart_items WHERE id = ?").bind(r).run(),e.json({success:!0})}catch(t){return e.json({success:!1,error:t.message},500)}});d.delete("/api/cart/clear/:userId",k,ua("cart"),async e=>{const{DB:s}=e.env,r=e.req.param("userId");try{return await s.prepare("DELETE FROM cart_items WHERE user_id = ?").bind(r).run(),e.json({success:!0,message:"장바구니가 비워졌습니다."})}catch(t){return e.json({success:!1,error:t.message},500)}});d.put("/api/cart/:cartItemId",k,async e=>{const{DB:s}=e.env,r=e.req.param("cartItemId");try{const t=await e.req.json(),{quantity:a}=t;if(!a||a<1)return e.json({success:!1,error:"Invalid quantity"},400);const n=await s.prepare(`
    ...
    `).bind(r).first();return n?n.stock<a?e.json({success:!1,error:"Insufficient stock"},400):(await s.prepare("UPDATE cart_items SET quantity = ? WHERE id = ?").bind(a,r).run(),e.json({success:!0})):e.json({success:!1,error:"Cart item not found"},404)}catch(t){return e.json({success:!1,error:t.message},500)}});d.post("/api/orders",k,async e=>{const{DB:s}=e.env;try{const r=await e.req.json(),{userId:t,cartItemIds:a,shippingInfo:n,items:o,shippingAddress:i,shippingAddressDetail:c,recipientName:u,recipientPhone:l,deliveryMemo:p,totalAmount:m,shippingFee:E,orderNumber:f,paymentKey:h,paymentMethod:g}=r;if(o&&o.length>0){const D=o.map(B=>B.productId),M=D.map(()=>"?").join(","),$=await s.prepare(`
        ...
        FROM products 
        WHERE id IN (${M})
      `).bind(...D).all(),C=new Map($.results.map(B=>[B.id,B])),q=[];for(const B of o){const Ae=C.get(B.productId);if(!Ae)return e.json({success:!1,error:`상품을 찾을 수 없습니다 (ID: ${B.productId})`},400);if(Ae.stock<B.quantity)return e.json({success:!1,error:`재고 부족: ${Ae.name} (남은 재고: ${Ae.stock}개)`},400);q.push({product_id:B.productId,option_id:B.optionId||null,quantity:B.quantity,price:B.price,product_name:Ae.name,product_stock:Ae.stock})}const V=new Date,Q=V.getFullYear().toString().slice(-2),R=(V.getMonth()+1).toString().padStart(2,"0"),we=V.getDate().toString().padStart(2,"0"),x=`${Q}${R}${we}`,F=Math.random().toString(36).substring(2,7).toUpperCase(),pe=f||`ORD-${x}-${F}`,oe=c?`${i} ${c}`:i,Ke=(await s.prepare(`
        INSERT INTO orders (
      ...
      JOIN products p ON ci.product_id = p.id
      WHERE ci.id IN (${w})
    `).bind(...a).all();if(y.results.length===0)return e.json({success:!1,error:"No items found"},400);for(const D of y.results)if(D.product_stock<D.quantity)return e.json({success:!1,error:`Insufficient stock for ${D.product_name}`},400);const A=y.results.reduce((D,M)=>D+M.price_snapshot*M.quantity,0),O=`ORD${Date.now()}${Math.floor(Math.random()*1e3)}`,P=(await s.prepare(`
      INSERT INTO orders (
        order_number, user_id, total_amount,
        shipping_address, shipping_name, shipping_phone
      ) VALUES (?, ?, ?, ?, ?, ?)
    `).bind(O,t,A,n.address,n.name,n.phone).run()).meta.last_row_id,j=[];for(const D of y.results){let M=!1,$="";for(let C=0;C<3;C++){if((await s.prepare(`
          UPDATE products 
          SET stock = stock - ?, 
              version = version + 1,
              updated_at = datetime('now')
  ...
  `).bind(t,JSON.stringify(s),a).run(),(t==="DONE"||t==="completed")&&(await e.prepare(`
      ...
      SET payment_status = 'approved',
          ...
          updated_at = CURRENT_TIMESTAMP
      WHERE order_number = ?
    `).bind(r).run(),console.log("[Webhook] ✅ 가상계좌 입금 완료 처리:",r))}async function wa(e,s){const{orderId:r,virtualAccount:t}=s;console.log("[Webhook] 가상계좌 발급:",{orderId:r,bank:t==null?void 0:t.bank,accountNumber:t==null?void 0:t.accountNumber}),await e.prepare(`
    UPDATE payments 
...
`),E=new Date,f=`orders_${E.toISOString().split("T")[0]}_${E.getTime()}.csv`;return new Response(m,{headers:{"Content-Type":"text/csv; charset=utf-8","Content-Disposition":`attachment; filename="${encodeURIComponent(f)}"`,"Cache-Control":"no-cache"}})}else return e.json({success:!1,error:"Unsupported format"},400)}catch(t){return console.error("Export error:",t),e.json({success:!1,error:t.message},500)}});d.patch("/api/seller/orders/:orderNumber/status",async e=>{const{DB:s}=e.env,r=await v(e);if(!r.success)return e.json({success:!1,error:r.error},401);try{const t=e.req.param("orderNumber"),{status:a}=await e.req.json();if(!["PAY_COMPLETE","PREPARING","SHIPPING","DELIVERED","CANCELLED"].includes(a))return e.json({success:!1,error:"Invalid status"},400);const o=await s.prepare("SELECT id FROM orders WHERE order_number = ?").bind(t).first();if(!o)return e.json({success:!1,error:"Order not found"},404);if(!await s.prepare("SELECT id FROM order_items WHERE order_id = ? AND seller_id = ?").bind(o.id,r.sellerId).first())return e.json({success:!1,error:"Unauthorized"},403);if(await s.prepare("UPDATE orders SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE order_number = ?").bind(a,t).run(),a==="DELIVERED")try{console.log(`[AUTO TAX INVOICE] 배송완료 감지: ${t}, 자동 발행 시작...`);const c=await s.prepare(`
        ...
        `).bind(t).first();if(c!=null&&c.buyer_business_number&&(c!=null&&c.buyer_business_name)){console.log(`[AUTO TAX INVOICE] 사업자 구매 확인: ${c.buyer_business_number}`);const u=await s.prepare("SELECT * FROM seller_business_info WHERE seller_id = ? AND is_verified = 1").bind(r.sellerId).first();if(!u)console.warn(`[AUTO TAX INVOICE] 판매자 사업자 정보 미승인: seller_id=${r.sellerId}`),await s.prepare(`
            ...
            `).bind(r.sellerId,t,g,u.business_number,u.business_name,u.ceo_name,u.address||"",u.business_type||"",u.business_category||"",u.email||"",u.phone||"",c.buyer_business_number,c.buyer_business_name,c.buyer_ceo_name||"",c.buyer_business_address||"",c.buyer_business_type||"",c.buyer_business_category||"",c.buyer_email||"",c.buyer_phone||"",m,E,p,`AUTO-${Date.now()}-${h}`).run()).meta.last_row_id;if(l.results.length>0){const A=l.results.map(O=>{const L=Math.floor(Number(O.price)*Number(O.quantity)/1.1),P=Number(O.price)*Number(O.quantity)-L;return s.prepare(`
                  INSERT INTO tax_invoice_items (
                    tax_invoice_id, product_name, quantity, unit_price,
                    supply_price, tax_amount, description, created_at
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                `).bind(y,O.product_name||"상품명 없음",O.quantity,O.price,L,P,O.option_name||"")});await s.batch(A)}await s.prepare(`
  ...
  `,{results:a}=await e.prepare(t).bind(s,r).all();return a}d.get("/api/admin/sellers",async e=>{const{DB:s}=e.env,r=await U(e);if(!r.success)return e.json({success:!1,error:r.error},401);try{const t=await s.prepare(`
    ...
    `).all();return e.json({success:!0,data:t.results,count:t.results.length})}catch(t){return e.json({success:!1,error:t.message},500)}});d.get("/api/public/seller/:sellerId",async e=>{const{DB:s,CACHE_KV:r}=e.env;try{const t=e.req.param("sellerId"),a=`public:seller:${t}`,n=await la(r,a);if(n)return e.json({success:!0,data:n,cached:!0});const o=await s.prepare(`
      SELECT 
    ...
    `).bind(t).all(),l=await s.prepare(`
      ...
      WHERE s.id = ?
    `).bind(t).first(),p={profile:o,live_streams:i.results,scheduled_streams:c.results,products:u.results,stats:l};return await Ge(r,a,p,60),e.json({success:!0,data:p})}catch(t){return console.error("셀러 프로필 조회 실패:",t),e.json({success:!1,error:t.message},500)}});d.get("/api/public/seller/username/:username",async e=>{const{DB:s}=e.env;try{const r=e.req.param("username"),t=await s.prepare(`
...
`});const E="\uFEFF";return new Response(E+m,{headers:{"Content-Type":"text/csv; charset=utf-8","Content-Disposition":`attachment; filename="settlement_${a||"all"}_${Date.now()}.csv"`}})}catch(t){return console.error("CSV 내보내기 실패:",t),e.json({success:!1,error:t.message},500)}});d.post("/api/orders/create",k,async e=>{const{DB:s}=e.env;try{const{userId:r,cartItems:t,totalAmount:a,shippingAddressId:n,sellerId:o,issueTaxInvoice:i,buyerBusinessNumber:c,buyerBusinessName:u,buyerCeoName:l}=await e.req.json();console.log("주문 생성 요청:",{userId:r,cartItems:t==null?void 0:t.length,totalAmount:a,shippingAddressId:n,sellerId:o,issueTaxInvoice:i});let p=10;if(o){const R=await s.prepare(`
      ...
      `).bind(n,r).first();if(!R)return e.json({success:!1,error:"배송지 정보를 찾을 수 없습니다"},400);f=R}if(!r)return e.json({success:!1,error:"User ID is required. Please login with Kakao first."},401);const h=r,g=new Date,w=g.getFullYear().toString().slice(-2),y=(g.getMonth()+1).toString().padStart(2,"0"),A=g.getDate().toString().padStart(2,"0"),O=`${w}${y}${A}`,L=Math.random().toString(36).substring(2,7).toUpperCase(),P=`ORD-${O}-${L}`,j=t.map(R=>R.product_id),D=j.map(()=>"?").join(","),M=await s.prepare(`
...
`}return new Response(l,{headers:{"Content-Type":"text/csv; charset=utf-8","Content-Disposition":`attachment; filename="settlement_${i}_${c}.csv"`}})}catch(s){return console.error("CSV download error:",s),e.json({success:!1,error:s.message},500)}});d.post("/api/seller/tax-invoices/issue",async e=>{const{DB:s}=e.env,r=await v(e);if(!r.success)return e.json({success:!1,error:r.error},401);try{const{order_number:t}=await e.req.json();if(!t)return e.json({success:!1,error:"주문번호는 필수입니다."},400);const a=await s.prepare(`
    ...
    `).bind(r.sellerId,t,"tax",p,l,n.business_number,n.business_name,n.ceo_name,n.address,n.business_type,n.business_category,a.buyer_business_number,a.buyer_business_name,a.buyer_ceo_name,c,u,i,f==="FAILED"?"failed":"issued",Ye()?"mock":"barobill",h,f).run()).meta.last_row_id;for(const y of o.results){const A=Math.floor(Number(y.price)*Number(y.quantity)/1.1),O=Number(y.price)*Number(y.quantity)-A;await s.prepare(`
        INSERT INTO tax_invoice_items (
...
대표전화: 0507-0177-0432 | 이메일: jiwon@ur-team.com
  `;return wr({to:"jiwon@ur-team.com",subject:`[유어 라이브] 🎉 새 라이브 스트림 생성: ${r}`,htmlContent:u,textContent:l})}const Ta=Object.freeze(Object.defineProperty({__proto__:null,sendEmail:wr,sendLiveStreamCreatedEmail:ba},Symbol.toStringTag,{value:"Module"}));export{Hs as default};

...
describe('도매 통합 현황 승인 큐 — wholesale-role 어드민 도달 가능 불변식', () => {
  ...
  it('큐 카드 목적지를 정상 추출했다(샘플 sanity)', () => {
    ...
    expect(dests).toContain('/admin/distributor-approval') // 판매사 승인 (2026-06-25 경로 일원화)
  })
  ...
  it('허용 경로 집합에 핵심 경로가 포함된다(추출 로직 sanity)', () => {
    ...
    expect(allowed.has('/admin/distributor-approval')).toBe(true) // wholesale nav 그룹에 포함 (2026-06-25)
  })

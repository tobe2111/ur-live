...
describe('도매 통합 현황 승인 큐 — wholesale-role 어드민 도달 가능 불변식', () => {
  ...
  it('큐 카드 목적지를 정상 추출했다(샘플 sanity)', () => {
    ...
    // 🔁 2026-06-24 retarget: 판매사 승인 타일이 out-of-scope `/admin/seller-approval` →
    //   in-scope `/admin/distributor-approval`(AdminDistributorApprovalPage) 로 변경됨.
    expect(dests).toContain('/admin/distributor-approval')  // 판매사(distributor) 승인
  })
  ...
  it('허용 경로 집합에 핵심 경로가 포함된다(추출 로직 sanity)', () => {
    ...
    expect(allowed.has('/admin/distributor-approval')).toBe(true)  // 도매 nav 그룹(판매사 승인 — 2026-06-24 retarget)
  })

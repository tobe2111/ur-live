...
describe('🚧 배선 — 스케줄러가 실제로 예산 분산을 쓰는가', () => {
  ...
  it('🔒 커서를 읽고 쓴다 — 안 그러면 매 회차 같은 레인만 돈다', async () => {
    ...
    // ⚠️ 자리표시자 **개수는 고정하지 않는다** — 이 한 번의 왕복에 키가 더 붙을 수 있고(2026-08-06
    //   에 이력 키가 세 번째로 들어왔다), 개수를 박아 두면 *정상적인 확장*이 빨간불이 된다.
    //   지키려는 것은 "커서를 **같은 왕복에서** 읽는다" 이지 키가 몇 개냐가 아니다.
    expect(src).toMatch(/SELECT key, value FROM platform_settings WHERE key IN \(\?(?:, \?)+\)/)
    expect(src, '커서 키가 읽기 바인딩에 없으면 커서는 영원히 초기값이다')
      .toMatch(/\.bind\(DISPATCH_CURSOR_KEY, LANE_LEARN_KEY[^)]*\)/)

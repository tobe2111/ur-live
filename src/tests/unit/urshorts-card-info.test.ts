...
describe('🔒 지키는 계약', () => {
  ...
  it('사진 위 스크림에서는 다크 표면용 할인색을 쓴다', () => {
    ...
    // 🔁 2026-09-21: 같은 값을 전체 보기 카드도 쓰게 되면서 **토큰**으로 올렸다
    //    (`--sale-on-media`). 손으로 박은 hex 로 되돌아가면 두 카드가 갈린다.
    expect(card).toMatch(/text-sale-on-media/)
    // ⚠️ `\b` 는 `text-sale-on-media` 의 하이픈에서도 끊겨 **자기 자신에 걸린다**(실제로 그랬다).
    expect(card, '체계의 라이트 할인색을 쓰면 스크림 위에서 안 읽힌다').not.toMatch(/text-sale(?![-\w])/)
  })

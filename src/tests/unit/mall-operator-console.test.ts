...
// 🩸 2026-09-15: 이 파일은 원문(`readFileSync`)을 읽었다. 그래서 아래 mall_id 검사가 **주석 덕에**
//    통과하고 있었다 — 주입(`UPDATE … WHERE id = ?` 로 mall_id 제거)에도 초록불이었다.
//    `readCode` 는 SSOT 주석 제거기(`stripComments`)를 거친다.
const src = readCode(ROUTES)
...
describe('몰 스코프 격리 (IDOR)', () => {
  ...
  it('mall_notices 를 만지는 모든 SQL 이 mall_id 를 동반한다', () => {
    ...
    //
    // 🩸 처음엔 따옴표/백틱 **짝짓기**로 문장을 잘라냈다(`` /`([^`]*mall_notices[^`]*)`/ `` 류).
    //    한국어 주석의 아포스트로피와 `${sets.join(', ')}` 의 작은따옴표가 짝을 어긋내
    //    **엉뚱한 구간 4개**를 잡았고, 그중 하나가 주석 "🔴 mall_id 를 WHERE 에 함께" 를 품어
    //    실제 UPDATE 에서 mall_id 를 빼도 통과했다. ⇒ 짝짓기 대신 **키워드 앵커 + 유계 창**으로 잰다.
    const touches = [...src.matchAll(/(?:FROM|INTO|UPDATE)\s+mall_notices/g)]
    expect(touches.length).toBeGreaterThan(0) // 측정 대상 0건이면 통과가 아니라 실패
    for (const m of touches) {
      // 그 키워드 뒤 유계 창(문장 하나가 넉넉히 들어가는 길이) 안에 mall_id 가 있어야 한다.
      const win = src.slice(m.index ?? 0, (m.index ?? 0) + 220)
      expect(win.replace(/\s+/g, ' '), `mall_id 미동반 쿼리: ${win.slice(0, 120)}`).toMatch(/mall_id/)
    }
  })

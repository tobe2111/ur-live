...
describe('② 대표 확정 — 없으면 아무것도 안 그린다', () => {
  ...
  it('기본 히어로는 무거운 미디어를 새로 들이지 않는다 (영상 0 · 하드코딩 URL 0 · 리사이즈 · lazy)', () => {
    ...
    // ③ 사진을 쓴다면 반드시 리사이저를 거친다(원본 1MB 직결 금지) — 이게 "무겁지 않게"의 핵심이다.
    if (/<img/.test(def)) {
      expect(def).toMatch(/cfImage\(/)
      /**
       * 🔄 2026-08-19 정정: 여기 원래 `loading="lazy"` 를 요구했다. **그게 틀렸다.**
       *   근거였던 "히어로 카피/검색보다 먼저 받을 이유가 없다" 는 히어로가 장식이던 시절의 말이고,
       *   지금 이 사진은 **첫 화면 최상단 = 사실상 LCP 요소**다. lazy 로 두면 브라우저가 다른 자원을
       *   다 받은 뒤에야 시작해 늦게 나타난다(대표 신고: "히어로에 있는 사진 이미지도 마찬가지고").
       *   ⇒ 즉시 로드로 바꾼다. **무게 보호는 ①영상 금지 ②하드코딩 URL 금지 ③리사이저 필수**가
       *     계속 담당한다 — 리사이즈된 900px 한 장이라 eager 여도 무겁지 않다.
       */
      expect(def).toMatch(/loading="eager"/)
      expect(def).not.toMatch(/loading="lazy"/)
    }
  })

  /**
   * 🖼️ 2026-08-27 — 금지의 **축이 바뀌었다**: "데모냐" → "사진 출처가 우리냐".
   *
   * 원래 규칙은 "데모는 홈 얼굴로 쓰지 않는다"(2026-08-04 타사 워터마크 보도사진 사고의 처방).
   * 그런데 라이브 카탈로그가 100% 데모가 되자 그 규칙이 **히어로를 영구히 빈 색면**으로 만들었다
   * (실측: 시드 50/50 데모 · 어드민 배너 0건 → 사진 소스가 아예 없음. 대표 신고의 실체).
   *
   * 사고의 원인은 *데모라는 사실*이 아니라 **남의 사진**이었다. 그래서 우리 버킷(R2)에서 온 것만
   * 마지막 수단으로 허용한다 — 실상품은 여전히 먼저 이기고, 외부 호스트 사진은 어느 단계에서도 안 온다.
   */
  it('🔴 히어로 사진: 실상품 우선 · 데모는 마지막 수단 · 남의 사진은 절대 안 쓴다', () => {
    ...
    expect(def).toMatch(/demo-deal-/)                 // 데모를 여전히 분간한다(= 뒤로 미룬다)
    // 데모 분기는 **우리 호스트일 때만** 후보로 잡는다 — 이 조건이 사라지면 워터마크 사고가 돌아온다.
    expect(def).toMatch(/isOwnMedia\(img\)/)
    expect(def).toMatch(/\/api\/media\//)
    expect(def).toMatch(/media\\?\.ur-team\\?\.com/)
    // 실상품이 나오면 즉시 반환한다(데모 후보를 들고 계속 도는 게 아니라).
    expect(def).toMatch(/return hit/)
  })

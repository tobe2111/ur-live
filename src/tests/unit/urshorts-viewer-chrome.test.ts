...
describe('🔒 층 위에 남아야 하는 것', () => {
  it('구매 바는 제스처 층보다 위다(z-20)', () => {
    // 🩸 2026-09-13: 여기서 닫기 버튼의 `top-3` 까지 박아 두는 바람에, 유튜브 로고와 겹쳐 자리를
    //   옮기자 이 검사가 빨간불이 됐다. 이 describe 가 지키는 것은 **층 순서(z-20)** 뿐이다.
    //   (그 닫기 버튼은 2026-09-16 대표 지시로 제거됐다 — 아래 describe 가 재등장을 막는다.)
...
describe('❌ 좌상단 닫기 X 는 없다 (2026-09-16 대표 지시 "X표시 없애주고")', () => {
  const V2 = readCode('src/pages/VideosPage.tsx')

  it('닫기 버튼이 다시 생기지 않았다', () => {
    // 🩸 이 자리는 2026-09-13 에 유튜브 Shorts 로고와 겹쳐 한 번 옮겼던 자리다(`top-3` → `top-14`).
    //   2026-09-16 대표가 아예 없애라고 확정했으므로, **자리를 지키는 대신 부재를 지킨다** —
    //   다음 세션이 "닫을 방법이 없네" 하고 조용히 되살리면 그 겹침 문제가 함께 돌아온다.
    //   ⚠️ 나가는 길은 브라우저 뒤로가기와 Esc 다(Esc 는 아래에서 따로 고정).
    expect(V2, '닫기 버튼이 되살아났다').not.toContain('aria-label="닫기"')
    expect(V2, 'lucide X 아이콘이 되살아났다').not.toMatch(/<X\b/)
  })

  it('Esc 로 나가는 길은 남아 있다', () => {
    expect(V2, 'X 를 지웠으니 Esc 마저 없으면 키보드 사용자는 갇힌다').toMatch(/Escape'\s*\)\s*navigate\(-1\)/)
  })

...
describe('nextSubreqCap — 회복', () => {
  it('🔒 예산을 남기고 완주해도 상한을 올린다 (데드락 회귀 테스트)', () => {
    ...
    // 🔀 병합(2026-07-29): 시그니처는 main(#837 플랫폼 천장 param), 기대값은 이 브랜치(RECOVER 1.25 → 1.15).
    //   두 변경은 서로 독립이다 — 천장은 '넘지 마라', 배율은 '부딪힌 값으로 되돌아가지 마라'.
    expect(nextSubreqCap(29, false, 63, 300, NO_CEILING)).toBe(73) // ceil(63 * 1.15)
  })

  it('예산을 다 쓴 경우에도 동일하게 회복', () => {
    expect(nextSubreqCap(63, false, 63, 300, NO_CEILING)).toBe(73)
  })
...
describe('nextSubreqCap — 반(反)진동 불변식', () => {
  it('🔒 한 번 부딪힌 값으로는 절대 되돌아가지 않는다', () => {
    for (const bad of [30, 44, 50, 55, 63, 100, 137, 250]) {
      // 🔀 NO_CEILING — 이 테스트가 잠그는 건 **배율의 성질**이지 천장이 아니다. 기본 천장(60)을 그대로
      //   쓰면 60 이상 지점이 전부 천장에 눌려 배율 성질을 검증하지 못한다(천장은 아래 별도 테스트가 담당).
      const down = nextSubreqCap(bad, true, bad, 300, NO_CEILING)!       // 부딪힘 → 하향
      expect(down).toBeLessThan(bad)
      const up = nextSubreqCap(1, false, down, 300, NO_CEILING)!          // 무사통과 → 회복
  ...
  it('하향은 항상 소비량보다 작다 (같은 지점에서 또 죽지 않게)', () => {
    for (const spent of [30, 44, 55, 100, 300]) {
      expect(nextSubreqCap(spent, true, spent, 300, NO_CEILING)!).toBeLessThan(spent)
    }
  })
})

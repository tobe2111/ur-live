...
export const ADS_ENV_KNOWN: readonly string[] = [
  ...
  'ADS_COLLECT_ROUNDS', 'ADS_COLLECT_TISTORY_DISABLED', 'ADS_COMMERCE_ENABLED', 'ADS_REMEASURE_AFTER_DAYS',
  ...
  // 🗄️ 2026-08-19: 리드 테이블 전용 D1 바인딩(`shared/ads/leads-db.ts`). 없으면 `env.DB` 로
  //   폴백하므로 미설정이 곧 고장은 아니다 — 다만 목록에 없으면 런타임이 "미사용"으로 오신고한다.
  'ADS_DB', 'ADS_COMPANY_DB', /* 업체 리드 전용 D1 바인딩(2026-08-23) — ADS_DB 와 같은 부류 */ 'ADS_DOMAINS', 'ADS_ENRICH_BUDGET', 'ADS_ENRICH_CONCURRENCY',
  ...
  'ADS_DAILY_WRITE_BUDGET', // ✍️ 일일 **쓰기** 예산(행) — 같은 원장. 0=끔, 기본 150만(월 4,500만 < 유료 포함 5,000만)
  ...
  'ADS_LICENSE_OPERATION',
  'ADS_MARKET_ENABLED', 'ADS_MARKET_ENDPOINT', 'ADS_MARKET_OP', 'ADS_MARKET_PAGES', 'ADS_MARKET_ROWS', 'ADS_MARKET_DEADLINE_MS',
  ...
  'ADS_NARA_ENDPOINT', 'ADS_NARA_CONTRACT_DAYS', 'ADS_NARA_CONTRACT_DEADLINE_MS',
  'ADS_NARA_CONTRACT_ENABLED', 'ADS_NARA_CONTRACT_ENDPOINT', 'ADS_NARA_CONTRACT_OP',
  'ADS_NARA_CONTRACT_PAGES', 'ADS_NARA_CONTRACT_ROWS',
  // 🏛️ 조달업체(사용자정보) 레인 — 2026-08-11 되살림. 코드 12 를 '주소 폐기'로 오독해 지웠던 것이고,
  //   진짜 원인은 오퍼레이션 이름의 '02' 누락이었다(대표 Swagger 확정). ENDPOINT/OP 는 무배포 교정용.
  'ADS_NARA_VENDOR_DAYS', 'ADS_NARA_VENDOR_ENABLED', 'ADS_NARA_VENDOR_ENDPOINT', 'ADS_NARA_VENDOR_OP',
  'ADS_NAVER_EXTRA',
  ...
  'ADS_WEBKR_BATCH', 'ADS_WEBKR_LANE_DISABLED', 'ADS_WEBKR_SUBREQUEST_BUDGET',
  'ADS_WORKER_ENABLED',
  'ADS_YT_PAGES', 'ADS_YT_PERF_UNITS', 'ADS_YT_SEARCH_BUDGET',
  // ── 🔑 외부 키 · 인프라 바인딩(`ADS_` 접두가 아니라 스캔에 안 걸린다 — 여기만 수동)
  'PUBLIC_DATA_SERVICE_KEY', 'NTS_API_KEY', 'NEIS_API_KEY',

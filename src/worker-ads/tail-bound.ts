...
/**
 * 회차 꼬리에서 레인 완료를 기다리는 상한(무료).
 *
 * 🔻 **2026-08-04: 25s → 10s** (대표 승인). 25s 로도 밤사이 **10회차 중 3회**(03·06·07시) 꼬리가 안 돌았고,
 *   그 구멍 때문에 `cap` 이 4 → 2 로 **되물러났다**(pinned 1). 상한이 있으면 최대 그만큼만 기다리므로,
 *   그래도 못 남긴다는 건 **부모가 25s 조차 못 버티는 회차가 있다**는 뜻이다.
 *   ⇒ 방향은 상한을 *올리는* 것이 아니라 **줄이는** 것이다.
 *
 * 손실이 작은 이유: 레인 마감선이 이미 12~20s 라 대부분 그 안에 끝나고, 못 기다린 레인은
 * **판정에서 빠질 뿐 실패로 세지 않는다**(`judgedLaneNames`). 다음 회차에 다시 잡힌다.
 *
 * ⚠️ **이건 처방이자 실험이다.** 부모가 왜 일찍 죽는지(CPU·대기·`waitUntil` 한도)는 여전히 미확정이다.
 *   `waited_ms` 가 스탬프에 남으므로, 10s 로 바꾼 뒤 이력 구멍이 줄면 인과가 확인된다.
 *   **줄지 않으면 원인이 대기 시간이 아니다** — 그때는 값을 또 만지지 말고 계측을 붙일 것.
 */
export const TAIL_WAIT_MS = 10_000
/** 유료는 부모 수명이 길고 레인 마감선도 길다(24~60s). 무료에서 검증되면 여기도 재검토. */

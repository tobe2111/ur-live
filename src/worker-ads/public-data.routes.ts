  ...
  try {
    ...
    const results = ladder.length && target !== 'all'
      ...
      : target === 'all'
        ...
        : [await m.probePublicData(c.env, target, undefined, { rows, page, path: c.req.query('path'), host: c.req.query('host'), params: c.req.query('params'), bodyMax: c.req.query('body') })]

...
async function scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
  ...
  // 🏷️ 실패 사유는 `cronErrorCode`(cron-heartbeat SSOT)로 짧은 코드 한 개 — 06:00 에 4개 레인이
  //   동시에 ok:false 로 죽었는데 어드민엔 `result: null` 뿐이라 *왜* 가 없던 갭(2026-07-29 실측).
  const adsBeat = async (name: string, ok: boolean, ms: number, err?: unknown): Promise<void> => {
    try {
      ...
      await recordCronBeat(env as never, `ads:${name}`, ok, ms, event.cron, ok ? undefined : { err: cronErrorCode(err) })

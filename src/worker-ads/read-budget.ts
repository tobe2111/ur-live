 ...
 *
 * ## 기본값 근거
 * 150만 행/일 = 월 4,500만 → 유료 포함분 5,000만 **안쪽**. 정상 수집의 실측 쓰기는
 * 월 3,200만(64%)이라 여유가 남고, 폭주는 시간당 300만이라 **반나절 안에 걸린다.**
 * ⚠️ 발굴을 크게 늘려도 안전하다: 리드 1건 저장 ≈ 1~2 쓴 행이라 하루 30만 건까지 이 예산 안이다.
 * `0` 이면 끈다(무제한).
 */
export const WRITE_BUDGET_ENV = 'ADS_DAILY_WRITE_BUDGET'
export const DEFAULT_DAILY_WRITE_BUDGET = 1_500_000

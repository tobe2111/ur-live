...
const inputCls = 'w-full h-12 rounded-xl border border-gray-200 dark:border-[#2A3446] bg-white dark:bg-[#1A2334] px-3.5 text-[14px] text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:border-gray-900 dark:focus:border-white'
      ...
      <div className="min-h-screen bg-white dark:bg-[#0F151D] flex items-center justify-center px-4">
        ...
        <div className="text-center max-w-sm">
          ...
          <button onClick={() => navigate('/')}
            className="mt-6 px-6 h-12 rounded-xl bg-gray-900 dark:bg-white text-white dark:text-[#0F151D] text-sm font-bold">
            홈으로
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-white dark:bg-[#0F151D] pb-24">
      ...
      <div className="ur-content-narrow px-4 pt-6">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-12 h-12 rounded-2xl bg-gray-100 dark:bg-[#1A2334] flex items-center justify-center">
        ...
        <form onSubmit={submit} className="space-y-3">
              ...
              <button key={tp.v} type="button" onClick={() => setForm(f => ({ ...f, type: tp.v }))}
                className={`px-3.5 h-10 rounded-full text-[13px] font-bold transition-colors ${
                  form.type === tp.v
                    ...
                    : 'bg-gray-100 text-gray-600 dark:bg-[#1A2334] dark:text-gray-300'
                }`}>
                {tp.l}
              </button>
            ))}
          ...
          <textarea value={form.message} onChange={set('message')} maxLength={4000} rows={6}
            ...
            className="w-full rounded-xl border border-gray-200 dark:border-[#2A3446] bg-white dark:bg-[#1A2334] px-3.5 py-3 text-[14px] text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 resize-none focus:outline-none focus:border-gray-900 dark:focus:border-white" />
          <button type="submit" disabled={submitting}
            className="w-full h-13 py-3.5 rounded-xl bg-gray-900 dark:bg-white text-white dark:text-[#0F151D] text-[15px] font-bold disabled:opacity-50">

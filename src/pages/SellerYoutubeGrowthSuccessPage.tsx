    ...
    async function confirm() {
      try {
        ...
        if (res.data.success) {
          ...
          setError(res.data.error || '결제 확인에 실패했습니다.')
        }
      } catch (err: any) {
    ...
    return (
      <div className="min-h-screen bg-[#fbfbfd] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">결제를 처리하는 중...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#fbfbfd] flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-2xl p-8 shadow-lg text-center">
        ...
        <p className="text-sm text-gray-500 mb-6">관리자 확인 후 구독자 늘리기가 진행됩니다.</p>
        <div className="bg-gradient-to-r from-red-50 to-pink-50 rounded-xl p-5 my-6">
          <div className="flex items-center justify-center gap-2 mb-2">
            <Youtube className="w-5 h-5 text-red-500" />
            <span className="text-sm text-gray-600">구독자 늘리기</span>
          </div>
          <p className="text-3xl font-bold text-red-600">+{result?.subscribers.toLocaleString()}명</p>
          <p className="text-sm text-gray-500 mt-2">결제 금액: {result?.amount.toLocaleString()}원</p>
        </div>
        <button

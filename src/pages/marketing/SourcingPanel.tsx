  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <div className="flex-1 flex flex-wrap gap-1.5">
        {segs.map(s => (
          <span key={s.label} className="inline-flex items-center gap-1 rounded bg-gray-50 dark:bg-[#0D0F12] px-1.5 py-0.5 text-[10.5px] text-gray-600 dark:text-gray-300">
  ...
  return (
    <div className={`mt-3 ${card}`}>
      <div className="flex items-center justify-between">
        ...
        <button onClick={run} disabled={busy} className="shrink-0 rounded-lg bg-gray-900 dark:bg-white px-3 py-1.5 text-[12px] font-bold text-white dark:text-[#0D0F12] disabled:opacity-50">{busy ? '분석 중…' : '트렌드 분석'}</button>
      </div>
            ...
            return (
              <div key={r.name}>
                <button onClick={() => toggle(r.cid)} disabled={!r.cid} className="w-full flex items-center gap-2 text-[12px] py-1 text-left disabled:cursor-default">
                  ...
                  <div className="flex-1 h-2 rounded-full bg-gray-100 dark:bg-[#1A1C21] overflow-hidden">

    ...
    <div className={`mt-3 ${card}`}>
      ...
      <div className="mt-2.5 flex gap-2">
        <input value={seed} onChange={e => setSeed(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') search() }}
          ...
          className="flex-1 h-9 rounded-lg border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] px-3 text-[13px] text-gray-900 dark:text-white placeholder:text-gray-400" />
        <button onClick={search} disabled={loading}
          className="shrink-0 rounded-lg bg-gray-900 dark:bg-white px-4 text-[12.5px] font-bold text-white dark:text-[#11141C] disabled:opacity-40">
          {loading ? '분석 중…' : '발굴'}
        </button>
      </div>

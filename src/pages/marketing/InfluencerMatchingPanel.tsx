...
export default function InfluencerMatchingPanel() {
  ...
  return (
    <div className={`mt-3 ${card}`}>
      <div className="flex items-center justify-between gap-2">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="shrink-0 rounded-full bg-gray-900 text-white dark:bg-white dark:text-[#11141C] px-2 py-0.5 text-[9.5px] font-bold tracking-wide">ADMIN</span>
          </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        {CATEGORIES.map((c) => (
          <button key={c.code} onClick={() => setCat(c.code)}
            className={`rounded-full px-3 py-1 text-[11.5px] font-semibold border ${cat === c.code ? 'bg-gray-900 text-white dark:bg-white dark:text-[#11141C] border-transparent' : 'border-gray-200 dark:border-[#2C2F35] text-gray-500 dark:text-gray-400'}`}>{c.label}</button>
        ))}
        <span className="mx-1 h-4 w-px bg-gray-200 dark:bg-[#2C2F35]" />
        {REGIONS.map((r) => (
          <button key={r.code} onClick={() => setRegion(r.code)}
            className={`rounded-full px-3 py-1 text-[11.5px] font-semibold border ${region === r.code ? 'bg-gray-900 text-white dark:bg-white dark:text-[#11141C] border-transparent' : 'border-gray-200 dark:border-[#2C2F35] text-gray-500 dark:text-gray-400'}`}>{r.label}</button>
        ))}
      ...
      {/* AI 매칭 근거(집계·가명만 전송) */}
      {(aiText || aiNote) && (
        <div className="mt-2 rounded-lg bg-gray-50 dark:bg-[#1D1F29] border border-gray-100 dark:border-[#2C2F35] p-3">
      ...
      <div className="mt-3 space-y-2">
        {rows.map((c, i) => {
          ...
          return (
            <div key={c.influencerId} className="rounded-xl border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C]">
              <div className="flex items-center gap-3 p-3">
                <div className="shrink-0 grid h-8 w-8 place-items-center rounded-full bg-gray-100 dark:bg-[#1D1F29] text-[12px] font-bold text-gray-400">{i + 1}</div>
              ...
              {/* 실측 지표 3종(차별점) */}
              <div className="grid grid-cols-3 gap-px bg-gray-100 dark:bg-[#1D1F29] text-center">
                ...
                <div className="border-t border-gray-100 dark:border-[#2C2F35] p-3 space-y-3">
                  {c.badge === 'cold' ? (
                    ...
                    <>
                      ...
                      <div className="rounded-lg bg-gray-50 dark:bg-[#1D1F29] px-3 py-2 text-[11.5px] text-gray-600 dark:text-gray-300">
...
function Cov({ label, v, accent }: { label: string; v: string; accent?: boolean }) {
  return (
    <div className="rounded-lg border border-gray-100 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] px-2 py-1.5 text-center">
...
function Metric({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="bg-white dark:bg-[#11141C] py-2">
...
function Bar({ label, pct, max, suffix, active }: { label: string; pct: number; max: number; suffix?: string; active?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2 py-0.5">
      ...
      <div className="h-2 flex-1 rounded-full bg-gray-100 dark:bg-[#1D1F29] overflow-hidden">

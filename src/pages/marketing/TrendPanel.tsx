        ...
        <div className="flex shrink-0 gap-1.5">
          {hasData && series && series.length > 0 && (
            <button onClick={() => downloadCsv('유어애즈_성과추세_30일.csv',
              ...
              className="rounded-lg border border-gray-200 dark:border-[#2C2F35] px-2 py-1 text-[11px] font-bold text-gray-700 dark:text-gray-200">CSV</button>
          )}
          <button onClick={snapshotNow} disabled={busy} className="rounded-lg border border-gray-200 dark:border-[#2C2F35] px-2.5 py-1 text-[11px] font-bold text-gray-700 dark:text-gray-200 disabled:opacity-50">{busy ? '갱신 중…' : '지금 갱신'}</button>
        </div>
      </div>
          ...
          {wow && (
            <div className="mt-3 grid grid-cols-3 gap-2">
              {[
                ...
                <div key={m.l} className="rounded-xl border border-gray-100 dark:border-[#2C2F35] p-2.5">

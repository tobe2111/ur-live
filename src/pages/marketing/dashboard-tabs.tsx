...
/** 🚫 ADS_AI_HIDDEN(2026-07-28 대표 "AI 안 씀") — 'AI 스튜디오' 탭 노출 제거. 정의는 위에 보존(플래그 false 면 즉시 복원). */
export const DASH_TABS: DashTab[] = ADS_AI_HIDDEN ? ALL_DASH_TABS.filter((t) => t.id !== 'ai') : ALL_DASH_TABS
...
export const SEC_TO_TAB: Record<string, string> = {
  ...
  // AI 숨김 시 옛 딥링크(#sec-content·#sec-ai)는 홈으로 — 존재하지 않는 탭으로 보내면 빈 화면이 된다.
  'sec-content': ADS_AI_HIDDEN ? 'home' : 'ai', 'sec-ai': ADS_AI_HIDDEN ? 'home' : 'ai',
...
export const CARD_CLS = 'rounded-2xl border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#1D1F29] p-4'
export const INPUT_CLS = 'w-full h-10 rounded-lg border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] px-3 text-[13px] text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500'

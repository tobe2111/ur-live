      ...
      <div className="flex items-center justify-between">
        ...
        {s && (
          <button onClick={() => patch({ enabled: s.enabled ? 0 : 1 })} disabled={saving}
            className={`px-2.5 py-1 rounded-lg text-[11.5px] font-bold ${s.enabled ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-gray-100 dark:bg-[#1A2334] text-gray-500 dark:text-gray-400'}`}>
      ...
      {s && (
        <>
          <div className="mt-3 space-y-2.5">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5">
                <input type="number" min={50} max={100} value={s.budget_pace_pct}
                  ...
                  className="w-16 h-8 rounded-lg border border-gray-200 dark:border-[#2A3446] bg-white dark:bg-[#0F151D] px-2 text-[12px] text-right text-gray-900 dark:text-white" />
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <button onClick={() => patch({ price_undercut: s.price_undercut ? 0 : 1 })} disabled={saving}
                className={`px-2 py-0.5 rounded text-[11px] font-bold ${s.price_undercut ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-gray-100 dark:bg-[#1A2334] text-gray-500 dark:text-gray-400'}`}>
          ...
          <div className="mt-3 flex items-center gap-2">
            <button onClick={preview} disabled={previewBusy} className="rounded-lg border border-gray-200 dark:border-[#2A3446] px-3 py-1.5 text-[12px] font-bold text-gray-700 dark:text-gray-200 disabled:opacity-50">{previewBusy ? '확인 중…' : '지금 확인'}</button>

...
const ta = 'w-full rounded-lg border border-gray-200 dark:border-[#2A2A2A] bg-white dark:bg-[#0A0A0A] p-2.5 text-[13px] text-gray-900 dark:text-white placeholder:text-gray-400'
...
const AdCopyList = ({ variants }: { variants: AdVariant[] }) => (
  <div className="space-y-1.5">
    {variants.map((v, i) => (
      <div key={i} className="rounded-lg border border-gray-100 dark:border-[#1A1A1A] p-2 text-[12px]">
...
const Actions = ({ onCopy, onSave }: { onCopy: () => void; onSave?: () => void }) => (
  <div className="mt-2 flex gap-1.5">
    <button onClick={onCopy} className="rounded-lg border border-gray-200 dark:border-[#2A2A2A] px-2.5 py-1 text-[11px] font-bold text-gray-700 dark:text-gray-200">복사</button>
    {onSave && <button onClick={onSave} className="rounded-lg border border-gray-200 dark:border-[#2A2A2A] px-2.5 py-1 text-[11px] font-bold text-blue-600 dark:text-blue-400">보관함 저장</button>}
  </div>
)
    ...
    <div className={`mt-3 ${card}`}>
        ...
        <div className="mt-2">
          ...
          {pack && (
            <div className="mt-3 space-y-2.5">
              {pack.summary && <div className="rounded-xl border border-gray-100 dark:border-[#1A1A1A] p-3"><div className="text-[12px] font-bold text-gray-900 dark:text-white">요약</div><p className="mt-1 text-[12.5px] text-gray-600 dark:text-gray-300 whitespace-pre-wrap">{pack.summary}</p></div>}
              {pack.blog && <div className="rounded-xl border border-gray-100 dark:border-[#1A1A1A] p-3"><div className="text-[12px] font-bold text-gray-900 dark:text-white">블로그 · {pack.blog.title}</div><pre className="mt-1 text-[12px] text-gray-600 dark:text-gray-300 whitespace-pre-wrap font-sans">{pack.blog.body}</pre><Actions onCopy={() => copy(`${pack.blog!.title}\n\n${pack.blog!.body}`)} onSave={() => save('blog', pack.blog!.body, pack.blog!.title)} /></div>}
              {pack.instagram && <div className="rounded-xl border border-gray-100 dark:border-[#1A1A1A] p-3"><div className="text-[12px] font-bold text-gray-900 dark:text-white">인스타그램</div><p className="mt-1 text-[12.5px] text-gray-600 dark:text-gray-300 whitespace-pre-wrap">{pack.instagram.caption}</p><p className="mt-1 text-[11.5px] text-blue-600 dark:text-blue-400">{pack.instagram.hashtags.join(' ')}</p><Actions onCopy={() => copy(`${pack.instagram!.caption}\n\n${pack.instagram!.hashtags.join(' ')}`)} onSave={() => save('instagram', pack.instagram!.caption, null, { hashtags: pack.instagram!.hashtags })} /></div>}
              {pack.tiktok && <div className="rounded-xl border border-gray-100 dark:border-[#1A1A1A] p-3"><div className="text-[12px] font-bold text-gray-900 dark:text-white">틱톡 · {pack.tiktok.hook}</div><pre className="mt-1 text-[12px] text-gray-600 dark:text-gray-300 whitespace-pre-wrap font-sans">{pack.tiktok.script}</pre><p className="mt-1 text-[11.5px] text-blue-600 dark:text-blue-400">{pack.tiktok.hashtags.join(' ')}</p><Actions onCopy={() => copy(`${pack.tiktok!.hook}\n${pack.tiktok!.script}\n${pack.tiktok!.hashtags.join(' ')}`)} onSave={() => save('tiktok', `${pack.tiktok!.hook}\n${pack.tiktok!.script}`, null, { hashtags: pack.tiktok!.hashtags })} /></div>}
              {pack.ad_copy.length > 0 && <div className="rounded-xl border border-gray-100 dark:border-[#1A1A1A] p-3"><div className="text-[12px] font-bold text-gray-900 dark:text-white mb-1.5">광고 문구</div><AdCopyList variants={pack.ad_copy} /><Actions onCopy={() => copy(pack.ad_copy.map(v => `[${v.title}] ${v.desc}`).join('\n'))} onSave={() => save('ad_copy', pack.ad_copy.map(v => `[${v.title}] ${v.desc}`).join('\n'), null, { variants: pack.ad_copy })} /></div>}
              {pack.seo && <div className="rounded-xl border border-gray-100 dark:border-[#1A1A1A] p-3"><div className="text-[12px] font-bold text-gray-900 dark:text-white">SEO</div><p className="mt-1 text-[12px] text-gray-600 dark:text-gray-300">제목: {pack.seo.metaTitle}</p><p className="text-[12px] text-gray-600 dark:text-gray-300">설명: {pack.seo.metaDescription}</p><p className="mt-1 text-[11.5px] text-gray-500 dark:text-gray-400">키워드: {pack.seo.keywords.join(', ')}</p></div>}
              <button onClick={() => copy(packText(pack))} className="rounded-lg border border-gray-200 dark:border-[#2A2A2A] px-3 py-1.5 text-[11.5px] font-bold text-gray-700 dark:text-gray-200">전체 복사</button>
            </div>
          )}
        </div>
      )}
      ...
      {mode === 'reply' && (
        <div className="mt-2.5">
          ...
          {reply && <div className="mt-3 rounded-xl border border-gray-100 dark:border-[#1A1A1A] p-3"><p className="text-[12.5px] text-gray-700 dark:text-gray-200 whitespace-pre-wrap">{reply}</p><Actions onCopy={() => copy(reply)} onSave={() => save('reply', reply)} /></div>}
        </div>
      )}
      ...
      {mode === 'analyze' && (
        <div className="mt-2.5">
          ...
          {analysis && <div className="mt-3 rounded-xl border border-gray-100 dark:border-[#1A1A1A] p-3"><pre className="text-[12.5px] text-gray-700 dark:text-gray-200 whitespace-pre-wrap font-sans leading-relaxed">{analysis}</pre><Actions onCopy={() => copy(analysis)} /></div>}
        </div>
      )}
        ...
        <div className="mt-2.5">
          {!libLoaded && busy ? <p className="text-[12px] text-gray-400 dark:text-gray-500">불러오는 중…</p>
            ...
            : <div className="space-y-1.5">{lib.map(it => (
                <div key={it.id} className="rounded-lg border border-gray-100 dark:border-[#1A1A1A] p-2.5">
                  <div className="flex items-center justify-between gap-2">

      ...
      {leads.length > 0 && (
        <div className="mt-3 space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {MIN_SUBS.map(m => (
              <button key={m.v} onClick={() => setMinSubs(m.v)} className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${minSubs === m.v ? 'bg-blue-600 text-white' : 'border border-gray-200 dark:border-[#2C2F35] text-gray-500 dark:text-gray-400'}`}>{m.l}</button>
            ))}
          </div>
        </div>
      )}

...
export default function SellerExperienceCampaignsPage() {
  ...
  const create = async () => {
    if (!form.product_id || !form.title.trim()) { toast.error('상품 ID·제목은 필수입니다'); return }
    setCreating(true)
    try {
  ...
  return (
    <SellerLayout title="체험 캠페인">
      <div className="mx-auto max-w-4xl p-4 sm:p-6 lg:p-8">
        <DashboardPageHeader
          title="체험 캠페인"
          subtitle="무료 응모·공정 추첨 체험단. 선정자에게 0원 체험권이 자동 발급됩니다 (매장 자기부담 · 정산·수수료 무관)."
        ...
        {enabled ? (
          <div className="bg-white rounded-2xl border border-gray-200 p-5 mt-4 mb-5">
            <h3 className="text-[14px] font-bold text-gray-900 mb-3">새 체험 캠페인</h3>
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              <label className="text-[12px] text-gray-600">제공 이용권 상품 ID
                <input value={form.product_id} onChange={e => setForm(f => ({ ...f, product_id: e.target.value.replace(/\D/g, '') }))} inputMode="numeric" className="mt-1 w-full px-3 py-2 border border-gray-200 rounded-xl text-[13px] text-gray-900" placeholder="내 상품 ID" />
              </label>
              ...
              <label className="text-[12px] text-gray-600 col-span-2 lg:col-span-3">캠페인 제목
                <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className="mt-1 w-full px-3 py-2 border border-gray-200 rounded-xl text-[13px] text-gray-900" placeholder="예: 아메리카노 체험단" />
              </label>
          ...
          <div className="mt-4 mb-5 flex items-start gap-2.5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
            ...
            <p className="text-[12px] leading-relaxed text-amber-800">
              셀러 셀프 캠페인 개설은 <b>준비 중</b>입니다. 지금은 유어딜 담당자가 대신 개설해 드립니다 —
              개설을 원하시면 관리자에게 문의해주세요. (개설된 캠페인의 응모자 조회·추첨·리포트는 아래에서 가능합니다.)
            </p>
          </div>
        )}
            ...
            <div key={c.id} className="border-t border-gray-100 first:border-t-0">
              <button type="button" onClick={() => void openDetail(c.id)} className="w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-gray-500">#{c.id} · {c.restaurant_name || c.product_name || `상품 ${c.product_id}`}</p>
                </div>
              ...
              {expanded === c.id && (
                <div className="px-4 pb-4 bg-gray-50/60">
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <p className="text-[12px] font-bold text-gray-900 px-3 py-2 border-b border-gray-100 flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5 text-blue-600" />응모자 ({entries.length})</p>

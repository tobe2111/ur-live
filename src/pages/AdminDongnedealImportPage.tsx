  ...
  const seedDemo = async () => {
    ...
    try {
      ...
      if (seedCategory === 'stay_voucher') {
        ...
        let sCreated = 0, sSkipped = 0, sPhotos = 0, sHealed = 0, sVaried = 0
        const SCHUNK = 6
        for (let done = 0; done < seedCount; done += SCHUNK) {
          ...
          sHealed += Number(d.healed ?? 0); sVaried += Number(d.varied ?? 0)
        }
        const healNote = `${sHealed ? ` · 기존 ${sHealed}개 보정(좌표·가격·이용권명)` : ''}${sVaried ? ` · 오퍼 다양화 ${sVaried}개` : ''}`
        if (sCreated > 0) {
          toast.success(`데모 숙소 ${sCreated}개 생성 · 실사진 ${sPhotos}${healNote}${sSkipped ? ` · ${sSkipped}개 건너뜀(실숙소 미매칭/중복)` : ''}`)
        } else if (sHealed > 0 || sVaried > 0) {

...
// 제조사 자가관리 — '승인한 유통채널' 상품의 허용 판매사 추가/해제.
export default function ChannelModal({ t, item, onClose }: { t: (k: string, o?: Record<string, unknown>) => string; item: CatalogItem; onClose: () => void }) {
  ...
  const add = async () => {
    ...
    if (!Number.isFinite(dsid) || dsid <= 0) { toast.error('판매사 ID를 입력하세요'); return }
    setBusy(true)
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 px-0 sm:px-4" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-6 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-lg font-bold text-gray-900">{t('supplier.manageChannel', { defaultValue: '승인 판매사 관리' })}</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
        </div>
        <p className="text-xs text-gray-500 mb-4">{item.name} · {t('supplier.channelHint', { defaultValue: '승인한 판매사에게만 이 상품이 노출·주문됩니다.' })}</p>
        <div className="flex gap-2 mb-4">
          <input value={sellerId} onChange={e => setSellerId(e.target.value)} type="number" placeholder={t('supplier.distributorId', { defaultValue: '판매사 ID' })} className={inputCls} />
        ...
        {list.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-6">{t('supplier.noChannel', { defaultValue: '승인된 판매사가 없습니다.' })}</p>
        ) : (

    ...
    <SellerLayout title="공동구매 관리">
      <div className="max-w-3xl mx-auto space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: '진행 중', value: products.filter(p => p.group_buy_status === 'active').length, color: 'text-blue-600', bg: 'bg-blue-50' },
            { label: '달성 완료', value: products.filter(p => p.group_buy_status === 'achieved').length, color: 'text-green-600', bg: 'bg-green-50' },
            { label: '전체 참여자', value: products.reduce((s, p) => s + (p.group_buy_current || 0), 0), color: 'text-pink-600', bg: 'bg-pink-50' },
          ].map(s => (
              ...
              return (
                <div key={p.id} className="bg-white rounded-xl border border-gray-200 p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                          ...
                          {isActive ? '진행 중' : isAchieved ? '달성 완료' : '마감'}
                        </span>
                      ...
                      <div className="mt-2">
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="text-gray-500">{p.group_buy_current}/{p.group_buy_target}명</span>
                      ...
                      {p.group_buy_deadline && (
                        <p className="text-[10px] text-gray-400 mt-1.5">
                          마감: {new Date(p.group_buy_deadline).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex items-center gap-2 p-2.5 bg-gray-50 rounded-lg">
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] text-gray-500">식당 사장 통계 링크</p>
                    ...
                    <button
                      onClick={() => {
                        ...
                        toast.success('링크 복사됨! 식당 사장에게 공유하세요.')
                      }}

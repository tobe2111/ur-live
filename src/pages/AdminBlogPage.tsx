...
export default function AdminBlogPage() {
  ...
  const [form, setForm] = useState({
    ...
    content: '',
    tagsInput: '',   // 콤마 구분 입력
    author: t('admin.blog.k001', { defaultValue: '유어딜 팀' }),
    thumbnail_url: '',
    is_published: false,
  })
  ...
  function openEdit(post?: BlogPost) {
    if (post) {
      setForm({
        ...
        author: post.author || t('admin.blog.k001', { defaultValue: '유어딜 팀' }),
        thumbnail_url: post.thumbnail_url || '',
        is_published: post.is_published === 1,
      })
    } else {
      setForm({ id: 0, slug: '', title: '', summary: '', content: '', tagsInput: '', author: t('admin.blog.k001', { defaultValue: '유어딜 팀' }), thumbnail_url: '', is_published: false })
    }
    setView('edit')
  }
  ...
  async function save(publish?: boolean) {
    if (!form.title.trim()) { toast.error(t('admin.blog.k003', { defaultValue: '제목을 입력해주세요.' })); return }
    if (!form.slug.trim()) { toast.error(t('admin.blog.k004', { defaultValue: '슬러그를 입력해주세요.' })); return }
    if (!form.content.trim()) { toast.error(t('admin.blog.k005', { defaultValue: '본문을 입력해주세요.' })); return }
    ...
    try {
      ...
      const payload = {
        ...
        author: form.author.trim() || t('admin.blog.k001', { defaultValue: '유어딜 팀' }),
      ...
      if (form.id) {
        ...
        toast.success(payload.is_published ? t('admin.blog.k006', { defaultValue: '발행되었습니다.' }) : t('admin.blog.k007', { defaultValue: '저장되었습니다.' }))
      } else {
        await api.post('/api/admin/blog', payload)
        toast.success(payload.is_published ? t('admin.blog.k006', { defaultValue: '발행되었습니다.' }) : t('admin.blog.k008', { defaultValue: '임시저장 되었습니다.' }))
      }
      ...
      toast.error(err_.response?.data?.error || t('admin.blog.k009', { defaultValue: '저장 실패' }))
  ...
  async function deletePost(id: number) {
    ...
    try {
      ...
      await loadPosts()
    } catch { toast.error(t('admin.blog.k012', { defaultValue: '삭제 실패' })) }
  }
  ...
  async function togglePublish(post: BlogPost) {
    try {
      ...
      toast.success(post.is_published === 1 ? t('admin.blog.k013', { defaultValue: '비공개로 변경됨' }) : t('admin.blog.k014', { defaultValue: '발행됨' }))
      await loadPosts()
    } catch { toast.error(t('admin.blog.k015', { defaultValue: '변경 실패' })) }
  }
    ...
    <AdminLayout title={t('admin.pages.blog')}>
      <div className="max-w-3xl mx-auto py-8 px-4 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
          ...
          {/* 요약 */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('admin.blog.k024', { defaultValue: '요약 (SEO 설명)' })}</label>
            <textarea
              value={form.summary}
          ...
          {/* 태그 */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('admin.blog.k026', { defaultValue: '태그' })}</label>
            <input
        ...
        <div className="bg-white rounded-xl border border-gray-200 px-5 py-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-900">{t('admin.blog.k032', { defaultValue: '발행 상태' })}</p>
            <p className="text-xs text-gray-500">{form.is_published ? t('admin.blog.k033', { defaultValue: '발행됨 — 공개적으로 노출됩니다' }) : t('admin.blog.k034', { defaultValue: '임시저장 — 외부에 노출되지 않습니다' })}</p>
          </div>
          <button

    ...
    <div className="min-h-[100dvh] bg-[#FAF7F5] dark:bg-[#0D0F12]">
      ...
      <main className="px-5 pb-32 max-w-xl mx-auto">
        ...
        <section className="pt-8 pb-10">
          ...
          <h1 className="text-[26px] leading-[1.35] font-extrabold text-[#16181C] dark:text-[#F5F3F1]">
        ...
        <section className="pb-10">
          <h2 className="text-[19px] font-extrabold text-[#16181C] dark:text-[#F5F3F1] mb-4">소비자에게는</h2>
          <div className="space-y-2.5">
            {BENEFITS.map(({ icon: Icon, t, d }) => (
              <div key={t} className="flex items-center gap-3.5 rounded-2xl bg-white dark:bg-[#1A1C21] p-4">
        ...
        {/* 3자 구조도 */}
        <section className="pb-10">
          <h2 className="text-[19px] font-extrabold text-[#16181C] dark:text-[#F5F3F1] mb-1">셋이 함께 커지는 구조</h2>
          ...
          <div className="grid grid-cols-3 gap-2">
            {TRIANGLE.map(({ icon: Icon, t, d }) => (
              <div key={t} className="rounded-2xl bg-white dark:bg-[#1A1C21] p-3 text-center">
                <div className="w-9 h-9 mx-auto rounded-xl bg-[var(--brand-tint)] dark:bg-[#3A2530] flex items-center justify-center">
                  <Icon className="w-[18px] h-[18px] text-brand" />
                </div>
                <p className="text-[13px] font-extrabold text-[#16181C] dark:text-[#F5F3F1] mt-2">{t}</p>
                <p className="text-[10.5px] text-gray-500 dark:text-gray-400 mt-1 leading-snug">{d}</p>
              </div>
            ))}
          </div>
          <div className="mt-3 grid grid-cols-2 gap-2">
            <Link to="/partners" className="h-11 rounded-xl border border-[#16181C]/15 dark:border-[#2C2F35] bg-white dark:bg-[#1A1C21] flex items-center justify-center text-[13px] font-bold text-[#16181C] dark:text-[#F5F3F1]">매장 입점 안내 →</Link>
            <Link to="/creators" className="h-11 rounded-xl border border-[#16181C]/15 dark:border-[#2C2F35] bg-white dark:bg-[#1A1C21] flex items-center justify-center text-[13px] font-bold text-[#16181C] dark:text-[#F5F3F1]">소개 파트너 모집 →</Link>
          </div>
        </section>

        {/* 🧭 2026-08-20 (대표 — 브랜드커넥트 참고): 사장님 진행 방법 4단계 */}
        <section className="pb-10">
          <h2 className="text-[19px] font-extrabold text-[#16181C] dark:text-[#F5F3F1] mb-1">사장님은 이렇게 진행해요</h2>
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={n} className="rounded-2xl bg-white dark:bg-[#1A1C21] p-3.5 flex items-start gap-3">
                <span className="text-[13px] font-extrabold text-brand shrink-0 mt-0.5">{n}</span>
                <div>
                  <p className="text-[14px] font-extrabold text-[#16181C] dark:text-[#F5F3F1]">{t}</p>
        ...
        <section className="pb-10">
          <h2 className="text-[19px] font-extrabold text-[#16181C] dark:text-[#F5F3F1] mb-4">숫자와 이야기</h2>
          <div className="rounded-2xl bg-white dark:bg-[#1A1C21] p-5">
            <div className="grid grid-cols-3 gap-2 text-center">
              {/* 📊 10월 방배 파일럿 실측치로 교체 예정 */}
              <div><p className="text-[18px] font-extrabold text-[#16181C] dark:text-[#F5F3F1]">서초구</p><p className="text-[10.5px] text-gray-500 dark:text-gray-400 mt-0.5">상권 활성화 사업 수행</p></div>
              <div><p className="text-[18px] font-extrabold text-[#16181C] dark:text-[#F5F3F1]">19조</p><p className="text-[10.5px] text-gray-500 dark:text-gray-400 mt-0.5">글로벌 동일 모델 시장</p></div>
          ...
          <Link to="/about/print" className="mt-3 flex items-center justify-center gap-1.5 h-11 rounded-xl border border-[#16181C]/15 dark:border-[#2C2F35] bg-white dark:bg-[#1A1C21] text-[13px] font-bold text-[#16181C] dark:text-[#F5F3F1]">

...
export default function AgencyWaitingPage() {
    ...
    (async () => {
      try {
        ...
        if (res.data?.success && res.data.data?.linked) {
          const s = res.data.data.agency?.status as Status
          setStatus(s || 'pending')
          setAgencyName(res.data.data.agency?.name || '')
          if (s === 'active') {
            navigate('/agency', { replace: true })
            return
          }
        } else {
          navigate('/agency/register/business?from=kakao', { replace: true })
          return
        }
      } catch {
      ...
      <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-sm">
        ...
        <div className="text-center space-y-2 mb-6">
          ...
          <p className="text-sm text-gray-500 leading-relaxed">
            {status === 'pending' && (
              <>
                {agencyName && <><span className="font-semibold text-gray-700">{agencyName}</span><br /></>}

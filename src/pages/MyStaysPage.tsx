 ...
 * 🎨 2026-07-02: 다크 전용 하드코딩 → 라이트 기본 + dark: variant (마이 위성 페이지 테마 규칙 정합).
...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  cancelled: { label: '취소됨', color: 'bg-gray-100 text-gray-500 dark:bg-[#1A1A1A] dark:text-gray-400' },
...
export default function MyStaysPage() {
  ...
  const { data: bookings = [], isLoading: loading, isError, refetch } = useMyStays()
  ...
  async function cancel(b: MyBooking) {
    // 🛡️ 2026-07-02: native prompt → 통일 디자인 promptDialog (사유는 선택 — 빈값 허용, 취소 시 null).
    const reason = await promptDialog({
      title: '예약 취소',
      message: '취소 사유를 입력해주세요. (선택)',
      prompt: { multiline: true, placeholder: '예: 일정 변경' },
      confirmText: '예약 취소',
      danger: true,
    })
    ...
    try {
      ...
      if (r.data?.success) {
        ...
        // 🛡️ 2026-07-02: refund_rate 서버 누락 시 NaN% 노출 방지 — 유효할 때만 비율 표시.
        const ratePart = Number.isFinite(refund_rate) ? ` ${(refund_rate * 100).toFixed(0)}%` : ''
        toast.success(`취소됨 — 환불${ratePart} (₩${formatNumber(refund_amount)})`)
        refetch()
      }
    } catch (err: unknown) {
      ...
      <div className="ur-content-wide px-4 lg:px-8 py-4">
        {loading ? (
          ...
          <div className="space-y-3">
            {bookings.map((b) => {
              const st = STATUS_LABELS[b.status] || { label: b.status, color: 'bg-gray-100 text-gray-600 dark:bg-[#1A1A1A] dark:text-gray-300' }

...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  pending: { label: '결제 대기', color: 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' },
  ...
  cancelled: { label: '취소됨', color: 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400' },
...
export default function MyStaysPage() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#020202] text-gray-900 dark:text-white pb-safe-nav">
      ...
      <div className="sticky top-0 z-30 bg-white/95 dark:bg-[#020202]/95 backdrop-blur-md border-b border-gray-100 dark:border-[#1A1A1A]">
        <div className="ur-content-wide px-4 lg:px-8 py-3 flex items-center gap-3">
          <button onClick={() => navigate(-1)} aria-label="뒤로 가기" className="p-1 text-gray-900 dark:text-white"><ChevronLeft className="w-5 h-5" /></button>
      ...
      <div className="ur-content-wide px-4 lg:px-8 py-4">
        {loading ? (
          <div className="text-center py-20 text-gray-500 dark:text-gray-400 text-sm">로딩 중...</div>
        ) : bookings.length === 0 ? (
          ...
          <div className="space-y-3">
            {bookings.map((b) => {
              const st = STATUS_LABELS[b.status] || { label: b.status, color: 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' }
              const canCancel = ['confirmed', 'pending'].includes(b.status)
              const canReview = b.status === 'checked_out'
              return (
                <div key={b.id} className="bg-white dark:bg-[#0A0A0A] border border-gray-200 dark:border-[#1A1A1A] rounded-xl p-4">
                  <div className="flex items-start gap-3">
                    <Link to={`/stays/${b.product_id}`} className="w-20 h-20 shrink-0 rounded-lg overflow-hidden bg-gray-100 dark:bg-[#1A1A1A]">
                      {b.image_url ? <img src={b.image_url} alt={b.product_name} className="w-full h-full object-cover" /> : null}
                    </Link>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <Link to={`/stays/${b.product_id}`} className="text-sm font-bold line-clamp-1 hover:underline text-gray-900 dark:text-white">{b.product_name}</Link>
                        <span className={`inline-flex px-2 py-0.5 text-[10px] font-bold rounded-full shrink-0 ${st.color}`}>{st.label}</span>
                      </div>
                      <p className="text-[11px] text-gray-500 dark:text-gray-400">
                        {b.room_name}
                        {b.sale_mode === 'voucher' && (
                          <span className="ml-1.5 px-1.5 py-0.5 text-[9px] bg-pink-100 text-pink-600 dark:bg-pink-500/20 dark:text-pink-300 rounded font-bold">🎫 숙소권</span>
                        )}
                      </p>
                      {b.sale_mode === 'voucher' ? (
                      ...
                      {b.check_in_code && b.status === 'confirmed' && (
                        <div className="mt-2 inline-flex items-center gap-2 px-2 py-1 bg-blue-50 dark:bg-blue-500/20 rounded">
                          <span className="text-[10px] text-blue-600 dark:text-blue-300">{b.sale_mode === 'voucher' ? 'voucher 코드' : '체크인 코드'}</span>
                          <span className="text-xs font-mono font-bold text-blue-700 dark:text-blue-200">{b.check_in_code}</span>
                        </div>
                      )}
                      <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-100 dark:border-[#1A1A1A]">
                        <p className="text-sm font-extrabold text-pink-600 dark:text-pink-400">₩{formatNumber(b.total_amount)}</p>
    ...
    <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur flex items-end sm:items-center justify-center" onClick={onClose}>
      <div className="bg-white dark:bg-[#0A0A0A] w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl border border-gray-100 dark:border-[#1A1A1A] max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-3">
          ...
          <div>
            <label className="block text-xs font-bold text-gray-700 dark:text-gray-200 mb-1">제목 (선택)</label>
            <input value={title} onChange={(e) => setTitle(e.target.value)} maxLength={200} placeholder="예) 깨끗하고 조용한 펜션" className="w-full px-3 py-2 bg-white dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#2A2A2A] rounded-lg text-sm text-gray-900 dark:text-white" />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-700 dark:text-gray-200 mb-1">코멘트 *</label>
            <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={5} maxLength={5000} placeholder="이용 경험을 자세히 알려주세요 (10자 이상)" className="w-full px-3 py-2 bg-white dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#2A2A2A] rounded-lg text-sm text-gray-900 dark:text-white resize-none" />
...
function VoucherInfo({ booking }: { booking: MyBooking }) {
  ...
  const colorClass = expired ? 'text-red-500 dark:text-red-400'

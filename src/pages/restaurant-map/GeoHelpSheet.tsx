...
export default function GeoHelpSheet({
  ...
  return (
    <div className="fixed inset-0 flex items-end sm:items-center justify-center" style={{ zIndex: Z.SHEET_BODY }}>
      ...
      <div className="relative w-full sm:max-w-sm bg-white dark:bg-[#0A0A0A] rounded-t-3xl sm:rounded-3xl p-5 pb-8 sm:pb-6 shadow-2xl">
        <button
          onClick={onClose}
          aria-label="닫기"
          className="absolute top-3 right-3 p-1.5 rounded-full bg-gray-100 dark:bg-[#1A1A1A] text-gray-500 dark:text-gray-400"
        >
          <X className="w-4 h-4" />
        </button>

...
export const RestaurantRow = memo(function RestaurantRow({ r, isSelected, userLoc, onSelect, fcfs }: {
  ...
  //   여기서 다른 숫자면 한 화면에서 두 값이 보인다(선언값이 계산값보다 크면 갈린다).
  //   📏 2026-09-09 실측 정정: 라이브 활성 50건은 `discount_rate` 가 **전부 0** 이라 오늘은 두 정의가
  //   같은 숫자를 낸다. 즉 이 통일은 **눈에 보이는 변화가 아니라 갈라질 자리를 막는 것**이다
  //   (누군가 discount_rate 를 채우는 순간 마커·행·카드가 서로 다른 값을 말하기 시작한다).

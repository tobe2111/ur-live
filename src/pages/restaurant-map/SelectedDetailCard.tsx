...
export default function SelectedDetailCard({ selected, userLoc, liveSellerIds, favorites, onClose, onToggleFavorite }: Props) {
  ...
  return (
    <div className="bg-pink-50 dark:bg-pink-900/20 border-2 border-pink-300 dark:border-pink-700 rounded-2xl p-4 mb-3 relative">
      <button onClick={onClose} aria-label={t('map.detail.deselect', { defaultValue: '선택 해제' })} className="absolute top-2.5 right-2.5 w-7 h-7 flex items-center justify-center rounded-full bg-white dark:bg-[#0D0F12]/80">
      ...
      <div className="flex gap-2 mt-3">
        <button
          ...
          className={`flex items-center justify-center w-10 h-10 rounded-xl transition-colors ${
            favorites.includes(selected.id) ? 'bg-[var(--brand-tint)] dark:bg-[#3A2530] text-brand' : 'bg-white dark:bg-[#0D0F12] text-gray-400 dark:text-gray-500'
          }`}
        >
          <Heart className="w-4 h-4" fill={favorites.includes(selected.id) ? 'currentColor' : 'none'} />
        </button>
        {selected.restaurant_phone && (
          <a href={`tel:${selected.restaurant_phone}`} aria-label={t('map.detail.call', { defaultValue: '전화' })} className="flex items-center justify-center w-10 h-10 bg-white dark:bg-[#0D0F12] rounded-xl text-gray-700 dark:text-gray-200">

...
// 카테고리별 핀 링 색상 (2026-06-22 대표 — "흑백일 필요 없음")
...
function bubbleHtml(thumbUrl: string, emoji: string, minPrice: number, count: number, photoClass: string): string {
  return `
    <div style="position:relative;width:64px;height:64px;transform:translate(-50%,-50%);cursor:pointer;">
      <div style="width:100%;height:100%;border-radius:16px;overflow:hidden;border:2.5px solid #fff;box-shadow:0 8px 18px rgba(0,0,0,0.28);position:relative;background:linear-gradient(135deg,#fca5a5,#fdba74);">
        <span style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:28px;line-height:1;">${emoji}</span>
...
/** img 로드 실패 시 제거 → 뒤 이모지 폴백 (CSP 로 inline onerror 불가 → addEventListener). */
...
export function buildAggContent(sc: ServerCluster): HTMLElement {
  ...
  el.innerHTML = bubbleHtml(thumb, '🍽️', sc.min_price, sc.count, 'ur-agg-photo')
...
export function buildClusterContent(items: Restaurant[], minPrice: number): HTMLElement {
  ...
  const emoji = categoryEmoji((rep?.category || '').toLowerCase())
  const thumb = rep?.image_url ? cfImage(rep.image_url, { width: 132, height: 132, fit: 'cover', format: 'auto' }) : ''
  const el = document.createElement('div')
  el.innerHTML = bubbleHtml(thumb, emoji, minPrice, items.length, 'ur-cluster-photo')
  wireImgFallback(el, 'img.ur-cluster-photo')
  return el
}

/** 딜 핀 — 상품 사진 원형 + 카테고리 컬러 링 + 모서리 배지 (2026-06-22 대표). */

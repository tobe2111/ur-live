      ...
      <div className="ur-content-narrow px-4 py-6 lg:py-10 mx-auto" style={{ width: '100%' }}>
        ...
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-24 bg-gray-100 dark:bg-[#1D1F29] rounded-xl animate-pulse" />
            ))}
          </div>
        ) : isError ? (
          ...
          <ul className="space-y-3">
            {items.map(it => {
              ...
              return (
                <li key={it.access_id} className={`bg-white dark:bg-[#11141C] border ${isExpired ? 'border-red-200 dark:border-red-900 opacity-70' : 'border-gray-200 dark:border-[#2C2F35]'} rounded-2xl p-4`}>
                  <div className="flex gap-3">
                    {it.image_url ? (
                      <img src={cfImage(it.image_url, { width: 200, quality: 82, format: 'auto' }) || it.image_url} alt="" loading="lazy" className="w-16 h-16 rounded-lg object-cover shrink-0 bg-gray-100 dark:bg-[#1D1F29]" onError={(e) => cfImageOnError(e.currentTarget, it.image_url)} />
                    ) : (

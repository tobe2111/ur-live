        ...
        <div className="bg-white rounded-2xl max-w-sm w-full p-6 text-center space-y-4 shadow-sm">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900">
              {existingStatus === 'active' ? '이미 에이전시 권한 활성화됨' : '승인 대기 중'}
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              {existingStatus === 'active'
                ? '에이전시 대시보드에서 소속 셀러를 관리하세요'
                : '관리자 승인 후 에이전시 기능을 이용할 수 있어요'}
            </p>
          </div>
          <button
            ...
            {existingStatus === 'active' ? '에이전시 대시보드로' : '홈으로'}
          </button>
        </div>
      </div>
    )
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-gray-100">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <h1 className="text-base font-bold text-gray-900 flex-1">에이전시로 시작하기</h1>
        </div>
      </div>
      ...
      <div className="p-4 space-y-4">
        <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-2xl p-5 text-center">
          ...
          <h2 className="text-base font-bold text-gray-900">카카오 계정에 에이전시 권한 추가</h2>
          <p className="text-xs text-gray-600 mt-1 leading-relaxed">
            현재 카카오 계정으로 에이전시 권한을 신청합니다.<br />
            별도 가입/로그인 없이 한 번에 연동돼요.
          </p>
        </div>

        <div className="bg-white rounded-2xl p-5 space-y-4 border border-gray-100">
          <Field label="에이전시명 / 회사명" required>
        ...
        <p className="text-[11px] text-gray-500 text-center leading-relaxed">
          ...
          승인 완료 시 카카오 로그인으로 바로 에이전시 기능 이용 가능.
        </p>

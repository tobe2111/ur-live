...
import { Package, Wallet, Receipt, Plus, LogOut, Clock, CheckCircle, XCircle, X, Truck, Tag, ShieldCheck, BarChart3, AlertTriangle, Upload, ChevronRight, MessageCircle, Loader2, Download, Bell } from 'lucide-react'
...
export default function SupplierDashboardPage() {
  ...
  const headerRight = (
    <>
      ...
      <SupplierNotificationsBell t={t} onNavigate={(link) => navigate(link)} />
...
// 🔔 2026-06-12 (도매몰 감사 fix): 제조사 알림 벨 — 출금 승인/반려·신규 도매주문 등
//   recipient_type='supplier' 알림을 읽을 UI 가 없어 데드 코드였던 것을 배선.
//   mount 시 1회 + 60s 간격(탭 숨김이면 skip) unread 갱신, 열면 목록 로드 + 전체 읽음.
interface SupplierNotificationRow { id: number; type: string; title: string; message: string | null; link: string | null; is_read: number; created_at: string }
function SupplierNotificationsBell({ t, onNavigate }: { t: (k: string, o?: Record<string, unknown>) => string; onNavigate: (link: string) => void }) {

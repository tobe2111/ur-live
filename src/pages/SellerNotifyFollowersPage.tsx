...
import { Send, Bell, Loader2, Megaphone, AlertCircle, Check } from 'lucide-react'
  ...
  return (
    <SellerLayout title="단골에게 알림 발송">
      <div className="mx-auto max-w-5xl space-y-5">
        ...
        {/* 결과 */}
        {result && (
          <div className="bg-white border border-rule rounded-[var(--dash-radius,16px)] p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-brand-tint text-brand-text flex items-center justify-center shrink-0"><Check className="h-5 w-5" /></div>
            <div>
              <p className="text-sm font-bold text-tone-ok">발송 완료</p>
              <p className="text-xs text-tone-ok mt-0.5">{result.sent}/{result.total}명에게 push 전송됨</p>
            </div>
          </div>
        )}

        {/* 발송 안내 */}
        <div className="bg-white border border-rule rounded-[var(--dash-radius,16px)] p-4 flex gap-3">
          <AlertCircle className="w-5 h-5 text-tone-warn shrink-0" />
          <div className="text-xs text-tone-warn space-y-1">

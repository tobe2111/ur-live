...
export default function MyStorePage() {
  ...
  if (forbidden) {
    return (
      <div className="bg-white dark:bg-[#11141C] min-h-screen flex flex-col items-center justify-center px-8 text-center">
  ...
  return (
    <div className="bg-white dark:bg-[#11141C] min-h-screen pb-8">
      <SEO title="내 매장 — 유어딜" description="내 매장 이용권 사용·정산 현황" url="/my-store" noindex />
      <div className="sticky top-0 z-30 bg-white/95 dark:bg-[#11141C]/95 backdrop-blur-md border-b border-gray-100 dark:border-[#2C2F35] px-4 h-12 flex items-center">
                    ...
                    return (
                      <div key={p.id} className="flex items-center gap-2 rounded-xl border border-gray-100 dark:border-[#2C2F35] bg-white dark:bg-[#1D1F29] px-3.5 py-3">
                        ...
                        <button
                          ...
                          disabled={busy === p.id}
                          className={`shrink-0 px-2.5 py-1 rounded-lg text-[11px] font-bold disabled:opacity-50 ${active ? 'bg-gray-900/10 dark:bg-white/15 text-gray-900 dark:text-white' : 'bg-gray-100 dark:bg-[#1D1F29] text-gray-400 dark:text-gray-500'}`}
                        >
                          {active ? '노출중' : '숨김'}
                        </button>
                      </div>

      ...
      <form onSubmit={onSubmit} className="space-y-4" id="bank-info-section">
        <div>
          <label className="mb-1.5 block text-xs font-medium text-gray-700">
            {t('seller.bankName', { defaultValue: '은행명' })} <span className="text-tone-bad">*</span>
          </label>
          <select
            value={bankInfo.bank_name}
            onChange={e => setBankInfo(prev => ({ ...prev, bank_name: e.target.value }))}
            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-brand focus:ring-2 focus:ring-blue-500"
          >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1.5 block text-xs font-medium text-gray-700">
              {t('seller.accountNumber', { defaultValue: '계좌번호' })} <span className="text-tone-bad">*</span>
            </label>
            <input
              type="text"
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium text-gray-700">
              {t('seller.accountHolder', { defaultValue: '예금주' })} <span className="text-tone-bad">*</span>
            </label>
            <input
              type="text"
        ...
        <div className="flex items-start gap-2 rounded-lg border border-rule bg-white p-3 text-xs text-gray-700">
        ...
        <Button
          type="submit"
          disabled={submitting}
          className="ur-btn ur-btn-lg ur-btn-primary w-full transition-colors disabled:cursor-not-allowed disabled:opacity-50"
        >
          {submitting ? (

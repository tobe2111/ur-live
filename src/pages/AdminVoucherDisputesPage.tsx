...
function LocationEvidence({ d }: { d: Dispute }) {
  ...
  return (
    <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-bold border ${far ? 'bg-tone-bad-bg text-tone-bad border-transparent' : 'bg-tone-ok-bg text-tone-ok border-transparent'}`}>
...
function CustomerResponseBadge({ resp }: { resp?: string | null }) {
  if (resp === 'contested') return <span className="inline-block px-2 py-0.5 rounded-full text-[11px] font-bold bg-tone-bad-bg text-tone-bad border border-transparent">손님: "이용했어요" 항변</span>
  ...
  return <span className="inline-block px-2 py-0.5 rounded-full text-[11px] font-bold bg-tone-warn-bg text-tone-warn border border-transparent">손님 응답 대기</span>
}
      ...
      <div className="max-w-3xl mx-auto p-4 space-y-4">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {items.map(d => (
              <div key={d.id} className="bg-white rounded-[var(--dash-radius,16px)] border border-gray-200 p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex flex-col gap-2 shrink-0">
                    <button
                      ...
                      disabled={busy === d.id}
                      className="ur-btn ur-btn-md ur-btn-primary text-[13px] disabled:opacity-50"
                    >정산 진행</button>
                    <button

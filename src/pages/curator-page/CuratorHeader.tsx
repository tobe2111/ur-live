...
export default function CuratorHeader({
  ...
  //   중간 스톱 → 브랜드 로즈(#1C69EF) — 회색→로즈→잉크 온-브랜드 히어로(배너 없는 유어샵만 노출).
  // 🎨 2026-08-30 (대표 결정 "C안"): 사진 없는 유어샵의 얇은 띠.
  //   이전 값은 `linear-gradient(135deg,#6b7280,#1C69EF 42%,#1D1F29)` 인라인 스타일 — 로즈가 강한
  ...
  const bannerBandClass =
    'bg-gradient-to-b from-[#F3ECE9] to-[#F8F7FC] dark:from-[#202A36] dark:to-[#11141C]'

  const hasSns = !!(curator.youtube_url || curator.instagram_url || curator.tiktok_url)

  return (
    <header className="bg-white dark:bg-[#11141C] border-b border-gray-100 dark:border-[#2C2F35]">
      <HeaderMarquee
        curator={curator}
        isOwner={isOwner}
      ...
      <div className="max-w-3xl mx-auto">
        ...
        >
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-b from-transparent to-white dark:to-[#11141C]" />
      ...
      <div className="max-w-3xl mx-auto px-4 pb-4">
          ...
          {editingField === 'bio' ? (
            <div className="mt-2.5 max-w-md mx-auto">
              <textarea
                ...
                rows={2}
                maxLength={200}
                className="w-full text-[13.5px] text-gray-900 dark:text-white bg-gray-50 dark:bg-[#11141C] border border-gray-900 dark:border-white rounded-lg p-2 focus:outline-none resize-none text-center"
              />
        ...
        {/* SNS 편집 패널 (소유자) */}
        {isOwner && editingSns && (
          <div className="mt-3 rounded-xl border border-gray-200 dark:border-[#2C2F35] bg-gray-50 dark:bg-[#11141C] p-3 space-y-2 max-w-md mx-auto">
            {([['youtube_url', '유튜브'], ['instagram_url', '인스타그램'], ['tiktok_url', '틱톡']] as const).map(([key, label]) => (
              <div key={key} className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="flex-1 min-w-0 px-2.5 py-1.5 rounded-lg border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#1D1F29] text-[13px] text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none"
                />
              </div>
            ))}
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={() => setEditingSns(false)} className="px-4 py-2 rounded-lg bg-gray-100 dark:bg-[#1D1F29] text-gray-500 dark:text-gray-400 text-[13px] font-bold">취소</button>
            </div>
          </div>
        )}
        ...
        {isOwner ? (
          <div className="mt-4 max-w-md mx-auto">
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-[#2C2F35] bg-gray-50 dark:bg-[#11141C] p-3.5">
              ...
              {editingHandle ? (
                <div>
                  <div className="flex items-center gap-1 px-3 py-2.5 rounded-xl border border-gray-300 dark:border-[#2C2F35] bg-white dark:bg-[#1D1F29]">
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    <button onClick={() => { setEditingHandle(false); setHandleVal(curator.handle); setHandleStatus('idle'); setHandleMsg('') }} className="px-4 py-2 rounded-lg bg-gray-100 dark:bg-[#1D1F29] text-gray-500 dark:text-gray-400 text-[13px] font-bold">취소</button>
                  </div>
                </div>
              ) : (
                // 🔗 2026-06-18 (사용자 요청 — 배너 우상단 공유 버튼과 중복): 주소 카드의 링크복사/카카오공유 제거.
                //   주소 표시 + '주소 변경'만 유지. 공유는 배너 히어로 우상단 공유 아이콘으로 일원화.
                <div className="flex items-center px-3 py-2.5 rounded-xl bg-white dark:bg-[#1D1F29] border border-gray-200 dark:border-[#2C2F35]">

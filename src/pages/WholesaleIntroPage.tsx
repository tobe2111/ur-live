 ...
 *   제조사 → /supplier/register, 유통사 → /wholesale/join.
 */
    ...
    <div className="min-h-screen bg-white text-[#0C2454]">
      <SEO
        ...
        jsonLd={[
          ...
          faqJsonLd([
            { q: '유통스타트는 어떤 도매사이트인가요?', a: '제조사와 유통사를 직접 잇는 B2B 도매 플랫폼입니다. 유통사는 검증된 제조사 상품을 등급별 도매가(공급가)로 사입할 수 있고, 제조사는 영업 없이 전국 유통 채널에 상품을 공급할 수 있어요.' },
            ...
            { q: '도매가(공급가)는 어떻게 확인하나요?', a: '도매가는 승인된 유통사 로그인 후에만 열람할 수 있습니다. 제조사 신원과 원가는 비공개로, 등급에 맞는 공급가만 표시돼요.' },
          ]),
        ]}
      />
      ...
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-[#ECEEF1]">
        <div className="ur-content-wide mx-auto px-4 lg:px-8 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 text-sm">
            <button onClick={() => navigate('/wholesale/login')} className="px-3 py-2 text-[#4E5560] hover:text-[#0C2454] font-medium">유통사 로그인</button>
      ...
      <section className="ur-content-wide mx-auto px-4 lg:px-8 pt-14 pb-16 lg:pt-24 lg:pb-24 text-center">
        <span className="inline-block px-3 py-1 rounded-full bg-[#FC5424]/10 text-[#FC5424] text-xs font-bold mb-5">제조사 ↔ 유통사 직거래</span>
      ...
      <section className="bg-[#F4F5F7] border-y border-[#ECEEF1]">
        <div className="ur-content-wide mx-auto px-4 lg:px-8 py-16 lg:py-20 grid lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl bg-white border border-[#ECEEF1] p-7">
            <div className="flex items-center gap-2 mb-4"><Store className="w-6 h-6 text-[#0C2454]" /><h3 className="text-xl font-extrabold">유통사라면</h3></div>
            <ul className="space-y-3 text-sm text-[#4E5560]">
            ...
            <button onClick={() => navigate('/wholesale/join')} className="mt-6 inline-flex items-center gap-1.5 text-[#FC5424] font-bold text-sm">유통사 가입하기 <ArrowRight className="w-4 h-4" /></button>
          </div>
        </div>
      </section>

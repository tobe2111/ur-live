...
export default function DigitalProductSection({ formData, onChange, onKindChange }: Props) {
  ...
  return (
    <div className="bg-white border border-rule rounded-lg p-4 space-y-3">
      <div>
        <label className="block text-sm font-bold text-gray-700 mb-2">
          {t('seller.products.productKindLabel', { defaultValue: '상품 유형' })} <span className="text-tone-bad">*</span>
        </label>
            ...
            <button
              ...
              className={`text-left p-3 rounded-lg border-2 transition-colors ${
                formData.product_kind === opt.value
                  ? 'bg-white border-brand ring-2 ring-purple-200'
        ...
        <>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1.5">
              {t('seller.products.contentUrlLabel', { defaultValue: '콘텐츠 URL' })} <span className="text-tone-bad">*</span>
            </label>
            <input
              type="url"
              name="content_url"
              ...
              className="w-full px-3 py-2 border border-rule rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-purple-500 bg-white"
            />
            <p className="text-[11px] text-gray-700 mt-1">{t('seller.products.contentUrlHint', { defaultValue: 'R2 또는 외부 CDN URL. 구매자만 접근 가능한 signed URL이 자동 발급됩니다.' })}</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">{t('seller.products.contentFormatLabel', { defaultValue: '파일 형식' })}</label>
              <select
                name="content_format"
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">{t('seller.products.accessDaysLabel', { defaultValue: '접근 기간 (일)' })}</label>
              <input
                type="number"
          ...
          <div className="bg-white border border-rule rounded p-3 text-[11px] text-tone-warn">

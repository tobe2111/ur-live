        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {Object.entries(CAT_LABELS).map(([k, v]) => (
            <button
              key={k}
              onClick={() => setCat(k)}
              className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-bold border ${cat === k ? 'bg-gray-900 text-white border-gray-900' : 'bg-white dark:bg-[#0F151D] text-gray-700 dark:text-gray-200 border-gray-200'}`}
            >
              {v}
            </button>
          ))}
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 border border-gray-200 dark:border-[#2A3446] rounded-full text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-pink-300"
            />
          </div>
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as 'latest' | 'popular' | 'deadline')}
            className="px-3 py-2 border border-gray-200 dark:border-[#2A3446] rounded-full text-xs text-gray-900 dark:text-white font-medium bg-white dark:bg-[#0F151D]"
          >

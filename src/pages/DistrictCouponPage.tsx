...
function ReceiptForm({ campaign, stores, onDone }: { campaign: Campaign; stores: StoreRow[]; onDone: () => void }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] p-4 space-y-3">
      ...
      <div>
        ...
        <select value={storeId} onChange={(e) => setStoreId(e.target.value)}
          className="w-full rounded-lg border border-gray-300 dark:border-[#2C2F35] bg-white dark:bg-[#1D1F29] px-3 py-2.5 text-sm text-gray-900 dark:text-white">
      ...
      <div>
        ...
        <input value={approvalNo} onChange={(e) => setApprovalNo(e.target.value)} placeholder="영수증에 표기된 승인번호"
          className="w-full rounded-lg border border-gray-300 dark:border-[#2C2F35] bg-white dark:bg-[#1D1F29] px-3 py-2.5 font-mono text-sm text-gray-900 dark:text-white" />
...
function RedeemModal({ coupon, stores, onClose, onRedeemed }: { coupon: MyCoupon; stores: StoreRow[]; onClose: () => void; onRedeemed: () => void }) {
  ...
  if (doneBridge) {
    return (
      <div className="fixed inset-0 z-[10600] flex items-end sm:items-center justify-center bg-black/60" onClick={onClose} role="presentation">
        <div className="w-full sm:max-w-xs sm:mx-4 rounded-t-3xl sm:rounded-3xl bg-white dark:bg-[#11141C] p-6 text-center" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
  ...
  return (
    <div className="fixed inset-0 z-[10600] flex items-end sm:items-center justify-center bg-black/60" onClick={onClose} role="presentation">
      <div className="w-full sm:max-w-xs sm:mx-4 rounded-t-3xl sm:rounded-3xl bg-white dark:bg-[#11141C] p-6" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        ...
        <div className="mt-4 space-y-3">
          ...
          <input inputMode="numeric" value={pin} onChange={(e) => setPin(e.target.value.replace(/[^0-9]/g, '').slice(0, 8))}
            ...
            className="w-full rounded-lg border border-gray-300 dark:border-[#2C2F35] bg-white dark:bg-[#1D1F29] px-3 py-2.5 text-center font-mono text-lg tracking-[0.3em] text-gray-900 dark:text-white" />
...
export default function DistrictCouponPage() {
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-50 dark:bg-[#1D1F29] pb-24">
      ...
      <div className="mx-auto ur-content-narrow space-y-4 px-4 py-4 lg:px-8">
        ...
        {!isMy && campaign && (
          <>
            <div className="rounded-2xl border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] p-4">
            ...
            {campaign.status === 'open'
              ...
              : <div className="rounded-2xl border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] p-5 text-center text-sm font-bold text-gray-500 dark:text-gray-400">접수가 종료된 캠페인입니다</div>}
            <div className="rounded-2xl border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] p-4">
        ...
        {(isMy || unusedCoupons.length > 0) && (
          <div className="space-y-2.5">
            ...
            {unusedCoupons.length === 0 && isMy && (
              <div className="rounded-2xl border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] p-6 text-center text-sm text-gray-400 dark:text-gray-500">아직 쿠폰이 없어요 — 영수증을 등록해보세요</div>
            )}
            {unusedCoupons.map((cp) => (
              <button key={cp.id} type="button" onClick={() => void openRedeem(cp)}
                className="flex w-full items-center gap-3 rounded-2xl border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] p-4 text-left active:scale-[0.99] transition-transform">
        ...
        {isMy && (receipts.length > 0 || pastCoupons.length > 0) && (
          <div className="space-y-2.5">
            ...
            {receipts.map((r) => {
              ...
              return (
                <div key={r.id} className="flex items-center gap-3 rounded-xl border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] px-4 py-3">
                  <div className="min-w-0 flex-1">
            ...
            {pastCoupons.map((cp) => (
              <div key={cp.id} className="flex items-center gap-3 rounded-xl border border-gray-200 dark:border-[#2C2F35] bg-white dark:bg-[#11141C] px-4 py-3 opacity-60">
                <div className="min-w-0 flex-1">

      ...
      <main className="ur-content-narrow mx-auto px-4 py-4 space-y-4">
        ...
        <div className="flex gap-2">
          <select value={period} onChange={(e) => setPeriod(e.target.value as 'month' | 'all')}
            className="flex-1 px-3 py-2 border border-line rounded-lg text-xs bg-white dark:bg-[#11141C]">
        ...
        {/* 랭킹 리스트 */}
        <div className="bg-white dark:bg-[#11141C] border border-line rounded-2xl overflow-hidden">
          {loading ? (
            ...
            <ul>
              {list.map((r) => {
                ...
                return (
                  <li key={r.rank} className={`flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-[#2C2F35] last:border-0 ${isTop3 ? 'bg-amber-50' : ''}`}>

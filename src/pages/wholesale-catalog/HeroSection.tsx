...
export default function HeroSection({ loggedIn, userSession, grade, me, monthSpend, orderCount, depositBalance, setGradeOpen, featured, hasOrdered }: {
  ...
  // 🏭 2026-06-29 (대표 "로그인 시 배너가 사라져 헷갈림 / 게스트 기준 통일"): 히어로 배너를 로그인 여부와
  //   무관하게 *항상* 표시 → 메인 구조 일관(배너 안 사라짐). 로그인 회원은 아래에 계정 요약(등급·예치금·
  //   매입)·OEM 을 '추가'만. (게스트 = 배너만 / 회원 = 배너 + 대시보드.)
  return (
    <>
      <div className="pt-4 pb-1">
        <div className="relative rounded-2xl overflow-hidden min-h-[244px] lg:min-h-[290px] flex" style={{ background: '#0C2454' }}>
          ...
          <div className="relative p-6 lg:p-9 flex flex-col justify-center text-white lg:max-w-[560px]">
            {/* 🏭 2026-06-29 (메인에 무재고 드랍십 노출): 배지 2개(회원 전용 + 무재고 드랍십) */}

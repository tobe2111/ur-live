...
export default function SellerPromoteBoostsPage() {
  ...
  async function activate(boost: Boost) {
    ...
    try {
      ...
      toast.success('🚀 ' + t('seller.boosts.activateSuccess', { defaultValue: '부스팅 활성화! 메인 피드 상단 노출 시작.' }))
      fetchAll()
    } catch (err: any) {
      toast.error(err?.response?.data?.error || t('seller.boosts.activateFailed', { defaultValue: '활성화 실패' }))
    }
  }
  ...
  return (
    <SellerLayout title={t('seller.boosts.title', { defaultValue: '노출 부스팅' })}>
      <div className="p-6 space-y-6">
        ...
        {!activeLive && unused.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800">
            ⚠️ {t('seller.boosts.noLiveWarning', { defaultValue: '부스팅 쿠폰을 사용하려면 먼저 라이브를 시작해주세요.' })}
          </div>
        )}
        ...
        {unused.length > 0 && (
          <div>
            <h3 className="text-sm font-bold text-gray-900 mb-3">
              📨 {t('seller.boosts.availableSection', { defaultValue: '사용 가능' })} ({unused.length}{t('seller.boosts.bidUnit', { defaultValue: '건' })})
            </h3>
        ...
        {others.length > 0 && (
          <div>
            <h3 className="text-sm font-bold text-gray-900 mb-3">
              📋 {t('seller.boosts.historySection', { defaultValue: '사용/만료' })} ({others.length}{t('seller.boosts.bidUnit', { defaultValue: '건' })})
            </h3>
                ...
                return (
                  <div key={b.id} className="p-3 flex items-center justify-between text-xs">
                    ...
                    <div className="text-gray-500">
                      {b.status === 'active'
                        ? <span className="text-red-600 font-bold">🔴 {t('seller.boosts.activeStatus', { defaultValue: '활성 중' })}</span>
                        : b.status === 'consumed'
                          ? t('seller.boosts.usedStatus', { defaultValue: '사용 완료' })
                          : t('seller.boosts.expiredStatus', { defaultValue: '만료' })}

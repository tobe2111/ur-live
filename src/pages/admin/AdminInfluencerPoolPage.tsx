...
export default function AdminInfluencerPoolPage() {
  ...
  const [sheetsGate, setSheetsGate] = useState<boolean | null>(null) // 📊 ur-ads env 의 시트 동기화 게이트 실값(멈춤 원인 판별)
  ...
  const applyMeta = useCallback((d: Record<string, unknown>) => {
    ...
    setStats(g<PoolStats>('stats') || {}); setRun(g<RunStats>('run') || null); setGate(!!d.gate); setSheetsSync(g<typeof sheetsSync>('sheets_sync') || null); setSheetsGate(typeof d.sheets_gate === 'boolean' ? d.sheets_gate : null)
  ...
  return (
    <AdminLayout title="인플루언서 풀">
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <CollectDiagPanel run={run} sheetsSync={sheetsSync} sheetsGate={sheetsGate} maintenance={maintenance} maintenanceRescan={maintenanceRescan} maintainRunning={maintainRunning}

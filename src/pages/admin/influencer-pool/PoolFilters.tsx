  ...
  return (
    <div className="flex flex-wrap gap-2 mb-3">
      <select value={p.platform} onChange={e => p.setPlatform(e.target.value)} className={SEL}>
        ...
        <option value="naver_cafe">지역·커뮤니티 매체(카페)</option>
      ...
      <select value={p.catSource} onChange={e => p.setCatSource(e.target.value)} className={SEL} title="본문·소개글로 확인된 분류인지, 발굴 키워드에서 물려받은 값인지">
        <option value="">분류 신뢰도 전체</option>
        <option value="content">본문 확인됨</option>
        <option value="keyword">키워드 상속(미확인)</option>
      </select>
      ...
      <label className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-rule text-sm text-gray-700 bg-white cursor-pointer" title="스스로 신청한 리드(사전동의 — 자유 연락 가능)">
      ...
      <input value={p.matchRegion} onChange={e => p.setMatchRegion(e.target.value)} placeholder="지역(예: 강남·서울)" className="w-[140px] px-3 py-2 rounded-lg border border-rule text-sm text-gray-900" title="유어딜 매장 매칭 시 지역(시/군구/동) 필터 — 위 지역 필터(리드 쪽)와 별개다" />
      <button onClick={p.onSellerMatch} disabled={p.matchLoading} className="px-3 py-2 rounded-lg border border-rule bg-white text-gray-700 text-sm font-medium disabled:opacity-50" title="선택 카테고리(+지역)의 유어딜 매장 목록(읽기 전용)">

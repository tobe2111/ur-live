...
export default function MaintenanceButtons({ onChanged, canMerge }: { onChanged: () => Promise<void>; canMerge: boolean }) {
  ...
  async function mergeDuplicates() {
    if (!window.confirm('중복 리드를 통합할까요?\n① 같은 이메일 ② 같은 인스타 핸들 ③ 공유 링크(linktr.ee/블로그/유튜브 교차링크) ④ 이름+카테고리(동명이인 방지: 이메일·인스타 둘 다 없는 잔여, 2개+ 플랫폼일 때만)\n상태·정보가 가장 앞선 1건만 남기고 나머지 삭제.')) return
    setMerging(true)
    try {
  ...
  async function reclassify() {
    ...
    try {
      ...
      if (r.data?.success) { toast.success(`${formatNumber(r.data.scanned)}명 스캔 · ${formatNumber(r.data.changed)}명 재분류`); await onChanged() }
  ...
  return (
    <>
      <button onClick={maintainAll} disabled={maintaining} className="ur-btn ur-btn-md ur-btn-primary disabled:opacity-50" title="야간 자동 정비와 동일한 순서로 한 번에 실행 — 중복 통합 → 연락처 재추출 → 카테고리 재분류 → 리드 점수 → 라이브 재보정. 아래 개별 버튼은 특정 단계만 다시 돌릴 때만 쓰세요.">{maintaining ? '정비 시작 중…' : '전체 정비'}</button>
      <button onClick={mergeDuplicates} disabled={merging || !canMerge} className={cls} title="같은 이메일 중복 리드 통합">{merging ? '통합 중…' : '중복 통합'}</button>
      <button onClick={sheetsSync} disabled={sheetsSyncing} className="px-4 py-2 rounded-lg border border-rule bg-white text-tone-ok text-sm font-medium disabled:opacity-50" title="풀 전체를 구글 스프레드시트 pool 탭에 미러(서비스계정 설정 필요 — 매시간 자동 + 이 버튼 즉시)">{sheetsSyncing ? '시트 동기화 중…' : '구글시트 동기화'}</button>
      <button onClick={recategorize} disabled={recategorizing} className="px-4 py-2 rounded-lg border border-rule bg-white text-gray-700 text-sm font-medium disabled:opacity-50" title="유튜브 전 풀의 카테고리를 라이브(channels.list 배치)로 한 번에 재보정 — 규칙+YouTube 신호, 버튼 한 번">{recategorizing ? '재보정 중…' : '카테고리 전체 재보정'}</button>
      <button onClick={reclassify} disabled={reclassifying} className={cls} title="채널 이름·소개글 신호로 카테고리 재분류(저장 소개글 기반 — 라이브 재보정으로 안 되는 네이버/티스토리 보조, 멱등)">{reclassifying ? '재분류 중…' : '카테고리 재분류(저장)'}</button>
      ...
      <button onClick={qualityPass} disabled={scoring} className="px-4 py-2 rounded-lg border border-rule bg-white text-tone-warn text-sm font-medium disabled:opacity-50" title="브랜드 공식 채널 태깅 + 리드 점수(0~100) 재계산 — 매일 밤 자동 실행되므로 평소엔 불필요, 즉시 반영이 필요할 때만">{scoring ? '채점 중…' : '리드 점수·브랜드 태깅'}</button>
      <button onClick={routeBizBlogs} disabled={routingBiz} className="px-4 py-2 rounded-lg border border-rule bg-white text-gray-700 text-sm font-medium disabled:opacity-50" title="블로그/카페 중 업체(사업자) 계정을 찾아 B2B 파트너풀(광고주 리드)로 넘깁니다 — 먼저 표본을 보여주고 확인 후 실행. 인플루언서 풀에서는 숨김 처리만(삭제 아님)">{routingBiz ? '검사 중…' : '업체 블로그 → 파트너풀'}</button>
      <button onClick={refetchLive} disabled={refetching} className="px-4 py-2 rounded-lg border border-rule bg-white text-gray-700 text-sm font-medium disabled:opacity-50" title="유튜브 채널의 현재 라이브 About을 다시 불러 이메일·카테고리 교정(재추출로 안 되는 케이스 — 현재 About에만 개인메일. YouTube API 사용)">{refetching ? '라이브 재조회 중…' : '유튜브 라이브 재조회'}</button>
    </>
  )
}

...
export default function MarkContactedPanel() {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className="px-4 py-2 rounded-lg border border-rule bg-white text-gray-700 text-sm font-medium"
      ...
      {open && (
        <div className="fixed inset-0 z-[10500] flex items-center justify-center bg-black/50 p-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-3 rounded-lg bg-white border border-rule px-3 py-2 text-xs text-gray-700 leading-relaxed">
              내려받은 엑셀의 <b>ID 열</b>에서 실제로 보낸 행만 복사해 붙여넣으세요. 구분자는 아무거나 됩니다(줄바꿈·쉼표·공백).
              <div className="mt-1 text-[11px] text-gray-700">이미 처리된 리드는 최초 발송일이 유지됩니다 — 두 번 눌러도 안전합니다.</div>
            </div>

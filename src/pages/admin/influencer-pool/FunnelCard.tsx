...
export default function FunnelCard({ stats, categories }: { stats: FunnelStats; categories?: CategoryFunnelRow[] }) {
  ...
  return (
    <div className="mb-4 rounded-lg border border-rule bg-white px-4 py-3">
      <div className="text-sm font-medium text-gray-700 mb-2">아웃리치 전환 퍼널 <span className="text-xs font-normal text-gray-400">— 최근 7일 컨택 {formatNumber(stats.contacted7 || 0)}</span></div>
      <div className="flex items-center gap-2 flex-wrap">
        <Step label="컨택함" n={reached} cls="border border-rule bg-white text-gray-700" />
        {opened > 0 && <><span className="text-gray-300">→</span>
          <Step label="개봉" n={opened} rate={`개봉률 ${pct(opened, reached)}%`} cls="border border-rule bg-white text-gray-700" /></>}
        <span className="text-gray-300">→</span>
        <Step label="관심(회신)" n={replied} rate={`전환 ${pct(replied, reached)}%`} cls="border border-rule bg-white text-tone-warn" />
        <span className="text-gray-300">→</span>
        <Step label="계약" n={contracted} rate={`전환 ${pct(contracted, replied)}%`} cls="border border-rule bg-white text-tone-ok" />
        ...
        {consented > 0 && <Step label="사전동의" n={consented} cls="border border-rule bg-white text-gray-700" />}
      </div>
      {chans.length > 0 && (
        <div className="mt-2 text-[11px] text-gray-500">컨택 채널: {chans.map(c => `${c.k} ${c.n}`).join(' · ')}
          {stats.bounced ? <span className="text-tone-bad"> · 반송/신고 {formatNumber(stats.bounced)}</span> : null}</div>
      )}
      ...
      {cats.length > 0 && (
        <div className="mt-3 overflow-x-auto">
          <table className="text-[11px] text-gray-600">
            ...
            <tbody>
              {cats.map(cf => (
                <tr key={cf.category} className="border-t border-rule">
                  ...
                  <td className={`text-right px-2 font-medium ${pct(cf.replied, cf.reached) >= 20 ? 'text-tone-ok' : ''}`}>{pct(cf.replied, cf.reached)}%</td>

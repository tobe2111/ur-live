  ...
  return (
    ...
    {collect && (
      <div className="mb-3 text-xs text-gray-500">
        레인 A 자동수집 <span className={collect.gate ? 'text-tone-ok font-semibold' : 'text-gray-400'}>{collect.gate ? 'ON · 홀수시' : 'OFF'}</span>
        ...
        상가정보 <span className={storeinfo?.gate ? 'text-tone-ok font-semibold' : 'text-gray-400'}>{storeinfo?.gate ? 'ON · 짝수시' : 'OFF'}</span>
        {storeinfo?.run?.diag?.error ? <span className="text-tone-warn"> · {storeinfo.run.diag.error}</span>
          : storeinfo?.run?.last_run ? <span> · 최근 {kstShort(storeinfo.run.last_run)} · 저장 {storeinfo.run.saved ?? 0} / 연락처보강 {storeinfo.run.enriched ?? 0}</span>
            : <span className="text-gray-400"> · 아직 실행 안 됨</span>}
        {storeinfo?.run?.diag?.enrich_note && <span className="text-tone-warn"> · {storeinfo.run.diag.enrich_note}</span>}
      </div>
    )}
    ...
    {agencyFunnel && agencyFunnel.total > 0 && (
      <div className="mb-3 text-xs rounded-lg border border-gray-200 bg-gray-50 p-2.5 text-gray-500">
        ...
        <span> · <span className="text-gray-700 font-semibold">이메일 보유 {formatNumber(agencyFunnel.with_email)}</span></span>
    ...
    {registryMatch?.last_run && (
      <div className="mb-3 text-xs text-gray-500">
        ...
        <span> · 최근 {kstShort(registryMatch.last_run)} · 대조 {formatNumber(registryMatch.scanned ?? 0)} · <b className="text-gray-700">이식 {formatNumber(registryMatch.matched ?? 0)}</b> (누적 {formatNumber(registryMatch.total_matched ?? 0)})</span>
    ...
    <div className="mb-3 text-xs text-gray-500">
      이메일 보강 레인
      {enrichLast?.partial && <span className="ml-1 text-tone-warn font-semibold" title="실행 도중 저장된 스냅샷 — 인보케이션이 끝나기 전 상태(중도 종료 시에도 계측이 남도록 25건마다 저장)">진행 중/중단 스냅샷</span>}
      {enrichLast?.last_run
        ? <span> · 최근 {kstShort(enrichLast.last_run)} · 처리 {formatNumber(enrichLast.processed ?? 0)} · <b className="text-gray-700">확보 {formatNumber(enrichLast.enriched ?? 0)}</b>
            {typeof enrichLast.crawls === 'number' && enrichLast.crawls > 0
              ? <span> · 크롤 {formatNumber(enrichLast.crawls)}(이메일 적중 <b className={(enrichLast.hit_rate ?? 0) >= 15 ? 'text-tone-ok' : 'text-tone-warn'}>{enrichLast.hit_rate ?? 0}%</b>)</span>
              : <span className="text-tone-warn"> · 크롤 0회 — 크롤까지 못 감(예산·대상 선정 확인 필요)</span>}
        ...
        : <span className="text-tone-warn"> · 아직 실행 기록 없음 — 매시간 자동이 안 돌고 있을 수 있음</span>}
      ...
      {typeof enrichLast?.spent === 'number' && (
        <div className="mt-1 text-[11px] text-gray-400">
          ...
          {typeof enrichLast.d1 === 'number' && (
            <span> (외부요청 {formatNumber(enrichLast.fetches ?? 0)} + DB쓰기 <b className={enrichLast.d1 > (enrichLast.fetches ?? 0) ? 'text-tone-warn' : ''}>{formatNumber(enrichLast.d1)}</b>)</span>
          )}
          ...
          {enrichLast.limit_hit
            ? <span className="text-tone-warn font-semibold"> · 플랫폼 요청한도 도달 → 이번 라운드 중단(실패 도장 미기록) · 다음 실행 상한 {formatNumber(enrichLast.learned_cap ?? 0)}</span>
            : <span> · 한도 여유</span>}
          {/* ⏱벽시계 상한 도달 — 예산이 남아도 시간이 라운드를 끝냈다는 뜻(처방이 다르다: 캡 조정이 아니라
              동시성·타임아웃). ADS_ENRICH_DEADLINE_MS 로 무배포 조정 가능. */}
          {enrichLast.deadline_hit && (
            <span className="text-tone-warn font-semibold"> · 시간 상한 도달 → 정상 종료(남은 백로그는 다음 라운드)</span>
          )}
      ...
      {enrichRollup && enrichRollup.rounds > 0 && (
        <div className="mt-1 text-[11px] text-gray-400">
          ...
          {' · '}<span className={enrichRollup.partial >= enrichRollup.rounds ? 'text-tone-warn font-semibold' : ''}>중단 {formatNumber(enrichRollup.partial)}</span>
          ...
          {enrichRollup.crash > 0 && <span className="text-tone-bad font-semibold"> · 예외 {formatNumber(enrichRollup.crash)}</span>}
          {' · '}처리 {formatNumber(enrichRollup.processed)} · <b className="text-gray-700">확보 {formatNumber(enrichRollup.enriched)}</b>
    ...
    {kakaoSweep?.last_run && (
      <div className="mb-3 text-xs text-gray-500">
        전화 스윕(카카오)
        <span> · 최근 {kstShort(kakaoSweep.last_run)} · 조회 {formatNumber(kakaoSweep.tried ?? 0)} · <b className="text-gray-700">전화 확보 {formatNumber(kakaoSweep.found ?? 0)}</b> (누적 {formatNumber(kakaoSweep.total_found ?? 0)})</span>
        {kakaoSweep.limit_hit && <span className="text-tone-warn font-semibold"> · 한도로 조기 중단</span>}
    ...
    {commerce?.run && (
      <div className="mb-3 text-xs rounded-lg border border-gray-200 bg-gray-50 p-2.5">
        ...
        {commerce.run.diag?.error ? <span className="text-tone-warn"> · {commerce.run.diag.error}</span>
          : <span className="text-gray-500"> · 최근 {kstShort(commerce.run.last_run)} · 발굴 {formatNumber(commerce.run.found ?? 0)} / <b className="text-gray-700">신규 {formatNumber(commerce.run.saved ?? 0)}</b>
        ...
        {commerce.probe && (
          <span> · <span className={commerce.probe.hasEmail ? 'text-tone-ok font-semibold' : 'text-tone-bad font-semibold'}>이메일 필드 {commerce.probe.hasEmail ? `있음 ${commerce.probe.emailField ? ` (${commerce.probe.emailField}, 선택입력이라 일부만 채워짐)` : ''}` : '없음 '}</span></span>
        )}
            ...
            <div className="mt-1 grid grid-cols-1 sm:grid-cols-2 gap-x-4 text-[11px]">
              {commerce.run.diag.coverage.map(c => (
                <div key={c.key} className="flex gap-1.5 py-0.5 border-b border-gray-100">
                  <span className={c.pct === 0 ? 'text-tone-bad font-medium w-40 shrink-0' : 'text-gray-600 w-40 shrink-0'}>{c.key}</span>
                  <span className={c.pct === 0 ? 'text-tone-bad w-10 text-right' : 'text-gray-500 w-10 text-right'}>{c.pct}%</span>
    ...
    {franchise?.run && (
      <div className="mb-3 text-xs text-gray-500">
        프랜차이즈 <span className={franchise.gate ? 'text-tone-ok font-semibold' : 'text-gray-400'}>{franchise.gate ? 'ON · 22시' : 'OFF'}</span>
        {franchise.run.diag?.error ? <span className="text-tone-warn"> · {franchise.run.diag.error}</span>
        ...
        {(franchise.run.health?.fail_streak ?? 0) > 0 && (
          <div className="mt-1 text-[11px] text-tone-warn">
            {franchise.run.health?.fail_streak}회 연속 실패
    ...
    {npsInfo?.run && (
      <div className="mb-3 text-xs text-gray-500">
        규모 조회(국민연금) <span className={npsInfo.gate ? 'text-tone-ok font-semibold' : 'text-gray-400'}>{npsInfo.gate ? 'ON · 01시' : 'OFF'}</span>
        ...
        {npsInfo.run.diag?.error && <span className="text-tone-warn"> · {npsInfo.run.diag.error}</span>}
      </div>
    )}
      {/* 매장 후보(인허가) — 소비자 공개면(/new-openings·상권 리포트)·개업 웰컴 큐의 유일한 데이터원.
          2026-07-28 실측: 이 라인이 없어 "0건"인 걸 아무도 몰랐음(전체 실행 목록에서도 누락돼 있었음). */}
      <div className="mb-3 text-xs text-gray-500">
        매장 후보(인허가) <span className={localdata?.gate ? 'text-tone-ok font-semibold' : 'text-gray-400'}>{localdata?.gate ? 'ON · 05시' : 'OFF'}</span>
        {localdata?.run?.last_run
          ...
          : <span className="text-tone-warn"> · 아직 실행 안 됨 — 개업/상권 리포트 공개면이 빈 상태(전체 실행 1회로 채워짐)</span>}
        {localdata?.run?.diag?.error && <span className="text-tone-warn"> · {localdata.run.diag.error}</span>}
        ...
        {typeof localdata?.run?.pending_days === 'number' && localdata.run.pending_days > 0 && (
          <span className="text-tone-warn"> · 밀린 날 {formatNumber(localdata.run.pending_days)}일(체인이 이어서 소진)</span>
        )}
        ...
        {typeof localdata?.run?.backfill_days === 'number' && (
          localdata.run.backfill_days > 0
            ...
            : <span className="text-tone-warn font-semibold" title="ADS_LOCALDATA_BACKFILL_DAYS=0 — 과거 데이터를 전혀 안 긁는다. 유입이 '전일 변동분'뿐이라 전국 음식점 DB 가 사실상 안 쌓인다."> · 과거 백필 OFF — 전일 변동분만 유입</span>
        )}
      </div>

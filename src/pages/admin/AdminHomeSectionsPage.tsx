 ...
 *
 * 홈 상단의 "지금 인기 이용권 / 주말에 떠나는 숙소" 같은 **줄**을 만든다.
 *   (2026-09-04 대표 "마감 개념은 없어" — '마감 임박순' 규칙 소스는 제거됐다.)
 *
    ...
    <AdminLayout title="홈 섹션 관리">
      <div className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6 lg:p-8">
        <DashboardPageHeader
          ...
          actions={
            <button onClick={() => { setEditing(null); setShowForm(v => !v) }} className="ur-btn ur-btn-md ur-btn-primary inline-flex items-center gap-1.5">
        ...
        {msg && (
          <div className={`p-4 rounded-xl text-sm font-medium ${msg.ok ? 'bg-white text-tone-ok border border-rule' : 'bg-white text-tone-bad border border-rule'}`}>
            {msg.text}
          </div>
        )}
        ...
        {isError ? (
          <div className="rounded-[var(--dash-radius,16px)] border border-rule bg-white py-16 text-center">
          ...
          <div className="rounded-[var(--dash-radius,16px)] border border-rule bg-white py-16 text-center text-sm text-gray-400">불러오는 중...</div>
        ) : sections.length === 0 ? (
          <div className="rounded-[var(--dash-radius,16px)] border border-rule bg-white py-16 text-center">
          ...
          <div className="space-y-3">
            {sections.map((s, i) => {
              ...
              return (
                <div key={s.id} className={`rounded-[var(--dash-radius,16px)] border border-rule bg-white p-4 ${s.is_active ? '' : 'opacity-60'}`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <h3 className="text-sm font-semibold text-gray-900">{s.title}</h3>
                      {s.subtitle && <p className="text-xs text-gray-400 mt-0.5">{s.subtitle}</p>}
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-500">
                        <span className="inline-flex px-2 py-0.5 rounded-full bg-tone-info-bg text-tone-info font-medium">
                        ...
                        {src === 'manual' && (
                          <span className={(s.products?.length ?? 0) === 0 ? 'text-tone-warn font-medium' : ''}>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {src === 'manual' && (
                        <button onClick={() => setPickerFor(pickerFor === s.id ? null : s.id)}
                          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium text-tone-info bg-tone-info-bg hover:bg-gray-100">
                      ...
                      <button onClick={() => handleDelete(s)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium text-tone-bad bg-tone-bad-bg hover:bg-gray-100">

 ...
 *
 *   제안서 원본은 `public/static/proposals/*.html` 에 있고 이 화면은 iframe 으로 그대로 띄운다.
 *   파일을 고쳐 배포하면 화면도 자동으로 최신본이 된다(별도 DB 도, 업로드 절차도 없다).
 *
 *   ⚠️ **`?raw` import 로 되돌리지 말 것.** 처음엔 AdminPlatformModelPage 처럼 docs/ 원본을 `?raw` 로
 *      가져왔는데, 제안서에 라이브 화면 캡처를 base64 로 심자 그 문자열이 그대로 JS 청크가 되어
 *      **AdminProposalsPage 청크가 254KB** 로 불었고 `check-bundle-size --budget`(총 raw JS 8.6MB)이
 *      빨간불을 냈다. 정적 자산으로 두면 번들에 1바이트도 안 들어간다.
 *
 *   ⚠️ 경로는 `/static/*` 아래여야 한다. `_routes.json` 이 그 접두사를 워커에서 제외하고 있어
 *      Pages 가 파일을 직접 서빙한다. 다른 경로에 두면 워커가 SPA 셸을 돌려준다.
 *      (라이브 실측 확인: `/static/app.js` → 200 `application/javascript`.)
 *
 *   ℹ️ Pages 의 clean-URL 때문에 `.html` 요청은 확장자 없는 주소로 **308 리다이렉트**된다
 *      (실측: `/static/cart.html` → `/static/cart` → 200, 실제 파일). 브라우저와 iframe 이 그대로
 *      따라가므로 동작에는 영향이 없고, 개발자도구에서 308 을 봐도 고장이 아니다.
 *      확장자를 뗀 주소를 직접 쓰지는 않는다 — `.html` 은 clean-URL 이 꺼져도 맞는 주소다.
 *
 *   ⚠️ PDF 를 서버에서 만들지 않는다. 슬라이드 원본이 이미 `@page 13.333in x 7.5in`(16:9)로 짜여 있어
 *      브라우저 인쇄 대화상자의 "PDF로 저장" 이 그대로 뽑는다. 워커 부담 0, 폰트도 그대로.
 *      same-origin 이라 iframe 의 `contentWindow.print()` 와 문서 높이 읽기가 모두 된다.
 */
...
export default function AdminProposalsPage() {
  ...
  // 같은 출처라 문서 높이를 직접 읽을 수 있다(하드코딩보다 안전).
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <p className="text-[13px] text-gray-500 mb-4">
        <code className="px-1 rounded bg-gray-100 text-[0.85em]">public/static/proposals/</code> 의 원본을 그대로 띄웁니다.

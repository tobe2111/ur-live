      ...
      <div className="ur-content-full px-4 lg:px-8 py-6">
        ...
        {loading && !report ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 py-16 text-center">
            ...
            <p className="text-gray-500">{t('admin.integrity.noReport', { defaultValue: '아직 점검 기록이 없습니다. "지금 점검 실행"을 눌러 시작하세요.' })}</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500 border-b border-gray-100">
                  <th className="py-2.5 px-4 font-medium">{t('admin.integrity.checkName', { defaultValue: '점검 항목' })}</th>
                  <th className="py-2.5 px-4 font-medium text-right">{t('admin.integrity.count', { defaultValue: '고아행 수' })}</th>
                  <th className="py-2.5 px-4 font-medium">{t('admin.integrity.sampleIds', { defaultValue: '샘플 ID' })}</th>
                </tr>
              </thead>
              <tbody>
                {checks.map(ch => (
                  <tr key={ch.key} className="border-b border-gray-50 align-top">

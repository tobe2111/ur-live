    ...
    <AdminLayout title="매장 후보">
      <div className="p-4 lg:p-6 max-w-7xl mx-auto">
        ...
        {laneHealth.length > 0 && (
          <div className="mb-4 rounded-xl border border-rule bg-white p-3">
            ...
            <ul className="mt-1 space-y-0.5">
              {laneHealth.map(h => (
                <li key={h.lane} className="text-xs text-tone-warn">
        ...
        <div className="grid grid-cols-2 md:grid-cols-7 gap-3">
          ...
          {statCard('신규 개업', stats?.new_open || 0, '최근 인허가 · 전환율 최고', 'text-tone-bad')}
          {statCard('전화 보유', stats?.with_phone || 0)}
          {statCard('이메일 보유', stats?.with_email || 0, '홈페이지 게시분', 'text-gray-700')}
          {statCard('입점 완료', stats?.onboarded || 0, undefined, 'text-tone-ok')}
                  ...
                  {bySource.map(r => {
                    ...
                    return (
                      <tr key={r.source} className="border-t border-gray-100">
                        ...
                        <td className={`py-1 text-right tabular-nums ${reach < 10 ? 'text-tone-bad font-semibold' : 'text-gray-500'}`}>{reach}%</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <button onClick={runCollect} disabled={collecting || !collect?.adsBinding} className="ur-btn ur-btn-md ur-btn-primary disabled:opacity-50" title="지방행정 인허가 전일 변동분 1회 수집(일반음식점·휴게음식점·미용업·숙박업·동물미용업)">{collecting ? '수집 중…' : '인허가 수집'}</button>
          <button onClick={runEnrich} disabled={enriching || !collect?.adsBinding} className="ur-btn ur-btn-md ur-btn-primary disabled:opacity-50" title="이메일 우선 연락처 보강 — 홈페이지 크롤 + 네이버 링크발견(게시된 것만, 추측 0)">{enriching ? '보강 중…' : '이메일 보강'}</button>
          ...
          <button onClick={() => runCollectSub('store-kakao')} disabled={busySub !== '' || !collect?.adsBinding} className="ur-btn ur-btn-md ur-btn-danger disabled:opacity-50" title="카카오 로컬로 음식점·카페·미용실·숙박 + 무인매장을 캅니다(전화가 함께 옵니다). 자동으로는 약 5회차에 한 번만 도는 레인 — 아래 조건을 바꾼 뒤 지금 확인하려면 이 버튼">{busySub === 'store-kakao' ? '수집 중…' : '카카오 매장 수집'}</button>
        ...
        {collect && (
          <div className="mb-3 text-xs text-gray-500">
            인허가 자동수집 <span className={collect.gate ? 'text-tone-ok font-semibold' : 'text-gray-400'}>{collect.gate ? 'ON · 매일 KST 05시' : 'OFF'}</span>
            ...
            {neis?.run && (
              <><span className="mx-2 text-gray-300">|</span>학원 <span className={neis.gate ? 'text-tone-ok font-semibold' : 'text-gray-400'}>{neis.gate ? 'ON' : 'OFF'}</span>
                {neis.run.diag?.error ? <span className="text-tone-warn"> · {neis.run.diag.error}</span>
                  : <span> · 최근 {kstShort(neis.run.last_run)} · {neis.run.office || ''} 저장 {neis.run.saved ?? 0} (누적 {neis.run.total_saved ?? 0})</span>}</>
            )}
            {hira?.run && (
              <><span className="mx-2 text-gray-300">|</span>병원 <span className={hira.gate ? 'text-tone-ok font-semibold' : 'text-gray-400'}>{hira.gate ? 'ON' : 'OFF'}</span>
                {hira.run.diag?.error ? <span className="text-tone-warn"> · {hira.run.diag.error}</span>
            ...
            {kakao?.run && (
              <><span className="mx-2 text-gray-300">|</span>카카오 <span className={kakao.gate ? 'text-tone-ok font-semibold' : 'text-gray-400'}>{kakao.gate ? 'ON' : 'OFF'}</span>
                {kakao.run.diag?.error ? <span className="text-tone-warn"> · {kakao.run.diag.error}</span>
        ...
        <TradePanel endpoint="/api/admin/store-prospects/trades" title="매장 수집 업태 설정" unit="블록" adapt={adaptStoreTrades} />
        ...
        {enrichRun && (
          <div className="mb-3 text-xs text-gray-500">
            ...
            <b className="text-gray-700"> 이메일 +{formatNumber(enrichRun.email_found ?? 0)}</b> ·
            ...
            <span className="text-gray-400"> · 예산 {enrichRun.spent ?? 0}/{enrichRun.budget_total ?? 0}
              {enrichRun.deadline_hit ? ' · 시간초과' : ''}{enrichRun.limit_hit ? ' · 요청한도' : ''}</span>
        ...
        {collect?.run?.diag?.error && (collect.run.diag.fail_probe || collect.run.diag.probe) && (
          <div className="mb-3 p-2 rounded-lg bg-white border border-rule text-[11px] text-tone-warn break-all">
            ...
            {collect.run.diag.fail_probe?.url && (
              <div className="mt-1 text-tone-warn">실패 요청 · {collect.run.diag.fail_probe.endpoint} p{collect.run.diag.fail_probe.page} — {collect.run.diag.fail_probe.url}</div>
            )}
          </div>
        )}
                ...
                {loading ? (
                  ...
                  <tr key={r.id} className={r.active ? '' : 'opacity-50'}>
                    <td className="px-3 py-2 text-gray-900">
                      {r.is_new_open === 1 && <span className="mr-1 text-[10px] px-1.5 py-0.5 rounded bg-tone-bad-bg text-tone-bad font-semibold">개업</span>}
                      {r.biz_name}
                    ...
                    <td className="px-3 py-2 text-gray-600">
                      {r.email ? (
                        <span className="inline-flex items-center gap-1">
                          <a href={`mailto:${r.email}`} className="text-brand-text hover:underline break-all">{r.email}</a>

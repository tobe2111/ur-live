...
function buildBookmarklet(token: string): string {
  ...
  const code = `(async()=>{try{var T=${JSON.stringify(token)},A=${JSON.stringify(api)};` +
    ...
    `else if(lg>=tR&&!tP){S('로그인 페이지가 읽혔어요 — buyKorea 재로그인 후 다시 눌러주세요')}` +
    `else if(!tP){S(''+tR+'개 읽었지만 파싱 0 — 상세 1건을 Ctrl+A→Ctrl+C 해서 관리자에게 보내주세요')}` +
...
export default function AdminBuyerPoolPage() {
  ...
  const deleteAll = async () => {
    ...
    const ans = prompt(`수집된 바이어 리드 전체를 삭제합니다(복구 불가).\n진행하려면 DELETE 를 입력하세요.`)
  ...
  const runSaved = async () => {
    ...
    try {
      ...
      if (res?.ran) {
        ...
        toast.success(`저장된 소스 ${res.sources?.length || 0}곳 · 총 ${res.saved}건 신규/보강` + (expired.length ? ` · 쿠키 만료: ${expired.join(', ')}` : ''))
      ...
      <div className="p-4 lg:p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-4 rounded-xl border border-rule bg-white p-4">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-gray-700">
            <span className="font-semibold text-gray-700">이렇게 쓰세요:</span>
          ...
          <div className="mt-2 text-xs text-gray-600 leading-relaxed">
            <b>바이어 이메일은 플랫폼이 원천 마스킹</b>합니다(그게 그들의 사업모델) — 긁어서 확보하는 건 구조적으로 막혀 있고,
        ...
        {loadError && (
          <div className="mb-4 rounded-xl border border-rule bg-white p-3 text-sm text-tone-bad flex items-center justify-between gap-2">
            <span>데이터를 불러오지 못했습니다(네트워크·서버 오류). 아래 숫자·목록은 정확하지 않을 수 있습니다.</span>
            <button onClick={() => { loadStats(); loadLeads() }} className="ur-btn ur-btn-sm ur-btn-danger shrink-0">다시 시도</button>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <button onClick={() => setShowGuide(v => !v)} className="ur-btn ur-btn-md ur-btn-primary">수집 방법 · 북마클릿 {showGuide ? '숨기기' : ''}</button>
          <button onClick={() => setShowDemand(v => !v)} className="ur-btn ur-btn-md ur-btn-primary" title="어느 나라가 무엇을 사는지 — 연락처 마스킹과 무관한 수요 신호">수요 인텔리전스 {showDemand ? '숨기기' : ''}</button>
          ...
          {showAdvanced && <>
            ...
            <button onClick={() => setShowAuto(v => !v)} className="px-3 py-2 rounded-lg bg-white border border-rule text-sm text-tone-bad">상세 자동 수집 {showAuto ? '숨기기' : ''}</button>
            ...
            <button onClick={runDiag} disabled={diagLoading} className="px-3 py-2 rounded-lg bg-white border border-rule text-sm text-tone-warn disabled:opacity-50" title="이메일이 왜 안 나오는지 서버에서 실측 진단">{diagLoading ? '진단 중…' : '이메일찾기 진단'}</button>
          </>}
        ...
        {diagResult && (
          <div className="mb-4 rounded-xl border border-rule bg-white p-4">
            ...
            <pre className="text-xs text-gray-800 whitespace-pre-wrap break-all max-h-96 overflow-auto bg-white rounded-lg p-3 border border-rule">{diagResult}</pre>
          </div>
        )}
        ...
        {showGuide && (
          <div className="mb-4 rounded-xl border border-rule bg-white p-4">
            ...
            <div className="mb-3 rounded-xl border-2 border-rule bg-white p-4">
              <div className="text-sm font-bold text-gray-700 mb-1.5">방법 1 (추천) · 원클릭 북마클릿 — F12·쿠키 필요 없음</div>
              ...
              <div className="flex items-center gap-2 flex-wrap mb-2">
                ...
                <a ref={bmRef} onClick={e => e.preventDefault()} draggable className="ur-btn ur-btn-md ur-btn-primary cursor-move select-none inline-block shadow" title="이 버튼을 브라우저 즐겨찾기 바로 드래그하세요">유어딜 바이어 수집</a>
                <span className="text-xs text-gray-700 font-semibold">← 즐겨찾기 바로 <u>드래그</u></span>
                <button onClick={copyBookmarklet} className="px-3 py-2 rounded-lg bg-white border border-rule text-gray-700 text-xs font-semibold">드래그 안 되면: 코드 복사</button>
                {ingestToken && <button onClick={resetToken} className="text-[11px] text-gray-400 underline ml-1">토큰 재발급</button>}
              </div>
              <details className="mb-2">
                <summary className="text-[11px] text-gray-700 cursor-pointer">드래그가 안 되나요? (크롬 최신 버전 등) — 코드 복사로 등록하기</summary>
              ...
              {BUYER_SOURCES.map((s, i) => (
                <div key={s.name} className="rounded-lg bg-white border border-gray-200 p-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <a href={s.url} target="_blank" rel="noreferrer" className="text-sm font-semibold text-brand-text underline">{s.name}</a>
                    <span className="text-[11px] text-gray-400">{s.org}</span>
                    {i === 0 && <span className="px-1.5 py-0.5 rounded bg-tone-ok-bg text-tone-ok text-[10px]">추천</span>}
                  </div>
            ...
            <div className="mt-3 rounded-lg bg-white border border-rule p-3 text-[11px] text-tone-warn">
              <b>방법 1(북마클릿)이 대부분 사이트에서 가장 빠릅니다</b> — 리스트 전체 페이지를 자동으로 돌아 상세까지 수집. 수동 복사는 북마클릿이 안 될 때만 쓰세요. 수집한 컨택으로의 콜드 발송은 대상국 규제(GDPR/CAN-SPAM/CASL)를 따르세요.
            </div>
          </div>
        )}
        ...
        {showAuto && (
          <div className="mb-4 rounded-xl border-2 border-rule bg-white p-4">
            ...
            <div className="text-sm font-semibold text-tone-bad mb-1">상세 페이지 서버 자동 수집 (실험 · 위험)</div>
            <div className="text-xs text-tone-bad border border-rule bg-white rounded-lg p-2 mb-3">
              이 기능은 <b>대표님 로그인 쿠키로 서버가 상세 페이지들을 자동 방문</b>합니다. buyKorea·tradeKorea·EC21·ECPlaza·GoBizKorea 각 사이트 약관은 자동·대량 수집을 금지하며, <b>계정이 정지될 수 있습니다.</b> 위험을 감수하고 사용하세요. (방어: 소량 배치 · 요청 간 지연 · 쿠키는 저장하지 않고 이 요청에만 사용)
            ...
            <textarea value={autoCookie} onChange={e => setAutoCookie(e.target.value)} rows={2} placeholder="로그인 쿠키(cookie: 헤더 값 전체) 붙여넣기" className="w-full px-2 py-1.5 rounded-lg border border-transparent text-xs font-mono text-gray-900 mb-2" />
            <div className="flex flex-wrap items-center gap-2">
              <input value={autoListUrl} onChange={e => setAutoListUrl(e.target.value)} placeholder="리스트 페이지 URL (예: https://www.buykorea.org/seller/ec/inq/selectInquiryList.do?...)" className="flex-1 min-w-[240px] px-2 py-1.5 rounded-lg border border-rule text-sm text-gray-900" />
              <label className="text-xs text-gray-600">최대 <input type="number" min={1} max={30} value={autoMax} onChange={e => setAutoMax(Math.min(30, Math.max(1, Number(e.target.value) || 10)))} className="w-16 px-2 py-1.5 rounded-lg border border-rule text-sm text-gray-900" />건</label>
              <button onClick={runAutoFetch} disabled={autoRunning} className="ur-btn ur-btn-sm ur-btn-danger disabled:opacity-50">{autoRunning ? '수집 중…' : '자동 수집 실행'}</button>
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-rule">
              <div className="flex items-center justify-between mb-1.5">
                <div className="text-xs font-semibold text-gray-800">저장된 소스 {autoConfig.sources.length > 0 && `(${autoConfig.sources.length})`} {autoConfig.cookieHosts.length > 0 && <span className="text-[11px] font-normal text-tone-ok">· 쿠키 저장됨: {autoConfig.cookieHosts.join(', ')}</span>}</div>
                {autoConfig.sources.length > 0 && <button onClick={runSaved} disabled={autoRunning} className="ur-btn ur-btn-sm ur-btn-primary disabled:opacity-50">{autoRunning ? '수집 중…' : '▶ 저장된 소스 전부 수집'}</button>}
              </div>
                ...
                <div className="space-y-1">
                  {autoConfig.sources.map(s => (
                    <div key={s.url} className="flex items-center gap-2 text-[11px] bg-white rounded-lg border border-gray-200 px-2 py-1">
                      ...
                      <button onClick={() => forgetSource(s.url)} className="text-gray-300 hover:text-tone-bad shrink-0">삭제</button>
                    </div>
                  ))}
                </div>
              )}
        ...
        {showAdd && (
          <div className="mb-4 rounded-xl border border-gray-200 bg-white p-3">
            ...
            <div className="mt-2 flex gap-2">
              <button onClick={submitAdd} disabled={saving} className="ur-btn ur-btn-sm ur-btn-primary disabled:opacity-50">{saving ? '저장 중…' : '추가'}</button>
            ...
            <div className="mt-4 pt-3 border-t border-gray-100">
              <div className="text-xs text-gray-500 mb-1.5"><b>buyKorea·tradeKorea·EC21 등 구매요청 리스트를 통째로 복사(Ctrl+A → Ctrl+C)해 붙여넣기(Ctrl+V)</b> → 제품·국가·카테고리 자동 추출(여러 건 한 번에). 상세 페이지를 붙여넣으면 회사명·담당자·이메일·현재수입국까지 채워집니다(마스킹된 연락처는 자동 제외). 엑셀/시트 표(첫 줄 헤더, 탭·쉼표 구분)도 인식. <button type="button" onClick={() => setShowGuide(true)} className="text-brand-text underline">수집 방법 보기</button></div>
              ...
              <button onClick={submitBulk} disabled={importing} className="ur-btn ur-btn-sm ur-btn-primary mt-2 disabled:opacity-50">{importing ? '가져오는 중…' : '붙여넣기 일괄 추가'}</button>
            </div>
          </div>
        )}
        ...
        {showTargets && (
          <div className="mb-4 rounded-xl border border-gray-200 bg-white p-3">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              <button onClick={addTarget} className="ur-btn ur-btn-sm ur-btn-primary">타깃 추가</button>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {targets.map(t => (
                <button key={t.id} onClick={() => toggleTarget(t)} title={`발굴 ${t.found_total} · 저장 ${t.saved_total}`}
                  className={`px-2 py-1 rounded-full text-xs ${t.active ? 'bg-tone-ok-bg text-tone-ok border border-transparent' : 'bg-gray-100 text-gray-400 line-through'}`}>
        ...
        <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
            ...
            <div className="px-3 py-2 flex flex-wrap items-center gap-3 border-b border-gray-100 bg-gray-50 text-sm">
              ...
              <div className="ml-auto flex items-center gap-2">
                <button onClick={deleteSelected} disabled={bulkDeleting || selected.size === 0}
                  className="ur-btn ur-btn-sm ur-btn-danger disabled:opacity-40">
                  선택 삭제{selected.size > 0 ? ` (${selected.size})` : ''}
                </button>
                <button onClick={deleteAll} disabled={bulkDeleting}
                  className="px-3 py-1.5 rounded-lg border border-transparent text-tone-bad text-xs font-medium disabled:opacity-40">
                  전체 삭제
                </button>
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="divide-y divide-gray-100">
              {leads.map(l => (
                <div key={l.id} className={`p-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm ${selected.has(l.id) ? 'border border-rule bg-white' : ''}`}>
                  ...
                  <div className="min-w-[180px] flex-1">
                    <div className="font-medium text-gray-900 flex items-center gap-1.5">
                      {l.company}
                      {l.imports_from_korea === 1 && <span className="px-1.5 py-0.5 rounded bg-tone-ok-bg text-tone-ok text-[10px]">수입이력</span>}
                    </div>
                  ...
                  <div className="min-w-[200px] text-xs text-gray-600">
                    ...
                    {l.website && <a href={l.website.startsWith('http') ? l.website : `https://${l.website}`} target="_blank" rel="noreferrer" title={l.website} className="text-brand-text underline break-all">{prettyUrl(l.website)}</a>}
                    {!l.email && !l.decision_maker_email && !l.phone && !l.website && <span className="text-tone-warn" title="상세 페이지를 붙여넣으면 이 행에 연락처가 채워집니다">상세 확인 필요</span>}
                  </div>
                  ...
                  <button onClick={() => remove(l.id)} className="text-xs text-gray-300 hover:text-tone-bad">삭제</button>
                </div>
              ))}
            </div>
          )}
        ...
        <p className="mt-3 text-xs text-gray-400">
          수집된 컨택은 공개된 비즈니스 정보입니다. 마케팅 발송(콜드 아웃리치)은 대상국 규제(EU GDPR·미국 CAN-SPAM·캐나다 CASL)를 따르세요 — 이 도구는 발굴·자격심사·매칭까지만 담당합니다.
        </p>
      </div>
    </AdminLayout>
  )

        ...
        {items.map((p) => {
          ...
          return (
            <button
              ...
              className="snap-start shrink-0 w-[160px] text-left rounded-2xl overflow-hidden border border-line bg-surface hover:shadow-md transition-shadow"
            >
              {/* 🏭 2026-06-04 (카드 로딩 체감): 큐레이션 스트립도 cfImage(리사이즈)+srcSet+dominant_color
                  — 기존 원본 풀사이즈 <img> → 첫 화면 이미지 지연. 메인 그리드와 동일 최적화. */}
              <div className="relative w-full aspect-square bg-gray-100 dark:bg-[#1D1F29]" style={p.dominant_color ? { backgroundColor: p.dominant_color } : undefined}>
                {p.image_url ? (
                  <img

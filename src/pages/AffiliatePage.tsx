    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-gray-100">
        <div className="ur-content-narrow flex items-center gap-3 px-4 lg:px-8 py-3">
          ...
          <h1 className="text-[17px] font-bold text-gray-900">{t('affiliate.title')}</h1>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-500">{t('affiliate.loginPrompt')}</div>
      ) : (
        <div className="ur-content-narrow px-4 lg:px-8 py-5 space-y-4">
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[15px] font-bold text-gray-900">{t('affiliate.myLinkTitle')}</p>
            </div>
          ...
          {/* 이용 방법 */}
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <p className="text-[15px] font-bold text-gray-900 mb-3">{t('affiliate.howToTitle')}</p>
          ...
          <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
            <div className="px-4 py-3 border-b border-gray-100">
              <p className="text-[15px] font-bold text-gray-900">{t('affiliate.feeTitle')}</p>
            </div>
          ...
          <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              <p className="text-[15px] font-bold text-gray-900">{t('affiliate.historyTitle')}</p>
              <span className="text-xs text-gray-400">{t('affiliate.historyCount', { count: (data.recent || []).length })}</span>
            </div>
            {(data.recent || []).length === 0 ? (
              <div className="py-12 text-center">
                <TrendingUp className="w-10 h-10 text-gray-200 mx-auto mb-3" />
                <p className="text-sm text-gray-400">{t('affiliate.historyEmpty')}</p>
                <p className="text-xs text-gray-300 mt-1">{t('affiliate.historyEmptyHint')}</p>
              </div>
            ) : (
              <div className="divide-y divide-gray-50">
                {(data.recent || []).map((r: { product_name?: string; created_at: string; commission: number; order_amount: number }, i: number) => (
                  <div key={i} className="flex items-center justify-between px-4 py-3">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-gray-900 truncate">{r.product_name || t('affiliate.productFallback')}</p>
                    ...
                    <div className="text-right ml-3">
                      ...
                      <p className="text-[10px] text-gray-400">{t('affiliate.purchaseAmount', { amount: formatNumber(r.order_amount) })}</p>
                    </div>
                  </div>
                ))}
              </div>

...
export function rateLimiter(config) {
    return async (c, next) => {
        ...
        const kv = (c.env.RATE_LIMIT_KV || c.env.SESSION_KV);
        if (!kv) {
        ...
        try {
            ...
            // KV에 카운터 저장
            await kv.put(key, JSON.stringify({ count, resetTime }), { expirationTtl: Math.ceil(config.windowMs / 1000) + 10 });
            // Rate Limit 헤더 추가
            c.header('X-RateLimit-Limit', config.maxRequests.toString());
 ...
 * 사용자 티어 감지
 */
export function detectUserTier(c) {

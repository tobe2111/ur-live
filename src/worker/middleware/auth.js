...
async function getFirebasePublicKeys() {
    ...
    const res = await fetch('https://www.googleapis.com/robot/v1/metadata/x509/securetoken@system.gserviceaccount.com', { cf: { cacheTtl: 3600, cacheEverything: true } });
    if (!res.ok) {
...
async function importCertPublicKey(pem) {
    ...
    const pemBody = pem
        ...
        .replace(/\s+/g, '');
    const derBuffer = base64UrlToUint8Array(pemBody).buffer;
...
async function verifyFirebaseToken(token, projectId) {
    try {
        ...
        const isValid = await crypto.subtle.verify('RSASSA-PKCS1-v1_5', publicKey, signature.buffer, signedData);
        if (!isValid) {
 ...
 */
export function requireUserType(...types) {
    return async (c, next) => {
        ...
        if (!types.includes(currentUser.type)) {
            return c.json(forbiddenResponse(`Access denied. Required user type: ${types.join(' or ')}`), 403);
        }
        return next();
    };
}
 ...
 */
export function hasUserType(c, type) {
...
export function requireOwnership(userIdParam = 'id') {
    return async (c, next) => {
        ...
        if (resourceUserId !== String(user.id)) {
            return c.json(forbiddenResponse('You can only access your own resources'), 403);
        }
        return next();
    };
}
 ...
 * Generate JWT token
 */
export async function generateJWT(payload, secret, expiresIn = 86400 // 24 hours
) {
    const now = Math.floor(Date.now() / 1000);

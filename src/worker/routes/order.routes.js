...
ordersRouter.post('/', async (c) => {
    try {
        ...
        const seller = await qb.queryOne('SELECT id, name, base_shipping_fee, free_shipping_threshold, status FROM sellers WHERE id = ?', [request.seller_id]);
        ...
        const shippingFee = calculateShippingFee(subtotal, seller.base_shipping_fee, seller.free_shipping_threshold ?? undefined);
        // ── 동시성 안전 재고 차감 (Optimistic Lock) ─────────────────
        // D1 batch()는 원자적으로 실행됩니다.
        // 각 UPDATE에 `AND stock_quantity >= qty` 조건을 포함하여
        // 동시 주문으로 인한 초과 판매를 방지합니다.
        const stockResult = await orderRepo.reserveStock(orderItems.map(i => ({
        ...
        try {
            ...
            await orderRepo['qb'].batch(restoreStmts).catch(e => console.error('[ORDERS] stock restore failed:', e));
...
ordersRouter.get('/', async (c) => {
    try {
        ...
        const orderRepo = new OrderRepository(c.env.DB);
        const { orders, total } = await orderRepo.findByUserId(userId, parseInt(page, 10), Math.min(parseInt(limit, 10), 100));
        return c.json({
            success: true,
            data: {
                items: orders,
...
ordersRouter.post('/:id/cancel', async (c) => {
    try {
        ...
        if (paymentMadeStatuses.includes(order.status)) {
            ...
            const tossResult = await tossCancelPayment(paymentKey, tossSecretKey, reason, cancelAmount);

/**
 * Smoke Test Routes (admin only)
 *
 * 운영자가 핵심 endpoint 의 5xx 여부를 빠르게 점검.
 *
 *   GET /api/_internal/smoke-test       — 모든 핵심 endpoint 호출 + 5xx 카운트
 *   GET /api/_internal/smoke-test-auth  — 인증 흐름 (로그인/리프레시) 진단
 *
 * 🛡️ 2026-04-27: TD-006 Phase D — worker/index.ts 인라인 핸들러 분리.
 */
import { Hono } from 'hono';
import type { Env } from '@/worker/types/env';
import { requireAdmin } from '../middleware/auth';

const smokeTestRoutes = new Hono<{ Bindings: Env }>();

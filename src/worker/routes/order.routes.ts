    ...
    if (paymentMadeStatuses.includes(order.status)) {
      ...
      if (((order as any).payment_method) === 'deal_points') {
        ...
        if (fullCancel) {
          ...
          // 🛡️ 2026-06-26 (소비자 감사 다): 전액 취소 시 부가 적립·쿠폰·공구권 역전 (refundOrderFully 대칭).
          try { await reverseOrderAncillaryOnRefund(c.env.DB, Number(order.id), String(order.order_number), reason); } catch (e) { console.error('[ORDERS] Ancillary reversal (deal cancel) error:', e); }
        }
      ...
      // 🛡️ 2026-06-26 (소비자 감사 다): 전액 취소 시 부가 적립·쿠폰·공구권 역전 (refundOrderFully 대칭).
      //   cancelAmount 미지정(전체) 또는 누적 취소가 total 도달 시만 — 부분 취소는 전체역전이 틀리므로 제외.
      if (cancelAmount === undefined || (Number(order.refunded_amount ?? 0) + cancelAmount) >= Number(order.total_amount ?? 0)) {
        try { await reverseOrderAncillaryOnRefund(c.env.DB, Number(order.id), String(order.order_number), reason); } catch (e) { console.error('[ORDERS] Ancillary reversal (cancel paid) error:', e); }
      }

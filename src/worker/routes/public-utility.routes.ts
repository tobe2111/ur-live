...
publicUtilityRoutes.get('/api/vouchers/categories', async (c) => {
  ...
  try {
    ...
    //   진단 결과:
    //     - gift_catalog: 2260개 / brand_name 85종 / goods_type_detail 정상.
    //     - products: 2260개 / brand_name 모두 NULL / category 모두 'voucher' / JOIN 100% 매칭.
  ...
  INNER JOIN gift_catalog gc ON gc.gift_code = p.kt_alpha_gift_code
    ...
    if (debug) {
      // 디버깅용 raw 데이터 (운영자가 ?debug=1 호출).
      return c.json({
        success: true,
        data: sections,
        debug: {
          categories_count: sections.length,
          total_brands: sections.reduce((sum, s) => sum + s.brands.length, 0),
          sample_categories: sections.slice(0, 3).map(s => ({ category: s.category, count: s.count, brand_sample: s.brands.slice(0, 2) })),
        },
      })
    }

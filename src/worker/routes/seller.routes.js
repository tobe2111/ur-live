...
sellersRouter.get('/', async (c) => {
    try {
        ...
        const countRow = await qb.queryOne(`SELECT COUNT(*) as count FROM sellers ${where}`, params);
        const rows = await qb.queryMany(`SELECT id, name, slug, description, logo_url, email,
...
sellersRouter.get('/:id', async (c) => {
    try {
        ...
        const seller = await qb.queryOne(`SELECT id, name, slug, description, logo_url, email, phone,
              base_shipping_fee, free_shipping_threshold,
              country, currency, timezone, status, is_verified,
              created_at, updated_at
       FROM sellers WHERE id = ? AND status = 'ACTIVE'`, [sellerId]);
        if (!seller) {

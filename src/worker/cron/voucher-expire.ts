/**
 * 🎫 2026-06-21: 식사권(교환권) 만료 임박 알림 cron.
 *
 * 매일 09:00 UTC 실행 (stay-voucher-expire 와 동시, KST 18:00):
 *   - vouchers.expires_at 이 D-7 / D-3 / D-1 인 미사용(unused) 식사권
 *   - 사용자에게 앱 내 알림(notifications) + 카카오 알림톡(인프라 설정 시) 발송
 *
 * 배경: 선결제 식사권은 유효기간(기본 90일)이 있어, 잊고 안 쓰면 낸 돈이 소멸.
 *   숙소(stay)는 이미 stay-voucher-expire.ts 로 알림 중이나 식사권은 없었음 → 동일 패턴 복제.
 *
...
export async function runMealVoucherExpireCron(env: Env): Promise<{ d7: number; d3: number; d1: number; alimtalk_sent: number; alimtalk_failed: number }> {
  ...
  // D-7 / D-3 / D-1 임계 — 미사용 식사권(KT 교환권 제외).
  const sql = `
  ...
  for (const r of (rows.results || [])) {
    ...
    try {
      const productName = r.product_name || '식사권'
      ...
      if (alimtalkConfigured && sendAlimtalk && r.user_phone) {
        ...
        if (/^01\d{8,9}$/.test(phone)) {
          ...
          const message =
            `[유어딜] 식사권 유효기간 안내\n\n` +
            `${productName}${where} 식사권이 ${daysLeft}일 후 만료됩니다.\n\n` +
            `· 코드: ${r.code}\n` +

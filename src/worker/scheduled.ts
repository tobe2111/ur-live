...
import { drainOutreachEmails } from '../features/marketing/api/outreach-email';
...
export async function handleCronScheduled(
  ...
  if (cron === '*/5 * * * *') ctx.waitUntil(safeCron('outreach-email-drain', () => drainOutreachEmails(env))); // 📮 제안 이메일 드립(일일캡·CAS)

 ...
 */
export async function requestTossRefund(paymentKey, reason) {
    try {
        // 실제로는 Toss API 호출
 ...
 * 재고 복구
 */
export async function restoreStock(db, orderId) {
    const items = await db
        .prepare('SELECT product_id, quantity FROM order_items WHERE order_id = ?')
        .bind(orderId)
        .all();
    if (items.results) {
        await db.batch(items.results.map((item) => db
 ...
 * 환불 내역 기록
 */
export async function recordRefundHistory(db, orderId, amount, reason) {
    await db
        .prepare('INSERT INTO refund_history (order_id, amount, reason, refunded_at) VALUES (?, ?, ?, datetime("now"))')
 ...
 * 통합 환불 처리 로직
 */
export async function processRefund(db, payload) {

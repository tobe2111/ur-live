 ...
 *
 *   판매사(distributor)·제조사(supplier) 가입 성공 직후 호출. 모두싸인 카카오 서명요청 발송 +
...
async function run(env: ModusignEnv, DB: D1Database, input: SignupContractInput): Promise<void> {
  const templateId = env.MODUSIGN_TEMPLATE_ID
  if (!env.MODUSIGN_API_KEY || !templateId) return // 미설정 → no-op(행 없음 = 미차단)
  ...
  const title = `유통스타트 ${isSupplier ? '제조사 공급계약서' : '판매사 거래계약서'} - ${(input.businessName || input.signerName || String(input.accountId)).slice(0, 60)}`
  const res = await sendContractFromTemplate(env, {
    ...
    metadata: { account_type: input.accountType, account_id: input.accountId, doc_type: isSupplier ? 'supplier_agreement' : 'distributor_agreement' },
  })
  if (res.ok) {
    await recordContractRequest(DB, {
...
export function dispatchSignupContract(c: ContractContext, input: SignupContractInput): void {
  const p = run(c.env, c.env.DB, input).catch(() => { /* fail-soft */ })

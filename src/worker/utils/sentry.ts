...
class SentryClient {
   ...
   */
  private parseStackTrace(stack?: string): { frames: any[] } | undefined {
...
export async function captureException(
  error: Error,
  context?: Parameters<SentryClient['captureException']>[1]
): Promise<void> {
  if (sentryInstance) {
...
export async function captureMessage(
  message: string,
  level?: 'fatal' | 'error' | 'warning' | 'info' | 'debug',
  context?: Parameters<SentryClient['captureMessage']>[2]
): Promise<void> {
  if (sentryInstance) {

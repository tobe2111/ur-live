 ...
 * 재고 확인
 */
export async function validateStock(db, products) {
 ...
 * 중복 주문 확인
 */
export async function validateDuplicateOrder(db, orderId) {
 ...
 * 주문 롤백 (에러 발생 시)
 */
export async function rollbackOrder(db, orderId) {
    try {
        // 재고 복구
        await db
            .prepare(`
 ...
 * 통합 결제 처리 로직
 */
export async function processCheckout(db, payload) {
    ...
    try {
        ...
        await db.batch([
            // 재고 차감
            ...products.map((item) => db
                .prepare('UPDATE products SET stock = stock - ? WHERE id = ?')
                .bind(item.quantity, item.productId)),
            // 주문 생성
            db
                .prepare('INSERT INTO orders (id, user_id, amount, status, payment_key, created_at) VALUES (?, ?, ?, ?, ?, datetime("now"))')
                .bind(orderId, userId, amount, 'paid', paymentKey),
            // 주문 아이템 생성
            ...products.map((item) => db
                .prepare('INSERT INTO order_items (order_id, product_id, quantity, price_snapshot) VALUES (?, ?, ?, ?)')

...
export function validatePassword(password, options = {}) {
    const { minLength = 8, maxLength = 128, requireSpecialChar = false, requireNumber = false, requireUppercase = false, } = options;
    ...
    if (password.length < minLength) {
        throw new ValidationError(`Password must be at least ${minLength} characters`, 'password', 'TOO_SHORT');
    }
    if (password.length > maxLength) {
        throw new ValidationError(`Password must be at most ${maxLength} characters`, 'password', 'TOO_LONG');
    }
    ...
    if (requireSpecialChar && !/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
        throw new ValidationError('Password must contain at least one special character', 'password', 'MISSING_SPECIAL_CHAR');
    }
    return password;
}
/**
...
export function validateArray(value, fieldName, options = {}) {
    ...
    if (value.length < minLength) {
        throw new ValidationError(`${fieldName} must contain at least ${minLength} items`, fieldName, 'TOO_SHORT');
    }
    if (value.length > maxLength) {
        throw new ValidationError(`${fieldName} must contain at most ${maxLength} items`, fieldName, 'TOO_LONG');
    }
    if (itemValidator) {
...
export function validateDate(value, fieldName, options = {}) {
    ...
    if (value instanceof Date) {
        ...
        throw new ValidationError(`${fieldName} must be a valid date`, fieldName, 'INVALID_TYPE');
    }
    if (isNaN(date.getTime())) {
        throw new ValidationError(`${fieldName} is not a valid date`, fieldName, 'INVALID_DATE');
    }
    const now = new Date();
    if (future && date <= now) {
        throw new ValidationError(`${fieldName} must be in the future`, fieldName, 'NOT_FUTURE');
    }
    if (past && date >= now) {
        throw new ValidationError(`${fieldName} must be in the past`, fieldName, 'NOT_PAST');
    }
    if (min && date < min) {
        throw new ValidationError(`${fieldName} must be after ${min.toISOString()}`, fieldName, 'TOO_EARLY');
    }
    if (max && date > max) {
        throw new ValidationError(`${fieldName} must be before ${max.toISOString()}`, fieldName, 'TOO_LATE');
    }
    return date;
}
/**

 ...
 *   3. 벤더사: 지속배분+시한부 — 영입 가게 GMV 1%(플랫폼 5%에서), 실판매 시에만,
 *      가게당 24개월 한도. 율·기간은 어드민이 벤더사별 조절(여기선 기본값만).
...
export interface FeeRates {
  ...
  platformPct: number;
  /** 벤더사 GMV % (플랫폼 수수료에서 분배). 기본 1. */
  agencyPct: number;
  /** 벤더사 지속배분 시한(개월). 기본 24. (리졸버는 withinTerm boolean 만 받음 — 참고용 상수) */
...
/** 벤더사 적립 조건 — 호출자가 DB 사실로 판정해 전달. */
export interface AgencyContext {
  /** 영입 벤더사 식별(있을 때만 적립 대상). 없으면 agency 슬라이스 0. */
...
export interface FeeBreakdown {
  ...
  ownership: Ownership;
  productKind: ProductKind;
  /** 플랫폼 수수료 총액(벤더사 포함). 1P=0. */
  platform: number;
  /** 벤더사 몫(platform 안에서 분배). */
  agency: number;
...
export function resolveOrderFees(ctx: FeeContext, rates: FeeRates = DEFAULT_FEE_RATES): FeeBreakdown {
  ...
  // 규칙 3: 벤더사 — 실판매 + 시한 내 + 영입 벤더사 있을 때만. platform 안에서 분배(≤ platform 가드).
  let agency = 0;
 ...
 *   fee_agency_pct        (기본 1)   — 벤더사 GMV %
 *   fee_agency_term_months(기본 24)  — 벤더사 지속배분 시한(개월)
 */

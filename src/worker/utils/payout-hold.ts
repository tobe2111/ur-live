 ...
 *
 * 대표: *"정산은 우리도 그럼 영업일 기준 7일 이렇게 하는게 좋을까?"* → 실측 후 **10일(역일)** 확정.
 *
 * ## 무엇을 막나
 ...
 *
 * ## 왜 "영업일"이 아니라 역일인가
 * 레포에 **공휴일 테이블이 없다.** 영업일을 쓰려면 한국 공휴일을 관리해야 하는데 설·추석은 음력이라
 * 매년 바뀌고 대체공휴일도 붙는다. 틀리면 **정산이 조용히 하루씩 어긋난다** — 에러가 안 나는 클래스다.
 * 역일로 근사하면 그 테이블 자체가 필요 없다. 영업일 10일을 원하면 `payout_hold_days=14` 로 두면 된다
 * (2주에 주말이 4일 + 공휴일 여유).
 *
 ...
 * 환불 역전(`debit`)까지 10일 미루면, 환불이 났는데 그 차감이 열흘 뒤에 반영된다 ⇒ **과다지급**.
 ...
 * 여기는 다르다 — 유보는 영구 정지가 아니라 열흘이고, 실패 방향의 손실이 비대칭이다:
 * **늦게 주는 실패는 회복되지만, 먼저 준 돈은 못 돌려받는다.**
 */
import type { D1Database } from '@cloudflare/workers-types'

/** 대표 확정값(2026-09-21). 영업일 10일을 원하면 `payout_hold_days=14`. */
export const DEFAULT_PAYOUT_HOLD_DAYS = 10

...
export class ProductRepository {
    ...
    async findById(id) {
        const row = await this.qb.queryOne(`SELECT p.*, s.name as seller_name, s.slug as seller_slug
       FROM products p
       LEFT JOIN sellers s ON p.seller_id = s.id
       WHERE p.id = ? AND p.status != 'DELETED'`, [id]);
    ...
    async findMany(params) {
        ...
        const countRow = await this.qb.queryOne(`SELECT COUNT(*) as count FROM products p ${where}`, queryParams);
        const total = countRow?.count ?? 0;
        const rows = await this.qb.queryMany(`SELECT p.*, s.name as seller_name, s.slug as seller_slug
       FROM products p
       LEFT JOIN sellers s ON p.seller_id = s.id
       ${where}
    ...
    async findByIds(ids) {
       ...
       FROM products p
       LEFT JOIN sellers s ON p.seller_id = s.id
       WHERE p.id IN (${placeholders}) AND p.status = 'ACTIVE'`, ids);

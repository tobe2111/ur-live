...
export class OrderRepository {
     ...
     */
    async createOrder(userId, request, items, subtotal, shippingFee) {
    ...
    async findById(orderId) {
        const row = await this.qb.queryOne(`SELECT o.*, s.name as seller_name, s.slug as seller_slug 
       FROM orders o
       LEFT JOIN sellers s ON o.seller_id = s.id
       WHERE o.id = ?`, [orderId]);
        if (!row)
            return null;
        const items = await this.qb.queryMany('SELECT * FROM order_items WHERE order_id = ? ORDER BY created_at', [orderId]);
    ...
    async findByOrderNumber(orderNumber) {
        const rows = await this.qb.queryMany(`SELECT o.*, s.name as seller_name, s.slug as seller_slug
       FROM orders o
       LEFT JOIN sellers s ON o.seller_id = s.id
       WHERE o.order_number = ?
       ORDER BY o.created_at`, [orderNumber]);
        const orders = await Promise.all(rows.map(async (row) => {
            const items = await this.qb.queryMany('SELECT * FROM order_items WHERE order_id = ? ORDER BY created_at', [row['id']]);
            return this.mapOrder(row, items);
        }));
        return orders;
    }
    ...
    async findByUserId(userId, page = 1, limit = 20) {
        ...
        const countRow = await this.qb.queryOne('SELECT COUNT(*) as count FROM orders WHERE user_id = ?', [userId]);
     ...
     * Update order status
     */
    async updateStatus(orderNumber, status, extra) {
        ...
        await this.qb.execute(`UPDATE orders SET ${setFields.join(', ')} WHERE order_number = ?`, params);
    }
    /**
     * Update status of a single order (by ID)
     */
    async updateStatusById(orderId, status, extra) {
        const setFields = ['status = ?'];
        const params = [status];
        if (extra?.toss_payment_key) {
            setFields.push('toss_payment_key = ?');
        ...
        params.push(orderId);
        await this.qb.execute(`UPDATE orders SET ${setFields.join(', ')} WHERE id = ?`, params);
    }
    /**
     * Restore stock for cancelled order items
     */
    async restoreStock(orderId) {
        const items = await this.qb.queryMany('SELECT product_id, quantity FROM order_items WHERE order_id = ? AND status != ?', [orderId, 'CANCELLED']);
        if (items.length === 0)
            return;
     ...
     */
    async reduceStock(orderId) {
        const items = await this.qb.queryMany('SELECT product_id, quantity FROM order_items WHERE order_id = ?', [orderId]);
        if (items.length === 0)
            return;
        const statements = items.map(item => ({
     ...
     */
    async isAlreadyProcessed(orderNumber, status) {
        const row = await this.qb.queryOne('SELECT COUNT(*) as count FROM orders WHERE order_number = ? AND status = ?', [orderNumber, status]);
    ...
    async findByIdempotencyKey(key) {
        const row = await this.qb.queryOne('SELECT * FROM orders WHERE idempotency_key = ?', [key]);
        if (!row)
            return null;
    ...
    async getPaymentInfo(orderId) {
        return this.qb.queryOne('SELECT toss_payment_key, total_amount, status FROM orders WHERE id = ?', [orderId]);
    }
    /**
     * Mark order as cancel-failed with a reason (rollback marker)
     */
    async markCancelFailed(orderId, reason) {
        await this.qb.execute(`UPDATE orders SET
         cancel_fail_reason = ?,
         updated_at = datetime('now')
       WHERE id = ?`, [reason, orderId]);
    }
    mapOrder(row, items) {
        return {
            id: String(row['id'] ?? ''),

...
export class FirebaseAdmin {
     ...
     * 라이브 스트림 상태 업데이트
     */
    async updateStreamStatus(streamId, data) {
        try {
     ...
     * 상품 재고 업데이트
     */
    async updateProductStock(productId, stock, additionalData) {
        try {
     ...
     * 셀러가 현재 상품 변경
     */
    async changeCurrentProduct(streamId, newProductId) {
        try {
     ...
     * 재고 부족 알림 전송 (채팅)
     */
    async sendLowStockAlert(streamId, productName, currentStock) {
        try {
     ...
     * 품절 알림 전송
     */
    async sendSoldOutAlert(streamId, productName) {
        try {
            const chatRef = `chats/stream${streamId}`;
            const timestamp = Date.now();
    ...
    async createCustomToken(uid, claims) {
        try {
            ...
            if (!this.privateKey || !this.clientEmail || !this.projectId) {
                const missingCreds = [];
                if (!this.privateKey)
                    missingCreds.push('FIREBASE_PRIVATE_KEY');
                if (!this.clientEmail)
                    missingCreds.push('FIREBASE_CLIENT_EMAIL');
                if (!this.projectId)
                    missingCreds.push('FIREBASE_PROJECT_ID');
            ...
            const cryptoKey = await crypto.subtle.importKey('pkcs8', privateKeyDer, {
            ...
            console.error('[Firebase Custom Token] Error name:', error.name);
            console.error('[Firebase Custom Token] Error message:', error.message);
            console.error('[Firebase Custom Token] Error stack:', error.stack);
            // Re-throw with more context
            throw new Error(`Failed to create Firebase custom token: ${error.message}`);
        }
    }
    /**
 ...
 */
export function initFirebaseAdmin(env) {
    return new FirebaseAdmin(env);
}
/**
 * D1 데이터를 Firebase에 동기화
 */
export async function syncD1ToFirebase(firebase, type, data) {
    try {
        if (type === 'stream') {

...
// 🛡️ 2026-05-01: URL 기반 401 디바운스 — useEffect 가 같은 endpoint 를
//   재호출 시 새 request config 객체로 들어와 _retry 우회 가능.
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      // 🛡️ 2026-05-01: URL 기반 디바운스 — useEffect 가 같은 endpoint 재호출 시
      //   _retry 우회 가능. 같은 URL 5초 내 401 반복 → 즉시 reject.
      const now = Date.now();
      const lastFail = _recent401.get(url);
      if (lastFail && (now - lastFail) < URL_401_DEBOUNCE_MS) {
        if (import.meta.env.DEV) console.warn('[API] URL-level 401 debounce:', url);
        return Promise.reject(error);
      }
      _recent401.set(url, now);
      // _recent401 size 제한 (메모리 leak 방어)
      if (_recent401.size > 50) {
        const oldestKey = _recent401.keys().next().value;
        if (oldestKey !== undefined) _recent401.delete(oldestKey);
      }

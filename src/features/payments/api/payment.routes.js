...
import { validateRequiredString, validateNumber, ValidationError } from '@/worker/utils/validation';
import { successResponse, notFoundResponse, badRequestResponse, validationErrorResponse, unauthorizedResponse, internalServerErrorResponse } from '@/worker/utils/response';
 ...
 */
async function confirmTossPayment(paymentKey, orderId, amount, secretKey) {
    try {
 ...
 */
async function cancelTossPayment(paymentKey, cancelReason, cancelAmount, secretKey) {
    try {
...
paymentRoutes.post('/confirm', requireAuth(), async (c) => {
    try {
        ...
        return c.json(internalServerErrorResponse(error.message || 'Payment confirmation failed'), 500);
    }
});
/**
...
paymentRoutes.post('/rollback', requireAuth(), async (c) => {
    try {
        ...
        return c.json(internalServerErrorResponse(error.message || 'Payment rollback failed'), 500);
    }
});

...
adminRoutes.post('/login', cors(), async (c) => {
    ...
    try {
        ...
        const admins = await executeQuery(DB, 'SELECT id, username, email, password_hash, name, role, created_at FROM admins WHERE email = ?', [email]);
        if (admins.length === 0) {
...
adminRoutes.post('/refresh', cors(), async (c) => {
    ...
    try {
        ...
        const admins = await executeQuery(DB, 'SELECT id, username, email, name, role FROM admins WHERE id = ?', [adminId]);
        if (admins.length === 0) {

export class KakaoAuthService {
    ...
    constructor(db, kakaoRestApiKey) {
        this.db = db;
     ...
     */
    async getServiceTerms(accessToken) {
    ...
    async upsertUser(kakaoUser) {
      ...
      `).bind(kakaoUser.kakaoId).first();
          ...
          WHERE id = ?
        `).bind(kakaoUser.name, kakaoUser.email || null, kakaoUser.profileImage || null, userId).run();
        ...
        `).bind(kakaoUser.kakaoId, kakaoUser.name, kakaoUser.email || null, kakaoUser.profileImage || null).run();

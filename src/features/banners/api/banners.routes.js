...
bannerRoutes.post('/api/admin/banners', requireAuth, async (c) => {
    ...
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `)
            .bind(title, image_url, link_url || null, description || null, is_active !== false ? 1 : 0, display_order || 0, start_date || null, end_date || null)
            .run();
...
bannerRoutes.put('/api/admin/banners/:id', requireAuth, async (c) => {
    ...
    try {
      ...
      WHERE id = ?
    `)
            .bind(title, image_url, link_url || null, description || null, is_active ? 1 : 0, display_order || 0, start_date || null, end_date || null, id)
            .run();

...
app.get('/intro-code', async (c) => {
  ...
  return c.json({
    ...
    data: {
      ...
      // 🏷️ 2026-07-02 대표 확정: 폴백 = SSOT 기본율(1%) — 하드코딩 2.0 제거.
      commission_pct: row?.store_intro_commission_pct ?? (await import('../../../shared/constants/policy')).COMMISSION_DEFAULTS.AGENCY_STORE_INTRO_PCT,

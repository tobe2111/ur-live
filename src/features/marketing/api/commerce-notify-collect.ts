 ...
 *
 *   ⇒ 역할을 갈라 놓는다:
 *       · `RUN_DEADLINE_MS`     = **I/O 예산** — 얼마나 기다릴 것인가.
 *       · `MAX_RECORDS_PER_RUN` = **CPU 손잡이** — 몇 건을 파싱할 것인가.
 *
 *   ⚠️ **정정(같은 날, CI 가 잡았다)**: 처음엔 *"마감선은 I/O 예산이니 올려도 CPU 비용 ≈ 0"* 이라고 썼다.
 *     **틀렸다.** 마감선을 올리면 페이지가 한 장 더 들어오고 그만큼 파싱량 = CPU 가 는다.
 *     기존 가드(`ads-cpu-deadline.test.ts`, 08-02 신설)가 정확히 그 이유로 13초 상한을 걸어 두고 있었다.
 *     맞는 문장은 이것이다 — **마감선은 CPU 를 *간접적으로*(페이지 수를 통해) 늘리고,
 *     `MAX_RECORDS_PER_RUN` 은 그 총량에 *직접* 천장을 씌운다.** 그래서 상한이 낮으면 마감선을 올려도
 *     총 파싱량은 상한이 정한다.
 *
 *   🧮 실효 상한 = `MAX_RECORDS_PER_RUN + PAGE_ROWS`(상한은 페이지를 **받기 전에** 보므로 한 장은 넘친다)
 *     = 700 + 500 = **1,200** < 죽던 회차의 **1,499**. 마감선을 20초로 올려도 이 천장은 그대로다.
 *

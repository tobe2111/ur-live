/**
 * 🌱 인플루언서 발굴 **시드 키워드**(데이터 전용) — `influencer-auto-collect.ts` 에서 분리(2026-07-29).
 *   탐색 *범위*라 구조 문서 갱신 대상이 아니고 자유롭게 확장한다. 로직이 아니라 목록이므로
 *   수집 엔진 본문과 섞여 있을 이유가 없다(엔진이 600줄 래칫에 닿아 분리 — 우회 대신 실제로 줄였다).
 */

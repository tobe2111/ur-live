...
import { envSubreqCap, envLaneBudget, companyRunDeadlineMs } from './collect-budget'
...
const KEYWORD_SEED_VERSION = 5 // 2026-08-23: 4단계 고수율 광맥 6업종×시군구 — 이메일 수율 실측 기반. 배열 끝 추가라 이어받는다
...
/**
 * ☎️ 카카오 전화 스윕은 2026-08-30 에 `kakao-sweep-lane.ts` 로 분리됐다(600줄 래칫 — 이동만, 로직 불변).
 *   worker-ads 가 이 경로로 동적 import 하므로 **재수출로 계약을 유지한다**(경로를 바꾸면 조용히 못 찾는다).
 */
export { runKakaoPhoneSweep } from './kakao-sweep-lane'

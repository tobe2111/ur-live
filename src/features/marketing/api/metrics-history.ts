...
import { loadSearchAdConnection, loadSearchAdConnectionByTenant, getActiveTenantId, listAllTenantPairs } from './searchad-connection'
...
export async function ensureMetricsHistorySchema(DB: D1Database): Promise<void> {
  ...
  _schemaDone.add(DB)
  // tenant = 네이버 고객사(customer_id). 계정이 여러 고객사를 연결하면 고객사별 1행(멀티테넌트 격리).
...
/** 특정 날짜·고객사의 계정 합계를 ad_daily_metrics 에 UPSERT(1행/계정/고객사/일). creds 없으면 skip(연동 전). */
export async function snapshotAccountMetricsForDate(env: Env, accountId: number, dateYmd: string, creds?: SearchAdCreds | null, tenant = ''): Promise<{ ok: boolean; reason?: string }> {
  ...
  await env.DB.prepare(`INSERT INTO ad_daily_metrics (account_id, tenant, snap_date, cost, conv_amt, clicks, conv, imp, roas, avg_rnk)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(account_id, tenant, snap_date) DO UPDATE SET cost = excluded.cost, conv_amt = excluded.conv_amt, clicks = excluded.clicks, conv = excluded.conv, imp = excluded.imp, roas = excluded.roas, avg_rnk = excluded.avg_rnk`)
    .bind(accountId, tenant, dateYmd, Math.round(T.salesAmt), Math.round(T.convAmt), Math.round(T.clkCnt), Math.round(T.ccnt), Math.round(T.impCnt), roas, avgRnk)
    .run().catch(() => null)
  return { ok: true }
}

/** 자기 계정 즉시 기록(대시보드 '지금 갱신'/첫 진입) — 활성 고객사의 어제(완전) + 오늘(부분) 2일. */
export async function snapshotAccountRecent(env: Env, accountId: number, nowMs?: number): Promise<{ ok: boolean; reason?: string }> {
  ...
  const tenant = (await getActiveTenantId(env.DB, accountId).catch(() => null)) || ''
  await snapshotAccountMetricsForDate(env, accountId, ymd(now - 86400000), creds, tenant).catch(() => null)
  await snapshotAccountMetricsForDate(env, accountId, ymd(now), creds, tenant).catch(() => null)
  return { ok: true }
}

/** cron — 연결된 모든 (계정, 고객사)의 '어제' 실적 스냅샷(고객사별 1행 — 멀티테넌트 전수 커버). */
export async function snapshotAllAccounts(env: Env, nowMs?: number, cap = 300): Promise<{ pairs: number; snapped: number; date: string }> {
  await ensureMetricsHistorySchema(env.DB)
  const now = nowMs ?? Date.now()
  const dateYmd = ymd(now - 86400000) // 어제(완전한 하루)
  const pairs = await listAllTenantPairs(env.DB, cap).catch(() => [])
  let snapped = 0
  for (const p of pairs) {
    const creds = await loadSearchAdConnectionByTenant(env.DB, p.seller_id, p.customer_id, env.DATA_ENCRYPTION_KEY).catch(() => null)
    if (!creds) continue // 삭제된 고객사 → creds 없음 → skip(안전)
    const res = await snapshotAccountMetricsForDate(env, p.seller_id, dateYmd, creds, p.customer_id).catch(() => ({ ok: false as const }))
    if (res.ok) snapped++
  }
  return { pairs: pairs.length, snapped, date: dateYmd }
}

/** 최근 N일 시계열(오름차순) — 차트 입력. tenant 지정 시 그 고객사만(멀티테넌트 격리). */
export async function getMetricsHistory(DB: D1Database, accountId: number, days = 30, tenant?: string | null): Promise<DailyMetric[]> {
  await ensureMetricsHistorySchema(DB)
  const span = Math.min(120, Math.max(1, Math.round(days)))
  const rows = (await (tenant != null
    ? DB.prepare(`SELECT snap_date, cost, conv_amt, clicks, conv, imp, roas, avg_rnk FROM ad_daily_metrics
        WHERE account_id = ? AND tenant = ? ORDER BY snap_date DESC LIMIT ?`).bind(accountId, tenant, span)
    : DB.prepare(`SELECT snap_date, cost, conv_amt, clicks, conv, imp, roas, avg_rnk FROM ad_daily_metrics
        WHERE account_id = ? ORDER BY snap_date DESC LIMIT ?`).bind(accountId, span)
  ).all<DailyMetric>().catch(() => null))?.results || []

...
export const SHEET_HEADER = ['ID', '플랫폼', '이름', '핸들', 'URL', '구독자수', '이메일', '인스타', '틱톡', '카테고리', '수집키워드', '상태', '컨택채널', '컨택일', '팔로업', '출처', '동의일', '메모', '수집일'] as const
export interface SheetLead {
  id: number; platform: string; name: string; handle: string | null; url: string
  subscriber_count: number; email: string | null; instagram: string | null; tiktok: string | null
...
export function leadToRow(l: SheetLead): (string | number)[] {
  return [l.id, l.platform, l.name, l.handle || '', l.url, l.subscriber_count || 0, l.email || '', l.instagram || '',
...
export async function syncInfluencerPoolToSheets(env: Env): Promise<{ ok: boolean; rows?: number; error?: string }> {
  ...
  for (let off = 0; ; off += PAGE) {
    const page = (await env.DB.prepare(`SELECT id, platform, name, handle, url, subscriber_count, email, instagram, tiktok,

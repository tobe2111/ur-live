...
import { adminAdsCollectGatesRoutes } from './admin-ads-collect-gates.routes' // 🎛️ 수집 트랙 원클릭 게이트(카페)
...
app.get('/stats', async (c) => {
  await ensureAdsAccountSchema(adsLeadsDb(c.env))
  const t = await adsLeadsDb(c.env).prepare(`SELECT
      COUNT(*) AS total,
...
app.get('/accounts', async (c) => {
  ...
  const like = `%${q}%`
  const rows = (await (q
    ? adsLeadsDb(c.env).prepare(`SELECT id, email, company_name, phone, status, access_unlocked, created_at, last_login_at FROM ad_accounts
        WHERE LOWER(email) LIKE ? OR LOWER(COALESCE(company_name, '')) LIKE ? ORDER BY id DESC LIMIT ?`).bind(like, like, limit)
    : adsLeadsDb(c.env).prepare('SELECT id, email, company_name, phone, status, access_unlocked, created_at, last_login_at FROM ad_accounts ORDER BY id DESC LIMIT ?').bind(limit)
  ...
  const connSet = new Set(((await adsLeadsDb(c.env).prepare('SELECT DISTINCT seller_id FROM ad_searchad_tenants').all<{ seller_id: number }>().catch(() => null))?.results || []).map(r => r.seller_id))
  const alertSet = new Set(((await adsLeadsDb(c.env).prepare('SELECT account_id FROM ad_alert_settings WHERE enabled = 1').all<{ account_id: number }>().catch(() => null))?.results || []).map(r => r.account_id))
  const planMap = new Map(((await adsLeadsDb(c.env).prepare('SELECT account_id, plan FROM ad_entitlements').all<{ account_id: number; plan: string }>().catch(() => null))?.results || []).map(r => [r.account_id, r.plan]))
...
app.get('/access-requests', async (c) => {
  await ensureAdsAccountSchema(adsLeadsDb(c.env)); await ensureAccessRequestSchema(adsLeadsDb(c.env))
  const rows = (await adsLeadsDb(c.env).prepare(`SELECT r.id, r.account_id, r.note, r.status, r.created_at, a.email, a.company_name, a.phone
...
app.post('/access-requests/:id/decide', async (c) => {
  ...
  const upd = await adsLeadsDb(c.env).prepare("UPDATE ad_access_requests SET status = ?, decided_at = datetime('now') WHERE id = ? AND status='pending'")
    .bind(approve ? 'approved' : 'rejected', id).run().catch(() => null)
  if (upd?.meta?.changes !== 1) return c.json({ success: false, error: '이미 처리된 요청입니다' }, 409)
  const req = await adsLeadsDb(c.env).prepare('SELECT account_id FROM ad_access_requests WHERE id = ?').bind(id).first<{ account_id: number }>().catch(() => null)
  if (approve && req) {
    await adsLeadsDb(c.env).prepare('UPDATE ad_accounts SET access_unlocked = 1 WHERE id = ?').bind(req.account_id).run().catch(() => null)
    // 승인 안내 메일 — Resend 키 있을 때만(best-effort, 실패해도 승인은 유효 — 재로그인 시 자동 입장).
    if (c.env.RESEND_API_KEY && c.env.RESEND_FROM) {
      const acc = await adsLeadsDb(c.env).prepare('SELECT email, company_name FROM ad_accounts WHERE id = ?').bind(req.account_id).first<{ email: string; company_name: string | null }>().catch(() => null)
...
app.patch('/accounts/:id', async (c) => {
  ...
  if (body.plan !== undefined) {
    ...
    await ensureEntitlementSchema(adsLeadsDb(c.env))
    await setPlan(adsLeadsDb(c.env), id, p as AdsPlan, body.period_end ? String(body.period_end) : null)
    if (!sets.length) return c.json({ success: true })
  }
  if (!sets.length) return c.json({ success: false, error: '변경할 항목이 없습니다' }, 400)
  await adsLeadsDb(c.env).prepare(`UPDATE ad_accounts SET ${sets.join(', ')} WHERE id = ?`).bind(...binds, id).run().catch(() => null)
...
app.post('/accounts/:id/reset-password', async (c) => {
  ...
  const r = await adminSetPassword(adsLeadsDb(c.env), id, newPassword)
...
app.get('/services', async (c) => c.json({ success: true, services: await listServices(adsLeadsDb(c.env), true) }))

// POST /api/admin/ads/services — 상품 생성/수정(id 있으면 수정)
app.post('/services', async (c) => {
  const b = await c.req.json().catch(() => ({} as Record<string, unknown>))
  const r = await adminUpsertService(adsLeadsDb(c.env), {
...
app.get('/service-orders', async (c) => {
  ...
  return c.json({ success: true, orders: await adminListOrders(adsLeadsDb(c.env), status) })
})
...
app.get('/service-orders/:id/outreach-stats', async (c) => {
  const id = Number(c.req.param('id'))
  if (!Number.isFinite(id)) return c.json({ success: false, error: '잘못된 ID' }, 400)
  const order = await adsLeadsDb(c.env).prepare('SELECT id, created_at, service_name FROM ad_service_orders WHERE id = ?')
    .bind(id).first<{ id: number; created_at: string; service_name: string }>().catch(() => null)
  if (!order) return c.json({ success: false, error: '주문을 찾을 수 없습니다' }, 404)
  const s = await adsLeadsDb(c.env).prepare(`SELECT
      COUNT(*) AS sent,
...
app.patch('/service-orders/:id', async (c) => {
  ...
  if (!Number.isFinite(id)) return c.json({ success: false, error: '잘못된 ID' }, 400)
  const b = await c.req.json().catch(() => ({} as Record<string, unknown>))
  const r = await adminUpdateOrder(adsLeadsDb(c.env), id, {
...
app.get('/service-reviews', async (c) => c.json({ success: true, reviews: await adminListReviews(adsLeadsDb(c.env)) }))
...
app.patch('/service-reviews/:id', async (c) => {
  ...
  if (!Number.isFinite(id)) return c.json({ success: false, error: '잘못된 ID' }, 400)
  const b = await c.req.json().catch(() => ({} as Record<string, unknown>))
  await adminSetReviewStatus(adsLeadsDb(c.env), id, b.status === 'hidden' ? 'hidden' : 'visible')
  return c.json({ success: true })
})

// ── 단축 링크 모더레이션 (피싱/스팸 신고 대응) ───────────────────────────────
// GET /api/admin/ads/short-links — 최근 링크(계정 포함)
app.get('/short-links', async (c) => c.json({ success: true, links: await adminListShortLinks(adsLeadsDb(c.env)) }))
...
app.patch('/short-links/:id', async (c) => {
  ...
  if (!Number.isFinite(id)) return c.json({ success: false, error: '잘못된 ID' }, 400)
  const b = await c.req.json().catch(() => ({} as Record<string, unknown>))
  await adminSetShortLinkActive(adsLeadsDb(c.env), id, !!b.active)
  return c.json({ success: true })
})
...
app.route('/', adminAdsCollectGatesRoutes)

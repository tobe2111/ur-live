 ...
 * ⚠️ 추가 상향(9 초과)은 다시 대표 판단 사항이다 — 테스트가 상한으로 잠근다.
 *
 * ## 🔓 9 → 14 (2026-09-02 — 대표 승인 "응 다 해줘")
 * 위 조건("차단 0 유지 + 측정이 받쳐 줄 때")을 **숫자로 채운 뒤의 승인**이다:
 * ```
 *   네이버 차단 blocked 0 / ok 54,383 (당일)      ← 9 로 올린 뒤 3주간 한 번도 안 걸렸다
 *   성과측정 167,848 / 177,325 = 94.7% 완료 · 백로그 9,477(이틀분)  ← 측정은 더 이상 병목이 아니다
 *   수확 곡선이 평평(epoch 1~4회 20.7 · 5~14회 29.3 · 15~39회 20.9) ← 고갈 신호 0
 * ```
 * ⚠️ **8-04 동결 주석의 논거는 이제 성립하지 않는다** — 그때는 "새 행은 이메일 1.3%, 그걸 25%로
 *   만드는 측정이 병목"이었다. 지금은 측정이 94.7% 끝났고 남은 백로그가 이틀분이라, 폭을 넓히면
 *   행 수만이 아니라 **발송 가능 리드가 같은 비율로 는다.**
 *
 * ⚠️ 유일한 리스크는 네이버 검색 호출 ~1.5배다. **하루 뒤 `ads_naver_crawl_block.blocked` 를 보고
 *   0 이 아니면 즉시 9 로 되돌린다** — 이 상수 하나가 롤백 전부다(env 로도 즉시 조정 가능).
 * ⚠️ 14 초과 상향은 **다시** 대표 판단 사항이다(테스트가 14 를 상한으로 잠근다).
 */
export const COLLECT_KEYWORDS_PER_ROUND = 14
 ...
 *
 * ⚠️ **YT 가 살아 있는 회차는 무접촉**(`COLLECT_KEYWORDS_PER_ROUND` = **14**). 그 회차는 08-12 실측
 *   당시 예산이 캡이라(56/56) 폭을 올려도 아무것도 안 늘었다 — 올리면 YT 쿼터만 더 빨리 태운다.
 *   🩸 이 줄은 2026-09-16 까지 `= 9` 라고 적혀 있었다(그 값은 2026-09-02 #1321 에서 14 가 됐다).
 *     **다른 상수의 값을 주석에 숫자로 베껴 적으면 반드시 갈린다** — 그래서 이제
 *     `ads-keyword-focus-split.test.ts` 가 이 줄의 숫자를 실제 상수와 대조한다.
 ...
 * ⚠️ 차단이 뜨면(`ads_naver_crawl_block.blocked > 0`) 이 상수를 **`COLLECT_KEYWORDS_PER_ROUND` 와
 *   같은 값으로** 되돌리는 것이 롤백 전부다(= 이 회차만의 확장을 끄고 보통 회차와 같은 폭으로).
 *   ⚠️ 원래 이 자리엔 `9` 라고 적혀 있었는데, 그건 08-12 당시 보통 폭이 9 였기 때문이다 —
 *     지금 9 로 되돌리면 네이버 전용 회차가 보통 회차보다 **좁아진다**(의도와 반대).
 */
...
/**
 * 📺 **유튜브 수집 시점 영상-스니펫 보강 상한** (2026-08-22, 대표 *"가장 이상적으로"*).
 *
 * ## 왜 8 에서 내렸나 — **재고 나서** 내렸다
 * `NAVER_COLLECT_ENRICH_MAX` 를 5→1 로 내린 세션이 *"유튜브는 그 몫의 실제 기여를 **아직 안 쟀다**.
 * 재기 전에 줄이지 않는다"* 고 명시해 뒀다. 그 측정을 여기서 한다.
 *
 * ### 회차 실측(`yt_calls`) — 예산의 대부분이 이 보강으로 간다
 * ```
 *   search 3 · channels 3 · videos 24        ← 30요청 중 24가 영상 보강
 *   videos_email 1 · videos_contact 3 · videos_cat 24
 * ```
 * 그리고 회차 예산 56은 **매번 100% 소진**된다. 즉 여기 쓰는 1요청은 네이버에서 **6.59명**을 포기하는 것이다
 * (실측 12회차 합계: YT 382요청→신규 77 = 0.20/요청 · 네이버 227요청→신규 1,497 = 6.59/요청).
 *
 * ### 이메일 몫은 **보강 레인이 어차피 한다**(네이버와 같은 논리)
 * ```
 *   유튜브 미측정    610건  이메일 13.3%
 *   유튜브 측정됨 13,873건  이메일 38.4%      ← 레인이 96%를 이미 커버, 수율 3배
 * ```
 * ⇒ 수집 시점에 이메일을 쫓는 건 **레인이 나중에 할 일을 비싸게 미리 하는 것**이다.
 *
 * ### 다만 **분류 신호는 레인이 못 한다** — 그래서 0 이 아니다
 * 레인은 링크인바이오·활동성을 보지 영상 제목을 안 본다. 영상 제목은 업종 판정의 재료다:
 * ```
 *                    n        content 분류율   카테고리 없음
 *   보강 없음    11,118          73.0%           6.4%
 *   보강 받음     3,365          82.1%           2.7%      ← 더 어려운 코호트인데도 더 높다
 * ```
 * (보강 대상이 *이메일·분류가 없는* 채널이라 이 코호트는 원래 더 어렵다. 그런데도 분류율이 9%p 높다
 *  = 영상 제목이 실제로 판정을 돕는다.)
 *
 * ⇒ **상한은 절반(4)으로, 대상은 '분류가 안 되는 채널'로 좁힌다**(`influencer-discovery` 의 후보 필터).
 *   이메일만 없는 채널은 레인에 맡긴다 — 레인이 더 잘하고 더 싸다.
 *
 * ⚠️ 0 으로 만들지 말 것: 경로가 죽었는지 살았는지 알 수 없게 되고(네이버가 1 을 남긴 이유와 동일),
 *   분류 신호는 대체재가 없다.
 * ⚠️ 되돌리기: 이 값 4 → 8 + 후보 필터에 `!l.email ||` 복원.
 */
export const YT_COLLECT_ENRICH_MAX = 4

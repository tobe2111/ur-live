...
export const KW_DDL: string[] = [
  ...
  // 🚀 **커서 1회 시딩**(2026-08-19, 배포 +2.5h 실측 후) — 커서만 넣으면 효과가 **몇 주** 뒤에 온다.
  //   이유: 커서는 "다음에 쓸 값"이라 어떤 키워드의 **첫 sim 회차는 여전히 1페이지**를 읽고 101 을
  //   저장만 한다. 그리고 한 키워드가 다시 뽑히는 주기가 실측 ~3일(활성 558 ÷ 회차당 8 × 시간당 1회차),
  //   sim 은 그 절반이라 **두 번째 sim 회차 = 약 6일 뒤**다. 배포 직후 회차의 신규율이 8.9% 로
  //   기준선과 같았던 게 그 증거다(그 회차에 7개가 1→101 로 저장만 됐다).
  //   ⇒ **몇 달간 1페이지만 훑힌 키워드는 그 페이지에 새 사람이 거의 없다.** 그러니 page 1 에서
  //      출발할 이유가 없다 — 다음 방문부터 바로 미개척지(2페이지)를 읽게 한다.
  //   비용 0(호출 수 불변) · 되돌리기 = 이 문장 제거(커서 규칙 자체는 그대로 동작).
  //   ⚠️ `saved_total >= 100` 가드: 갓 만든 키워드는 1페이지가 아직 미개척지다 — 건드리지 않는다.
  //   ⚠️ 2페이지(101)로만 보낸다. 네이버가 `start` 를 존중하는지 **아직 라이브 증거가 없어서**
  //      가장 얕고 안전한 한 칸만 민다(존중하면 이후는 커서가 알아서 깊어진다. 무시하면 결과가
  //      종전과 같아 손실 0 — 어느 쪽이든 다음 회차 신규율로 판정된다).
  'UPDATE ad_discovery_keywords SET nb_start = 101 WHERE COALESCE(nb_start, 1) = 1 AND COALESCE(saved_total, 0) >= 100',
]

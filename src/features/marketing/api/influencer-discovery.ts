 ...
 *   `limitHit` = 이번 실행에서 **플랫폼 한도 오류를 실제로 관측**했나(2026-07-28). 예산(left)은 우리가 세는
 *   숫자일 뿐 실제 한도가 아니므로, 한도에 부딪힌 사실은 fetch 실패 메시지에서만 알 수 있다 →
 *   각 레인이 이 플래그를 세우고 호출부가 즉시 중단 + 학습 상한 하향(collect-budget.ts)에 쓴다. */

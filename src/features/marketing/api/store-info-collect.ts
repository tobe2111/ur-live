 ...
 *   tier 2~5(주류도매·식자재·간판·인테리어·POS·세무 …)를 **검색이 아닌 통째 조회**로 전국 발굴.
 ...
 *
 *   🔧 활용신청 전엔 게이트 OFF. **첫 실전 조회 시 stats.diag.sample(원응답 첫 항목)로 필드/업종코드 검증·교정**
 *      (아래 STOREINFO_TARGETS 업종코드는 문서 기준 placeholder — 실응답으로 확정).
 */
...
/** 🗺️ 조회 대상 = 업종중분류코드(indsMclsCd) × 시드. ⚠️ 코드는 data.go.kr 업종분류표 기준 placeholder —
 *   활용신청 후 diag.sample 로 확정·교정. 대행사(tier1)는 B2B 서비스업이라 상가정보 미포착 → 네이버 유지. */
type StoreTarget = { indsMclsCd: string; category: string; subcategory: string; tier: number }
const STOREINFO_TARGETS: StoreTarget[] = [
  { indsMclsCd: 'G208', category: '정기납품', subcategory: '주류·음료 도매', tier: 2 },
  { indsMclsCd: 'G209', category: '정기납품', subcategory: '식자재 도소매', tier: 2 },
  { indsMclsCd: 'F128', category: '매장인프라', subcategory: '간판·광고물', tier: 3 },
  { indsMclsCd: 'F125', category: '매장인프라', subcategory: '실내건축·인테리어', tier: 3 },
  { indsMclsCd: 'S011', category: '전문서비스', subcategory: '세무·회계', tier: 5 },
]
...
async function fetchStoreInfoPage(serviceKey: string, t: StoreTarget, page: number, budget?: FetchBudget): Promise<{ items: RawStore[]; total: number }> {
  ...
  spendBudget(budget)
  const url = `${STOREINFO_BASE}/storeListInUpjong?serviceKey=${encodeURIComponent(serviceKey)}&type=json&numOfRows=50&pageNo=${page}&divId=indsMclsCd&key=${encodeURIComponent(t.indsMclsCd)}`
...
export async function runStoreInfoCollect(env: Env): Promise<StoreInfoStats> {
  ...
  for (let i = 0; i < batch; i++) {
    ...
    const leads: CompanyLead[] = items.map(it => {
      ...
      return {
        ...
        source: 'storeinfo', source_keyword: t.indsMclsCd,
      }
  ...
  const s: StoreInfoStats = { last_run: stamp, found, saved, enriched, target: `${t.subcategory}(${t.indsMclsCd})`, page, total_runs: (prev?.total_runs || 0) + 1, total_saved: (prev?.total_saved || 0) + saved, diag: { configured: true, sample } }
  await persist(s)
  return s
}

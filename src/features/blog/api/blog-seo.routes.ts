...
app.get('/blog/og/:slug', async (c) => {
  ...
  // 🎨 2026-07-19 확정 브랜드(Ur Deal Final): 네이비 #16181C 배경(마케팅 배너) + 로즈 액센트 —
  //   다크 배경 로즈 글자는 밝힌 로즈 #4D8DF5(지시서 §6), 본문 웜화이트 #F8F7FC. 구 핑크(#ec4899)/검정 폐기.
  const titleSvg = lines
    .map((ln, i) => `<text x="90" y="${260 + i * 84}" font-size="66" font-weight="800" fill="#F8F7FC">${xmlEscape(ln)}</text>`)
    .join('')

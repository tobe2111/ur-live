 ...
 *
 * 설계 원칙:
 *   1) 소비자 홍보/마케팅 콘텐츠만 생성 — 이용권/교환권/동네딜/유어샵 혜택·활용법.
 *   2) 운영/내부 정보 유출 금지 — 수수료율·정산·원천징수·커미션·관리자·매출·도매(B2B) 등
 *      은 컨텍스트(brief)에 아예 넣지 않고, 출력 검증에서 나타나면 거부.
 *   3) 명칭 SSOT 준수(유저/사업자 유저/이용권/교환권/동네딜/유어샵) + 폐기 용어 금지
...
export const PROMO_BRIEF = [
  ...
  '- 유어샵: 가입한 유저 누구나 자동으로 갖는 나만의 이용권 진열대(도메인/u/{핸들}). 별도로 만들 필요 없이 가입과 동시에 생긴다. 마음에 드는 이용권을 담아 진열하고 링크 하나로 소개한다. 매장을 등록하면 같은 자리에서 내 이용권을 직접 판다.',
  '- 딜 포인트: 미리 충전해 두면 결제가 빠르다. 마음에 드는 유저를 후원할 수도 있다.',
  '- 결제는 토스페이먼츠로 안전하게 처리된다.',
  '- 회원은 "유저"라 부르고, 사업자등록·판매 승인을 받으면 "사업자 유저"가 되어 유어샵에서 자기 상품을 판다.',
  '',
  '## 톤 & 형식',
...
export const PROMO_TOPICS: readonly PromoTopic[] = [
  ...
  { slug: 'linkshop-share-your-taste', title: '내 취향을 링크 하나로, 유어샵 자랑하기', angle: '유어샵에 좋아하는 혜택을 핀으로 모아 친구와 공유하는 재미', tags: ['유어샵', '공유', '취향'] },

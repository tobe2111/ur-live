/**
 * Seller Alimtalk Management Routes
 *
 * Endpoints:
 * - GET  /alimtalk         - 알림톡 계정 + 템플릿 조회
 * - POST /alimtalk         - 알림톡 계정 등록/수정
 * - GET  /alimtalk/balance - 잔액 조회
 * - POST /alimtalk/test    - 테스트 발송
 * - POST /alimtalk/send    - 알림톡 발송
 * - GET  /alimtalk/messages - 발송 내역
 * - POST /alimtalk/charge  - 크레딧 충전
 */
...
export const sellerAlimtalkManagementRoutes = new Hono<{ Bindings: Bindings }>();

// CORS 설정
sellerAlimtalkManagementRoutes.use('*', cors({
  origin: [...ALLOWED_ORIGINS],
  credentials: true,
}));

// ─── GET /alimtalk — 계정 + 템플릿 조회 ──────────────────────────────────
sellerAlimtalkManagementRoutes.get('/alimtalk', requireSeller(), async (c) => {
  const { DB } = c.env;
  try {
...
// ─── POST /alimtalk — 계정 등록/수정 ─────────────────────────────────────
sellerAlimtalkManagementRoutes.post('/alimtalk', requireSeller(), async (c) => {
  const { DB } = c.env;
  try {
    const authUser = getCurrentUser(c);
    const sellerId = authUser?.id;
...
// ─── GET /alimtalk/balance — 잔액 조회 ──────────────────────────────────
sellerAlimtalkManagementRoutes.get('/alimtalk/balance', requireSeller(), async (c) => {
  const { DB } = c.env;
  try {
    const authUser = getCurrentUser(c);
    const sellerId = authUser?.id;
...
// ─── POST /alimtalk/test — 테스트 발송 ──────────────────────────────────
sellerAlimtalkManagementRoutes.post('/alimtalk/test', requireSeller(), async (c) => {
  const { DB } = c.env;
  try {
    const authUser = getCurrentUser(c);
    const sellerId = authUser?.id;
...
// ─── POST /alimtalk/send — 알림톡 발송 ──────────────────────────────────
sellerAlimtalkManagementRoutes.post('/alimtalk/send', requireSeller(), async (c) => {
  const { DB } = c.env;
  try {
    const authUser = getCurrentUser(c);
    const sellerId = authUser?.id;
...
// ─── GET /alimtalk/messages — 발송 내역 ──────────────────────────────────
sellerAlimtalkManagementRoutes.get('/alimtalk/messages', requireSeller(), async (c) => {
  const { DB } = c.env;
  try {
    const authUser = getCurrentUser(c);
    const sellerId = authUser?.id;
...
// ─── POST /alimtalk/charge — 충전 요청 ──────────────────────────────────
sellerAlimtalkManagementRoutes.post('/alimtalk/charge', requireSeller(), async (c) => {
  const { DB } = c.env;
  try {
    const authUser = getCurrentUser(c);
    const sellerId = authUser?.id;

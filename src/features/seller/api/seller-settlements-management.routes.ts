/**
 * Seller Settlements Management API Routes
 *
 * Endpoints:
 * - GET  /api/seller/settlements           - 정산 목록 (페이지네이션)
 * - POST /api/seller/settlements/request   - 정산 신청
 * - GET  /api/seller/settlements/stats     - 정산 통계
 * - GET  /api/seller/settlements/summary   - 정산 요약
 * - GET  /api/seller/dashboard/stats       - 대시보드 일별 통계
 * - GET  /api/seller/settlements/:id/download - 정산 내역서 다운로드
 */
...
import { getSellerSettlementSummary } from '../../../lib/settlement-automation';
...
export const sellerSettlementsManagementRoutes = new Hono<{ Bindings: Bindings }>();

sellerSettlementsManagementRoutes.use('*', cors({
 ...
 * 정산 목록 (페이지네이션)
 */
sellerSettlementsManagementRoutes.get('/settlements', async (c) => {
  const db = c.env.DB;
 ...
 * 정산 신청
 */
sellerSettlementsManagementRoutes.post('/settlements/request', async (c) => {
  const db = c.env.DB;
  const authorization = c.req.header('Authorization');
  if (!authorization?.startsWith('Bearer ')) return c.json({ success: false, error: '인증이 필요합니다' }, 401);
  try {
 ...
 * 정산 통계
 */
sellerSettlementsManagementRoutes.get('/settlements/stats', async (c) => {
  const db = c.env.DB;
  const authorization = c.req.header('Authorization');
  if (!authorization?.startsWith('Bearer ')) return c.json({ success: false, error: '인증이 필요합니다' }, 401);
  try {
...
sellerSettlementsManagementRoutes.get('/settlements/summary', async (c) => {
  const db = c.env.DB;
  const authorization = c.req.header('Authorization');
...
sellerSettlementsManagementRoutes.get('/dashboard/stats', async (c) => {
  const { DB } = c.env;
  const authorization = c.req.header('Authorization');
  const sellerId = await getSellerIdFromToken(authorization, c.env.JWT_SECRET);
...
sellerSettlementsManagementRoutes.get('/settlements/:id/download', async (c) => {
  const { DB } = c.env;
  const authorization = c.req.header('Authorization');
  const sellerId = await getSellerIdFromToken(authorization, c.env.JWT_SECRET);
  if (!sellerId) return c.json({ success: false, error: 'Unauthorized' }, 401);

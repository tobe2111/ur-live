/**
 * Seller Registration & Role Switching Routes
 *
 * - POST /register           — 셀러 회원가입
 * - POST /register-from-user — 카카오 유저 → 셀러 전환 신청
 * - GET  /my-seller-status   — 현재 유저의 셀러 전환 상태 확인
 * - POST /switch-to-seller   — 유저 세션 → 셀러 JWT 발급
 * - POST /switch-to-user     — 셀러 JWT → 유저 세션 복귀
 */
...
sellerRegistrationRoutes.post('/register-from-user', rateLimit({ action: 'seller_register_from_user', max: 5, windowSec: 3600 }), async (c) => {
  try {
    ...
    createDashboardNotification(db, 'admin', null, 'seller_registered', '유저→셀러 전환 신청', `${userName} (유저 #${userId})`, '/admin/sellers').catch(() => {});

...
- 어드민/제조사가 상품ID 조회 후 **선정 유통회원**을 추가/해제(\`POST/DELETE /api/supplier/products/:id/distributor-access\`).`,
  },
  {
    ...
    content: `### 등급가 산출 공식 (2026-06-16 신모델 — 판매가 대비 보장마진)
**유통사 공급가 = max(제조사 원가, 판매가 × (1 − 등급 보장마진율))**. 원가가 하한(플랫폼 손실 차단).
- 마진율 = **판매가(권장소비자가) 대비** 유통사 보장마진. 예) 판매가 10,000 → 일반 8,500 / 프로 7,000 / 프리미엄 6,200.
- 정산 분배: **제조사 = 공급가 × (1 − 플랫폼수수료%)** (단, 제조사 원가 이상 보장) / **플랫폼 = 공급가 × 수수료%**(기본 10%, \`/admin/distributor-grades\` 에서 조정). 예) 공급가 8,500 → 플랫폼 850 · 제조사 7,650. 제조사 원가·신원은 유통사에 **비노출**.
- ⚠️ 제조사는 **판매가(권장소비자가) 필수 입력** — 공급가보다 높게(안 넣으면 마진 0, 등록 차단).
- 표시가 = 결제가 (서버 재계산 SSOT — 카탈로그/상세/홈/재주문/주문생성/미리보기/내보내기/제안/찜/어드민 일괄 반영).

### 등급 마진율 (\`/admin/distributor-grades\`)
- A/B/C/D/OEM/SPECIAL 마진율 편집. **고등급(A)일수록 큰 보장마진 = 더 낮은 공급가**.
- 기본: **프리미엄 A 38% / 프로 B 30% / 일반 C 15%(기본 배정)** / D 8% / OEM 40% / 특별 45%.
- 프로(B)는 유통사가 **연 구독료(기본 100만원)** 를 예치금에서 결제 시 1년 적용. 프리미엄(A)은 매출 임계 자동 승급.

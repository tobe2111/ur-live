...
const GUIDE_SEED_VERSION = 36 // 2026-09-26 운영백서 숫자표 꺼진 기능 12→13 — `SELLER_COUPONS_HIDDEN`(셀러 쿠폰 메뉴 숨김) 추가. 안 올리면 라이브 백서가 12 그대로라 **운영자가 쿠폰이 아직 켜져 있는 줄 안다**(라이브 실측: 셀러가 만든 쿠폰 0건이라 내렸다). 이전: 35 // 2026-09-24 영입 커미션 중단 — 셀러 가이드가 "매출의 2% 1년"을 약속하고 있었다(안 올리면 라이브 가이드가 없어진 제도를 계속 약속한다). 이전: 34 // 2026-09-21 정산 유보 10일 + 채널별 수수료(중개 5%/직접 10%) 명시 — 안 올리면 라이브 가이드가 '기본 5%·유보 없음'으로 남아 매장이 정산일을 오해한다. 이전: 33 // 2026-09-19 대행사 확정 플로우 — 사장님 승계 코드·협업 코드·딜 % 조정·중개사 몫 스위치(OFF) 안내(안 올리면 라이브 가이드가 코드 입구를 모른다). 이전: 32 // 2026-09-16 이용권 상세 주소 이전 `/group-buy/:id` → `/pass/:id` — 가이드가 옛 주소를 안내하고 있었다(안 올리면 라이브 가이드가 301 을 타는 주소를 계속 알려준다). 이전: 31 // 2026-09-16 업체 정보 한 페이지 통합 — 가이드의 '미니샵' 섹션이 없어진 화면을 안내하고 있었다(안 올리면 라이브 가이드가 옛 경로 그대로). 이전: 30 // 2026-09-15 운영백서 숫자표에 `VOUCHER_CART_UI_ENABLED`(꺼짐) 추가 — 꺼진 기능 11→12. 안 올리면 라이브 백서가 옛 목록 그대로라 운영자가 '담기'를 켜진 줄 안다. 이전: 29 // 2026-09-08 auto-reference 꼬리말에서 '마지막 생성: <ISO 시각>' 제거(생성물 결정론 — 매 커밋 바뀌어 동시 PR 이 충돌했다). 이전: 28 // 2026-09-07 셀러 가이드 '소개 협업' — 커미션 % 플랫폼 상한 없음(결재 Q2-1). 이전: 27 // 2026-09-05 **26 도 두 갈래가 각자 선점**했다(main = 5%→2% 정정 + 운영백서 재생성 / 이 브랜치 = 에이전시 일몰). main 이 26 으로 먼저 배포되면 같은 번호인 이 브랜치의 재시드는 **무음 스킵**된다 → 27 로 올려 양쪽을 함께 재시드한다

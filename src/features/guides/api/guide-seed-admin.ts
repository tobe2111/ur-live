...
- **상품별 마진(특가/전략상품)**: 상품ID + 마진율 입력 → \`특가 적용\`. 설정 시 **등급 무관 모든 유통사가 동일 공급가**(전략·특가·임박품). \`해제\`하면 등급별 마진으로 복귀. 상품ID는 \`상품정보 엑셀\`(product_id 컬럼)에서 확인. 표시가=결제가 일괄 반영.
- **수량 구간 할인(volume tier)**: 상품별 \`수량:할인%\` 쌍(예 \`100:5, 500:10\` = 100개↑ 5%, 500개↑ 10%). 등급가 위에 **구매 수량별 추가 할인**(많이 살수록↓). ⚠️ 단가는 **공급원가 이하로 안 내려감**(역마진 차단 — 할인이 마진 초과 시 원가로 floor). 표시가=주문 결제가 동일(서버 재계산 SSOT).
- **유통채널 선정(product-access)**: 공급범위가 '승인한 유통채널'/'유통스타트 유통채널'인 상품 → 상품ID 조회 후 **선정 유통회원만 노출**(선정/해제).
- **플랫폼 사업자정보**: 세금 섹션 폼(상호/사업자번호/대표/주소/업태/종목/이메일/전화) → 저장. 국세청 전자세금계산서 발행 전제. 유통사는 본인 발행 자료를 \`/wholesale/documents\`(거래명세서/세금계산서 탭)에서 조회·인쇄.
- **공급가 이력**: 제조사가 공급가 수정 시 수정 전 금액 기록(관리자만 확인).
- **온라인 최저가 검수 + 가격변경 승인(2026-06)**: \`/admin/products\` → \`공급자 상품\` 탭.
  - 신규 등록 상품: 제조사가 제출한 **온라인 최저가 참고 링크**(네이버쇼핑 등)를 열어 확인 → \`온라인 최저가 확인함\` 체크 후 \`승인\`. (검수 완료 상품은 \`최저가 검수됨\` 뱃지 표시)
  - **가격변경 요청** 필터: 이미 판매 중인 상품의 가격을 제조사가 바꾸려면 \`가격 수정 요청\`을 보냄 → 이 탭에서 **현재가 → 요청가**와 참고 링크를 검토 후 \`반영\`(승인 시 라이브 가격 갱신 + 이력 기록) / \`거부\`. **승인 전까지 기존 노출 가격이 유지**되어 갑작스러운 가격 변동/잘못된 가격 결제가 없음.

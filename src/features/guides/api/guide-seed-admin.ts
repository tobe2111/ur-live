...
유통스타트(utongstart.com) 도매몰의 제조사·유통사 온보딩, 등급/마진, 상품 검수(온라인 최저가 + 가격변경 승인), 주문/배송(합배송·송장), 정산, 세금계산서, OEM/ODM 운영 전체는 단일 진실원천(SSOT) 유지를 위해 **전용 도매몰 운영 가이드**로 분리되었습니다.
...
- **입금 계좌 설정** \`/admin/wholesale-deposit-account\` — 유통사에게 안내할 은행/계좌번호/예금주 등록.
- **배너 관리** \`/admin/wholesale-banners\` — 도매 카탈로그 상단 배너.
- **프리미엄 상품 토글** \`/admin/wholesale-products\` — 프리미엄/회원공개 전용관 노출 설정.
- **제안/신고 처리** \`/admin/wholesale-proposals\` — 유통사 상품 제안·문제 신고 큐.
- **세금계산서** \`/admin/wholesale-tax\` — 거래단위 매출/매입 역발행 계산서 조회 + 재발행(\`TAX_INVOICE_API_KEY\` 설정 시 실발행).
- **가입 승인** \`/admin/suppliers\` · \`/admin/distributor-grades\` — 제조사/유통사 승인 시 대표자·담당자·사업자 정보 확인.

> 도매 관련 코드(제조사/유통사/공급 API)가 바뀌면 전용 가이드 끝의 "코드 자동 참조" 섹션이 자동 갱신됩니다(\`npm run generate:guide-refs\`).`,
  },
  {
...
1. **필터 선택**: 대상 역할(셀러/제조사/유통사 등) + 상태(전체/승인 등) + 등급으로 수신자를 좁힙니다. 수신자 목록은 **항상 서버에서 필터로만 해석**(클라이언트가 명단을 보내지 않음).

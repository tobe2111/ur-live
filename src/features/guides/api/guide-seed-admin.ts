...
- 이용권 기본 5% (\`commission_rate_meal_voucher\`)
...
### 카카오맵 리뷰 인증 (\`/admin/kakao-reviews\`) — 2026-07-02 게이미피케이션
- 손님의 카카오맵 후기 인증은 **매장(셀러 \`/seller/review-verifications\`)이 1차 확인** — 어드민 큐는 샘플링 감사 + 상위 권한
- 승인 시 보너스 딜(\`kakao_review_bonus_amount\`) + **동네 리뷰어 점수/레벨** 자동 적립 (레벨 임계값: \`review_level_thresholds\`)
- OCR "자동검증 통과" 는 **참고 라벨** — 자동 지급 없음 (지급 판정은 사람)
- 부정 패턴 감사 포인트: 같은 매장 반복 승인, 스크린샷 재사용, 매장-유저 공모 (실매출 동반 여부 확인)
- 레벨 전용 이용권: 동네딜 등록/수정 폼의 "동네 리뷰어 레벨 전용" (meta \`min_review_level\`, 구매 시 서버 게이트)
...
7. **VoucherMap**: MyVouchersPage 에 카카오 멀티 마커 지도 (미사용 이용권 위치)
...
이미지/설명 유사도 확인 → 원본 셀러에게 증빙 요청 → 허위 판매자 경고 → 반복 시 정지.`,
  ...
  {
    key: 'voucher-categories-admin', icon: '🎫', title: '이용권 3종 운영 (식사·뷰티·헬스)', order: 600,
    content: `### 이용권 카테고리 enum

\`products.category\` 컬럼에 다음 3종 값:
- \`meal_voucher\` — 식사 이용권 (맛집)
- \`beauty_voucher\` — 뷰티 이용권 (헤어·네일·피부) NEW
- \`health_voucher\` — 헬스 이용권 (PT·요가·필라테스) NEW
...
- \`/map\` 에서 카테고리 칩으로 필터, API: \`GET /api/group-buy/products?category=...\`

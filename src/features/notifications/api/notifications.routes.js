...
notificationsRoutes.get('/unread-count', async (c) => {
    ...
    try {
        const userId = c.get('userId');
        const userType = c.get('userType');
        const result = await DB.prepare(`SELECT COUNT(*) as count FROM notifications WHERE user_id = ? AND user_type = ? AND is_read = 0`).bind(userId, userType).first();
...
notificationsRoutes.put('/:id/read', async (c) => {
    ...
    try {
        ...
        const userId = c.get('userId');
        const userType = c.get('userType');
        const notif = await DB.prepare(`SELECT id FROM notifications WHERE id = ? AND user_id = ? AND user_type = ?`).bind(id, userId, userType).first();
        if (!notif)

...
async function ensureRefundTable(DB: D1Database) {
  if (_done_ensureRefundTable.has(DB)) return
  _done_ensureRefundTable.add(DB)
  try {
    await DB.prepare(`
...
communityGroupBuyRoutes.post('/:id/refund', rateLimit({ action: 'community_gb_refund', max: 10, windowSec: 3600 }), requireAuth(), async (c) => {
  const user = getCurrentUser(c);
  if (!user) return c.json({ success: false, error: '로그인이 필요합니다' }, 401);
...
async function ensureMessagesTable(DB: D1Database) {
  if (_done_ensureMessagesTable.has(DB)) return
  _done_ensureMessagesTable.add(DB)
  try {
    await DB.prepare(`
...
const _done_ensureMessagesTable = new WeakSet<object>()
const _done_ensureTables = new WeakSet<object>()
const _done_ensureRefundTable = new WeakSet<object>()

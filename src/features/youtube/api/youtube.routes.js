 ...
 */
async function getValidAccessToken(db, sellerId, youtubeService) {
...
app.post('/oauth/callback', async (c) => {
    ...
    `).bind(sellerId, googleEmail, tokens.access_token, tokens.refresh_token, tokens.expires_at, channel.id, channel.title, channel.thumbnail, channel.subscriberCount).run();
        return c.json({
            success: true,
            data: {
                channel,
...
app.post('/live/create', async (c) => {
    ...
    try {
        ...
        const liveSetup = await youtubeService.setupLiveStream(accessToken, title, description || '', scheduledTime);
        // Save to database
    ...
    `).bind(sellerId, title, description || '', 'scheduled', liveSetup.broadcast.id, liveSetup.broadcast.id, liveSetup.stream.ingestionInfo.streamName, liveSetup.broadcast.liveChatId || null, liveSetup.rtmpUrl, liveSetup.rtmpKey, liveSetup.embedUrl, scheduledTime).run();
...
app.post('/live/:id/start', async (c) => {
    ...
    `).bind(streamId, sellerId).first();
        ...
        // Transition to live
        await youtubeService.transitionBroadcastToLive(accessToken, stream.youtube_broadcast_id);
        // Update database
        await c.env.DB.prepare(`
      UPDATE live_streams 
...
app.post('/live/:id/end', async (c) => {
    ...
    `).bind(streamId, sellerId).first();
        ...
        // End broadcast
        await youtubeService.endBroadcast(accessToken, stream.youtube_broadcast_id);
        // Update database
        await c.env.DB.prepare(`
      UPDATE live_streams 

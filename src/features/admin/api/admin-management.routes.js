...
adminManagementRoutes.get('/sellers', cors(), async (c) => {
    try {
        const { DB } = c.env;
        const sellers = await executeQuery(DB, `
...
adminManagementRoutes.patch('/sellers/:id/approve', cors(), async (c) => {
    try {
        ...
        const rows = await executeQuery(DB, 'SELECT id, status FROM sellers WHERE id = ?', [sellerId]);
        if (rows.length === 0)
...
adminManagementRoutes.patch('/sellers/:id/reject', cors(), async (c) => {
    try {
        ...
        const rows = await executeQuery(DB, 'SELECT id FROM sellers WHERE id = ?', [sellerId]);
        if (rows.length === 0)
            return c.json({ success: false, error: '판매자를 찾을 수 없습니다' }, 404);
...
adminManagementRoutes.get('/orders', cors(), async (c) => {
    ...
    `;
        ...
        const orders = await executeQuery(DB, query, params);
...
adminManagementRoutes.get('/products', cors(), async (c) => {
    try {
        const { DB } = c.env;
        const products = await executeQuery(DB, `
...
adminManagementRoutes.get('/stats', cors(), async (c) => {
    try {
        ...
        const [ts, as, tst, ast] = await Promise.all([
            executeQuery(DB, 'SELECT COUNT(*) as count FROM sellers'),
            executeQuery(DB, "SELECT COUNT(*) as count FROM sellers WHERE status = 'approved'"),
            executeQuery(DB, 'SELECT COUNT(*) as count FROM live_streams'),
            executeQuery(DB, "SELECT COUNT(*) as count FROM live_streams WHERE status = 'live'"),
        ]);
...
adminManagementRoutes.get('/dashboard/stats', cors(), async (c) => {
    try {
        ...
        const [sales, orders, live] = await Promise.all([
            executeQuery(DB, `SELECT COALESCE(SUM(total_amount),0) as total FROM orders WHERE DATE(created_at)=? AND payment_status='approved'`, [today]),
            executeQuery(DB, 'SELECT COUNT(*) as count FROM orders WHERE DATE(created_at)=?', [today]),
            executeQuery(DB, "SELECT COUNT(*) as count FROM live_streams WHERE status='live'"),
        ]);
...
adminManagementRoutes.get('/settlement/stats', cors(), async (c) => {
    try {
        ...
        let df = '';
        if (period === 'today')
            df = `AND DATE(o.created_at) = '${new Date().toISOString().split('T')[0]}'`;
        else if (period === 'week')
            df = "AND DATE(o.created_at) >= DATE('now','-7 days')";
        else if (period === 'month')
            df = "AND DATE(o.created_at) >= DATE('now','-30 days')";
...
adminManagementRoutes.get('/settlement/records', cors(), async (c) => {
    ...
    `;
        ...
        if (status && status !== 'all') {
            query += " AND COALESCE(o.settlement_status,'pending')=?";
            params.push(status);
        }
        query += ' ORDER BY o.created_at DESC LIMIT 1000';
...
adminManagementRoutes.delete('/streams/:id', cors(), async (c) => {
    try {
        ...
        const rows = await executeQuery(DB, 'SELECT id FROM live_streams WHERE id=?', [streamId]);
        if (rows.length === 0)

...
export const adminBannersRoutes = new Hono();
// 모든 배너 조회
adminBannersRoutes.get('/', cors(), async (c) => {
    try {
        const { DB } = c.env;
        const banners = await executeQuery(DB, `
...
adminBannersRoutes.post('/', cors(), async (c) => {
    try {
        ...
        const result = await executeRun(DB, `INSERT INTO banners (title, image_url, link_url, description, is_active, display_order, start_date, end_date, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, [title, image_url, link_url || null, description || null,
...
adminBannersRoutes.put('/:id', cors(), async (c) => {
    try {
        ...
        const { title, image_url, link_url, description, is_active, display_order, start_date, end_date } = await c.req.json();
        const rows = await executeQuery(DB, 'SELECT id FROM banners WHERE id = ?', [bannerId]);
        if (rows.length === 0)
            return c.json({ success: false, error: '배너를 찾을 수 없습니다' }, 404);
        await executeRun(DB, `UPDATE banners SET title=?, image_url=?, link_url=?, description=?, is_active=?,
       display_order=?, start_date=?, end_date=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`, [title, image_url, link_url || null, description || null,
            is_active !== undefined ? (is_active ? 1 : 0) : 1,

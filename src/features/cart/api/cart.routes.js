...
import { validateNumber, validateOptionalString, ValidationError } from '@/worker/utils/validation';
...
cartRoutes.post('/', requireAuth(), async (c) => {
    try {
        ...
        // 상품 존재 여부 및 재고 확인
        const product = await dbHelper.findById('products', product_id);
        if (!product) {
        ...
        // 이미 장바구니에 있는지 확인
        const existingItem = await dbHelper.findOne('cart', { user_id: userId, product_id });
        if (existingItem) {
            // 기존 아이템 수량 업데이트
...
cartRoutes.delete('/:id', requireAuth(), async (c) => {
    try {
        ...
        // 장바구니 아이템이 해당 사용자의 것인지 확인
        const cartItem = await dbHelper.findOne('cart', { id: cartItemId, user_id: userId });
        if (!cartItem) {
            return c.json(notFoundResponse('Cart item'), 404);
        }
        // 삭제

...
import { loadPlatformCommissionPct, mallCommissionPctOverride } from '../wholesale-settlement'
    ...
    try {
      ...
      const rows = await c.env.DB.prepare(`
        SELECT p.id, p.name, p.category, p.stock, p.barcode, COALESCE(p.supply_visibility,'ALL') AS supply_visibility,
               COALESCE(p.supply_price,0) AS supply_price, COALESCE(p.price,0) AS retail_price, p.supply_margin_override_pct AS margin_override, COALESCE(p.mall_id,1) AS mall_id, sup.business_name AS supplier_name
      ...
      `).all<{ id: number; name: string; category: string | null; stock: number; barcode: string | null; supply_visibility: string; supply_price: number; retail_price: number; margin_override: number | null; mall_id: number; supplier_name: string | null }>()
      // 🏬 2026-07-04 몰별 수수료 — export 도 라이브 결제가와 정합(몰 override ?? 전역). 몰 수 적어 사전 맵.
      const mallIds = [...new Set((rows.results || []).map(r => Number(r.mall_id) || 1))]
      const pctByMall = new Map<number, number>()
      for (const mid of mallIds) pctByMall.set(mid, (await mallCommissionPctOverride(c.env.DB, mid)) ?? commPct)
      const out = (rows.results || []).map(r => {
        ...
        const priceFor = (g: string) => resolveDistributorPrice({
          ...
          marginOverridePct: r.margin_override, defaultPlatformMarginPct: pctByMall.get(Number(r.mall_id) || 1) ?? commPct,
        }).price
        return [

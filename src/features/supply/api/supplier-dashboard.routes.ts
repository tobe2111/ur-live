...
supplierDashboardRoutes.get('/me', async (c) => {
  ...
  try {
    ...
    // 🛡️ 2026-06-27: 출금 예약분(reserved_amount, 미지급 출금신청이 잠근 금액) 별도 안전조회.
    //   '출금 가능' 표시 = available - reserved (출금 페이지 spendable 과 동일). 컬럼 없으면 0 폴백.
    ...
    return c.json({
      ...
      data: {
        ...
        balance: {
          ...
          reserved_amount: reserved?.reserved ?? 0,

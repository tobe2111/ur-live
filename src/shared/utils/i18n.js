 ...
 */
export function getLocaleFromHeader(acceptLanguage) {
    if (!acceptLanguage)
        return 'ko';
 ...
 */
export function formatPrice(amount, currency, locale = 'ko') {
    const localeMap = {
        ko: 'ko-KR',
        en: 'en-US',
        ja: 'ja-JP',
 ...
 */
export function createI18nContext(locale) {
    return {
        locale,
        dir: LOCALES[locale]?.dir ?? 'ltr',
        t: (key) => t(key, locale),
        formatPrice: (amount, currency) => formatPrice(amount, currency, locale),
    };
}

 ...
 *   배경: 유어샵 콘텐츠의 핵심은 '추천 핀'(addPin — 상품 상세/검색에서 1탭 핀). /host/new("공구 열기"
 *         어드민 큐레이션 이용권 호스팅)는 별개 시스템이고, 유어샵 버튼 폴백으로 떠서 혼란 + 동네공구
 ...
 *   "온라인 판매·소싱은 필요없다 — 이용권 파는 매장 업주만을 위한 형태로. 상품은 유어샵에서만").
 *   true: 셀러 nav 에서 **온라인 상품 관리(/seller/products)·도매 소싱(/seller/supply)** 을 전 셀러
 *         타입에서 숨김(심플모드 여부 무관). 상품(물건) 판매는 유어샵(/u/{handle})으로 일원화 —
 *         nav 에 '내 유어샵' 진입 추가. 대납 검토(/seller/proxy-products)는 이용권 그룹으로 이동.

...
app.get('/api/brandpay/callback', async (c) => {
  try {
    const { code, customerKey } = c.req.query()
    
    if (!code || !customerKey) {
      return c.json({ 
        success: false, 
        error: 'code와 customerKey가 필요합니다.' 
      }, 400)
    }
    ...
    if (!response.ok) {
      ...
      console.error('[BrandPay] Access Token 발급 실패:', errorData)
      return c.json({ 
        success: false, 
        error: errorData.message || 'Access Token 발급에 실패했습니다.' 
      }, response.status)
    }

    const tokenData = await response.json()
    console.log('[BrandPay] Access Token 발급 성공:', { customerKey })
    ...
    `).bind(
      ...
      expiresAt,
      customerKey.replace('customer_', '') // customer_1 → 1
    ).run()

    // 결제 페이지로 리다이렉트
    return c.redirect('/checkout?brandpay=registered')
  } catch (error) {
    console.error('[BrandPay] Callback 오류:', error)
    return c.json({ 
      success: false, 
...
// 브랜드페이 토큰 갱신
app.post('/api/brandpay/refresh-token', async (c) => {
  try {
    ...
    const user = await c.env.DB.prepare(`
      SELECT brandpay_customer_key, brandpay_refresh_token
      FROM users
      WHERE id = ?
    `).bind(userId).first()

    if (!user || !user.brandpay_refresh_token) {
      return c.json({ 
        success: false, 
        error: '브랜드페이 토큰이 없습니다.' 
      }, 404)
    }
    ...
    const response = await fetch('https://api.tosspayments.com/v1/brandpay/authorizations/access-token', {
      ...
      body: JSON.stringify({
        ...
        refreshToken: user.brandpay_refresh_token,
        customerKey: user.brandpay_customer_key,
      }),
    })

    if (!response.ok) {
      const errorData = await response.json()
      console.error('[BrandPay] 토큰 갱신 실패:', errorData)
      return c.json({ 
        success: false, 
    ...
    console.error('[BrandPay] 토큰 갱신 오류:', error)
    return c.json({ 
      success: false, 
      error: error instanceof Error ? error.message : '알 수 없는 오류가 발생했습니다.' 
    }, 500)
...
// 브랜드페이 토큰 조회
app.get('/api/brandpay/token/:userId', async (c) => {
  try {
    const userId = c.req.param('userId')

    const user = await c.env.DB.prepare(`
      SELECT 
        brandpay_customer_key,
        brandpay_access_token,
        brandpay_token_expires_at
      FROM users
      WHERE id = ?
    ...
    if (!user || !user.brandpay_access_token) {
      return c.json({ 
        success: false, 
        hasToken: false 
      })
    ...
    // 토큰 만료 확인
    const expiresAt = new Date(user.brandpay_token_expires_at as string)
    ...
    return c.json({ 
      ...
      hasToken: true,
      customerKey: user.brandpay_customer_key,
      expiresAt: user.brandpay_token_expires_at
    })
  } catch (error) {
    console.error('[BrandPay] 토큰 조회 오류:', error)
    return c.json({ 
      success: false, 
      error: error instanceof Error ? error.message : '알 수 없는 오류가 발생했습니다.' 
    }, 500)

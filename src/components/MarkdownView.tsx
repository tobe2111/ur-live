...
export default function MarkdownView({ source, className = '' }: { source: string; className?: string }) {
  ...
  while (i < lines.length) {
    ...
    if (/^>\s?/.test(t)) {
      ...
      blocks.push(
        <blockquote key={key++} className="my-3 border-l-4 border-gray-300 dark:border-[#3A3A3A] bg-gray-50 dark:bg-[#1D1F29] px-3 py-2 text-[13px] text-gray-600 dark:text-gray-300 rounded-r-lg">

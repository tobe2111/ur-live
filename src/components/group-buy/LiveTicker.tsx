...
export default function LiveTicker({ className = '' }: { className?: string }) {
  ...
  return (
    <Link
      ...
      className={`flex items-center gap-2 px-3 py-2 rounded-full bg-white dark:bg-[#1A2334] border border-gray-200 dark:border-[#2A3446] shadow-sm hover:shadow transition-shadow ${className}`}
      role="status"
      aria-live="polite"
    >
      {/* 깜빡이는 빨간 dot */}

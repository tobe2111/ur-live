...
interface TeamPointsBadgeProps {
  streamId?: number
  donationGoal?: number | null  // stream.donation_goal (셀러 설정값, 없으면 기본 10000)
}

// 딜 포인트 후원 게이지 뱃지 (상단 왼쪽, LIVE 뱃지 아래)
export function TeamPointsBadge({ streamId, donationGoal }: TeamPointsBadgeProps) {
  const [donated, setDonated] = useState(0)
  const goal = donationGoal && donationGoal > 0 ? donationGoal : DEFAULT_GOAL
  ...
  useEffect(() => {
    const handler = () => { setTimeout(fetchDonated, 1000) }
    ...
    <div className="flex flex-col gap-1 px-3 py-1.5 rounded-xl bg-black/60 backdrop-blur-md min-w-[100px]">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[11px] font-bold text-white/90">
          {formatNumber(donated)}<span className="text-white/50">/{formatNumber(goal)}딜</span>
        </span>
        {achieved && (
          <span className="text-[10px] font-bold text-yellow-400 animate-pulse">달성!</span>
        )}
      </div>

 ...
 *
 * ## 그럼 `common` 에는 무엇이 남는가 (이 판단을 매번 다시 하지 않으려고 적는다)
 *
 ...
 *     전사 상황판이 맞다. ⚠️ 한때 "실제로는 유어딜"이라고 적었는데 **틀렸다.**
 */
...
export const NAV_GROUPS: NavGroup[] = [
  {
    ...
    items: [
      ...
      //   ⚠️ 체리픽(2026-09-16): `/admin/proposals` 는 그 뒤 main 에 생긴 전사 항목이라 여기 남긴다.
  ...
  },
  {
    ...
    items: [
      ...
      // 📮 2026-09-16 체리픽(#1149 ⑩): 이 가드가 **그 사이 생긴 네 번째 고아**를 잡았다 —
      //   `AdminInfluencerOutreachPage` 는 실존하는데 nav 어디에도 없어 URL 직접 입력으로만 열렸다.
      //   발송은 대표가 직접 한다(유어애즈 방향) — 그러려면 화면에 닿을 수 있어야 한다.
      { path: '/admin/influencer-outreach', label: '제휴 제안 발송', icon: Send },
    ],
  },
  {
...
export const NAV_SECTIONS: Array<{ key: NavSectionKey; label?: string; accent?: string }> = [
  ...
  /* 🏪 2026-08-14 체리픽(#1149 ⑧): 공구 서비스는 **유어딜이 아니다** — 서비스 넷이면 밴드도 넷이다.
     ⚠️ 원 커밋의 `#6ee7b7`(밝은 민트)은 **검정 사이드바 시절 색**이라 흰 면에서 대비 1.6:1 로 무너진다.
     위 세 값과 같은 처방으로 같은 계열 진한 값(emerald-700, 약 5:1)으로 내리고 이모지도 뺀다
     (`admin-tones` 가 섹션 라벨 이모지 0 을 잠가 뒀다). */
  { key: 'mall', label: '공구 서비스 · 운영자 몰 (SaaS)', accent: '#047857' },
  /* 📣 2026-08-16 체리픽(#1149 ⑩): 유어애즈는 **네 서비스 중 하나**인데 밴드가 없어 '공통' 아래
     렌더되고 있었다. 원 커밋의 `#f0abfc`(밝은 자홍)도 검정 사이드바 시절 색이라 흰 면에서
     대비가 무너진다 → 같은 계열 진한 값(fuchsia-700, 약 5:1). */
  { key: 'ads', label: '유어애즈 · 마케팅 (인플루언서 DB)', accent: '#A21CAF' },

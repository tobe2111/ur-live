...
export function CartTab({ cartItems, onUpdateQuantity, onRemoveItem, onCheckout }: CartTabProps) {
  ...
  return (
    <div className="space-y-4 pb-28">
      {/* Cart Items */}
      <div className="bg-white dark:bg-[#0A0A0A] rounded-2xl border border-gray-100 dark:border-[#1A1A1A] overflow-hidden divide-y divide-gray-100">
        {cartItems.map(item => (
          <div key={item.id} className="p-4">
            ...
            <div className="flex items-center justify-between">
              <div className="inline-flex items-center rounded-full border border-gray-200 dark:border-[#2A2A2A] overflow-hidden">
                <button
      ...
      {/* Cart Summary */}
      <div className="bg-white dark:bg-[#0A0A0A] rounded-2xl border border-gray-100 dark:border-[#1A1A1A] p-5">
        <div className="space-y-3 pb-4 border-b border-gray-100 dark:border-[#1A1A1A]">
          <div className="flex justify-between">
            <span className="text-[13px] text-gray-500 dark:text-gray-400">상품 금액</span>
            <span className="text-[14px] font-semibold text-gray-900 dark:text-white">
              {formatNumber(totalAmount)}원
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-[13px] text-gray-500 dark:text-gray-400">배송비</span>
            <span className="text-[14px] font-semibold text-gray-900 dark:text-white">무료</span>
          </div>
        </div>

        <div className="pt-4 flex justify-between items-baseline">
          <span className="text-[15px] font-bold text-gray-900 dark:text-white">총 결제금액</span>

...
export function CartTab({ cartItems, onUpdateQuantity, onRemoveItem, onCheckout }: CartTabProps) {
  ...
  return (
    <div className="space-y-4 pb-28">
      <div className="bg-white dark:bg-[#0F151D] rounded-2xl border border-gray-100 dark:border-[#2A3446] overflow-hidden divide-y divide-gray-100">
        {cartItems.map(item => (
          <div key={item.id} className="p-4">
            <div className="mb-3">
              <h3 className="text-[15px] font-semibold text-gray-900 dark:text-white line-clamp-2 leading-snug">
                {item.product_name}
              </h3>
              {item.option_value && (
            ...
            <div className="flex items-center justify-between">
              <div className="inline-flex items-center rounded-full border border-gray-200 dark:border-[#2A3446] overflow-hidden">
                <button
                  onClick={() => onUpdateQuantity(item.id, item.quantity - 1)}
                  disabled={item.quantity <= 1}
                  aria-label={t('cart.decreaseQty', { defaultValue: '수량 감소' })}
                  className="w-9 h-9 flex items-center justify-center text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-[#1A2334] active:bg-gray-100 dark:active:bg-[#1A2334] disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                >
                ...
                <button
                  ...
                  className="w-9 h-9 flex items-center justify-center text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-[#1A2334] active:bg-gray-100 dark:active:bg-[#1A2334] transition-colors"
                >
              ...
              <button
                ...
                className="flex items-center gap-1 text-[13px] font-medium text-gray-500 dark:text-gray-400 hover:text-red-500 transition-colors"
              >
                <Trash2 className="h-3.5 w-3.5" />
                {t('cart.remove', { defaultValue: '삭제' })}
              </button>
            </div>
          </div>
        ))}
      ...
      <div className="bg-white dark:bg-[#0F151D] rounded-2xl border border-gray-100 dark:border-[#2A3446] p-5">
        <div className="space-y-3 pb-4 border-b border-gray-100 dark:border-[#2A3446]">
      ...
      <div
        className="app-frame-bar fixed bottom-0 left-0 right-0 xl:left-56 bg-white dark:bg-[#0F151D] border-t border-gray-100 dark:border-[#2A3446] px-4 py-3 z-40 safe-bottom"

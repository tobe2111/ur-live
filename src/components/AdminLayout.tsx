...
export default function AdminLayout({ title, children, headerRight, pendingCount = 0 }: AdminLayoutProps) {
  ...
  // 🆕 2FA 강제 등록 게이트 — 강제 대상(도매 파트너/슈퍼)인데 미등록이면 2FA 설정 페이지로 가둠.
  //   로그인 시 must_enroll_2fa 플래그 설정 → 등록(verify 성공) 시 해제. /admin/2fa 자신은 면제(루프 방지).
  useEffect(() => {
    if (typeof window === 'undefined') return
    if (localStorage.getItem('admin_must_enroll_2fa') === '1' && location.pathname !== '/admin/2fa') {

...
export default function MallOriginBanner({ className = '' }: { className?: string }) {
    ...
    fetch(`/api/mall/${encodeURIComponent(slug)}`)
      ...
      .then((j) => {
        ...
        setBrand({
          ...
          colorLight: String(j.mall?.colorLight || ''),
        })
      })
  ...
  // 🎨 가게가 색을 안 정했으면 **클래스가 기본값을 준다**(`bg-gray-800`). 여기에 hex 를 박으면
  //   손으로 박은 색이 하나 더 늘고, 나중에 팔레트를 바꿀 때 이 자리만 남는다.
  ...
  return (
    <div className={`flex items-center gap-3 rounded-xl bg-surface shadow-lift px-3.5 py-3 ${className}`}>
      ...
      <button
        ...
        className="shrink-0 rounded-lg border border-rule px-3 py-2 text-[12px] font-semibold text-gray-700 dark:text-gray-200 hover:bg-warm transition-colors"
      >
        가게로 →
      </button>
    </div>

 ...
 */
export function clearAuthData(type) {
    console.log(`[Auth] Clearing ${type} auth data (selective removal)`);
    const keysToRemove = [];
    if (type === 'seller') {
        // Seller 전용 키 (user_id, user_name은 User 전용이므로 제거 안 함)
        keysToRemove.push('seller_token', 'seller_refresh_token', 'seller_id', 'seller_name', 'seller_email', 'user_type', 'access_token', // 레거시 호환
        'refresh_token' // 레거시 호환
        );
    }
    else if (type === 'admin') {
        // Admin 전용 키 (user_id, user_name은 User 전용이므로 제거 안 함)
        keysToRemove.push('admin_token', 'admin_refresh_token', 'admin_id', 'admin_name', 'admin_email', 'user_type', 'access_token', // 레거시 호환
        'refresh_token' // 레거시 호환
        );
    }
    else {
        // User 전용 키
        keysToRemove.push('firebase_token', 'user_type', 'user_id', 'user_name', 'user_email', 'user_profile_image', 'user_session_token', // 레거시 세션 토큰
        'hasCartItems', 'tempCartItem', 'loginReturnUrl', 'lastLoginUid');
    }
    // 선택적 삭제
...
export function isLoggedInSync() {
    ...
    if (userId && (userType === 'user' || !userType))
        return true;
    // 레거시: firebase_token이 localStorage에 있는 경우 (구버전 호환)
    if (localStorage.getItem(FIREBASE_STORAGE_KEYS.FIREBASE_TOKEN))
        return true;
    return false;
}
 ...
 */
export function requireLogin(navigate, message = '로그인이 필요합니다.', force = false) {
 ...
 */
export function saveTempCartItem(productId, quantity, priceSnapshot, liveStreamId, productName) {
    const tempCart = {
        productId,
        quantity,
        priceSnapshot,
 ...
 * 임시 장바구니 아이템 가져오기
 */
export function getTempCartItem() {
 ...
 * 임시 장바구니 아이템 삭제
 */
export function clearTempCartItem() {
    localStorage.removeItem(FIREBASE_STORAGE_KEYS.TEMP_CART_ITEM);
}
/**
 ...
 */
export function saveFirebaseTokens(firebaseToken, userId, userName, userType, userEmail, profileImage) {
    // Firebase 토큰 저장
 ...
 */
export function saveJwtTokens(accessToken, refreshToken, userId, userName, userType, userEmail, profileImage) {
 ...
 * @deprecated Firebase 전환 후 이 함수는 saveFirebaseTokens로 대체됩니다.
 */
export function saveUserInfo(userId, userName, sessionToken, userEmail, profileImage) {

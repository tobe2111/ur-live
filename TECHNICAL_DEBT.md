...
- **Tier 2 — 데이터페칭 RQ 이전**: ✅ **10건 이전 완료** — `MyFollowsPage`(useMyFollows), `NotificationsPage`(useNotifications — 읽음 시 unreadCount 벨배지 자동갱신), `InterestListPage`(useMyInterests), `MyCouponsPage`(useMyCoupons), `MyReviewsPage`(useMyOrders 재사용), `MyAppointmentsPage`(useMyAppointments 재사용), `FollowingPage`(useFollowing), `MyReturnsPage`(useMyReturns + 송장등록 캐시반영), `MyDealHistoryPage`(useDealHistory — page/filter + keepPreviousData), `WishlistPage`(useWishlist). 패턴/컨벤션(`queryKeys` SSOT, `localCache` fallback, `refetchOnMount:'always'`, optimistic+롤백, 기존 훅 재사용, 파라미터 쿼리 keepPreviousData) 정착. **남은 안전 후보**(다음 PR): `MyStaysPage`(리뷰 mutation), `MyGroupBuysPage`(멀티 fetch — 복잡), `MyDigitalLibraryPage`, `AddressManagementPage` 등. ⚠️ **고위험 흐름 영구 제외**(checkout/payment-success/group-buy-confirm/toss-widget/points-charge/kakao·youtube-callback/login — 부작용 순서 민감). `MyLedgerPage`는 seller/agency 게이트 대시보드 → 저ROI 제외. 대시보드(167개)는 후순위.

...
- **Tier 2 — 데이터페칭 RQ 이전**: ✅ **user-facing My*/소셜 페이지 16건 전부 완료** — MyFollows/Notifications/InterestList/MyCoupons/MyReviews/MyAppointments/Following/MyReturns/MyDealHistory/Wishlist/MyDigitalLibrary/MyStays/MyGroupBuys/AddressManagement/MyOrders/MyCommissions (+ 기존 MyVouchers). 신규 훅 11종(`useMyFollows`/`useNotifications`/`useMyInterests`/`useMyCoupons`/`useFollowing`/`useMyReturns`/`useDealHistory`/`useWishlist`/`useDigitalLibrary`/`useMyStays`/`useMyGroupBuys`/`useAddresses`/`useMyCommissions`) + `useMyOrders`/`useMyAppointments` 재사용. 패턴(`queryKeys` SSOT, `localCache` fallback, `refetchOnMount:'always'`, optimistic+롤백, keepPreviousData, 복합 멀티-fetch queryFn 캡슐화) 정착.
  ...
  - 🟡 **대시보드 트랙 착수**(seller/admin/agency, 내부·저ROI): **제네릭 `useApiQuery`(2026-05-31)** 로 per-page 훅 없이 3줄 이전. **8/136 완료** — SellerReviews/AgencyReturns/SellerStays/AgencyCompare/AgencyRanking/AgencyStreams/AdminPendingSellers/AgencyTargets. 패턴: `useApiQuery(key, url, {params, select, enabled})` + mutation 후 `refetch()`/`setQueryData`. api 인터셉터가 seller/agency/admin_token 자동 주입. **남은 128개는 동일 패턴의 기계적 반복**(점진). ⚠️ 1회성 config/register/2FA/status 페이지는 캐싱 가치 0 → 이전 불필요.
  - 🟡 **God 파일 실제 분해**=스테이징 PR 필요(안전망 완비).
  - ✅ **돈 라우트 테스트 안전망 신설**(2026-06-01, 테스트 0→15) — `returns`(환불·재고복원·공급사/영입자 역적립 9ep) / `referral-tree`(commission 적립·출금승인 10ep) / `agency-settlements`(4ep) / `admin-settlements`(8ep, 인증=부모 requireAdmin) / `wholesale`(B2B 주문·결제 9ep) / `donations`(후원 init/confirm 3ep). 계약(핸들러 소실 차단) + 미인증 401/403(인증 게이트 누락→무권한 환불/출금/정산/결제 트리거 차단). network/DB 없이 실행.

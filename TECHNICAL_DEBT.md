...
- **Tier 2 — 데이터페칭 RQ 이전**: ✅ **user-facing My*/소셜 페이지 16건 전부 완료** — MyFollows/Notifications/InterestList/MyCoupons/MyReviews/MyAppointments/Following/MyReturns/MyDealHistory/Wishlist/MyDigitalLibrary/MyStays/MyGroupBuys/AddressManagement/MyOrders/MyCommissions (+ 기존 MyVouchers). 신규 훅 11종(`useMyFollows`/`useNotifications`/`useMyInterests`/`useMyCoupons`/`useFollowing`/`useMyReturns`/`useDealHistory`/`useWishlist`/`useDigitalLibrary`/`useMyStays`/`useMyGroupBuys`/`useAddresses`/`useMyCommissions`) + `useMyOrders`/`useMyAppointments` 재사용. 패턴(`queryKeys` SSOT, `localCache` fallback, `refetchOnMount:'always'`, optimistic+롤백, keepPreviousData, 복합 멀티-fetch queryFn 캡슐화) 정착.
  ...
  - ✅ **non-My user-facing 추가 7건**: `BlogDetailPage`(useBlogPost), `LiveRecapPage`(useLiveRecap), `AffiliatePage`(useAffiliate 3종), `StaysSearchPage`(useStaysSearch — qs commit), + 유통스타트 도매 `WholesaleOrders`/`WholesaleStatement`/`WholesaleCatalog`(useWholesale 5종, seller_token). 검색/날짜 필터는 commit 패턴으로 기존 동작 보존.
  - ⚠️ **영구 제외**(올바른 설계): checkout/payment-success/group-buy-confirm/toss-widget/points-charge/kakao·youtube-callback/login (부작용 순서 민감). **SSR 잠금 페이지 제외**(`__SSR_INITIAL_*`: Browse/Vouchers/GroupBuyDetail/ProductDetail/Voucher·StayDetail/SellerPublic/**LiveList**). `MyLedgerPage`=seller/agency 대시보드. 대시보드(167개)=저ROI 후순위.
  - 🟡 **남은 user 후보**(복잡/저빈도): `ReferralPage`(512줄 코드조회+hydration), `SearchPage`(debounce), `CouponClaim`/`GiftClaim`(claim mutation), `RestaurantMapPage`(534줄 지도SDK), `WholesaleProductPage`(주문 POST 동반).

...
- **Tier 2 — 데이터페칭 RQ 이전**: ✅ **user-facing My*/소셜 페이지 16건 전부 완료** — MyFollows/Notifications/InterestList/MyCoupons/MyReviews/MyAppointments/Following/MyReturns/MyDealHistory/Wishlist/MyDigitalLibrary/MyStays/MyGroupBuys/AddressManagement/MyOrders/MyCommissions (+ 기존 MyVouchers). 신규 훅 11종(`useMyFollows`/`useNotifications`/`useMyInterests`/`useMyCoupons`/`useFollowing`/`useMyReturns`/`useDealHistory`/`useWishlist`/`useDigitalLibrary`/`useMyStays`/`useMyGroupBuys`/`useAddresses`/`useMyCommissions`) + `useMyOrders`/`useMyAppointments` 재사용. 패턴(`queryKeys` SSOT, `localCache` fallback, `refetchOnMount:'always'`, optimistic+롤백, keepPreviousData, 복합 멀티-fetch queryFn 캡슐화) 정착.
  - 🐛 **부수 발견·수정**: `useMyOrders` 가 `/api/orders` 의 `data:{items:[...]}` 를 `Array.isArray` 로만 검사 → 항상 `[]` 반환하던 잠복 버그(유일 소비자였던 신규 MyReviews/MyOrders 와 함께 정상화). `refetchOnMount` 잠금 불변.
  - ✅ **non-My user-facing 11건**: `BlogDetailPage`(useBlogPost), `LiveRecapPage`(useLiveRecap), `AffiliatePage`(useAffiliate 3종), `StaysSearchPage`(useStaysSearch — qs commit), 유통스타트 도매 `WholesaleOrders`/`WholesaleStatement`/`WholesaleCatalog`/`WholesaleProduct`(useWholesale 6종, seller_token), `ReferralPage`(useReferral — 그룹+상품 hydration), `RestaurantMapPage`(useMapProducts — products만, live-poller 유지). 검색/날짜 필터는 commit 패턴으로 기존 동작 보존.
  - ✅ **user-facing 캐시 가능 read 페이지 RQ 이전 사실상 완료**(SSR 잠금/대시보드 제외). `SearchPage`=이미 `useSearchInfinite`(useInfiniteQuery).
  - ⚠️ **의도적 제외**(RQ 부적합 — imperative 가 올바른 설계): **claim 액션**(`CouponClaim`/`GiftClaim` — 1회성 side-effect, already_claimed state-machine), 결제/인증 흐름(checkout/payment-success/group-buy-confirm/toss-widget/points-charge/kakao·youtube-callback/login), **튜닝된 polling**(RestaurantMap live-poller — visibility/배터리). **SSR 잠금**(`__SSR_INITIAL_*`: Browse/Vouchers/GroupBuyDetail/ProductDetail/Voucher·StayDetail/SellerPublic/LiveList). `MyLedgerPage`=대시보드.
  - 🟢 **대시보드 트랙 완료**(seller/admin/agency, 내부·저ROI): **제네릭 `useApiQuery`(2026-05-31)** 로 per-page 훅 없이 3줄 이전. **~120/128 완료(2026-06-03)** — 캐싱 가치 있는 read/list/편집폼 + **대형 대시보드 3개(Admin/Seller/Agency) 전부 이전**. 패턴: `useApiQuery(key, url, {params, select, enabled, refetchInterval, headers, initialData})` + mutation 후 `refetch()`/`setQueryData`. api 인터셉터가 seller/agency/admin_token 자동 주입; 미주입 prefix(`/api/seller-public`·`/api/supply`·`/api/seller-marketing`·`/api/youtube-growth`·`/api/agency-public`)는 `headers` 옵션으로 수동 전달. 탭/필터/페이지는 queryKey 반응형 또는 `enabled` 게이트, 편집형 폼은 데이터 도착 시 로컬 state 시드, optimistic toggle 은 `queryClient.setQueryData(key)`.
    - **useApiQuery 옵션 확장(2026-06-03)**: `refetchInterval`(폴링 — foreground only) + `headers`(수동 토큰) + `initialData`(동기 캐시 seed).
    - **대형 대시보드 이전(2026-06-03)**: `AgencyPage`(useApiQuery bundle)/`SellerPage`/`AdminPage`(raw useQuery multi-fetch) — **sessionStorage 5분 즉시렌더 캐시를 `initialData` 로 보존 + `refetchOnMount:'always'` 백그라운드 fresh** → 로딩최적화 회귀 0. fetcher 가 동일 캐시 갱신. SellerPage 실시간 주문 폴러 / AdminPage dashboard-stats 30s 폴러+이상거래 알림은 명령형 유지.
    - **의도적 명령형 유지(이전 부적합 — 잔여 ~8)**: `SellerPublicPage`(공개 SSR `__SSR_INITIAL_SELLER__` 잠금 — 절대 금지), `AdminLiveMonitor`(snapshot-diff 알람+사운드 폴링), `AdminEnvCheck`(커스텀 헤더+503 body), `SellerOverlay`(OBS 오버레이 다중 폴링+dedup), `SellerLiveBroadcast*`(실시간 WS+폴링), `SellerWaiting`/`AgencyWaiting`(상태별 redirect 라우팅), 순수 POST 폼(login/register/reset/2FA/notices/bulk-tracking/notify-followers — 캐싱 가치 0). 부분 이전(읽기 섹션만, 편집 테이블 명령형): `AdminKtAlpha`(consumer stats/categories), `AdminDistributorGrades`(proposals).
  ...
  - ✅ **돈/보안 계산 로직 테스트**(route 가드뿐 아니라 **로직 자체** 보호, mock DB):
    ...
    - `idempotency`(0→7): **이중결제 방지** — (key,user) 1회 실행, done→캐시 replay, pending→409, 실패 시 키삭제.
    - `order-refund`(0→6): 전액환불 가드 — 주문없음404/IDOR/멱등/상태검증/결제키누락422.
    - `agency-store-intro-commission`(0→6): 입점 signup_bonus ₩30k + sales_commission 2%.

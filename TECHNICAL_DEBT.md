...
- **Tier 2 — 데이터페칭 RQ 이전**: ✅ **user-facing My*/소셜 페이지 16건 전부 완료** — MyFollows/Notifications/InterestList/MyCoupons/MyReviews/MyAppointments/Following/MyReturns/MyDealHistory/Wishlist/MyDigitalLibrary/MyStays/MyGroupBuys/AddressManagement/MyOrders/MyCommissions (+ 기존 MyVouchers). 신규 훅 11종(`useMyFollows`/`useNotifications`/`useMyInterests`/`useMyCoupons`/`useFollowing`/`useMyReturns`/`useDealHistory`/`useWishlist`/`useDigitalLibrary`/`useMyStays`/`useMyGroupBuys`/`useAddresses`/`useMyCommissions`) + `useMyOrders`/`useMyAppointments` 재사용. 패턴(`queryKeys` SSOT, `localCache` fallback, `refetchOnMount:'always'`, optimistic+롤백, keepPreviousData, 복합 멀티-fetch queryFn 캡슐화) 정착.
  ...
  - 🟢 **대시보드 트랙 대량 진행**(seller/admin/agency, 내부·저ROI): **제네릭 `useApiQuery`(2026-05-31)** 로 per-page 훅 없이 3줄 이전. **~106/128 완료(2026-06-03)**. 패턴: `useApiQuery(key, url, {params, select, enabled, refetchInterval, headers})` + mutation 후 `refetch()`/`setQueryData`. api 인터셉터가 seller/agency/admin_token 자동 주입; 미주입 prefix(`/api/seller-public`·`/api/supply`·`/api/seller-marketing`·`/api/youtube-growth`·`/api/agency-public`)는 `headers` 옵션으로 수동 전달. 탭/필터/페이지는 queryKey 반응형 또는 `enabled` 게이트, 편집형 폼은 데이터 도착 시 로컬 state 시드.
    - **useApiQuery 옵션 확장(2026-06-03)**: `refetchInterval`(폴링 페이지 — foreground only) + `headers`(수동 토큰).
    - **의도적 명령형 유지**(이전 부적합): 대형 대시보드 `AdminPage`/`SellerPage`/`AgencyPage`(sessionStorage 5분 즉시렌더 캐시 = 로딩최적화), `AdminLiveMonitor`(snapshot-diff 알람+사운드 폴링), `AdminEnvCheck`(커스텀 헤더+503 body), `SellerLiveBroadcast*`(실시간), 순수 POST 폼(login/register/reset/2FA/notices/bulk-tracking/notify), per-row lazy 확장 캐시(AdminAgency 셀러/AgencyCampaigns 참여자/AdminUsers 상세/AgencySellers stats).
    - **남은 ~22**: 대부분 위 "명령형 유지" 군 + 잔여 편집 폼(SellerProfileEdit/ProductEdit/StayDetail/StayNew/MealVoucherNew/BusinessInfo/StreamingSetup/Overlay) — 점진 이전.

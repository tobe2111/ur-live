...
- **Tier 2 — 데이터페칭 RQ 이전**: ✅ **5건 이전 완료** — `MyFollowsPage`(useMyFollows), `NotificationsPage`(useNotifications — 읽음 시 unreadCount 벨배지 자동갱신 보너스), `InterestListPage`(useMyInterests), `MyCouponsPage`(useMyCoupons), `MyReviewsPage`(useMyOrders 재사용 — 중복 fetch 제거). 패턴/컨벤션(`queryKeys` SSOT, `localCache` fallback, `refetchOnMount:'always'`, optimistic+롤백, 기존 훅 재사용) 정착. **다음 안전 후보**: `MyStaysPage`/`MyAppointmentsPage`/`MyDealHistoryPage`/`MyLedgerPage`/`MyGroupBuysPage`/`MyReturnsPage`/`WishlistPage`/`FollowingPage`. ⚠️ **고위험 흐름 영구 제외**(checkout/payment-success/group-buy-confirm/toss-widget/points-charge/kakao·youtube-callback/login — 부작용 순서 민감, 수동 유지가 올바른 설계). 대시보드(seller/admin/agency 167개)는 저ROI(내부) — 후순위.

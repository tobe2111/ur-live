...
const TARGET_RES = [
  ...
  /\bhref=(?:"([^"]+)"|'([^']+)'|\{`([^`]+)`\})/g,                              // <a href="/...">
  // 🕳️ 2026-08-12: **객체 리터럴 `to: '/x'` 가 사각지대였다** — JSX 속성 `to=` 만 잡고 있었다.
  //   링크를 배열로 선언하고 `.map()` 으로 렌더하는 흔한 패턴(`NotFoundPage` 의 "인기 페이지
  //   둘러보기", 각종 칩·탭 목록)이 통째로 무검사였다. 이 한 줄로 검사 타깃 **868 → 904**(+36 — 되돌려-검증 실측).
  //
  //   ⚠️ **정직하게**: 이 구멍으로 난 죽은-링크 사고는 아직 없다(추가 시점 위반 0). 같은 파일에서
  //   2026-08-11 에 고친 결함은 죽은 링크가 아니라 **라벨↔목적지 불일치**였다
  //   (`label: '공동구매'` 인데 `to: '/referral'`(추천 수익) — 라우트는 실재한다).
  //   그건 이 가드가 원리적으로 못 잡는다(문자열이 유효한지만 보지 의미가 맞는지는 모른다).
  //   여기 추가하는 이유는 사고 이력이 아니라 **66개가 지금까지 한 번도 검사되지 않았다**는 사실이다.
  /\bto:\s*(?:"([^"]+)"|'([^']+)'|`([^`]+)`)/g,                                 // { to: '/...' }
]

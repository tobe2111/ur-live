...
function main() {
  ...
  for (const role of ROLES) {
    ...
    // 🕐 2026-09-08: 여기 있던 `마지막 생성: <ISO 시각>` 을 뺐다. 아래 배너 주석의 이유와 같다 —
    //   시각이 매번 달라 **모든 커밋이 이 파일을 건드리고**, 그래서 동시에 도는 PR 끼리
    //   코드가 안 겹쳐도 반드시 충돌했다. 가이드는 배포마다 재생성되므로 값의 의미도 얕았다.
    md += `> 코드에서 자동 생성 · 배포마다 갱신됩니다.\n`
    sections[role] = md
  }
 ...
 *
 * 🕐 생성 시각을 일부러 적지 않는다 (2026-09-08).
 *   pre-commit 훅이 **매 커밋마다** 이 생성기를 돌리는데, 시각을 박으면 내용이 안 바뀌어도
 *   파일이 늘 달라진다 → 모든 커밋이 이 파일을 스테이지하고, 동시에 도는 두 PR 은
 *   **코드가 전혀 안 겹쳐도** 이 줄에서 충돌한다(2026-09-07 하루에 세 번 났다).
 *   읽는 곳도 없었다. 시각이 빠지면 이 파일은 **라우트·엔드포인트가 실제로 바뀐 커밋에서만**
 *   변한다. ⚠️ 되살리지 말 것 — 되살리면 그 충돌이 그대로 돌아온다.
 *

...
const BUDGET = {
  ...
  // ⬆️ 2026-09-24: 7.45 → **7.5**(+0.05). **무엇이 왜 늘었는가**: 결재 파일 한 개
  //   (`docs/decisions/2026-09-24-settlement-single-rail.md`, 2.5KB) 추가로 `AdminDecisionsPage`
  //   청크가 **+3,096 B**. main 실측 7.448959 → 이 브랜치 7.451912(초과 2,005 B).
  //   🔴 **이건 유기적 성장이 아니라 구조적으로 예정된 성장이다.** 그 페이지는 결재함을
  //   `import.meta.glob(..., '?raw', eager)` 로 **빌드 때 인라인**한다(2026-09-08 대표
  //   *"매번 깃헙에서 보기에는 불편해"* → 배포마다 자동 최신, SSOT 는 파일). 즉 **결재를 쓸 때마다
  //   번들이 커진다** — 지금 19개 164KB 이고, 그중 14개가 이미 approved/rejected 다.
  //   ⇒ 다음에 또 닿으면 **올리지 말고 `docs/decisions/archive/` 로 처리 끝난 것을 옮겨라**
  //     (glob 에서 빠진다). 상향은 그 정리를 미루는 값이지 해결이 아니다.
  //   ✅ 사용자 체감 0 인 것은 확인했다: 늘어난 것은 **어드민 lazy 청크 하나**이고
  //     critical path 는 194.1 / 240 KB, gzip 2.367 / 2.45 로 둘 다 여유가 그대로다.
  totalRawMB: 7.5,

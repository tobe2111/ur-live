...
const BUDGET = {
  ...
  // ⬆️ 2026-09-26 4차: 7.54 → **7.57**(+0.03). 무엇이 왜 늘었나 —
  //   대표 *"모두 다 마이로 가능하게끔 하고 최적화 필요해. 로딩 속도를 줄이고"*.
  //   실측 7.53 → **7.56 MB**. 늘어난 것은 **선반 총량**이고, 줄어든 것은 **사람이 실제로 받는 양**이다:
  //     · 마이 정적 폐쇄  30 청크 1,081.2 KB → **27 청크 912.5 KB (−168.7 KB)**
  //     · `UserProfilePage` 청크 118.3 → **46.9 KB (−60%)**
  //     · `app-seller-components`(83.8) + `app-dashboard`(9.9) **폐쇄에서 완전히 사라짐**
  //   +30 KB 의 정체: 시트 열넷을 `lazy` 로 가르면서 생긴 **청크 경계 비용**(경계마다 붙는 래퍼 +
  //   함께 최적화되지 못하는 손해) + 새 파일 셋(임베드 컨텍스트·도구 지도·범용 시트).
  //   ⚠️ 한 봉투로 묶어 그 비용을 되찾으려 해 봤고 **실패했다** — 실측 총량이 7.56 으로 동일한데
  //     `useSellerWork` 가 봉투로 끌려가 셀러 폐쇄만 936.9 → 1,003.4 KB 로 **커졌다**. 되돌렸다.
  //     (다음 사람이 같은 시도를 반복하지 않도록 남긴다.)
  //   🔴 **9번째 상향이다.** 앞선 여덟과 다른 점은 이번이 **사용자가 받는 양을 줄이면서** 올린
  //     첫 번째라는 것뿐이고, ①(결재 아카이브 — `AdminDecisionsPage` 가 처리 끝난 결재 15건을
  //     `eager: true` 로 인라인, 140.4 KB = 예산의 1.9%)은 **여전히 2026-09-25 부터 대표 대기**다.
  //     승인되면 이번 +30 KB 를 포함해도 **7.50 아래로 되돌아간다.**
  //   ✅ critical-chunks **17 불변**(신규 진입 0) · gzip **2.410 / 2.45** (여유 그대로).
  //   ⚠️ 헤드룸 10 KB.
  totalRawMB: 7.57,

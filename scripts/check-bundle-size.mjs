...
const BUDGET = {
  ...
  // 🛡️ 2026-08-04: 8.9 → 8.95 상향(**8번째**). 실측 **8.9012 MB — 초과분 1,298 바이트**.
  //   늘린 것: 홈 히어로 기본 배경(`HomeHeroDefault`) + 홈 섹션 시드/편집 UI — 전부 **lazy**.
  //   살아 있는 두 감지기는 여유: critical-path **217.7/240 KB gzip**(신규 진입 0) · 총 gzip **2.780/2.9 MB**.
  //   🔴 **직전 상향(7번째)이 "다음엔 상향이 아니라 lazy 청크 정리"라고 적어 뒀고, 그 말이 맞다.**
  //     그런데 지목된 firebase(378KB)는 `useAuthKR`·`useAuthWorld`·`RouteGuards`·`firebase-auth.ts` 에
  //     아직 엮여 있어 **인증 변경**이다 — UI PR 에 끼워 넣으면 "1 commit = 1 원인" 을 어긴다.
  //     ⇒ **다음 세션의 우선 과제로 넘긴다**(단독 PR + 로그인 E2E). 그때까지의 임시 상향이다.
  //     이 줄이 9번째로 늘어나면, 그건 정리를 계속 미뤘다는 뜻이다.
  totalRawMB: 8.95,

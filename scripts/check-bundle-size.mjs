...
const BUDGET = {
  ...
  // ⚠️ 2026-09-24: 7.45 → **7.47**(+0.02). 무엇이 왜 늘었나 — **코드가 아니라 문서다.**
  //   실측 델타 5,326 B 중 **3,339 B 가 `docs/decisions/*.md` 프로즈**다:
  //   `admin/AdminDecisionsPage.tsx:15` 가 `import.meta.glob('...docs/decisions/*.md', ?raw, eager)`
  //   로 **결재 문서 18개를 통째로 번들에 인라인**한다(그 청크 혼자 **141 KB** = 예산의 1.9%).
  //   나머지 1,987 B 는 `/store/find` PC 2단(대표 지시)이고 **critical path 는 194.1 KB 불변**이다.
  //   🔴 **다음 사람에게**: 이 값을 또 올리기 전에 구조를 보라 — 결재 문서는 **어드민만 읽는 프로즈**인데
  //     소비자 번들과 같은 예산을 먹고, 결재가 쌓일수록 **영원히 커진다**. 진짜 처방은 임계값이 아니라
  //     그 `?raw` 인라인을 걷어내는 것(워커가 서빙하거나 lazy fetch). 그건 새 API 표면이라 별건이다.
  totalRawMB: 7.47,

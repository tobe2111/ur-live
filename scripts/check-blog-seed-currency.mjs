...
// 시드 콘텐츠는 blog-seed.ts, 동기/레거시 로직은 blog.routes.ts — 둘 다 스캔.
const FILES = [
  path.join(ROOT, 'src/features/blog/api/blog-seed.ts'),
  path.join(ROOT, 'src/features/blog/api/blog.routes.ts'),
]
...
for (const FILE of existing) {
  ...
  lines.forEach((line, i) => {
    ...
    for (const { re, why } of FORBIDDEN) {
      if (re.test(line)) hits.push({ file: FILE, ln: i + 1, term: re.source, why, text: line.trim().slice(0, 90) })
    }
  })
}
...
for (const h of hits) {
  console.log(`   ${path.relative(ROOT, h.file)}:${h.ln}  [${h.term}] ${h.why}`)
  console.log(`      ${h.text}`)
}

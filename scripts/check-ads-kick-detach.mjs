 ...
 *
 * ## 왜 가드여야 하나 (라이브 실측)
 * 하트비트를 오래된 순으로 세우면 계단이 보인다. ⚠️ 그 계단 전부가 굶주림은 아니다 —
 * `maintenance?phase=*`(hourUTC 로테이션)·짝/홀수시 레인은 원래 매시간이 아니다.
 * **결정적 신호는 하나뿐**: *게이트 없는 매시간 레인*이 자기 슬롯을 놓쳤는가.
 * 실측 — `enrich-prospects`·`collect-neis`·`collect-hira`·`collect-localdata(백필)`·`match-registry` 는
 * 매시간 무조건 도는데 09:00/10:00 이 마지막이었고, **즉시응답 레인 2개만 한 번도 안 빠졌다**.
 * 수집이 0인 건 코드가 틀려서가 아니라 **레인이 돌지 않아서**였다. 그리고 이 실패는
 * **에러를 내지 않는다** — 하트비트를 세워 보기 전까지 아무도 몰랐다.

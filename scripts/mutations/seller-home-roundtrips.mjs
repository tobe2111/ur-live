...
export default [
  {
    ...
    file: HELPERS,
    find: "    if (haveProducts.has(col.trim().split(/\\s+/)[0])) continue",
    replace: '',
    test: TEST,
  ...
  },
  {
    ...
    file: HELPERS,
    find: "    if (haveVouchers.has(col.trim().split(/\\s+/)[0])) continue",
    replace: '',
    test: TEST,
  ...
  },
  {
    ...
    file: HELPERS,
    find: "    .all<{ name: string }>().catch(() => ({ results: [] as { name: string }[] }))",
    replace: "    .all<{ name: string }>().catch(() => ({ results: [{ name: 'restaurant_name' }] as { name: string }[] }))",
    test: TEST,

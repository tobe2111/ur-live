// NicePay 실제 인증 정보 테스트

const NICEPAY_MID = 'BItobe211m';
const NICEPAY_KEY = 'bPEmjXIPGt8FvmRxt7Q8nrbi4J3Z20I237Xbyp5pOxUP7A3UnyDbY5dOKBSrZxyflDzMop1kfVQwOsnl3D+wLA==';

const auth = Buffer.from(`${NICEPAY_MID}:${NICEPAY_KEY}`).toString('base64');

console.log('========================================');
console.log('NicePay 실제 인증 정보 테스트');
console.log('========================================');
console.log('MID:', NICEPAY_MID);
console.log('Key 길이:', NICEPAY_KEY.length, '문자');
console.log('Key 형식:', NICEPAY_KEY.endsWith('==') ? '✅ Base64 형식' : '❌ 형식 이상');
console.log('========================================\n');

async function testAuth() {
    try {
        console.log('🔵 NicePay API 인증 테스트 중...\n');
        
        const response = await fetch('https://api.nicepay.co.kr/v1/payments/approval', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Basic ' + auth
            },
            body: JSON.stringify({
                tid: 'test_tid_12345',
                amt: '1000',
                ediDate: '20260206',
                authToken: 'test_token'
            })
        });

        const result = await response.json();
        
        console.log('응답 상태:', response.status);
        console.log('응답 본문:', JSON.stringify(result, null, 2));
        console.log('');
        
        if (response.status === 401) {
            console.log('❌ 인증 실패: MID 또는 Key가 잘못되었습니다.');
        } else if (response.status === 400) {
            console.log('✅ 인증 성공! (요청 데이터는 잘못되었지만 인증은 통과)');
            console.log('✅ MID와 Merchant Key가 올바릅니다!');
        } else if (response.status === 200) {
            console.log('✅ 완전히 성공!');
        } else {
            console.log('⚠️  응답 상태:', response.status);
            console.log('⚠️  메시지:', result.resultMsg);
        }
        
    } catch (error) {
        console.error('❌ 오류 발생:', error.message);
    }
}

testAuth();

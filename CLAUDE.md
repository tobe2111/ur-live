...
#### 📏 무료 천장 = **하루 약 5,400명. 이미 붙어 있다** (2026-08-05 실측 — 대표 *"무료의 천장을 지금 보고싶다"*)

30시간 시간당 수집량을 재서 **투사 아닌 관측으로** 확정했다:
```
시간당 평균 224명 · 합계 6,728명 / 30시간   →  하루 약 5,400명
```
...
```
이유: 발굴 레인(`collect`)은 cron 레인 순환이 아니라 **DO 알람**으로 돈다
(하트비트에 `lane-alarm-boot:collect` 가 찍힌다). ⇒ **cap 회복은 측정·정비만 살리고
발굴 수집량은 안 늘린다.** 2026-08-05 세션이 "cap 6 회복하면 공짜 2배" 라고 대표에게
보고했다가 이 실측으로 **정정**했다 — 같은 오판을 반복하지 말 것.
...
시간당 12회 × 회차당 6키워드 = **72키워드/시간**이 무료의 구조적 상한이고, 실측 224명/시간이
거기서 나온다. **30시간 내내 평평한 것이 포화의 증거다** — 더 밀 여지가 있으면 들쭉날쭉해야 한다.
(고/저가 번갈아 나오는 것은 `MEASURE_SHARE=0.5` 의 측정↔수집 교대다.)

⇒ **무료에서 발굴을 더 늘릴 방법은 없다.** 네이버 쿼터가 0.7% 남아돌아도 알람 빈도와 회차당
예산이 먼저 막는다. 발굴을 늘리는 유일한 레버는 **유료 전환**이다(대표 판단 대기).

...
>   - "벤더사" = 매니징 조직(옛 "에이전시") / "도매 공급자(제조사)" = B2B 공급 (개인 축과 별도)
...
| **벤더사** | 여러 사업자 유저/매장을 관리하는 B2B 조직 (옛 "에이전시") | `agencies` |
...
>
> 🏷️ **2026-07-05 대표 확정 — "에이전시" → "벤더사" 로 통일** ("에이전시 대시보드를 이름을 모두 벤더사 대시보드로, 벤더사로 명칭 통일하자"). 사용자-가시 "에이전시" 전량(src 702건 + locale 6개 언어 324값) 일괄 치환 완료 — "에이전시 대시보드"→"벤더사 대시보드", "에이전시 파트너 약관"→"벤더사 파트너 약관" 포함. 6개 언어 값: **벤더사 / Vendor / ベンダー / 供应商 / Proveedor / Fournisseur**. **코드 식별자(`agencies`·`AgencyLayout`·`agency_token`·`/agency/*` 라우트·`AGENCY_HIDDEN`·i18n 키 `agency*`)는 전부 불변** — 한글/현지어 라벨만 변경. Korean 토큰 "에이전시" 는 식별자에 안 쓰여(식별자는 romanized `agency`) 안전 치환. 아래 audit log 의 "에이전시" 표기는 *작성 당시 historical record*(소급 변경 X). (에이전시는 현재 `AGENCY_HIDDEN=true` 셸브 상태 — 재개 시 새 라벨로.)
>

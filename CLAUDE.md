...
### 변경 audit log
- 2026-06-11 `[LOADING_ADDITIVE]` 카드 이미지 외부호스트(기프티쇼 등) 변환 경로 수리 (사용자 신고 "현저히 느림" — prod 실측 기반) — `cf-image.ts` EXTERNAL_PROXY 분기를 `/api/image/resize` 프록시 → **zone 리사이저 직접 래핑**(`/cdn-cgi/image/<옵션>/<외부절대URL>`)으로. 근거(GitHub Actions 실측): 프록시는 워커 내부 cdn-cgi subrequest 에 리사이저 미적용 → 항상 원본 폴백(143KB 그대로, 기프티쇼 origin 1~4.5s, 캐시 미적중) / 브라우저→cdn-cgi 직접은 cf-resized OK(143KB→18KB, zone 캐시). same-origin 응답이라 canvas 대표색(CORS, 06-05 프록시 도입 사유) 동일 안전. **SUPPORTED_HOSTS/EXTERNAL_PROXY_HOSTS 목록·Save-Data·`/api/media` 프록시 분기(06-06 사고로 cdn-cgi 불가) 전부 불변.** `/api/image/resize` 엔드포인트는 존치(업로드 분기 + 레거시 URL).

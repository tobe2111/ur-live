...
## 📖 어드민 운영 가이드 자동 업데이트 규칙

`src/pages/admin/AdminOperationsGuidePage.tsx` 는 관리자용 운영 핸드북입니다.
**새 기능 추가, API 변경, 운영 정책 변경 시 반드시 이 가이드도 함께 업데이트하세요.**

업데이트 대상:
- 새 API 엔드포인트 추가 → "유용한 링크" 섹션 또는 해당 운영 섹션에 반영
- 새 셀러/어드민/에이전시 기능 → 해당 역할 관리 섹션에 설명 추가
- 정산/주문 플로우 변경 → 주문 관리 / 정산 처리 섹션 업데이트
- 장애 발생 및 해결 → "기술 장애 대응" 섹션에 사례 추가
- FAQ 추가 → "자주 묻는 문제" 섹션에 Q&A 추가

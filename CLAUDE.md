...
| **사업자 유저** | 유저 + **사업자등록 → 판매 승인**. 자기 상품·공구권 판매 + 현금 정산 | `users` + 연결된 승인 `sellers` |
...
### 🏭 도매몰(유통스타트) 명칭 — **유통사 / 제조사** 로 무조건 통일 (2026-06-21 대표 확정 "앞으로도 마찬가지")

도매몰의 **모든 사용자-가시 라벨**은 아래 둘만 사용. 새 UI/문구/번역/약관/안내 작성 시 필수.

| 명칭 | 정의 | 비고 |
|---|---|---|
| **유통사** | 도매가로 사입해 재판매하는 B2B 회원(=구매자측) | `sellers`(is_distributor) |
| **제조사** | 도매몰에 상품을 공급하는 B2B 회원(=공급자측) | suppliers |

- ❌ **사용 금지(도매몰 사람/회원 지칭)**: "판매사", "공급사", "유통회원", "제조회원", "제조(브랜드)회원", "판매파트너", "사입 바이어", "셀러"(사람), "큐레이터". 괄호 병기("판매사(유통사)", "제조사(공급사)")도 금지 — **유통사 / 제조사 단독**.
- ✅ **유지 OK**: "공급가"/"공급가격"(가격 용어 — 회사 아님), "도매가", "공급하다/공급망"(동사·일반명사), "제조사 대시보드"·"유통사 대시보드"(도구 명칭), 오프라인 "매장"(가게 자체).
- 법적 문서(이용약관/공급계약서)에서 법률상 당사자 정의가 필요하면 "유통사(이하 …)" 식으로 1회 정의 후 유통사/제조사로 통일.
- **코드 식별자는 무관**(suppliers/sellers/is_distributor 등 내부 식별자 그대로). 이 규칙은 사용자-가시 문자열에만 적용.
...
- 🎯 **타겟 포지셔닝 (2026-06-18 대표)**: 사업자 유저의 메인 타겟 = **"자신의 쇼핑몰을 갖고 싶은 유저"**. 그들의 `/u/{handle}` 링크샵 = **본인 쇼핑몰**이고 **본인 상품이 주인공**. 공구권은 부가 채널(주인공 아님). → 사업자 유저 관련 신규 UI/문구/기본 강조는 "내 쇼핑몰" 언어·상품 우선으로(향후 구현 시 적용; 현재는 방향 메모만 — 코드 미변경).
...
>   - **역할 토큰(seller/agency/유통사/미래 역할)**: transfer 쿠키(`ur_pending_*`) **금지** — iOS 미영속.

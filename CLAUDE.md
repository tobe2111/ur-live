...
### 변경 audit log
- 2026-09-21 `[UNLOCK_LOADING]` `index.html` **로고 폰트 `display=swap` → `optional` (단어 하나)** (대표 신고 *"로딩 중에 urdeal 로고도 안보이고"* → *"셀러대시보드에서 로더가 문제 있는거야"* · `AskUserQuestion` 승인 **"단어 하나 교체"**). **배경**: 로더에 **브랜드 점 하나만** 뜨고 워드마크가 없었다. 결정적 단서는 **점이 정중앙이 아니라는 것** — 대표 스크린샷 폭 1915 의 중앙은 957 인데 점은 ~1026(**+69px**). 글자가 아예 없었다면 점이 가운데 왔을 테니, **글자가 자리는 차지하는데 안 칠해진** 상태다(오프라인 하네스로 같은 마크업 재현 시 오프셋 **+59px**). ⇒ 색이 아니라 **폰트**다. 🩸 **내 첫 가설은 틀렸다** — *"밝은 표면에 다크 워드마크(`forceLight` 미전달)"* 라고 보고했는데, `/seller/*` 는 `isDashboardLoaderSurface` 가 잡아 `DashboardLoader`(forceLight)로 가고 글자는 잉크색(`#16181C`)이라 대비가 충분하다. 🩸 **같이 보고한 "에이전시 로더 누락"도 틀렸다** — 에이전시는 2026-09-04 완전 일몰이라 `/agency/**` 라우트가 **없다**. 정규식에 없는 게 정상이고 고치지 않았다(CLAUDE.md 의 옛 문장 `/seller|/admin|/agency|/ads` 를 보고 오판). **수정**: `display=swap` → `optional`. `swap` 은 폰트를 기다리는 블록 구간 동안 글자를 **안 그린다**(그 구간이 하필 로더가 떠 있는 순간과 겹친다). `optional` 은 제때 안 오면 기다리지 않고 폴백으로 **바로 그린다** → 안 보이는 구간이 구조적으로 사라진다. **이 레포는 본문 폰트(Pretendard)에 이미 같은 처방을 쓰고 있다**(같은 파일 인라인 `font-display: optional`) — 두 폰트가 이제 같은 규칙이다. **대가**: 느린 망에서 로고가 Poppins 대신 폴백 글꼴로 잠깐 보일 수 있다(안 보이는 것보다 낫다는 판단 — 대표 확정). **⚠️ 잠긴 항목 전부 byte-불변**: preload `crossOrigin` 부재 · Speculation Rules 대상 · preconnect 목록 · Pretendard dynamic-subset 링크 · 인라인 critical CSS — 바뀐 것은 **쿼리 파라미터 한 단어**뿐이다. 가드: `seller-product-edit-save-top-2026-09-21.test.ts` 의 폰트 2건 + 주입 1건 **되돌려-검증 빨간불 확인**. ⚠️ **배포 후 판정**: 셀러 대시보드 콜드 진입에서 로더에 `urdeal` 글자가 보이는지. 롤백: `optional` → `swap`.

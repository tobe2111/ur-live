...
### 🎟️ 소비자 명칭 — **공구권 → 이용권** 으로 통일 (2026-06-27 대표 확정)

> **2026-06-27 변경**: 동네딜 공동구매 티켓의 사용자-가시 명칭을 **공구권 → 이용권**으로 일괄 변경(대표 "공구권이 아니라 이용권"). 코드베이스 전체(src+locales) 사용자-가시 "공구권" 155건 일괄 치환 완료. **코드 식별자(`group-buy`/`groupBuy`/`공구`/라우트 `/group-buy`)·활동명 "공동구매·동네딜"·"교환권"(기프티콘)·"식사권"은 불변** — "공구권"(티켓 명칭)만 "이용권"으로. 아래 이력 audit log 의 "공구권" 표기는 *작성 당시 historical record*(소급 변경 X).
> - ✅ **이용권** = 동네딜 공동구매로 구매해 매장에서 쓰는 티켓 (구 "공구권")
> - **교환권**(기프티콘/KT-Alpha) · **식사권**(식당) 은 별개 명칭 그대로 유지.

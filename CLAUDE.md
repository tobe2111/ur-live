...
## 🤖 AI 팀 운영 모델 — 역할 7(판정 6 + 운영 1) · 결재함 1 · 완료 판정 E1~E5 (2026-09-07 대표 확정, 모든 세션 준수)

대표: *"세계 최고의 전문가 담당자(경영·기획·마케팅·디자인·개발·정산) 팀을 빌딩해서 최대한 자동화 운영"* +
*"미완성·에러·문제가 곳곳에 숨어있었는데도 완성되었다고 판단하는 경우가 많았어."*

**SSOT = `docs/design/ai-team-operating-model.md`.** 세 가지가 파일로 고정돼 있다:
1. **역할** — `.claude/agents/{ceo-office,planning,marketing,design,dev,finance,ops}.md`. 세션이 어떤 역할로 일하는지는 이 파일의
   프론트매터(허용 도구·결정권·금지 레일·읽을 SSOT·보고 형식)가 정한다. 채팅에서 즉석으로 역할을 부여하지 않는다.
2. **결재함** — 대표가 판단할 것은 **`docs/decisions/<날짜>-<슬러그>.md` 로만** 올린다(필수 필드는 `_template.md`).
   handoff·PR·채팅에 "대표 판단 필요" 라고만 적고 끝내는 것 금지. 결정은 대표가 한 말 **그대로** 옮긴다.
3. **완료 판정** — "완료" 라는 말은 **증거 등급 E4(라이브에서 의도한 효과를 실측) 이상**에서만 쓴다.
   E1 작성됨 · E2 검증됨(tsc·vitest·가드·되돌려-검증) · E3 머지·배포됨 · **E4 라이브 판정됨** · E5 사용자 판정됨.
   **모든 보고 첫 줄에 `[E등급]`** 을 적는다. E3 에서 멈추면 "배포됐다"까지만 말한다. 머니 경로의 E4 는 staging 실결제다.

결정권은 세 등급뿐이다 — **A 자율 / B 세 줄 보고 후 진행 / C 결재(승인 전 실행 금지)**. 결제·정산·요율·환불·원장·커미션 ·
게이트 ON · 발행/발송 · 삭제/purge · 잠금표 파일 · 유료 전환은 **전부 C**. 애매하면 한 등급 위.
행위자·베네핏의 "지금 살아 있는 값"은 `docs/design/actor-benefit-map.md` 한 장(코드·`platform_settings` 실측이 진실).
가드: `src/tests/unit/ai-team-operating-model.test.ts` + 주입 매니페스트 2건.
...
### 변경 audit log
- 2026-09-19 `[UNLOCK]` `TossWidgetPayPage.tsx` **결제 화면에서 딜 사용액 조절(대표 확정 "C안")** (대표 *"나는 여기 페이지에서 딜 결제 내용 변경을 할 수 있으면 좋겠어 … 원래는 여기서 포인트 사용을 하듯이 원래 하잖아 다른 사이트들도"* → 시안 3안 중 **C안** 확정 · `AskUserQuestion` 승인 **"허가 — 최소 범위로 진행"**). **배경**: 딜 조절이 **이용권 상세**에만 있어서, 결제 화면은 결과만 읽어 주고 고칠 수가 없었다(다른 커머스는 전부 결제 단계에서 포인트를 쓴다). **수정(잠금 파일 접촉 = import 1 + 상태/파생 3줄 + effect 1개 + 렌더 1블록 + 라벨 3곳)**: ① `goodsAmount = amount + summary.dealUsed`(URL 값, 고정) · `chargeAmount = goodsAmount − dealUsed` ② 딜이 바뀌면 **별도 effect** 가 `setAmount` 재호출 — `state === 'ready'` 가드로 **초기화 effect 와 겹치지 않는다**(그 줄은 byte-불변) ③ 신규 `pages/pay/DealUseCard.tsx`(독립 카드 — 입력 + 전액 사용). **⚠️ 잠긴 계약 전부 byte-불변**: `requestPayment`·`widgets()`·`customerKey`·`successUrl/failUrl`+`safePaymentReturnPath`·`USER_CANCEL` 분기·SDK 마운트 계약(id 2개 + `hidden`)·`STEP_TIMEOUT_MS 4000`·키 분기·초기화 `setAmount` 줄 — grep 카운트로 확인(전부 동일). `setAmount` 호출만 1 → 2. **🔒 서버 무수정**: 청구액이 바뀌어도 서버는 이미 `derivePartialDeal` 로 **청구액에서 딜을 역산**해 게이트·최소카드액(`MIN_CARD_AMOUNT` 100)·잔액을 재검증하고 실제 차감은 원자 CAS 다 — 위조해도 남의 딜을 쓸 수 없다(자기 잔액 안에서만 움직인다). **전부-딜은 이 화면의 일이 아니다**(0원 결제는 PG 거절 — `/join` payment_method='deal' 이 따로 처리) ⇒ 상한이 `총액 − 100` 이라 productId·별도 엔드포인트가 필요 없어 변경이 최소로 끝났다. **🔒 화면이 금액을 지어내지 않는다**: 상한은 서버 `deal-plan` 의 `max_deal_usable` 이 쿼리(`dealMax`, 표시용 파라미터와 같은 클래스)로 실려 온 값이고, **이 화면은 잔액을 조회하지 않는다**(테스트가 고정 — 2026-09-04 에 값을 치르고 배운 규칙). `MIN_CARD_AMOUNT` 는 `shared/pay-summary.ts` 로 **SSOT 단일화**(서버가 재수출 — 두 벌이면 화면이 허용한 금액을 서버가 거절하는 날이 온다). **🕳️ 기존 가드 둘이 내 변경을 잡았고, 둘 다 풀지 않고 재조준했다**: ① *"딜 사용 블록의 렌더 조건"* — `summary.dealUsed`(URL 초기값)를 앵커했는데 그건 **이 화면에서 새로 고른 딜을 못 본다**. 0 에서 시작해 여기서 딜을 고른 사람에게 그 줄이 안 뜨는 **실제 결함**이었다 → 조건을 `goodsAmount > chargeAmount` 로 고치고 `displayGoodsAmount` 제거. ② *"요약 값이 금액 판단에 새어들지 않는다"* — 그 규약(`display*` 이름)의 전제는 *"요약은 표시 전용"* 이었고 C안이 그 전제를 바꿨다 → 이름 규약 대신 **지키려던 것**(화면이 잔액을 조회하지 않는다 · `setAmount` **모든 줄**에 요약 값 금지 · 청구액 재대입 금지)으로 재작성. 🩸 **그 과정에서 가드 자신의 눈먼 자리도 둘 드러났다**: ⓐ `setAmount` 를 **첫 줄 하나만** 검사하고 있었다(호출이 둘이 되면 새 줄이 샌다) → 전부 검사 ⓑ 요약 파생값 탐지 정규식이 산술 기호가 `summary.x` **뒤**에 오는 모양만 봐서, `amount + (summary.dealUsed ?? 0)` 에 매치 0 이 됐다 — 그 검사 자신의 *"0건이면 헛도는 가드"* 단언이 잡았고, 합성 누수(`summary.origAmount * 2`)로 **되돌려-검증**해 빨간불 확인. 가드: `pay-deal-use-card-2026-09-19.test.tsx` 20건 + 주입 5건 **되돌려-검증 전부 빨간불 확인**. 검증: tsc 0 · 결제 관련 유닛 pass · theme/design-slop/light-input/modal-zindex/mobile-viewport GREEN. ⚠️ **staging 실결제 1회 필수**(딜 조절 → 카드 청구액이 바뀐 채 승인까지 — `STAGING_CHECKLIST.md` S13). 롤백: `<DealUseCard>` 렌더 블록 + setAmount 재호출 effect + 상태 3줄 제거(그때 위 가드가 빨간불이 되므로 함께 판단할 것).
- 2026-09-13 `[UNLOCK]` `TossWidgetPayPage.tsx` **결제 콜백 주소의 쿼리 보존 — `safeInternalPath` → `safePaymentReturnPath`** (대표 신고 *"토스페이먼츠 pg로 결제 중인데 안되고"* · `AskUserQuestion` 승인 "잠금 해제 · 근본 수정"). **배경(재현 확인)**: 성공 주소 `/group-buy/confirm-payment?productId=2888&qty=1` 을 `safeInternalPath()` 에 통과시켰는데 **그 함수는 쿼리를 통째로 지운다** — 그 삭제는 2026-05-01 에 *카카오 returnUrl* 의 `?error=` 누적을 막으려 넣은 **OAuth 규칙**이지 결제 콜백의 규칙이 아니었다. 토스는 `paymentKey·orderId·amount` 만 붙여 돌려주므로 "어느 상품·몇 개·어느 주문" 은 **우리가 실어 보낸 쿼리로만** 돌아온다 → `productId` 가 0 이 되어 `!productId` 분기가 **"결제 정보가 올바르지 않습니다"** 를 띄웠다(대표 화면의 `결제 정보: GB-3-1788967989484` 가 그것). **에러 로그도 실패 알림도 없이 마지막 화면만 틀려서** 아무도 신고하지 않았고, 라이브 `orders` 마지막 행이 2026-06-26 인 것이 그 방증이다. **한 원인에 세 흐름이 깨져 있었다**: 이용권 카드결제(`?productId=&qty=`) · 숙소 예약(`?order_id=` — `STAY-{id}` 역산 폴백에 의존) · 알림톡 충전(`?charge=success&orderId=`). 딜 충전(`/points/charge/success`, 쿼리 없음)만 무사했다. **수정(식별자 2개)**: 신규 SSOT `safePaymentReturnPath`(`utils/safe-internal-path.ts` — 잠금 아님)로 교체. **오픈 리다이렉트 방어는 한 글자도 안 약해진다** — 경로 판정은 `isSafeInternalPath` **같은 함수** 그대로이고(외부 URL·`//`·역슬래시·제어문자·인증 경로 차단), 달라진 건 통과 뒤 **쿼리를 붙여 돌려주는 것** 하나다. 쿼리도 그대로 믿지 않는다: `#` 이하 폐기 · `URLSearchParams` 왕복 재인코딩 · 길이 상한 1024 · 역슬래시/제어문자는 **쪼개기 전에** 통째 차단. 🔒 **금액 위조 자리는 안 생긴다** — 청구액의 진실은 `/confirm` 의 서버 재검증이고 쿼리는 "어느 주문을 확인할지" 고르는 손잡이일 뿐이다. **⚠️ 잠긴 계약 전부 byte-불변**: `requestPayment`·`widgets()`·`setAmount`·금액 검증·키 분기·상태 전이·SDK 마운트 계약(id 2개 + `hidden`)·요약 카드 — diff 는 **import 1줄 + 호출 2줄 + 주석**뿐. 가드: `payment-return-path-2026-09-13.test.ts` 23건(쿼리 보존 5 + 오픈 리다이렉트 12 + 배선 3 …) + 주입 5건 **되돌려-검증 빨간불 확인**. 🩸 **주입이 내 시험 둘을 헛돈다고 잡았다**: ① 공격 목록이 전부 *경로*에 나쁜 문자를 넣어서, 쪼개기 전 사전 차단을 통째로 지워도 초록이었다(그 방어가 실제로 일하는 자리는 **쿼리 쪽**) ② 조각 검사를 `not.toContain('#')` 로 썼는데 재인코딩이 `%23` 으로 바꿔 늘 통과했다 → **모양이 아니라 값**(`q.get('productId') === '5'`)을 보도록 교체. 검증: tsc 0 · 관련 66건 pass · build 0. ⚠️ **staging 실결제 1회 필수**(카드 1건 — 승인 후 confirm 까지 도달하는지). 롤백: 호출 2곳을 `safeInternalPath` 로 환원(그때 위 가드가 빨간불이 되므로 함께 판단할 것).
- 2026-09-04 `[UNLOCK]` `TossWidgetPayPage.tsx` **부분결제 내역 표시(딜 사용 · 상품 금액 · 카드 결제)** (대표 *"최대한 가장 이상적으로 해줘"* — 같은 날 *"머지하고, 너가 직접 켜줘"* 의 마무리). **배경**: #1296 머지로 이용권을 **딜 일부 + 카드**로 살 수 있게 됐는데, 10,000원짜리를 누르면 결제창에 **8,000원**이 뜨고 화면이 그 이유를 한 마디도 안 했다. **수정(표시 2줄 + 라벨 삼항)**: `summary.dealUsed` 가 있을 때만 `상품 금액 / 딜 사용(−N딜, brand-text)` 을 얹고, 아래 큰 숫자의 라벨을 `결제 금액` → `카드 결제` 로. 값은 **서버 `/join` 이 계산해 돌려준 것**만 쓴다 — 화면이 잔액으로 추정하면 게이트가 꺼진 순간 그 안내가 거짓말이 된다(주입 매니페스트가 그 유혹을 고정). 파생 숫자는 `displayGoodsAmount` 로 **이름에 표시-전용임을 못 박았다**. **⚠️ 잠긴 계약 전부 byte-불변**: `requestPayment`·`widgets()`·`setAmount`·키 분기·상태 전이·`safeInternalPath`·SDK 마운트 계약(id 2개 + `hidden`) — 바뀐 것은 요약 카드 마크업뿐이고 **청구액 `amount` 는 어디서도 재대입되지 않는다**(테스트가 고정). 🕳️ **기존 가드가 내 변경을 잡았다**: *"요약 값이 금액 계산에 새어들지 않는다"* 가 표시용 덧셈까지 빨간불을 냈다 — 가드를 푸는 대신 **규약으로 재조준**(요약 파생값은 `display*` 로 이름 짓는다). 🩸 그리고 **내가 만든 가드 하나가 헛돌았다**: `toContain('summary.dealUsed')` 는 그 이름이 파일 곳곳에 있어 블록을 통째로 꺼도 통과했다 — 되돌려-검증이 잡아 렌더 조건(`{summary.dealUsed ? (`)으로 앵커 교체. 검증: tsc 0 · vitest 616파일 7,658건 pass · 주입 3건 되돌려-검증 빨간불 확인. ⚠️ **staging 실결제 1회 권장**(부분결제 켠 뒤 딜+카드 1건). 롤백: 요약 블록 + 라벨 삼항 제거(딜 0 이면 원래 안 뜨므로 게이트 OFF 면 현행과 byte-동일).
- 2026-09-03 `[UNLOCK]` `TossWidgetPayPage.tsx` **결제 화면 chrome 전면 — 안 2-D(요약 카드 + 다크 + 브랜드 CTA)** (대표 `AskUserQuestion` 승인 "CTA 색 + 우리 chrome 전체" → 시안 확정 "안 2" → "안 2-d 진행"). **배경**: 대표 *"결제 상품 카드 부분에 이용권 이미지랑 등등으로도 나오게 할 수는 없나? 지금은 밋밋하잖아."* 실측하니 **밋밋한 게 디자인이 아니라 데이터였다** — 이 페이지는 쿼리로 `orderId·amount·orderName·clientKey` **넷만** 받아서 사진·매장명·정가가 **화면에 도착조차 안 했다.** 그릴 게 문자열 한 줄과 숫자 한 개뿐. **수정(우리 chrome 만)**: ① 신규 SSOT `shared/pay-summary.ts` — `image·merchant·origAmount·qty` 를 **표시 전용**으로 싣고 읽는다(호출부가 이미 화면에 띄우고 있는 값 → **새 fetch 0 · 서버 변경 0 · 지연 0**). 호출부 배선 2곳(`GroupBuyDetailPage`·`checkout/StayCheckout`). ② 요약 카드를 [76px 사진 + 매장·상품·할인] / [결제 금액 27px] 로 재구성(종전 20px·상품명 아래 → 결제 화면에서 사람이 마지막에 확인하는 건 *얼마인가* 하나다). ③ 다크 대응 신설 — 이 파일은 `dark:` 가 **0개**였다. ④ CTA `bg-gray-800` → `bg-brand`. ⑤ 에러 상자 → `tone-bad`. **🏝️ 토스 위젯 자리는 `light-island`**: 위젯은 토스가 흰색으로 그리고 우리가 넘기는 건 `selector`+`variantKey` 뿐이라 **테마를 못 바꾼다(실측)** → 다크에서도 흰 섬으로 남기되 **의도임을 클래스로 못 박는다**(안쪽 `dark:` 차단 + 전역 `.dark input`(0,5,1)이 위젯 이메일 칸을 흰 글자로 덮는 것 차단 — 09-03 지도 검색창이 정확히 그 사고였다). **⚠️ 결제 로직 전부 byte-불변**: `requestPayment`·`widgets()`·`setAmount`·금액 검증·키 분기·상태 전이·successUrl/failUrl `safeInternalPath`·**SDK 마운트 계약(id `toss-widget-pay-method`/`toss-widget-pay-agreement` + `hidden={state==='error'}`)** — 바뀐 것은 className 과 요약 마크업뿐. **🔒 요약은 표시 전용**: 쿼리는 위조 가능하나 실제 청구액은 `/confirm` 이 `serverTotal !== parsedAmount` 로 재검증하고, `orderName` 은 이미 오늘도 같은 성질의 값이라 새 위험 0. 사진은 `cfImage`(호스트 화이트리스트) + `onError` 폴백 경유, `http(s)` 스킴만 수용. **🕳️ 함께 드러난 가드 사각지대**: `check-theme-consistency` 는 `dark:` 가 **한 개도 없는 파일을 통째로 건너뛴다**("순수 다크인지 강제 화이트인지 모호 → 오탐 방지") — 그래서 결제 화면이 다크 지원 0인 채로 계속 초록불이었다. 이번에 `dark:` 가 생겨 그 구멍도 닫혔고, 테스트가 `dark:` 개수 하한을 지킨다. 가드: `pay-screen-summary.test.ts` 11건 + 주입 3건(되돌려-검증 빨간불 확인). 검증: tsc 0 · 테마/대시보드테마/design-slop/status-tone GREEN. ⚠️ **staging 실결제 1회 권장** — 위젯이 정상 마운트되고 결제가 끝까지 가는지(마크업만 바뀌었지만 마운트 자리를 건드린 커밋이라). 롤백: 요약 섹션·`dark:`·`light-island`·CTA 색 환원(호출부는 파라미터를 안 읽으면 자동 무시).
- 2026-09-01 `[UNLOCK]` `TossWidgetPayPage.tsx` + `TossPaymentWidget.tsx` **결제 실패 상태의 빈 위젯 상자 숨김** (대표 `AskUserQuestion` 승인 "허가 — 상자만 숨김"). **배경**: SDK 로드가 실패하면 Toss 가 그릴 자리(`#toss-widget-pay-method` 180px · `#toss-widget-pay-agreement` 60px / `#toss-payment-method` · `#toss-agreement`)가 **빈 테두리 상자**로 남아 비활성 회색 버튼 위에 떠 있었다(하네스 실측 — 이용권 결제의 유일한 화면 `/pay/widget` 이 그 얼굴이었다). **수정**: 두 컨테이너에 `hidden={state === 'error'}` / `hidden={loadingState === 'error'}` 만 추가 — id·순서·loading/ready 렌더는 그대로라 SDK 마운트 계약 불변. **⚠️ requestPayment/widgets()/금액 검증/키 분기/상태 전이 전부 byte-불변 — 속성 1개씩.** 롤백: `hidden` 2×2 제거.
- 2026-09-01 `[UNLOCK]` `TossWidgetPayPage.tsx` + `TossPaymentWidget.tsx` **소비자 오류 문구만 교체** (대표 `AskUserQuestion` 승인 "허가 — 문구만 수정", #1294 머지). ① 화면에 붙던 `[SDK 원본]: <영문 스택 조각>` 제거(원인 값은 `console.error` 가 그대로 남긴다) ② *"Toss 콘솔 → 결제 → 결제수단 → 활성화"* 는 **운영자용 지시**라 소비자가 할 수 있는 행동으로 ③ `payment.errors.systemError` "결제 시스템 오류 (새로고침 필요)" → "지금은 결제할 수 없어요"(그 버튼은 error 에서 비활성이라 행동을 약속하면 안 된다) · `payment.initError` → 소비자 문장, **6개 언어**(코드 `defaultValue` 만 고치면 locale 값이 이겨 소용없다). **⚠️ 분기 조건(정규식)·requestPayment·widgets()·금액 검증·키 분기·상태 전이 전부 byte-불변 — 문자열만.** 롤백: 문자열 환원.
- 2026-09-01 `[UNLOCK_LOADING]` `VouchersPage.tsx` **교환권 탭 상단 두 층 정리 — 잔액 슬래브 조건화 · 브랜드 스트립 기본 접기** (대표 시안 3안 중 **"나안"** 승인). **배경(미리보기 하네스 430px 실측)**: 첫 상품 위에 층이 다섯이었다 — 잔액 슬래브 250 + 고아 링크 40 + 카테고리 칩 60 + 브랜드 스트립 175 + 섹션 헤더 60 ≈ **700px**, 상품이 1.5개. 그리고 **비로그인 방문자는 `dealBalance`가 0**(l.304)이라 첫 진입이 "당신은 0" 이라고 알리는 검정 상자로 시작했다. **수정**: ① 잔액이 있으면 슬래브 유지(설명 1겹), **0 이면 한 줄 바(44px)** ② 브랜드 스트립을 기본 접고 `브랜드로 찾기 ⌄` 로 펼침 — **없애지 않는다**(2026-05-19 대표 요청 "카테고리 + 브랜드 2단 구조"). 딥링크로 브랜드가 잡혀 있으면 펴서 시작(접힌 채 선택 상태면 왜 걸러졌는지 알 수 없다). 상품 1.5개 → 4.5개. **⚠️ 잠긴 항목 byte-불변**: `__SSR_INITIAL_VOUCHERS__` 즉시 소비 · 기본 정렬 `price_low` · VoucherCard/VoucherRow 이미지 속성(width/height/srcSet/lazy/dominant_color) · 카테고리/브랜드 선택 동작 — 바뀐 것은 **잔액 블록 마크업 + 스트립 접기 게이트**뿐. **⚠️ PC 레이아웃은 무접촉**(같은 파일 안에 별도 마크업이 있고, 세로 여유가 달라 같은 처방이 맞는지 별도 판단). 가드: `vouchers-top-chrome.test.ts` 4건 + 주입 매니페스트 2건 — **되돌려-검증 2종 빨간불 확인**. 검증: tsc 0 · theme/design-slop/image-fallback GREEN · 파일 981줄(동결 987 이내). 롤백: 잔액 삼항을 슬래브 단일로 환원 + `brandsOpen` 게이트 제거.
- 2026-09-01 `[UNLOCK]` `PaymentSuccessPage.tsx` **이모지 6곳 → lucide (시각만)** (대표 `AskUserQuestion` 승인 "허가 — 시각만 변경"). **배경**: 대표 *"결제 페이지도 다 했어? 유어딜의 세부페이지들 말이야"* 로 구매 흐름을 재 보니 이모지 45곳이 남아 있었고(그중 이 파일 6곳), 더 나쁜 것은 **`check-design-slop` 이 그걸 하나도 못 보고 0 을 보고하고 있었다** — 그 가드의 `UI_SLOT` 이 `icon:`/`emoji:` 필드·`label:`·`<h1~6|button>` 만 봐서 `<span className="text-2xl">📅</span>` 형태를 통째로 놓쳤다. **수정(글리프만)**: `📅`→`CalendarClock` · `📄 영수증 보기`→`Receipt` + 텍스트 · `🧾 현금영수증`→`ReceiptText` · `⚠️`/`🎭`/`💬` 는 문장 앞 장식이라 제거(문구 불변). **⚠️ Toss 잠금 계약 전부 byte-불변**: `confirmPayment` 호출·금액검증(`serverTotal!==parsedAmount`)·`TossPaymentObject` 표시(receipt.url·cashReceipt·easyPay·card·approvedAt)·`pendingBookings` 조회·autoReturn·셀러 전환 넛지 — grep 카운트로 확인(3/3/2/6/5/4/7/2). 검증: tsc 0 · design-slop 되돌려-검증 빨간불 확인. 롤백: 글리프 6곳 환원(결제 동작 무관).
- 2026-09-16 `[UNLOCK]` `PaymentSuccessPage.tsx` **몰 손님용 간판 + 가장 큰 버튼의 목적지** (대표 `AskUserQuestion` 승인 "허가 — 배너 + 버튼 둘 다"). ⚠️ **원래 2026-08-12 에 PR #1149 로 승인받았던 변경을 조각내 되살린 것**이고, 그때 본문은 *"렌더 1줄 + import 1줄"* 이었으나 **오늘 재승인 범위는 버튼 교체를 포함**한다(그 PR 자신이 뒤에 덧붙인 부분 — 간판만으론 부족하다는 실측). **배경**: 🏪 공구 서비스(운영자 몰) 분리가 **장바구니 앞까지**만 돼 있었다. 손님이 담는 순간부터 결제·완료·주문내역이 전부 유어딜 화면이라, 운영자가 카톡으로 데려온 손님이 결제 순간 남의 간판 아래로 넘어간다. 게다가 이 화면의 셀러 전환 넛지(*"내 쇼핑몰에서도 팔 수 있어요"*)가 **몰 손님에게도** 떠서 유어딜이 운영자 손님에게 셀러 전환을 권하고 있었다 — 브랜딩이 아니라 **고객 가로채기**. **수정(잠금 파일 접촉 = import 2줄 + 렌더 1줄 + 버튼 1개 교체)**: ① 셀러 전환 넛지 다음에 `<MallOriginBanner className="mt-4 sm:mt-5" />`(신규 `components/mall/MallOriginBanner.tsx`) ② `쇼핑 계속하기` 버튼을 `<ContinueShoppingLink onFallback={() => navigate('/')} />` 로 — **흔적이 없으면 종전과 동일하게 `/` 로** 간다(본진 손님 화면 불변). **🔴 잠금 파일에서 몰을 판정하지 않는다**(`readMallOrigin`/`isFromMallSession`/`mall_id` 참조 0 — 테스트가 고정). 판정을 여기 심으면 다음 세션이 잠금 파일에 조건을 계속 얹게 되고 승인 절차가 형해화된다. 조회 실패도 `null`(잘못된 가게 이름이 결제 화면에 뜨는 것이 아무것도 없는 것보다 나쁘다). **⚠️ Toss confirm/금액검증(`serverTotal!==parsedAmount`)/`TossPaymentObject` 표시(receipt.url·cashReceipt·easyPay·card·approvedAt)/pendingBookings/autoReturn/셀러 전환 넛지 렌더 조건 전부 byte-불변.** 동반: 넛지가 몰 세션이면 스스로 미노출 · `CartPage` "쇼핑 계속하기" → `ContinueShoppingLink` · `CheckoutPage` 동일 배너 · 주문 내역에 가게 표시(서버 `stampOrdersMall` 이 찍음). **같은 커밋에서 잠금표의 `src/shared/types/index.ts` 도 닿는다 — `Order` 인터페이스에 선택 필드 `mall_slug?`/`mall_name?` 2줄 추가뿐이고 그 파일이 잠긴 이유인 `TossEventType`/`TossWebhookPayload` 는 byte-불변**(본진 주문엔 이 필드가 아예 없어 `undefined` → 종전 표시 그대로). ⚠️ **staging 결제 성공 1회 권장**(몰 흔적 없는 본진 결제에서 화면이 이전과 동일한지 — 배너가 **안 떠야** 정상). 롤백: import 2줄 + 배너 1줄 제거 + 버튼을 종전 `<Button onClick={() => navigate('/')}>쇼핑 계속하기</Button>` 로 환원.
...
### 변경 audit log
- 2026-09-21 `[UNLOCK_LOADING]` `index.html` **로고 폰트 `swap` → `optional` 로 바꿨다가 같은 날 되돌림 — 내 진단이 측정으로 뒤집혔다** (대표 신고 *"로딩 중에 urdeal 로고도 안보이고"* → *"셀러대시보드에서 로더가 문제 있는거야"*). **내가 한 주장**: 로더에 브랜드 점만 뜨고 워드마크가 없는 건 `swap` 의 블록 구간 동안 글자가 투명해지기 때문이고, 대표 스크린샷에서 점이 정중앙이 아니라 +69px 인 것이 "글자가 자리는 차지하는데 안 칠해졌다"는 증거다 — 그래서 `display=swap` → `optional` 한 단어를 바꿔 배포했다(#1527, 머지 `25d3866`). 🩸 **배포 후 판정하려고 재 봤더니 그 전제가 통째로 틀렸다**: 폰트를 영영 안 오게 막아 두고 `20·50·80·110·150·250·500ms` 에서 워드마크 박스의 **칠해진 픽셀**을 세니 `swap`·`optional` **모든 시점에서 1,626px(38.3%)로 완전히 동일**했다. `swap` 의 블록 구간은 명세상 "extremely small"(Chrome 실측 ~0)이라 폴백 글자가 **즉시** 그려진다 — 애초에 그게 `swap` 의 존재 이유(anti-FOIT)다. ⇒ **폰트는 그 증상의 원인이 아니다.** **되돌린 이유**: 근거가 없어졌고, `optional` 은 폰트가 늦으면 **그 로드에서는 아예 안 쓴다**(다음 로드용으로만 받는다) — 로더는 1초쯤 떠 있으므로 첫 방문자가 로고를 Poppins 로 거의 못 보게 된다(얻는 것 없이 잃는다). ⇒ 잠금 파일은 승인된 상태(`swap`)로 환원. **❌ 원인은 아직 못 찾았다.** 코드로 배제한 후보 넷(전부 성립 안 함, 다시 파지 말 것): ① App.tsx Suspense 폴백 — `/seller|admin|ads` 는 `DashboardLoader`(forceLight)라 글자가 잉크색(#16181C) on #F4F5F7 ② `seller-light-theme` 가 흰 글자를 만든다? — `tailwind.config` 의 darkMode 변형이 `:not(.seller-light-theme *)` 로 **이미 `dark:` 를 차단**한다 ③ 워커 `#root` — 대시보드는 `needsRootBlank` 라 `#F4F5F7` 빈 오버레이뿐이고 **워드마크 자체가 없다** ④ `BrandLoader` 소비자 기본값 — `fsBg`(`bg-white dark:bg-[#11141C]`)와 글자색이 **같은 방향**이라 대비가 안 깨진다. **다음 세션의 첫 액션**: 대표에게 (a) 정확히 어느 화면·URL 인지 (b) 그때 테마가 다크였는지 (c) 가능하면 새 스크린샷을 받는다 — 이 컨테이너 브라우저는 라이브 자산을 못 받아(청크 자가복구가 `chrome-error://` 로 튄다) 프레임 캡처로 재현할 수 없다. 가드: `seller-product-edit-save-top-2026-09-21.test.ts` 가 **`swap` 을 고정**하고 위 측정표를 파일에 박아 뒀다(주입 1건). 🧭 **교훈: "그럴듯한 기제"를 증거로 쓰지 말 것.** 점의 +69px 오프셋은 "글자가 자리를 차지한다"만 말해 주지 "칠해졌는가"는 **전혀 구분하지 못한다**(투명해도 자리는 그대로다). 두 가설을 가르는 측정을 먼저 설계했어야 했다.
- 2026-09-16 `[UNLOCK_LOADING]` `detail-ssr-body.ts` + `GroupBuyDetailPage.tsx` + `worker/index.ts` **서버 첫 화면 경계를 히어로 → 제목까지** (대표 *"남은 후속들 모두 진행 끝까지"*). **배경**: 09-15 에 히어로에서 멈춘 이유는 제목 **위**에 per-user 블록이 둘 있어서였다. 이번에 그 둘을 치웠다 — ① `ShareRewardBanner`(딜 보유자에게만)를 **가격 아래로 이동**. 이 배너는 서버 조회가 끝나야 뜨는데(첫 렌더는 항상 `null`) 제목 위에 있어서 **딜 가진 사람에게만 뒤늦게 제목·가격이 밀렸다** — 서버 렌더와 무관하게 **오늘도 나던 밀림**이라 이건 결함 수정이다(빈 칸은 `empty:hidden` 으로 접어 안 쓰는 사람에겐 16px 도 안 남는다. PC 가시성 불변). ② `?ref=` 추천 진입 배너는 **URL 로 결정**되므로 그 파라미터가 붙은 요청에서만 제목을 접는다(`search` 인자 추가) = 09-15 동작 그대로라 무회귀. **수정**: `titleHtml`(신규, 매장명 + 온누리 뱃지 + 오픈예정 뱃지 + h1) — React inline style 을 그대로 CSS 문자열로 옮겼고, 감싸는 노드에 `class="gbd"` + 페이지 루트와 같은 배경·글자색을 준다(`--gbd-*` 는 `.gbd` 안에서만 풀리고, 없으면 바탕이 `--bg`→`--surface` 로 **마운트 때 바뀐다**). **❌ 주소·가격은 여전히 안 그린다** — 제목 바로 아래 주소 줄의 `· 1.2km` 가 **내 위치 기준**이라 서버가 모르고, 거리 없이 그리면 그 줄이 한 줄→두 줄로 되감겨 **바로 아래 가격이 밀린다.** 경계는 "서버가 확실히 아는 마지막 것" = h1 이다(서버가 **안 그린** 것이 나중에 채워지는 건 밀림이 아니다). **❌ PC(lg+)는 이번에 재고해서 안 하기로 했다** — ⓐ PC 첫 화면은 **2열 그리드**(`lg:grid-cols-[minmax(0,1fr)_360px]`)라 반쪽만 그리면 마운트 때 **가로로 줄어든다**(세로 밀림을 없애려다 가로 밀림을 만드는 셈) ⓑ 그리드 밖 유일 블록 `DetailTitleHeader` 에 `StarRating`·lucide `MapPin` **SVG** 가 있어 path 를 손으로 옮기면 두 벌이 갈리고 기계 검사가 없다 ⓒ 전제(V8 파싱 1.2초)가 폰 CPU 현상이다. ⚠️ PC 상세 비중은 **실측 못 했다** — CF 토큰은 읽기 4종뿐이라 Analytics Read 가 없고(CLAUDE.md 가 추가 스코프 요청을 금지), 앱 계측에도 기기 차원이 없다. 높은 것으로 드러나면 ⓐⓑ를 푸는 별건으로 다시 올릴 것. 가드: `detail-ssr-first-screen-2026-09-15.test.ts` ③~③-3 재작성(h1 inline style 을 **페이지 소스에서 파싱해 대조** · 배너가 제목 아래인가 · `?ref=` 게이트 · `.gbd`) + 주입 6건 **되돌려-검증 빨간불 확인**. 파일: 상세 905줄(동결 유지 — 주석 압축으로 상쇄). 검증: tsc 0 · build 0 · 상세 관련 19파일 329건 pass. ⚠️ **배포 후 판정**: `/group-buy/:id` 하드로드 프레임에서 제목이 사진과 함께 첫 프레임에 뜨고 **마운트 때 안 움직이는지**. **✅ E4 판정 통과(배포 후 라이브 2회, iPhone 13 · `urdeal.kr/group-buy/2888`)**: `사진과 제목이 **같은 프레임**`(2,792ms / 608ms) · `제목 박스 1종 [18,385,354,27]` · `히어로 박스 1종` · `URL 1종` · `풀스크린 로더 0회` · 둘 다 React(4,097ms / 1,357ms) 보다 먼저. 10초 동안 **상태 변화가 셋뿐**(없음 → 사진+제목 → 마운트) — 제목이 마운트 때 한 픽셀도 안 움직인다. ⚠️ 두 회차의 ms 차이는 프록시 릴레이 편차이지 성능 변화가 아니다(판정은 순서·개수). 롤백: `titleHtml(d)` 호출을 `''` 로(그때 위 가드가 빨간불이 되므로 함께 판단할 것).
- 2026-09-16 `[UNLOCK_LOADING]` `vite.config.ts` manualChunks **아이콘 다이어트 — 첫 페인트가 안 쓰는 아이콘 214개를 같이 받던 것** (같은 지시, 후속 ④ JS 다이어트). **배경(번들러 모듈→청크 그래프 실측)**: 트리쉐이킹은 멀쩡했다 — lucide 1,534개 모듈 중 **260개만** 남는다. 문제는 그 260개가 `manualChunks` 의 **한 봉투**(64.8KB)라, 어드민·셀러 페이지에서만 쓰는 아이콘까지 소비자 첫 화면이 통째로 받고 있었다는 것이다(셸 폐쇄가 닿는 건 **43개**뿐). **⚠️ 규칙을 지우면 될 것 같지만 아니다** — 지워 보니 Rollup 이 256개를 통째로 `app-shell` 에 넣어 총량이 그대로였다(695.0 → 694.0KB). **명시 분할만** 효과가 있다. **수정 2곳**: ① `lucide` → `lucide-shell`(셸이 쓰는 아이콘 + 코어/배럴) / `lucide`(나머지) ② `auth/` 버킷을 **`RouteGuards` 로 좁힘** — 폴더 통째로 묶여 있어서, 셸이 쓰는 `RouteGuards` 때문에 같은 폴더의 `KakaoLinkButton`·`SellerPinPrompt`(셀러 프로필 편집 **한 페이지만** 쓴다)가 13.7KB + 아이콘 6개를 달고 첫 페인트에 실려 왔다. **실측**: 첫 페인트 폐쇄 **695.0 → 633.1KB raw · 211.8 → 193.1KB gzip(−8.8%)**, 청크 수 17 불변. 🩸 **첫 판이 헛돌았다**: 아이콘 이름을 kebab 으로 넣었는데 `AlertCircle`·`Home`·`CheckCircle2` 는 lucide 에서 **별칭**이고 구현은 `circle-alert`·`house`·`circle-check` 에 있다 — 별칭만 넣으니 구현이 큰 봉투에 남아 셸이 그 봉투를 다시 끌고 왔다(**빌드는 성공하고 총량만 안 준다**). 가드가 별칭 체인을 따라가도록 고쳤다. 가드: `lucide-shell-icons.test.ts` 7건(소스에서 셸 폐쇄를 **다시 계산**해 목록과 대조 · 별칭 체인 · 배선 · "0개면 통과가 아니라 고장") + 주입 4건 **되돌려-검증 빨간불 확인**. `critical-chunks` 는 `lucide`→`lucide-shell` 로 **리베이스라인**(`_measured` 에 사유 기재). ⚠️ **남은 것(실측·미착수)**: `i18n` 20.1 · `axios` 17.3 · `app-routes` 8.3 KB gzip. 앞 둘은 첫 페인트에 실제로 쓰이고, `app-routes`(admin/seller/supplier 라우트 표)는 `<Routes>` 직계 자식이라 중첩 라우트로 바꿔야 해 **별건**(라우트 가드·리다이렉트 파급). 롤백: 규칙 2곳 환원.
- 2026-09-16 `[UNLOCK_LOADING]` `VouchersPage.tsx` + `vouchers/DealBalanceCard.tsx` **잔액이 늦게 와서 목록 전체가 한 번 밀리던 것** (같은 지시, 후속 ⑤ per-user chrome). **배경**: 잔액은 마운트 뒤 `/api/points/balance` 로 온다 → 첫 커밋은 **누구든 `null`** 이고 부품은 그걸 44px 한 줄 바로 그렸다. 응답이 오면 170px 카드로 바뀌며 **아래 이용권 목록 전체가 내려갔다.** 딜을 가진 사람일수록 매번 겪는다. **수정**: 로그인 여부는 **동기로 알 수 있다**(`getUserIdSync`) → 로그인이면 **숫자만 비운 같은 카드**를 먼저 그리고(높이 동일 → 밀림 0), 비로그인이면 종전 한 줄 바. ⚠️ 빈 자리에 **0 을 적지 않는다** — 모르는 것과 0 은 다르고, 잠깐 0 을 보여 주면 딜 보유자에게 "잔액 없음" 이라고 말하는 셈이다. 조회 실패는 `setDealBalance(b => b ?? 0)` 로 빠져나온다 — 2026-06-26 룰(**읽은 값은 0 으로 안 덮는다**)을 지키면서 **한 번도 못 읽은 경우**에만 종전 실패 동작(한 줄 바)으로 떨어진다(안 그러면 숫자 없는 빈 카드가 영원히 남는다). **⚠️ 잠긴 항목 전부 byte-불변**: `__SSR_INITIAL_VOUCHERS__` 즉시 소비 · 기본 정렬 `price_low` · VoucherCard/VoucherRow 이미지 속성 · 카테고리/브랜드 선택 · 웜 시드 — 바뀐 것은 **잔액 부품의 분기 하나와 prop 하나**뿐이다. **❌ 목록 표면은 서버가 첫 화면을 안 그린다** — `/vouchers`·홈의 첫 화면은 곧 **상품 그리드**인데, 그 카드(`VoucherCard`/`VoucherRow`/`GroupBuyFeedCard`)는 **잠금표 부품**이고 지키는 것이 이미지 속성(width/height/srcSet/lazy/fetchPriority/dominant_color)과 prefetch 배선이다. 상세 히어로는 배경 이미지 `div` 하나였지만 그리드는 **N장 × 약 10개 속성**이라 두 벌이 갈리는 클래스에 정면으로 들어간다. per-user chrome 은 이걸로 풀렸으니, 다시 시도한다면 **카드 마크업을 서버·클라가 같은 SSOT 로 만드는 것**이 선행 조건이다. 가드: `deal-balance-awaiting-2026-09-16.test.tsx` 7건(**실제로 렌더해** 기다리는 카드와 도착한 카드의 구조가 같은가 · 0 을 안 적는가 · 비로그인은 한 줄 바인가 · 배선 2) + 기존 `deal-balance-card`·`vouchers-top-chrome` 재조준 + 주입 5건 **되돌려-검증 빨간불 확인**. 파일: VouchersPage 829줄(동결 유지). **✅ 라이브 판정(하네스로 로그인 상태를 만들어 측정)**: 필요한 입력은 둘뿐이다 — `getUserIdSync()` 가 읽는 `localStorage.user_id` 와 `/api/points/balance` 응답. 그 둘만 심고·stub(1.5초 지연)해 재니 `1,612ms 기다리는 카드(h=152) → 3,293ms 도착한 카드(h=152)` — **높이가 같아 목록을 안 밀어낸다**(불변식 확인). ⚠️ 그 사이 한 프레임의 '한 줄 바' 는 **하네스 아티팩트**다 — 같은 프레임에 `user_id` 가 `9999 → null` 로 바뀜다(진짜 세션 없는 가짜 신원을 앱이 정리 → `!userId` 분기가 0 으로). **그 값을 같이 찍어야 아티팩트와 결함이 구분된다.** 별건으로 카테고리 칩 행이 늦게 로드되며 첫 사진이 10px 내려간다(`sectionsReady` 예약 높이 미세 불일치 — 다음 후보). 롤백: `awaiting` 분기 + `loggedIn` prop 제거.
...
- 2026-09-16 `[UNLOCK_LOADING]` `VouchersPage.tsx` **상단 두 블록의 자리 예약을 숫자 → 같은 마크업으로** (대표 *"권하지 않는 것 말고 다 해줘"*). **배경(실측 — 응답을 2.5초 늦춰 프레임 캡처)**: 카테고리 칩 줄과 브랜드 줄은 서버에서 오고, 도착 전 자리를 **손으로 박은 숫자**(`h-[50px]`·`h-[113px]`)로 잡고 있었다. 그런데 2026-09-02 에 칩이 **흰 알약 + `shadow-lift`** 로 바뀌었는데 그 숫자가 안 따라와 **칩 50 vs 실제 56(−6) · 브랜드 113 vs 117(−4)** — 교체 순간 첫 사진이 `348 → 358` 로 **10px 내려앉았다.** 에러가 없어 아무도 신고하지 않는 종류다(소스 주석은 하필 *"border 를 더하면 1px 밀린다"* 고 경고까지 하고 있었다 — 경고를 적어 둔 그 자리가 어긋나 있었다). **수정**: 신규 `pages/vouchers/TopChromeReserve.tsx` 가 **진짜 블록과 같은 클래스**로 세우고 `invisible` 로 감춘다 ⇒ 높이가 같은 CSS 에서 나오므로 칩 디자인을 바꿔도 예약이 **저절로** 따라온다. 브랜드 예약은 `brandsOpen` 을 받아 **접힌 사람에겐 접힌 높이**를 잡는다(숫자였다면 2026-09-01 에 생긴 접기 토글을 못 따라갔다). **⚠️ 잠긴 항목 전부 byte-불변**: `__SSR_INITIAL_VOUCHERS__` 즉시 소비 · 기본 정렬 `price_low` · VoucherCard/VoucherRow 이미지 속성 · 카테고리/브랜드 선택 동작 · 웜 시드 — 바뀐 것은 **자리표시자 두 줄**뿐이다. **⚠️ 재방문자는 원래 이 자리를 안 본다** (카테고리가 localStorage 에 1시간 캐시돼 첫 렌더에 진짜 블록이 그려진다) — 이 밀림은 **첫 방문자 전용**이었다. 파일 829 → **824**(동결 이내 — 자리표시자를 모듈로 추출). 실측(수정 후): 예약 `[56, 117]` · 사진 y **1종 358**(직전 2종 `348 | 358`). 가드: `top-chrome-reserve-2026-09-16.test.ts` 8건(마법의 숫자 0 · 배선 · 치수 클래스 진짜 블록과 대조 · 접힘 추종 · 안 보임) + 주입 6건 **되돌려-검증 빨간불 확인**. 검증: tsc 0 · 관련 유닛 pass. **✅ E4 판정 통과(배포 후 라이브 2회, iPhone 13 · `urdeal.kr/vouchers` 첫 방문)**: `첫 상품 카드 y 1종(340)` · 칩 행 **56** · 브랜드 줄 **117** — 예약이 진짜 블록과 같은 값이라 교체 순간 안 밀린다(직전 예약 50/113, 카드 y 2종). 🩸 **판정 하네스가 내 눈을 두 번 속였다**: ① 처음 앵커가 `main img, …, img` 라 단계마다 **다른 사진**을 집었다 ② 고쳐서 "사진 가진 첫 `<button>`" 으로 잡았더니 **브랜드 칩도 `<button>` 안에 `<img>`** 라, 로고가 늦게 뜨는 순간 앵커가 상품 카드(340) → 브랜드 칩(200)으로 갈아타 **140px 밀린 것처럼** 보였다(두 번 다 재현돼서 진짜로 보였다). ⇒ 브랜드 스트립 안쪽을 명시적으로 제외한 뒤에야 1종이 나왔다. **하네스도 가드처럼 "무엇을 재고 있는지"를 의심할 것.** 🩸 **jsdom 은 레이아웃이 없어 "높이가 같은가" 를 단위시험으로 못 잰다** — 시험은 *치수를 정하는 클래스가 같은지*만 대조하고 실제 밀림은 브라우저 프레임 캡처가 판정한다(그 한계를 테스트 머리말에 적어 뒀다). 롤백: 두 자리표시자를 `h-[50px]`·`h-[113px]` 로 환원(그때 위 가드가 빨간불이 되므로 함께 판단할 것).
...
- 2026-09-08 `[UNLOCK_LOADING]` `GroupBuyFeedCard.tsx` **할인율 계산을 SSOT 로 분리(행동 불변)** (대표 *"구매바는 안 b 로 하지만 기존 가격정보도 넣으면 되잖아"*). **배경**: 유어쇼츠 구매 바에 정가·할인율을 넣는데, 그 계산을 구매 바가 **따로** 하면 같은 상품이 홈 카드에서 30%, 구매 바에서 0% 로 보이는 날이 온다 — 에러가 안 나서 아무도 모른다(2026-09-03 에 서버 정렬 정의와 클라 `discountOf` 가 갈려 "인기순"이 인기순이 아니었던 것과 같은 클래스). **수정(계산 3줄 → 함수 호출 1줄)**: 이 카드가 갖고 있던 `Math.max(선언값, 계산값)` 규칙을 신규 `shared/price-display.ts` 로 **행동 그대로** 옮기고 `VideosPage` 구매 바가 같은 함수를 쓴다. 가격의 `current_price ?? price` 폴백(공구 확정가)은 **이 카드의 계약이라 호출부에 남긴다.** **⚠️ 잠긴 항목 전부 byte-불변**: `React.memo` · viewport prefetch `rootMargin:'100px'` · hover/touch/focus prefetch · **aboveFold eager** · 이미지 fade-in · 대표색 추출 — 렌더 JSX 는 한 글자도 안 바뀌었다(diff 는 계산 블록뿐). **🕳️ 가드가 즉시 잡은 것**: 새 import 가 `manualChunks` catch-all 로 떨어져 **홈 폐쇄가 청크를 하나 더 끌고 왔다** — `home-chunk-diet` 가 빨간불을 냈고, 형제 `deal-card-grid` 와 같은 자리에 `app-shared` 규칙을 추가해 해소(그 가드의 OK_PARTS 미러도 함께 갱신했고, 미러 자기검증이 실제로 발화하는지 vite 규칙을 빼서 확인). 가드: `videos-buy-bar.test.ts` 13건 + 주입 4건 **되돌려-검증 빨간불 확인**. 검증: tsc 0 · vitest 641파일 7,961건 pass · build 0. 롤백: 계산 블록 환원 + import 제거(SSOT 파일은 구매 바가 계속 쓰므로 유지).
- 2026-09-07 `[UNLOCK_LOADING]` `GroupBuyFeedCard.tsx` **'마감 임박' 빨간 배지 제거 — #1349 가 놓친 마지막 한 곳** (대표 *"잔재 정리도 하고"* → *"또 해야할 건?"*). **배경**: 09-04 대표 확정 *"마감 개념은 없어"* 로 상세의 'D-N 마감' 띠·구매 차단 가드·정렬 칩을 지웠는데, **홈·지도 피드 카드의 배지만 살아남았다.** 눈에 안 띈 이유가 고약하다 — 리스트 API 가 판매 마감을 `p.group_buy_deadline AS expires_at` 으로 **이름을 바꿔** 내려서 카드 코드만 보면 *사용 기한*처럼 읽히고(원 필드명으로 grep 하면 안 걸린다), 조건이 `시간|분` 단위일 때만이라 **평소엔 화면에 아예 안 뜬다**. **라이브 실측(2026-09-07)**: 활성 50건 중 `expires_at` 이 있는 건 유일한 실제 매장 상품(2888 치즈돈가스) 하나뿐이고 값이 `2026-09-10 07:21` — 즉 **09-09 부터 24시간 동안** 그 카드에 빨간 `마감 23시간 → 마감 30분` 이 켜질 예정이었다. 아무것도 막지 않는 날짜로 만든 가짜 긴박감이고, 하필 유일한 실제 매장의 카드다. **수정**: `timeRemaining`·`remaining`·`isUrgent`·배지 렌더 제거 + `FcfsBadge` 위치를 항상 `top-2`(위 배지가 사라져 좌상단이 비었다) + 죽은 prop `expires_at`·고아 `safeDate` import 정리. 함께 죽은 코드 둘 제거 — `main-home/utils.ts fmtEnd`(`N분 후 마감`, 참조 0) · `group-buy/detail-derived.ts daysLeft`(#1349 가 띠를 지우며 소비처 0). **⚠️ 잠긴 항목 전부 byte-불변**: `React.memo` · viewport prefetch `rootMargin:'100px'` · hover/touch/focus prefetch · **aboveFold eager(`eager={aboveFold}`)** · 이미지 fade-in · 대표색 추출 — 바뀐 것은 **오버레이 배지 하나**뿐이다. **⚠️ 서버는 무접촉**: `group-buy-public.routes.ts:221` 의 별칭은 그대로 둔다(캐시키·외부 계약 — 소비자가 안 그리면 해가 없다). **구매 후 사용 기한은 다른 축이라 그대로다**(대표 *"결제 이후 유저가 이용권을 쓸 수는 있어야 하잖아"* — 지갑 `VoucherTicket`·`PaymentCompleteTicket` 의 D-N 은 `vouchers.expires_at` 으로 직접 센다). 가드: `no-deadline-sort.test.ts` +4건(마감 문구 0 · `expires_at` 미소비 · 잠금 계약 4종 잔존 · 죽은 포매터 부활 금지) + 주입 매니페스트 1건 **되돌려-검증 빨간불 확인**(2건 실패 → 복원 후 통과). 🩸 **이번에 가드 자신의 결함 둘을 잡았다**: ① 테스트의 주석 제거기가 **줄 단위**라 여러 줄 JSX 주석(`{/* … */}`)의 가운데 줄이 남아, 내가 방금 쓴 설명의 '마감' 에 걸려 빨간불이 났다(세 번째 재발) → 블록 주석을 **먼저 통째로** 지우도록 강화 ② 그 강화가 즉시 **헛돌던 단언**을 드러냈다 — `toMatch(/fetchPriority/)` 는 그 파일에서 **주석에만** 있는 단어라 배선을 지워도 초록이었다(실제 배선은 `eager={aboveFold}`). 검증: tsc 0 · 관련 유닛 55 pass · theme/design-slop/image-fallback/mobile-viewport/modal-zindex GREEN. 롤백: 배지 블록 + `timeRemaining` 복원(그때 위 4건이 빨간불이 되므로 함께 판단할 것).
- 2026-09-14 `[UNLOCK_LOADING]` `BrowsePage.tsx` **목록 복원 장치 배선(교환권과 동일 처방)** (대표 *"모두 고쳐"* — 전날 교환권 수리의 나머지 화면). **배경**: `App.tsx` 는 경로가 바뀌면 페이지를 통째로 리마운트하므로(`key={location.pathname}`) 목록이 `useState` 로 들고 있던 것이 전부 사라진다. 그리고 **그게 스크롤 복원까지 무력화한다** — `ScrollToTop` 은 멀쩡한데 돌아갈 문서가 없다(그 파일이 "못 하는 것"으로 적어 둔 한계). **수정**: 신규 SSOT `src/lib/list-view-cache.ts`(탭 수명 메모리, TTL 30분, 최대 8키) 를 POP 일 때만 조회해 `products·page·hasMore·showCount` 를 **첫 렌더에 동기 복원**하고, 마운트 리셋(`setProducts([])` + `loadProducts(1,true)`)의 **첫 실행만** 건너뛴다. 키는 `browseViewKey(cat, sort)` SSOT 로 복원·보관이 같은 문자열을 쓰게 했다(갈리면 조용히 안 살아난다). **⚠️ 잠긴 `__SSR_INITIAL_BROWSE__` 즉시 소비는 불변** — 복원본이 없을 때의 경로가 종전과 byte-동일이고, 있을 때만 복원본이 시드를 이긴다(시드는 늘 1페이지라 덮으면 목록이 도로 짧아진다). 가격대가 기본('all')일 때만 보관한다(priceRange 는 URL 에 없어 마운트마다 'all' 로 돌아간다). **🩸 내가 틀린 것**: 전날 PR 에서 *"/map · 유어샵에도 같은 버그"* 라고 보고했는데 **`useState` 선언만 보고 단정했고 실측에서 뒤집혔다** — `/map` 은 `useMapProducts` 의 모듈 캐시 동기 소비 + `data-scroll-restore="map-list"` 로 이미 정상(행 40→40 · 시트 스크롤 1200→1200, **캐시 TTL 2분을 넘겨도** 통과), 유어샵도 `getCuratorCache` 동기 소비로 이미 정상(카드 3→3 · 스크롤 208→208). 그 둘은 고치지 않고 **있는 장치를 주입 2건으로 잠갔다.** 🧭 **교훈: 스크롤을 px 로 판정하지 말 것** — 되돌려-검증에서 `scrollY` 는 **같은 값이 나오는데** 화면 맨 위 항목이 달랐다(문서가 우연히 그 오프셋만큼은 길었다). 대표가 말한 "이전 위치가 아님" 은 px 가 아니라 **내용**이다 ⇒ 판정 기준은 맨 위 항목. 가드: `list-view-restore-2026-09-13.test.ts` 24건 + 주입 8건(되돌려-검증 전부 빨간불 확인). 롤백: `restored` 참조 4곳 + 보관 effect + 첫 실행 스킵 제거(그때 위 테스트가 빨간불이 되므로 함께 판단할 것).
- 2026-09-03 `[UNLOCK_LOADING]` `useFeedWindow.ts` + `RestaurantMapPage.tsx` **수요 로딩이 만든 결함 2건 자가 수리** (대표 *"계속 끝까지 해"* — 자기 diff 재검토). ① **`near` 와 `sort` 를 같이 보내고 있었다**: 서버는 `baseOrder = hasNear ? 거리 : sort` 라 **near 가 sort 를 이긴다**. `/map` 은 위치가 있으면 늘 near 를 보냈으므로 위치를 켠 사용자가 '할인율순'을 골라도 서버는 가까운 50개를 주고 화면은 그 안에서만 정렬 — **전량을 받던 시절엔 맞았고, 수요 로딩으로 바꾼 그 순간 결함이 됐다**(에러 0, 테스트도 통과했다). ⇒ 거리순일 때만 near 를 넘긴다. ② **위치 캐시가 있으면 진입 요청이 2회**: `useNearMeAuto` 의 자동 '가까운 순' 전환이 정렬-포함 캐시키를 갈라 두 번째 요청을 만든다 ⇒ 위치가 캐시돼 있으면 `sortBy` 를 처음부터 `distance` 로 시작(칩 깜빡임도 함께 제거). 하네스 실측: `/map` 위치無 1회(`sort=discount`) · 위치有 1회(`near=…`, sort 미동반) · 홈 1회(`sort=popular`). 가드 +1 · 주입 매니페스트 7건 되돌려-검증 빨간불 확인 · 전체 유닛 7,373 pass · audit-gate 101 ✓. 🧭 **교훈: 테스트 초록 ≠ 옳음** — 두 결함 다 내가 쓴 테스트를 통과했다(배선은 봤지만 *서버 우선순위*와 *마운트 시퀀스*는 안 봤다). diff 적대적 재독 패스를 생략하지 말 것.
- 2026-09-03 `[UNLOCK_LOADING]` `group-buy-public.routes.ts` **정렬 정의를 화면과 일치시킴 + 홈 피드 서버 정렬 배선** (대표 *"마저 다 해줘"* — 같은 날 지도 피드 수요 로딩의 마지막 조각). **배경**: 홈 피드(`GroupBuyFeed`)는 이미 page1+무한스크롤이라 전량을 받진 않았는데, **정렬을 클라가 로드된 것 안에서만** 했다 — 전체 338건인데 "인기순"이 사실은 *최근 50개 중 인기순*. 게다가 서버 정렬 정의가 화면과 **달랐다**: 서버 `popular`=`group_buy_current DESC`인데 클라 `soldOf`는 `sold_count ?? group_buy_current`(2026-07-16 대표 "PC 정렬 무반응" 때 sparse 라 고친 그 이유), 서버 `discount`=`discount_rate DESC`인데 클라 `discountOf`는 값이 비면 정가·판매가로 계산. 정의가 갈리면 서버가 고른 상위 50이 클라가 원하는 상위 50이 아니다. **수정**: ① 서버 `popular`→`COALESCE(p.sold_count, p.group_buy_current, 0) DESC`, `discount`→`MAX(COALESCE(discount_rate,0), 정가·판매가 계산값) DESC`(클라 SSOT 미러, **실제 SQLite 로 6종 ORDER BY 실행 검증**) ② `GroupBuyFeed` 가 page1·loadMore 모두 `sort`(거리순은 `near`, 좌표는 서버 캐시키 단위 0.02° 반올림)를 넘김 ③ 캐시키를 정렬별로 분리(`queryKeys.groupBuyList(status, category, sort?)` — 미전달이면 종전 키와 byte-동일) + `placeholderData:(prev)=>prev`(정렬 전환이 빈 화면이 되지 않게) + 정렬 바뀌면 누적 밴드 리셋. **⚠️ SSR 0-RTT 유지**: `__SSR_INITIAL_MAIN__` 시드는 그대로 `initialData` 로 쓰고(홈 기본 정렬 화면), 클라 `sortBand`(밴드별 정렬, 2026-07-16 "스크롤 시 배치가 제멋대로" 수리)도 불변 — 서버는 **후보 집합**을 정하고 화면 최종 순서는 종전대로 클라가 매긴다. **⚠️ `discount` 정의 변경은 `/map` 기본 정렬에도 적용된다**(둘 다 "할인 큰 순"이고 새 정의가 카드에 찍히는 값과 일치 — 더 정확해진다). 실측(하네스): 홈 진입 요청 1회에 `?sort=popular`, page2~7 도 같은 정렬. 가드: `feed-sort-and-sentry-noise.test.ts` 13건 + 주입 3건 **되돌려-검증 빨간불 확인**. 파일: 라우트 1154(래칫 상한 유지 — 주석 1줄 압축으로 상쇄), GroupBuyFeed 560→577(cap 600). 롤백: `feedParams` 3곳 + 캐시키 3번째 인자 제거(서버 정의는 남아도 무해).
- 2026-09-03 `[UNLOCK_LOADING]` `group-buy-public.routes.ts` + `useMapProducts.ts` + `RestaurantMapPage.tsx` **지도 피드 전량 순회 제거 — 화면이 요구하는 만큼만** (대표 *"이거 페이지 들어갈 때마다 모든 이용권을 다 불러오는 것 같은데 … 가장 이상적이고 합리적인 방법은 맞아?"* → *"가장 이상적으로 하자. 모두 이상적으로 진행"*). **배경(라이브 실측)**: 활성 이용권 **338건**인데 `/map` 진입마다 클라가 50개씩 **끝까지 스스로 걸어가** 전부 받았다 — **요청 7회 · 66KB(gzip)**, 화면에 뜨는 카드는 10~20장. 게다가 지도는 그렇게 받아 놓고도 idle 마다 `bbox` 로 같은 지역을 또 받았고(중복), `SOFT_CAP=500` 은 **에러 없이 조용히 끊기는 절벽**이었다(500건을 넘는 순간 그 뒤 이용권은 목록에서 그냥 사라진다). 같은 레포의 **PC/모바일 홈 피드(`GroupBuyFeed`)는 이미 page1 + 무한스크롤**이라 두 화면이 서로 다른 전략을 쓰고 있었다. **수정**: ① `useMapProducts` = page1 + **요청형**(`loadMore`/`loadAll`) — 초기 effect 의 자동 순회 루프 삭제 ② **정렬을 서버로**(`sort`) — 50개 안에서 정렬한 "할인 큰 순"은 거짓말이다. 서버 화이트리스트에 `price`·`rating` 추가(클라 `SortBy` 4종과 1:1, 거리순은 `near` 가 담당) ③ **`total` additive** — 다 안 받고도 "N곳"을 정확히 말한다(필터·검색 없을 때만 계산, `feed-total.ts`, 조합 5종·900s 캐시, fail-soft null) ④ 서버가 못 거르는 필터(지역 텍스트·반경·가격대·즐겨찾기)**와 필터 시트를 여는 순간**에만 `loadAll()` ⇒ **비용을 그 기능을 쓰는 사람에게만** 부과 ⑤ 리스트 센티넬이 로드된 행을 다 보여 준 뒤 **서버 다음 페이지**를 부른다(`RestaurantList.onLoadMore`). **⚠️ 잠긴 계약 byte-불변**: 워커 SSR 시드(`__SSR_INITIAL_MAIN__` — 무파라미터 기본 요청)·`Cache-Control`/`CDN-Cache-Control` 분리·materialized 경로·`near` 거리랭킹·bbox/클러스터 3계층 — 바뀐 것은 **클라가 언제 무엇을 요청하는가**와 응답의 `total` 필드뿐이다. **실측(브라우저 하네스, 338건 가짜 카탈로그)**: `/map` 진입 요청 **7회 → 1회**, 화면 카운트 "338곳"(로드된 50이 아님), 바닥까지 스크롤해야 page2~7. 파일: `RestaurantMapPage` 957→954(래칫 준수 — `viewport-list.ts` 순수 추출), 라우트 1146→1154(래칫 상한). 가드: `map-feed-demand-loading.test.ts` 14건 + 주입 매니페스트 3건 — **되돌려-검증 3종 빨간불 확인**. 🩸 **교훈**: 첫 판에서 `s.index("// 🗺️ 2026-07-15 (대표 — …")` 로 블록을 잘라냈는데 **같은 접두사 주석이 위에 또 있어** 174줄이 통째로 날아갔다(tsc 가 잡았다) — 긴 주석은 앵커로 쓰면 안 되고, 쓰려면 고유 꼬리("왜 18곳만?")까지 포함해야 한다. ⚠️ **이번 범위 밖(남은 것)**: 홈 피드(`GroupBuyFeed`)의 정렬도 **로드된 것 안에서만** 돈다(서버 화이트리스트엔 이미 popular/newest/deadline/discount 가 있다) — 그쪽은 SSR 시드 캐시키 계약을 건드려야 해 별건. 롤백: `useFeedWindow` → `useMapProducts` 직접 호출 환원 + 훅의 초기 effect 에 순회 루프 복원(그때 가드가 빨간불이 되므로 함께 판단할 것).
- 2026-09-02 `[UNLOCK_LOADING]` `VouchersPage.tsx`(+비잠금 `vouchers/shared.tsx`) **교환권 B안 — 칩(지도 B안과 같은 그림) + 브랜드 기본 펼침(로고) + 잔액 흰 카드** (대표 확정 *"B · 칩(지도와 같은 B안) + 브랜드 펼침 으로 하자. 브랜드는 당연히 로고가 보이는게 맞겠지?"*). **① 브랜드 스트립 기본 펼침** — 09-01 "브랜드 스트립 기본 접기(나안)" 를 **대표가 이 결정으로 대체**했다. 접기 토글(`brandsOpen`)은 남긴다. `useState(() => !!searchParams.get('brand'))` → `useState(true)`(딥링크 분기는 항상 참이라 불필요). **② 카테고리 칩**: 잉크 면 선택 → **흰 알약 + 선택 블루 면**(`bg-brand text-white` / `bg-white … shadow-lift`), 칩 바의 `border-b` 제거(표면 규칙 ① 테두리 0). PC 세로 카테고리 리스트 선택도 블루 면. **③ 잔액**: 검정 슬래브(`#16181C` inline) → **흰 카드 + 숫자 36px 잉크 + `충전/내역` 알약만 블루**(모바일·PC 레일 둘 다). 0 잔액 바도 흰 카드. **④ 목록**: `VoucherRow` 구분선 행 → 흰 카드(`shadow-lift`, `gap-2`), `VoucherCard`·브랜드 타일 테두리 → 들림, 브랜드 선택 링 블루. **⚠️ 잠긴 항목 byte-불변**: `__SSR_INITIAL_VOUCHERS__` 즉시 소비 · 기본 정렬 `price_low` · VoucherCard/VoucherRow **이미지 속성**(width/height/srcSet/lazy/fetchPriority/dominant_color/onLoad) · 카테고리/브랜드 선택 동작 · `React.memo` — 바뀐 것은 컨테이너 className 과 `brandsOpen` 초기값뿐. 파일 980 → 979(동결 이내). 가드: `vouchers-top-chrome.test.ts` ②④ 재정의(펼침이 정본) + 매니페스트 "기본 접힘으로 되돌아간다" 교체 — **되돌려-검증 빨간불 확인**. 롤백: `useState(true)` → 09-01 식 + className 환원.
- 2026-09-02 `[UNLOCK_LOADING]` `cf-image.ts` **미지 호스트 기본값을 원본 → cdn-cgi(+onerror=redirect) 로 · 핫링크 목록 모듈 스코프로 hoist** (대표 "그것도 다 진행해줘. 모두 다 해결해줘. 가장 이상적으로" — 같은 날 호스트 승격의 근본 처방). **배경**: 승격은 사후약방문이다. 데모 사진 출처가 카카오 플레이스라 **임의 CDN 이 계속 섞이므로 호스트 목록은 원리상 샌다** — `if (!isSupported && !isExternalProxyable) return src` 한 줄이 구멍이었고 라이브 홈 한 화면에서 5장이 리사이저를 안 거쳤다. **수정 2곳**: ① 그 `return src` → `HOTLINK_BLOCKED_HOSTS` 검사 후 `/cdn-cgi/image/…,onerror=redirect/${cdnCgiSafe(src)}`(+`cropFrag`). ② `HOTLINK_BLOCKED_HOSTS` 를 블록 안 → **모듈 스코프**(값·순서 불변 — `hotlink-rehost-scope.test` 가 SSOT 와 동일성 대조). **⚖️ 세 경우 전부 현행과 같거나 낫다**: 리사이저가 받으면 리사이즈(이득) / 못 받으면 `onerror=redirect` 로 원본 302 = 현행(대가는 리다이렉트 한 홉) / 브라우저도 못 받으면 오늘도 깨짐 = 동일. **2026-06-11 카카오 프로필 깨짐은 `onerror=redirect` 도입(2026-07-02) 전** 사고라 그 실패 모드는 지금 두 번째 경우로 흡수된다. **SUPPORTED_HOSTS·EXTERNAL_PROXY_HOSTS·CDN_CGI_VERIFIED·Save-Data·`/api/media`·`/api/upload` 분기 전부 byte-불변(제거 0).** 가드 `qa-image-host-coverage.test.ts` 18(함수 호출 검증 포함) + 주입 4. ⚠️ **주입 검증이 내 오류를 두 번 잡았다**: 앵커 공백 불일치, 그리고 **픽스처 전제 오류**(`sub.postfiles.pstatic.net` 은 apex `pstatic.net` 때문에 목록 경로라 핫링크 hoist 를 발동 못 시킨다 — 현재 핫링크 7호스트는 **전부** 목록 경로이고 hoist 는 새 apex 대비 죽은 가지다. 함수로 못 재는 배선은 소스로 고정). 롤백: 그 분기를 `return src` 로 환원 + 선언을 블록 안으로.
- 2026-09-02 `[UNLOCK_LOADING]` `cf-image.ts` **실측 승격 2호스트 추가(cloudfront.net · digitaloceanspaces.com)** (대표 "QA 도와줘" — 라이브 홈 브라우저 실측). **배경**: 홈 한 화면에서 **사진 5장이 리사이저를 아예 안 거치고 원본 호스트로 직행**했다(브라우저 요청 로그로 확인 — cdn-cgi 요청 자체가 없었다). `cfImage` 는 목록에 없는 외부 호스트면 원본 URL 을 그대로 돌려주는데, 데모 사진 출처가 카카오 플레이스라 **임의 CDN 이 섞인다**. D1 실측: 활성 상품 15개(커버 4 · 갤러리 11)가 이 상태. **수정(목록 추가만 — 잠금표 예외 "EXTERNAL_PROXY_HOSTS 추가 OK, 제거 X")**: 두 호스트를 `EXTERNAL_PROXY_HOSTS` + `CDN_CGI_VERIFIED` 에 **함께** 추가(한쪽만이면 프록시로 새어 리사이즈가 안 된다). 승격 근거는 문서가 요구하는 실측: `cf-resized: internal=ok`, w200/400/600 전부 200(각 12/28/54KB). `onerror=redirect` 안전판 그대로라 재차단 시 원본 302 폴백 = 현행 동작. **Save-Data·`/api/media` 분기·HOTLINK_BLOCKED·기존 목록 전부 byte-불변.** 동반(비잠금): `demo-image-rehost` recondition 큐를 **차단(언론사·스톡) 사진 우선**으로 정렬 — QA 에서 **동아일보 사진이 만화카페 딜 카드에 라이브**였는데(id 2879) 큐가 `ORDER BY p.id` 라 백로그 40건·시간당 3건 기준 **13시간**이 남아 있었다(저작권 사진에 "나중"은 없다). 가드 `qa-image-host-coverage.test.ts` 12 + 주입 2(되돌려-검증 red). 롤백: 목록 2줄 + ORDER BY 절 환원.
- 2026-09-02 `[UNLOCK_LOADING]` `worker/index.ts` **상세 히어로 preload 를 갤러리와 같은 SSOT URL 로 · UA 로 폰/PC 분기** (대표 *"사진 불러오는 속도가 느리다 · 클릭하면 반응이 늦다"*). **배경(라이브 워터폴)**: 08-31 에 모바일 슬라이드만 3:2 크롭(`900×600,gravity=auto`)으로 바뀌고 preload(`width=900`)·감시 `<img>`(`width=1200`)는 옛 폭에 남아 **같은 히어로를 세 벌** 받았다 — preload 111KB 는 버려지고(브라우저는 URL 이 한 글자만 달라도 안 쓴다) 폰이 PC 폭 179KB 를 받았다. 에러가 없어 아무도 몰랐다. 그 위에 갤러리 5장이 동시에 내려와(각 136~220KB, 콜드 2~4s) 첫 사진과 대역폭을 나눴다. **수정**: ① 신규 SSOT `shared/detail-hero-image.ts` — preload(`home-card-preload.ts`) · `DetailGallery` 감시 `<img>`(`useMediaQuery(lg)` 로 폰은 히어로 한 장만) · 슬라이드가 **같은 함수**로 URL 을 만든다. 워커는 `isMobileUserAgent(c.req.header('user-agent'))` 로 폰/PC 를 갈라 넘긴다(**호출 인자 1개 추가 + import 1줄 — SSR inject·`caches.default`·HOT_PATHS·#root 로더 전부 byte-불변**, 파일크기 래칫 2685 안). ② 슬라이드는 `active ±1` 만 요청. ③ `utils/image-warm.ts` — 카드 커버가 실제로 받은 `currentSrc` 를 기억하고 상세가 `url(고해상), url(카드 변형)` 두 겹 배경으로 **클릭 즉시** 사진을 보인다(추가 요청 0). 가드: `detail-image-continuity.test.ts` 8건 + 주입 3건 되돌려-검증 red. 롤백: 각 항목 독립(preload 인자 제거 → 기본값 폰 · 게이트 2개 제거 · `layered` → 단일 url).
- 2026-09-02 `[UNLOCK_LOADING]` `RestaurantMiniMap.tsx` **지도 SDK 를 히어로 뒤로 — 관측 여백 300→120px + 교차 후 idle 호출** (대표 *"모두 다 진행"* — 로딩 후속 ④). **배경(클릭 프로브)**: 홈 카드 탭 → 상세에서 카카오 지도 SDK 가 0.28초, 타일 6장이 1.3초에 내려왔다 — 히어로 사진이 오는 바로 그 시간. 폰에서 지도 칸이 첫 화면 바로 아래라 `rootMargin: 300px` 가 마운트 즉시 교차 판정이었고, 교차하자마자 SDK 를 불렀다. **수정**: 여백 120px + `requestIdleCallback`(최대 2.5초) 뒤 `setShouldLoadSdk`. **⚠️ 잠금표가 지키는 lazy 구조(`shouldLoadSdk` — 스크롤 안 오면 SDK 0 fetch)는 그대로이고 강화만 했다.** 동반(비잠금): `analytics.routes` vitals 표본율 0.01→0.25 + `web-vitals-report` LCP 를 `pagehide`·10초 폴백으로도 전송(4일간 실사용자 LCP 표본 0 이던 것 — 카카오 인앱은 `visibilitychange→hidden` 이 안 온다). 가드 `loading-followups-2026-09-02.test.ts` 5 + 주입 3. 롤백: 여백·idle 환원(각 1곳).
- 2026-09-02 `[UNLOCK_LOADING]` `vite.config.ts` manualChunks **app-utils 다이어트 — 홈 미도달 모듈을 `app-utils-deferred` 로** (대표 *"모두 다 진행"* — 로딩 후속 ②). **배경(번들러 실측 그래프, #1310 과 같은 방법)**: app-utils 104.6KB 중 **73.8KB(56모듈)가 홈 미도달**인데 엔트리 필수(api.ts 등)와 한 봉투라 홈이 매번 통째로 받았다 — sentry·performance-monitor·web-vitals-report(엔트리가 *동적* import 하지만 결제/로그인 페이지가 정적으로 써서 공유 봉투로 끌려옴)·errorHandler·kakao-login-overlay·도매 유틸(read-table-file/supplier-api/useChatPoll)·마이 쿼리 훅(useMy*)…. **수정(규칙 1블록 additive — 제거 0)**: 파일 이름 열거 정규식으로 `app-utils-deferred`(폴더 규칙 금지 — #1310 에서 두 번 밟음). **실측(실제 `npm run build`)**: app-utils 107 → **63KB raw**, deferred 42KB 는 홈 preload 에 없고 상세·유어샵·목록 표면만 preload(그 표면들은 종전 총량과 같고 요청 1개 추가). `check-critical-chunks` 17 동일 · `surface-role-leak` 0 · 번들 예산 통과. **⚠️ 기존 잠금 규칙(env-validator·Layout 분리·폴더별 청크·app-home·toss-preload·useWholesale) 전부 불변.** 가드 `app-utils-diet.test.ts` 24건(규칙 위치·걸림/안 걸림·홈 진입 파일 정적 import) + 주입 1(catch-all 을 앞에 두는 되돌려-검증 red). 롤백: 규칙 1블록 제거.
- 2026-09-02 `[UNLOCK_LOADING]` `vite.config.ts` manualChunks **홈 청크 다이어트 — 홈이 실제로 닿는 모듈을 `app-home` 청크로** (대표 *"홈 청크 다이어트도 지금 해줘"*). **배경(라이브 실측)**: 홈 콘텐츠 완성이 0.9~3.7s 로 편차가 크고 첫 화면 modulepreload 가 26개 — 8/27 에 "app-components 166KB 안에 홈이 안 쓰는 게 더 있다, 정확한 판정엔 번들러의 실제 모듈→청크 그래프가 필요하다" 고 남겨 둔 자리. **이번엔 그 그래프를 실제로 떴다** — `vite build` 에 `generateBundle` 플러그인으로 `chunk.modules` + 모듈 import 그래프를 덤프하고, 엔트리+홈 페이지에서 정적 import 로 도달 가능한 모듈 집합을 계산. 결과: 홈 정적 폐쇄 27청크 956KB 중 `app-components` 281KB 는 홈이 **21/66 모듈(70KB)** 만 쓰고, 그 봉투의 안 쓰는 45개(온보딩 모달·이미지 업로드·교환권 스캐너·선물 모달·PWA 프롬프트…)가 같이 preload 됐다. 더 나쁜 건 **딸려오는 것** — 그 안 쓰는 모듈들이 `components/ui/button` 등을 import 해 `app-ui-utils`(**tailwind-merge 97KB, 홈 도달 0/3**)·`radix-ui`·`app-kakao-sdk` 를 청크 단위로 끌고 왔고 `app-features`(48KB) 도 홈은 2/25. **수정(규칙 1블록 additive — 제거 0)**: 실측 도달 집합(`components/home/`·`pages/pc-home/`·`GroupBuyFeedCard`·`deal/{DealCardMedia,WishlistHeart,StarRating}`·`region/`·`SEO`·`ui/sort-menu`·`shared/seo/`·`shared/home-*`·`product-flow`·`deal-category-icon`·`group-buy/{FcfsBadge,useFcfs}`)을 `app-home` 으로. **2차 실측에서 밟은 것**: `deal/` 을 폴더째 넣었더니 상세 전용 `DetailFloatingHeader` 가 WishlistButton·PinButton·KakaoShareButton 을 import 해 app-components 를 **도로 끌고 왔다** → 파일 3개로 좁힘. 그리고 Vite 는 전이 import 를 `chunk.imports` 에 끌어올리므로 "누가 끌고 오나" 는 chunk.imports 가 아니라 **모듈 간선**으로 찾아야 한다. **3차 실측 결과**: 홈 폐쇄 **27청크 → 20청크(−7), 956KB → 774KB(−19%)**, 다섯 봉투 전부 폐쇄에서 사라짐. `/browse` 도 26→21청크. **⚠️ 기존 잠금 규칙(env-validator/Layout 분리·폴더별 청크·seller-roles/icons/seller-tracking→app-shared·toss-preload) 전부 불변 — 규칙 추가만.** 가드: `home-chunk-diet.test.ts` 7건(규칙 존재·catch-all 보다 앞·**홈 진입 4모듈의 `@/` import 가 app-home 또는 기존 홈 청크 규칙에 걸리는지** — 새 import 하나가 catch-all 로 떨어지면 빨강) + 주입 2건(규칙 소실·deal 폴더 통째) 되돌려-검증 red. ⚠️ **남은 것**: `app-utils` 102KB 중 72KB 가 홈 미도달(performance-monitor·errorHandler·sentry·kakao-login-overlay…)인데 `api.ts` 등 필수와 한 봉투라 별건. 롤백: `app-home` 블록 제거(그때 위 테스트가 빨간불이 되므로 함께 판단).
...
- 2026-08-31 `[UNLOCK_LOADING]` `GroupBuyFeedCard.tsx` **할인율을 사진 밖으로 · 쿠팡식 2줄 가격 · 줄 여백 조임** (대표 *"할인율이 사진 안으로 들어가면 안돼. 할인율을 강조하기도 해야하고 그리고 줄마다 여백을 줄여야 할 것 같아. 쿠팡처럼"* — A안 승인). **배경**: 같은 날 오전에 *6자리 가격(숙소 119,000원)에서 할인 pill 이 다음 줄로 밀려 그 카드만 높이가 늘어나는* 구조적 깨짐을 고치려고 배지를 **사진 좌하단으로** 옮겼는데, 그건 사진을 가린다. **하루에 두 번 뒤집힌 자리다.** ⇒ 쿠팡·11번가·G마켓이 쓰는 구조로 간다: 사진은 비우고, 가격 블록을 `[할인율(로즈 굵게) 정가취소선]` / `[판매가]` **2줄**로 나눈다. 줄로 나누면 6자리 가격이 **구조적으로 안 깨지고**, 할인율이 판매가 바로 위에 붙어 커머스에서 가장 먼저 읽힌다. 여백은 함께 조여(본문 pt-2.5→2 · 머천트 mb-1→0.5 · 제목 leading-snug→tight · 주소/평점 mt-1→0.5) 줄이 하나 늘었는데도 카드 높이는 종전과 거의 같다. **⚠️ 잠긴 항목 전부 byte-불변**: `React.memo` · viewport prefetch `rootMargin:'100px'` · hover/touch/focus prefetch · aboveFold eager+fetchPriority · 이미지 fade-in(opacity transition) · 대표색 추출 — 바뀐 것은 **오버레이 배지 제거 + 본문 가격 마크업 + 여백 수치**뿐. 가드: `deal-card-price-block.test.ts` 4건(① 할인율 사진 위 금지 ② 본문에 존재 ③ 정가·판매가 다른 줄 ④ 로즈 강조) + 주입 매니페스트 2건 — **되돌려-검증 2종 빨간불 확인**(사진 위로 환원 · 한 줄로 합침). 검증: tsc 0 · 카드 계약 30건 pass · design-slop/image-fallback/theme GREEN. 롤백: 가격 블록을 한 줄 `<p>` 로 환원 + 오버레이 배지 복원(그때 위 테스트가 빨간불이 되므로 함께 판단할 것).
- 2026-08-31 `[UNLOCK_LOADING]` `cf-image.ts` **크롭 위치 옵션(`gravity`) 추가 — 상세 히어로 고정 프레임용** (대표 승인 "너가 얘기해준 가장 이상적인 방법들로 할게"). **배경**: 대표가 *"지금 사진 비율이 지금이 가장 이상적일까? 네이버에서 가져오는 사진들 비율 중 가장 많은 비율로 해줘. 그루폰은 가로에 비해 세로가 짧잖아"* 물어 **직접 쟀다**(라이브 네이버 사진 70장): 세로가 더 긴 것 **37%** · 정사각 21% · 4:3 23% · 3:2 11% · 16:9 7%, **중앙값 정확히 1:1**. 즉 "가장 많은 비율"은 넓은 게 아니라 정사각~세로라, 그루폰처럼 16:9 로 가면 **열 장 중 여섯이 위아래로 잘린다.** ⇒ 답은 프레임을 고르는 게 아니라 **자를 위치를 고르는 것**: 프레임 3:2 고정 + `gravity=auto`(Cloudflare 가 피사체를 찾아 자름). **수정(additive·게이트드)**: `ResizeOptions.gravity` 신설 + `cropFrag(o)` 헬퍼(**`o.gravity` 가 있을 때만** `,height=..,fit=..,gravity=..` 반환)를 `/api/media` 경로·검증호스트 빠른경로·일반 빌더 세 곳에 append. ⚠️ **이 게이트가 핵심**: 두 빠른 경로는 옵션 문자열을 손으로 조립해서, 조건 없이 붙이면 **기존 모든 호출의 URL 이 바뀌어 엣지 캐시가 통째로 미스** 난다. 지금 `gravity` 를 넘기는 곳은 상세 히어로뿐이라 나머지는 URL 이 byte-동일. **`SUPPORTED_HOSTS`/`EXTERNAL_PROXY_HOSTS`/`HOTLINK_BLOCKED_HOSTS`/`CDN_CGI_VERIFIED`/Save-Data/`onerror=redirect`/워커 프록시 분기 전부 불변 — 제거 0.** 라이브 실측: `width=780,height=520,fit=cover,gravity=auto` → `cf-resized: internal=ram`, center 대비 **결과 바이트가 다름**(28,019 vs 28,217 = 실제로 다른 곳을 자른다) + 세로 사진 육안 비교에서 피사체 보존 확인. 동반(비잠금): `DetailGallery` 모바일 프레임 1:1 → **3:2** + `heroUrl()` 로 크롭 요청(이용권·숙소 상세가 **같은 컴포넌트라 둘 다** 적용). ⚠️ **PC(대형/썸네일)는 미적용** — 그쪽은 프로브 `<img>` 와 URL 을 공유해 추가 트래픽 0 을 유지하는 구조라 한쪽만 바꾸면 요청이 두 배가 된다(별건). ⚠️ **검증호스트가 아닌 사진**은 워커 프록시로 가는데 그 경로는 리사이즈 자체를 못 해(2026-06-11 실측) 프레임만 3:2 이고 크롭은 브라우저 center 다. 가드: `detail-hero-crop.test.ts` 5건 + **되돌려-검증 6건 전부 빨간불 확인**. 검증: tsc 0 · vitest 6,714 pass · build 0. 롤백: `cropFrag` 호출 3곳 + `gravity` 필드 제거(호출부가 안 넘기면 자동 no-op).
- 2026-09-02 `[UNLOCK_LOADING]` `worker/index.ts` **요청 경로 D1 읽기 2건 — 봇 OG 셀러 조회 OR 분리 · 공구 상세 API TTL 30→120** (대표 "유료 전환 전에 … 가장 이상적인 최적화, 특히 유어딜"). **배경**: 9/1 계정 D1 일일 읽기 한도(500만) 초과. 정적 감사(`docs/handoff/2026-09-02-d1-read-diet.md` §2-2)에서 ① 봇 UA 분기(`app.get('*')`)가 `/profile/*`·`/s/*` 마다 `sellers WHERE slug = ? OR username = ?` — OR 라 `idx_sellers_username` 을 못 써 **sellers 전수**(sitemap 이 셀러 200건 제출) ② `/api/group-buy/products/*` 가 상세 슬롯 중 **가장 짧은 30초** TTL 이라 크롤러(sitemap 상세 500건)가 3분 뒤 재방문마다 콜드 D1. **수정**: ① `username = ?` 점 조회 하나로 — 🩸 **pre-commit 스키마 가드가 `sellers.slug` 가 존재하지 않는 컬럼임을 잡았다**: 종전 OR 문장은 늘 예외 → catch → 기본 OG 였고, /profile/*·/s/* 크롤은 셀러 카드가 아니라 **홈 카드**를 받아 왔다(읽기 낭비가 아니라 항상 실패하던 쿼리) ② TTL 120(participants 60s·클라 폴링은 별도). 동반(비잠금): `sitemap.xml` `publicCache(3600)` · `/api/flash-deals` PRAGMA 를 D1 당 1회 메모 + `publicCache(300)` · `AdminHealthPage` 폴링 10s→60s · 부분 인덱스 `products(price / created_at) WHERE deal_only=1 AND is_active=1`(sitemap 캐시 마운트는 `index.ts` — 라우트 모듈은 유닛이 직접 불러 `caches.default` 에 못 묶는다). **⚠️ SSR inject·`caches.default`·HOT_PATHS·Cache-Control 분리·SSR head rewriter 전부 byte-불변.** 가드 `d1-request-diet.test.ts` + 주입 2건. 롤백: 각 항목 독립(OR 환원 · TTL 30 · 미들웨어 1줄 제거).
- 2026-09-02 `[UNLOCK_LOADING]` **유어딜 D1 읽기 다이어트 — 5분 cron 셋의 주기·게이트** (대표 "유료 전환 전에 유어딜, 유어애즈 모두 가장 이상적인 최적화 … 특히 유어딜. 문제가 생기면 이제 안돼"). **배경**: 9/1 23:55 KST 계정이 D1 무료 **일일 읽기 한도(500만 행, 계정 단위)** 에 닿아 소비자 API 전체 500(CF 7500). 정적 감사(`docs/handoff/2026-09-02-d1-read-diet.md`)가 본진 읽기의 3대 원인으로 5분 cron 셋을 지목: ① `cache-prewarm`(틱당 자체호출 ~34 + 동적 상세 12 + products 정규화 UPDATE 2) ② `group-buy-feed-cache`(틱당 정렬 쿼리 20, 상품 무변화에도) ③ `scheduled-cleanup`(인덱스 없는 GC 스캔 ~33). **수정(잠금 항목 무접촉 — 주기·게이트만)**: ① 동적 워밍 :00/:30 · 정규화 19:35 UTC 하루 1회 — **`HOT_PATHS`(SSR 키)는 매 틱 그대로**(잠금표 "제거 금지" 준수) ② 지문 게이트(활성 이용권 행만 읽어 카드 값 지문 → 같으면 `computed_at` 만 touch, 다르거나 60분 경과면 전체 갱신 — 라우트의 10분 신선도 규칙 불변·materialized 키/컬럼 불변) + `(status = ? OR ? = 'all')` OR 분리(결과 byte-동일) ③ 청소를 티어로(매 틱: 공구 마감·타임딜·달성 알림 / 매시 :10: 시간 규칙 / 매일 04:20 KST: GC) — 본문 byte-동일, 게이트만. daily 13개 섹션은 `scheduled-cleanup-daily.ts` 로 **이동**(섹션마다 감싼 브레이스 86줄이 1,041줄 래칫을 넘겨 CI 가 막았다 — 게이트는 호출부 `if (tiers.daily) await runDailyCleanup()` 1곳). 소품: 백업 `*/5` 슬롯 **중복 배선 제거**(08-25 전용 트리거와 시간당 8회 → 4회, 하루 ~110만 행) · 하트비트 리더 `LIKE 'cron_hb:%'` → PK 범위 · 알림톡 COUNT → 존재 프로브 + `idx_alimtalk_failures_retry` · `prospects-commission-activate` 매시 :40 · `SELECT DISTINCT id FROM users` DISTINCT 제거. **⚠️ SSR inject·`caches.default`·HOT_PATHS 목록·Cache-Control 분리·materialized 경로 전부 byte-불변.** 가드: `d1-read-diet.test.ts` 13건 + 주입 매니페스트 3건(되돌려-검증 red). ⚠️ **배포 후 판정은 하트비트 `rr`**(PR #1299 계량기)로 — 09:00 KST 리셋 창에서 작업별 읽기량을 본다. 롤백: 각 항목 독립(호출부 옵션/티어 인자 제거 → 종전 byte-동일 · `FEED_FP_KEY` 블록 제거 · 백업 슬롯 블록 복원).
- 2026-08-27 `[UNLOCK_LOADING]` **홈 부팅 JS 비용 3건 — 렌더마다 강제 리플로 · 캐시 컬럼 드리프트 · 대표색 동기 리드백** (대표 "지금 인기 이용권, 숙소 카테고리 등이 안보이네 … 이유 찾는게 매우 중요해" + "아직 로딩이 매우 느려"). **🔴 진단이 뒤집힌 사건 — 이게 이 항목에서 제일 값진 부분이다**: 처음엔 *"SSR SECTIONS 시드가 콜드 콜로에서 빈다"* 로 보고 그쪽을 고쳤는데(PR #1227, 그 자체는 유효), **라이브를 실제로 재 보니 시드는 5/5 멀쩡히 실려 있었다**(`__SSR_INITIAL_SECTIONS__` 에 두 섹션·각 4상품 정상). 그런데도 **패널이 뜨는 시각이 2,975ms** 였다. ⇒ 원인은 데이터가 아니라 **부팅 JS** 였다. **실측 근거**: JS 는 1,300ms 에 다 받았는데 앱의 **첫 XHR 이 2,488ms 에야 나갔다**(그 사이 1,165ms 가 순수 실행) · CPU 프로파일에서 **한 함수가 self 1,108ms 로 홈 최대**. **수정 3건**: ① `DesktopTopNav` 카테고리 스크롤 화살표 — **의존성 배열 없는 `useEffect`** 가 렌더마다 `scrollWidth`/`clientWidth`/`scrollLeft` 를 읽어 **강제 동기 레이아웃**을 돌고 resize 리스너를 해제+재등록했다(부팅 중 i18n·인증·쿼리가 차례로 도착하며 렌더가 수십 번 난다) → `useCallback` 고정 + 리스너 1회 + **`ResizeObserver`** 로 폭이 실제 변할 때만 재측정(i18n 라벨 지연 도착까지 덮는다 — dep 없는 effect 의 원래 목적을 정확히 대체). ② `group-buy-feed-cache`(잠금 `group-buy-public.routes` 의 cron 짝) — **`dominant_color` 가 2026-05-28 에 라이브 `buildCols` 에만 들어가고 materialized COLS 에는 3개월간 없었다.** 홈 기본 피드는 이 캐시가 서빙하므로 **그 값이 소비자에게 한 번도 간 적이 없다**(라이브 실측: 응답 50건 전부 키 자체 부재) → 캐시에도 추가 + 컬럼 부재 graceful 재시도(라우트와 동일 방식). ③ `GroupBuyFeedCard` — ② 탓에 카드가 **매번** canvas 로 대표색을 다시 뽑았고, `getImageData`(GPU→CPU 리드백)가 사진 `onLoad` 안 즉 **첫 화면 그리는 한복판에서 동기로** 돌았다(프로파일 self 166ms) → 이미 색이 있으면 건너뛰고, 뽑을 때도 `requestIdleCallback` 으로 미룬다(**기능 불변** — 색은 여전히 뽑히고 보고된다). **⚠️ 잠긴 계약 전부 승계**: `Cache-Control`/`CDN-Cache-Control` 분리·캐시키·SSR 0-RTT·`React.memo`·viewport prefetch `rootMargin:'100px'`·hover/touch/focus prefetch·aboveFold eager+fetchPriority·이미지 fade-in — **컬럼 1개 추가 + 실행 시점 이동뿐**. 가드: `home-boot-cost.test.ts` 6건 + 주입 매니페스트 3건(**6건 전부 되돌려-검증 빨강 확인**). **⚠️ 교훈 2건**: ⓐ 컬럼 드리프트 테스트를 **콤마 split** 로 짰더니 `${...}` 표현식 **안의 콤마** 때문에 `s.name` 이 통째로 사라져 **가짜 빨강**이 떴다 → 토큰 스캔으로 교정. ⓑ `check-guard-mutations --only` 는 **정규식이 아니라 부분일치**인데 `"a|b|c"` 로 부르니 **0건을 돌고도 "전부 빨간불 확인" 초록**을 찍었다 — 이 레포가 반복해 당한 "검사가 실패할 수 없음" 을 하필 그 검사기 자신이 하고 있었다 → 0건 매칭이면 exit 1. ⚠️ **배포 후 실측이 유일한 판정**(이 컨테이너는 CPU 스로틀이라 절대 ms 는 부풀려져 있다): 패널 표시 시각이 2,975ms 보다 당겨졌는지 + `/api/group-buy/products` 응답에 `dominant_color` 키가 생겼는지. **④ (대표 `AskUserQuestion` 승인 "한가할 때로 미루기")** 맨 위 카드 prefetch 를 idle 로 — 실측상 카드 6장의 XHR 6개 + 상세 청크 4개가 `/api/sections`·`/api/banners`(=지금 화면에 필요한 것)와 **같은 순간 2,488ms 에 동시 발사**됐다. **미루기이지 제거가 아니다** — 사용자가 카드를 읽고 누르기까지 최소 1~2초라 "클릭 시 0ms" 목적은 그대로 달성되고, 잠금표가 지키는 성질(클릭 시 fetch 워터폴 방지)도 보존된다. `rootMargin:'100px'`·화면 밖 카드 IntersectionObserver 는 불변. 롤백: 각 항목 독립(① dep 배열·ResizeObserver 제거 ② `dominantColorFrag` → `''` ③④ `requestIdleCallback` → 직접 호출).
- 2026-08-27 `[UNLOCK_LOADING]` `vite.config.ts` **manualChunks — 소비자 임계 경로에서 역할 전용 청크 분리** (대표 "계속 작업" — 번들 마운트 지연 후속). **배경(라이브 실측)**: 홈 첫 화면 JS 가 압축 384KB / 해제 1,396KB 였고, 그 안에 **셀러 대시보드 65KB**(`app-seller-components` — SellerLayout·BulkUploadModal·SellerKpiDashboard…)와 도매 훅·admin/wholesale/marketing 컴포넌트가 섞여 있었다. 어느 것도 소비자 화면에서 렌더되지 않는다. **원인은 큰 코드가 아니라 작은 공용 모듈이 역할 전용 봉투 안에 있었던 것**: ① 공용 `RoleGate.tsx` → `shared/seller-roles.ts`(116줄 역할판별 SSOT — CLAUDE.md 가 모든 UI 에 강제하는 그 파일)가 규칙이 없어 Rollup 이 셀러 컴포넌트들과 한 청크로 묶었다 ② `lib/seller-tracking.ts` 는 **이름만 셀러**이고 실제론 쇼핑·상품상세·공구상세·이용권상세 **4개 소비자 페이지**가 쓰는 70줄 URL 추천코드 캡처 유틸인데 2026-05-28 주석의 전제("셀러 페이지만 사용")가 틀린 채 셀러 청크에 있었다. **수정(규칙 추가만 — 제거 0)**: `shared/seller-roles`·`lib/seller-tracking` → `app-shared` · `components/admin/` → `app-admin-components` · `components/wholesale/`+`wholesale-catalog/WholesaleLogo`+`wholesale/wholesale-theme` → `app-wholesale-components`(신규) · `MarketingDashboardShell`+`marketing/dashboard-tabs` → `app-marketing`(신규). **실측**: 홈 폐쇄 **26청크 972KB → 22청크 840KB raw(−132KB)**, `app-components` 211→166KB, `app-seller-components`/`app-wholesale-hooks`/`app-misc` 가 홈에서 완전히 사라짐. `/browse` 의 65KB 도 제거. **⚠️ 기존 잠금 항목(env-validator/AdminLayout/AgencyLayout/SellerLayout 분리·폴더별 청크·hoisted 훅) 전부 불변 — 규칙 추가만이고 critical path 는 줄어들었다**(critical-chunks 17개 동일, bundle 예산 GREEN). **영구 가드 신설 `check-surface-role-leak.mjs`**: 기존 `check-critical-chunks` 는 **`index.html` 만** 보는데, 2026-07-12 부터 워커가 표면별 modulepreload 를 **추가 주입**한다(route-chunk-map) — 홈의 실제 첫 페인트는 둘의 합이고 **후자는 어느 가드도 안 보고 있었다.** 그 사각지대가 이 누수가 살던 자리다. 되돌려-검증 2건(seller-roles·seller-tracking 각각 원복 → 빨간불) 확인. 검증: tsc 0 · build 0 · 단위 5,957 pass(472파일) · guard-registry 110 · audit-registry 90. ⚠️ **남은 것**: `app-components` 가 아직 166KB(58모듈)이고 그 안에 소비자 첫 화면이 안 쓰는 것이 더 있다 — 다만 정확한 판정에는 번들러의 실제 모듈→청크 그래프가 필요하다(이번에 만든 정적 추적기는 `export … from` 재수출과 앱 셸 경유를 놓쳐 오탐이 났다). 별도 작업. 롤백: 추가한 규칙 줄 제거(가드는 그때 빨간불이 되므로 함께 판단할 것).
- 2026-08-19 `[UNLOCK_LOADING]` **홈 이용권 카드 그루폰식 통일 + hover 캐러셀 · 상세 갤러리 2단** (대표 시안 — "이용권 UI 이렇게 다 바꿔줘. 인기 이용권 카드와 동네딜 카드가 다르네. 마우스 올리면 좌우로 사진 돌려볼 수 있어" + "상세도 사진 여러 장이면 그루폰처럼"). **배경**: 홈에 카드가 **두 벌**이었다 — 섹션(`HomeSections` 자체 `DealCard`: 이미지+제목+가격) ↔ 피드(`GroupBuyFeedCard`: 평점·거리·할인까지). 한쪽만 고쳐지니 계속 갈렸다. **수정**: ① 섹션의 자체 카드를 **삭제**하고 피드 카드를 재사용 → 카드 1벌. ② 이미지 영역을 신규 SSOT `components/deal/DealCardMedia.tsx`(hover 좌우 화살표 캐러셀 + 도트)로 교체. ③ 카드 본문을 그루폰 위계로 재배열(머천트 → 제목 → 주소·거리 → ★평점 → 정가취소선·판매가·할인 pill). ④ 상세(`GroupBuyDetailPage`)는 갤러리를 `pages/group-buy/DetailGallery.tsx` 로 추출하고 **사진 2장 이상일 때만** PC 에서 [좌 대형 + 우 썸네일 + `+N` 전체보기 모달]로 편다(1장이면 종전 그대로 — 빈 칸을 만들지 않는다). 모바일 스와이프+도트는 불변. **⚠️ 잠긴 계약 전부 승계**: `React.memo`·viewport prefetch `rootMargin:'100px'`·hover/touch/focus prefetch·**aboveFold eager+fetchPriority**·이미지 fade-in(opacity transition)·대표색 추출(`extractDominantColor`/`reportDominantColor`) — 캐러셀이 커버에 그대로 적용한다. **💰 트래픽 보호가 1원칙**: 카드 50개 × 갤러리를 다 받으면 첫 화면 요청이 몇 배가 되므로 (a) **커버만 즉시 로드**하고 사용자가 *실제로 넘긴 장면*만 `<img>` 생성(`seen` 집합 — hover 만으론 아무것도 더 안 받는다) (b) **서버가 잘라 보낸다**: 리스트 응답에서 커버 중복 제거 후 **최대 3장**(`CARD_GALLERY_MAX`), materialized cron(`group-buy-feed-cache`)도 **저장 시점에 동일 규칙**(안 자르면 캐시 row 가 원본 전량을 안고 그 크기를 hit 마다 파싱). **SELECT 에 `p.images` 추가**(group-buy-public buildCols · feed-cache COLS · section-rules CARD_COLS) — Cache-Control/CDN-Cache-Control 분리·캐시키·`group_buy_tiers` 서버 parse·SSR 0-RTT·materialized 경로 전부 불변(body enrich 만, 2026-05-30 current_price·2026-07-02 fcfs enrich 와 동일 additive 패턴). 가드: `deal-card-gallery.test.ts` 13건(화살표 preventDefault/stopPropagation·안 본 장면 미로드·aboveFold 계약·서버 자르기·모달 z-index SSOT·1장이면 2단 미전개) + `check-guard-mutations` 매니페스트 2건 등록. **되돌려-검증 4건 전부 red 확인**(화살표 가드/트래픽 보호/서버 자르기/섹션-피드 위임). ⚠️ 교훈: 위임 가드를 처음 `/GroupBuyFeedCard/` 로 썼더니 **import 줄 때문에** 카드를 `<div>` 로 바꿔도 초록이 떴다 → `<GroupBuyFeedCard\b`(JSX) 로 교정. 검증: tsc 0 · build 0 · vitest 5,79x pass. ⚠️ **배포 후 실측 권장**: `/api/group-buy/products` 응답 크기(갤러리 3장 캡이 실제로 먹는지)와 홈 첫 화면 이미지 요청 수. 롤백: 각 항목 독립 — 카드 통일은 `HomeSections` 의 `<GroupBuyFeedCard>` → 옛 `DealCard` 환원, 캐러셀은 `<DealCardMedia>` → 기존 `<img>` 블록 환원, 상세는 `<DetailGallery>` → 인라인 스와이프 환원, 서버는 `p.images` 3곳 제거.
...
- 2026-08-11 `[UNLOCK_LOADING]` `cf-image.ts` **cdn-cgi 경로에 박히는 원본 URL 의 `%` 이스케이프** + `worker/index.ts` **별칭 301 쿼리 병합** (대표 "모두 빠짐없이 다 AB테스트" — 라이브 브라우저 스윕). **배경(실측)**: 네이버 지도 사진 중 **파일명이 한글**인 것은 URL 이 이미 EUC-KR 퍼센트 인코딩돼 있다(`…/%B8%DE%B4%BA%C6%C7_….jpg`). 이걸 그대로 `/cdn-cgi/image/<옵션>/<URL>` **경로**에 이어붙이면 리사이저가 경로를 디코딩해 원본 fetch 주소가 달라지고 **404** 가 난다 — 그리고 **`onerror=redirect` 안전판이 안 걸린다**(404 를 리사이저 자신이 반환하기 때문). 실측: `raw 200/429KB · cdn-cgi 그대로 404/151B · %→%25 한 번 200/151KB`. 라이브 영향: 갤러리에 `%` 를 가진 활성 상품 **134개**(전부 데모 이용권, 커버는 1개) — 상세 갤러리 사진이 안 뜬다. **수정**: 새 헬퍼 `cdnCgiSafe(src)` = `%` 가 있을 때만 `%25` 로 치환, **`CDN_CGI_VERIFIED` 분기 1곳에만 적용**. ⚠️ `encodeURIComponent` 는 쓰면 안 된다(`:` `/` 까지 먹어 URL 이 통째로 깨진다). **`SUPPORTED_HOSTS`/`EXTERNAL_PROXY_HOSTS`/`HOTLINK_BLOCKED_HOSTS`/`CDN_CGI_VERIFIED` 목록·Save-Data·`/api/media` 분기·워커 프록시 분기 전부 byte-불변 — 제거 0, URL 조립만 수정.** 동반: `worker/index.ts` 별칭 301 이 `origin+alias+url.search` 로 잇는데 **목적지가 이미 쿼리를 갖는 별칭**(`/meal-vouchers` → `/?category=meal_voucher`)이 새로 생겨 `?a=1?b=2` 가 되던 것을 `&` 병합으로. **SSR inject·0-RTT·`caches.default`·#root 로더·기존 별칭 전부 불변.** 검증: tsc 0 · 신규 `consumer-surface-fixes-2026-08-11.test.ts` 27 pass(**되돌려-검증 확인** — `cdnCgiSafe` 를 무력화하니 빨강) · audit-gate 87 GREEN. ⚠️ **배포 후 판정**: 데모 상세(`/group-buy/2846`)에서 `/cdn-cgi/image/…%25B8…` 요청이 200 인지 curl. 롤백: `cdnCgiSafe(src)` → `src` 환원 + 301 삼항 제거.
...
## 📔 Notion 자동 갱신 (2026-08-13 대표 지시 — "구현이 될 때마다 업데이트를 Notion 에다가")

**의미 있는 구현·수정을 머지(또는 배포)했으면 같은 세션에서 Notion 에 한 줄 남긴다.** 레포 안의 인계
문서(`docs/handoff/**`)는 *다음 세션*을 위한 것이고, Notion 은 **대표가 보는 창**이다 — 둘 다 필요하다.

**어디에 쓰나** (유어팀 워크스페이스, MCP `mcp__Notion__*`):

| 대상 | ID | 언제 |
|---|---|---|
| 개발 업데이트 로그 (DB) | 데이터소스 `571b32b9-cefb-4453-9b84-78928c619451` | **매번** — `notion-create-pages` 로 행 1개 |
| 🚀 서비스 개발 현황 (허브) | `3bb0adee-652b-8105-a518-cb4d2983b26e` | 서비스 상태·판단 대기가 바뀔 때 |
| 🎟️ 유어딜 | `3bb0adee-652b-8126-ab32-f159ca7a89be` | 구현 목록·꺼진 기능이 바뀔 때 |
| 🏪 공구 서비스 | `3bb0adee-652b-81d8-a8b2-d8a22069cc07` | 〃 |
| 📣 유어애즈 | `3bb0adee-652b-812b-bab4-d5c9e7973365` | 〃 (수치는 **라이브 실측**으로 갱신) |
| 🏭 유통스타트 | `3bb0adee-652b-81e5-8025-ede6cc30c9f2` | 〃 |

**로그 행에 넣을 것**: `작업`(제목) · `서비스`(유어딜/공구 서비스/유어애즈/유통스타트/공통·인프라 —
**넷 중 어디인가를 먼저 판별**) · `유형` · `date:배포일:start` · `무엇이 달라졌나` · `머니 경로`(체크박스) ·
`PR`(github URL).

> 🔑 **`무엇이 달라졌나` 는 사용자·운영 관점 한 문장으로 쓴다.** 커밋 메시지를 복사하지 말 것 —
> 대표가 읽는 자리다. "resolveGbPricing 을 order.routes 에 배선" (✗) → "화면엔 특가인데 결제는 상시가로
> 되던 것을 고쳤습니다" (○).

**쓰지 않아도 되는 것**: 문서/인계만 바뀐 커밋, 리팩토링만 한 커밋, 가드 추가만 한 커밋(단 그 가드가
막는 사고가 대표에게 의미 있으면 쓴다). **애매하면 쓴다** — 빠진 것보다 많은 게 낫다.

⚠️ **MCP 미연결 세션이면 건너뛰고 handoff 에 "Notion 미기록"을 남긴다.** 없는 도구를 기다리지 말 것.
...
✅ **07-21 의 12,533 은 규명됐다** (2026-08-09 #1106 — 이전 판 "아직 설명 못 한다"를 대체):
07-20 에 키워드 862개 생성·202개 활성화된 **다음날**이다. 신선한 키워드의 첫 회차가 기존 글 백로그를
수확한다(고갈 곡선 실측: 회당 저장 5~14회차 14.3 → 30회차+ 2.4). 그 조건을 상시화한 것이
`MAX_AUTO_KEYWORDS` 60→120 + 수율 은퇴(+livelock 차단·30일 가석방 — 후속 PR). 판정 ~08-11.
⇒ 무료 발굴량 공식: **레인 안정성 × 키워드 신선도.** 여전히 "하루 N건 영구" 단정은 금물이다.

💡 활성화의 답도 나왔다(2026-08-09 실측): 활성 키워드는 실행당 **157.6명**, 비활성 출신은 **49.7명**
(3분의 1) — **무차별 활성화 확대는 수율 3배 낮은 키워드를 섞는 일**이다. 늘릴 가치가 있는 것은
"신선한(안 돌아 본) 승격 후보"이고, 그게 cap 60→120 처방의 근거다(위 ✅ 참조).
...
시간당 12회 × 회차당 9키워드(2026-08-11 대표 승인 6→9) = **108키워드/시간**이 무료의 구조적 상한이다.
...
회차 수가 같아도 **키워드가 신선하면 수확이 뛴다.** 그 여지는 2026-08-09 #1106 이 실측·상시화했다
(cap 120 + 수율 은퇴 — 위 ✅ 참조). 판정 ~08-11, 기대치는 기준선 5,700 → 8천~1.2만.
...
> 🌙 **2026-08-12 대표 승인("응 해") — YT 쿼터 소진 회차만 예산까지 확장.** YT 검색 쿼터(일 90회)는
> 이른 시간에 소진되고 그 뒤 회차는 네이버 전용인데, 키워드당 비용이 3분의 1(실측 29/9≈3.2)인데도
> **폭 9에서 멈춰 `spent 29 / 56`** 으로 끝났다(예산 27 유휴). 그 회차만 `COLLECT_KEYWORDS_PER_ROUND_NAVER_ONLY`
> (18, 실제 상한은 예산·계획폭)로 넓힌다. **YT 살아있는 회차는 9 그대로** — 지금도 예산이 캡(56/56)이라
> 넓혀도 안 늘고 YT 쿼터만 빨리 태운다. 해제 근거: 측정 22,764/일 > 유입 6,434/일(3.5배) · 실백로그 ~8시간분 ·
> `blocked 0` · 네이버 쿼터 0.3% ⇒ 동결 사유("측정이 병목")가 더 이상 사실이 아니다.
> ⚠️ 롤백은 그 상수를 9 로 되돌리는 것 하나다. 차단(`ads_naver_crawl_block.blocked > 0`) 뜨면 즉시.
> 정책 SSOT 는 `influencer-round-width.ts`(2026-08-12 분리).
>
> ✅ **발굴 폭 잠금은 2026-08-11 대표 승인("폭 9로 올려")으로 6→9 해제됐다** — 그 주석이 명시한
> 해제 조건("측정 처리량이 올라간 뒤")을 숫자로 채운 뒤였다: 측정 8,018/일(샤딩) > 유입 5,045/일 ·
> 백로그 감소 · blocked 0, 그리고 §16 판정이 "폭이 캡"임을 규명(경보 원문 "예산 37/56 · 키워드 6/16").
> ⇒ **9 초과 상향은 다시 대표 판단 사항이다**(테스트가 9 를 상한으로 잠금). 올리려면 (a) 측정 처리량이
> 받쳐 주는지 (b) `ads_naver_crawl_block.blocked` 가 0 을 유지하는지 **두 값을 먼저 보고** 근거와 함께.
...
> 현재 **100개** 불변식 GREEN (서비스분리·인증세션RBAC·머니패턴·DB스키마·상품종류·UI테마·시각KST·배포·번들). 상세: `docs/AUDIT_INVARIANTS.md`.
...
**🛍️ 유어샵 (2026-09-02 대표 확정 — 모바일 안3 · PC 안P1)**: 주인 상단 안내 띠 없음. 주인·방문자 차이는 헤더 버튼 **한 자리**뿐([유어샵 편집] 블루 ↔ 없음). **방문자에게 팔로우 같은 대체 버튼을 두지 않는다**(대표: "그냥 방문자는 안보이면 되잖아"). 헤더는 아바타+왼정렬(배너 히어로 렌더 없음), 카테고리 칩은 지도 B안과 같은 그림(핀 7개 이상), PC 는 `/u/:handle` 한 세그먼트만 액자를 벗고 좌 300px 프로필 열 + 우 3열. ⚠️ 2026-07-07 "아바타·스탯 삭제"는 이 시안으로 갱신됐다. SSOT: `docs/design/ticket-completion-reference-2026-09.md` §9 · 가드 `ushop-a3-p1.test.ts`.

**🏝️ 라이트 섬 `light-island` (2026-09-02 대표 확정 "안A · 다크에서도 패널은 흰색")** — 배경이 테마와 무관하게 정해진 자리(잉크 색면 위 홈 패널 `ur-home-panel`, 지도 타일 위 오버레이)는 이 클래스 하나로 안쪽 `dark:` 유틸을 전부 끄고(`tailwind.config` `darkMode: ['variant', '&:is(.dark *):not(.light-island *)']`, 섬 밖 출력은 byte-동일) **전역 라이트 입력 규칙(색·placeholder·autofill, `!important`)을 켠다**. 손으로 `dark:` 를 빼거나 `!important` 로 덮지 말 것.
> 🔴 **`light-island`(클래스) ≠ `light-fixed`(주석). 늘 밝은 표면에는 반드시 클래스를 붙인다.** `light-fixed` 는 `check-theme-consistency` 를 면제하는 **부표**일 뿐 런타임엔 아무 일도 안 한다. 2026-09-03 에 지도 검색창이 주석만 달고 클래스가 없어서, 전역 `.dark input`(0,5,1)이 `text-gray-900`(0,1,0)을 이겨 **흰 배경 위 흰 글자**가 됐다(실측 1.1:1 — 대표 신고 "글자가 또 하얘"). 클래스 유틸로는 이 싸움을 못 이긴다. 회귀 감시: `scripts/check-dark-contrast.mjs`(실제 렌더 측정). 가드: `home-panel-light-island.test.ts` + 주입 매니페스트 2건.
...
(`Leonxlnx/taste-skill`, MIT. 대표가 지정해 레포에 벤더링 — 세션마다 자동으로 잡힌다.)

가장 자주 걸리는 것 다섯 개(이 레포에서 실제로 전부 위반했었다):
- **em-dash `—` 0개.** "적게 쓰기"가 아니라 0개다. 마침표·쉼표·괄호·`-` 로 바꾼다.
- **섹션번호 eyebrow 금지** (`01 — 서비스`, `05 · 정산`). 제목만으로 충분하다.
- **eyebrow 총량 ≤ ceil(섹션수/3)**. 모든 섹션 위에 작은 대문자 라벨을 다는 것이 가장 흔한 AI 티다.
...
🛡️ **자동 강제 (2026-09-01 신설)**: `scripts/check-anti-slop-copy.mjs` — 위 다섯 개 중 **기계로 판정 가능한 셋**
(em-dash · 섹션번호 뱃지 · 한글 라벨의 `uppercase`)을 대외 표면에서 0 으로 동결(verify.yml + audit-gate strict).
나머지 둘(eyebrow 예산 · 레이아웃 계열 반복)은 **렌더해서 눈으로** 봐야 한다 — 가드가 못 한다.
> 이 가드는 **실제 사고 뒤** 생겼다: 이 절이 다섯 규칙을 적어 두고도 강제가 없어서, 대외 랜딩 `/about` 이
> 다섯 개를 **전부** 위반한 채 배포돼 있었다(2026-09-01 실측). 룰만 있고 강제가 없으면 결국 놓친다.

적용 대상: 제안서·랜딩·소비자 마케팅 표면. **어드민/셀러 대시보드는 대상 아님**(스킬 자신이 dashboard 제외를 명시).
...
| 명칭 | 정의 | 코드 실체 |
|---|---|---|
| **유저** | 회원가입한 누구나. **유어샵(`/u/{handle}`) 자동 생성**, 담기(소개)·구매 가능 | `users` + handle |
| **유어샵** | 그 사람의 **진열대**(`/u/{handle}`). 담은 이용권·내 가게 이용권이 한 곳에 뜬다. 매장은 *설정할 때 고르는 스코프*이지 진열 단위가 아님 | `CuratorPage` + `SellerPublicPage` |
...
> 🏷️ **2026-08-26 대표 확정 — "링크샵" → "유어샵" 으로 통일.** 이유: ① 유어딜·유어애즈·유어팀과 **브랜드 패밀리**가 이어진다 ② "링크샵"은 무엇을 파는지도 누구 것인지도 안 알려준다. 사용자-가시 "링크샵" **src 552건 + 6개 언어 90건** 일괄 치환. **URL(`/u/{handle}`)은 불변** — 이미 공유된 링크가 하나도 안 깨진다. **코드 식별자(`linkshopPath`·`useLinkshopPath`·`nav.linkshop`·`LinkshopVisitorRails`·`isLinkshopSurface`)는 불변**(한글 라벨만). 다국어에서도 **번역하지 않고 `UrShop`** — 유어딜(UrDeal)과 같은 브랜드 취급. 아래 audit log 의 "링크샵" 표기는 *작성 당시 historical record*(소급 변경 X).
>
> 🏷️ **2026-08-26 대표 확정 — 사람을 "인플루언서 / 대행사" 로 나누지 않는다. 행위 2개로 말한다.** 둘은 실제로 정반대 일을 한다(인플루언서=**손님**을 만든다 / 대행사=**매대**를 만든다)는데 화면에선 똑같이 "남의 가게로 번다"로 보여 모호했다. **신분이 아니라 행위**이고, 경계는 이미 코드에 있다 — `seller_operators` 위임 권한 유무.
> - **담기(소개)** = 누구나. 남의 이용권을 내 유어샵에 담는다 → 팔리면 소개 커미션.
> - **운영(대행)** = 매장 사장님이 위임한 사람만. 그 매장을 대신 판다 → 운영 몫.
>
> 유어샵에는 셋이 **한 진열대**에 뜨고 배지만 다르다: `내 가게`(owner) · `관리 중`(operator) · `추천`(핀).
> ⚠️ 이건 새 규칙이 아니라 **이미 있는 규칙("사람 지칭에 인플루언서/크리에이터/큐레이터 금지")을 지키는 것**이다.
...
> 🔄 **2026-08-31 대표 재검토 — 위 원칙의 전제가 바뀌었다(확정 아님, 방향 메모).** 대표:
> *"매장 promo 재원은 이제 5%가 아니라 수수료 10%도 받기 때문에 지금은 변경된 얘기 같아."*
> 07-08 원칙은 **플랫폼 수수료가 5% 하나**이던 시절의 계산이다. 그 뒤 채널별 요율이 들어와
> (`fee_channel_rates_enabled=true` · **직접 10% / 중개 5%**) 직접 입점 매장에서는 원래 폭이 훨씬 넓다.
> ⇒ **"5% 는 커미션에 안 쓴다"를 지금 그대로 적용하지 말 것** — 대표가 전제 변경을 명시했다.
> 다만 **폐기된 것도 아니다**: 중개(5%) 매장에서는 07-08 계산이 여전히 그대로 유효하다.
> 재확정 전까지는 `promo_funding_source` flip 을 밀지 말고, 아래 8-31 결정 두 개로 간다.
>
> ⭐ **2026-08-31 대표 확정 2건** (라이브 실측 후 — 중개 매장은 `5% − PG 2.75% − 영입 2% = +0.25%` 로
> 사실상 0, 커미션 하나만 더 겹치면 적자):
>   1. **영입 2% 는 직접 입점(`store_channel='direct'`) 매장에만.** 적립 코드
>      (`influencer-store-intro-commission.ts` compute/credit 양쪽)에 채널 조건 추가.
>      🔴 **미지정 매장 처리를 먼저 정할 것** — 실측상 매장 11곳 중 채널이 박힌 곳은 1곳(그것도
>      brokered)뿐이라, 순진하게 `=== 'direct'` 로 막으면 **아무도 못 받는다**. 방향은
>      `fee-resolver` 의 fail-soft 와 일관되게: *매장에서 떼는* 쪽은 관대, *우리가 주는* 쪽은 엄격.
>   2. **예산 아비터를 켠다**(`commission_budget_enabled`) — 커미션 총합이
>      `플랫폼수수료 − PG준비금` 을 구조적으로 못 넘게. ⚠️ **프로덕션에 바로 켜지 말 것**:
>      위 2026-07-04 audit log 가 활성 전 staging 실결제를 조건으로 달아 뒀다.
> 둘 다 머니 경로 → **단독 세션 + staging 실결제.**
>
> ⭐ **2026-09-07 대표 확정(결재 Q4-2, "기본안대로 모두 승인") — 07-08 원칙 폐기.** 성장 커미션은 플랫폼 수수료 안에서 유어딜이 부담하되 주문당 총합 ≤ `수수료 − PG 준비금` 을 예산 아비터가 강제한다. 전 축 owner-promo flip·불변식 #44 는 추진하지 않는다. **아비터 ON 은 staging S1 뒤 대표가 켠다**(코드는 있고 기본 OFF). SSOT: `urdeal-platform-model.md` §5-3 ⭐ 2026-09-07 · 결재 파일 `docs/decisions/2026-09-07-actor-benefit-conflicts.md`.
...
| **텍스트 가드가 자기 주석 제거기를 만듦** | `check-comment-stripper.mjs` (래칫) | `verify.yml` (strict) + audit-gate | 2026-09-13 — 이 레포가 같은 자리에서 **네 번** 넘어졌다. 소스를 문자열로 읽는 가드는 주석을 걷어내야 하는데, 그 걷어내기를 테스트마다 `.replace(/\/\*[\s\S]*?\*\//g,'')` 로 새로 쓰고 있었다. 이 정규식은 **문자열·정규식 리터럴 안의 `/*`** 를 블록주석 시작으로 읽어 다음 `*/` 까지 먹는다 — 실측 `worker/index.ts` 코드의 **76%**, `wholesale-main.routes` **81%** 증발. 증발한 구간은 검사에서 사라지므로 `not.toContain(...)` 류가 **무조건 통과**하고, 실패가 아니라 **초록불**이라 아무도 모른다. ⇒ SSOT `src/tests/helpers/source-text.ts` 의 `stripComments`. 🩸 **그리고 그 SSOT 자신에게 JSX 구멍이 있었다**(같은 날 발견): `</div>` 의 `/` 를 정규식 시작으로 읽어 다음 `/` 까지 삼켰고, 그 안의 주석이 전부 살아남았다 — `RegisterPage.tsx` 는 15,428자 중 **141자만** 지워지고 있었다(= 사실상 주석 제거를 안 한 것). `.tsx` 전부가 이 구멍에 걸려 있었다. 수리: `prev !== '<'` + "그 줄에서 안 닫히면 문자열·정규식이 아니다" 안전판(JSX 본문의 `don't` 도 함께 해소). ⚠️ **못 보는 것**: 줄 단위 필터형 자체 제거기(지뢰는 안 밟지만 JSX 주석 둘째 줄부터를 놓친다) · 남은 98개는 **동결만**(줄이는 건 자유, `--rebaseline`). 예외 `comment-stripper-ok` |
| **CI 는 막는데 로컬은 안 막음**(57분 피드백) | `check-local-ci-parity.mjs` + **`pre-push` 게이트**(`scripts/pre-push-gate.mjs`, 실측 ~20초) | `verify.yml` (strict) + audit-gate | 2026-09-14 대표 *"왜 계속 이렇게 잘 안되는거지? 시간도 많이 걸리고"* — 재 보니 **가드 121개 중 91개가 CI 는 차단하는데 로컬은 안 막고 있었다**(pre-commit 에 아예 없음 63 · `|| true` 로 경고만 28). 그래서 사소한 위반 하나가 **CI 한 바퀴 = 실측 57분**(그중 45분이 가드 주입 검증 한 스텝)을 태웠고, 그날 하루에 그 루프를 **두 번** 돌았다 — ① 테스트가 자체 주석 제거기를 만든 것 ② `BrowsePage.tsx` 625줄. **둘 다 로컬에서 1초면 알 수 있었다.** 실제로 91개를 전부 재 보니 **합쳐 20초**다 — 느려서 못 넣은 게 아니라 아무도 안 넣은 것이다. ⇒ `pre-push` 훅이 `verify.yml` 의 strict 가드를 **전부** 돌린다(커밋은 계속 가볍게 두고 57분 루프만 끊는다). 🔑 **목록을 손으로 관리하지 않는다** — `verify.yml` 에서 매번 뽑으므로 새 가드가 조용히 새지 못한다. 제외는 `EXCLUDE` 3건뿐이고 각각 **왜 로컬에서 못 도는지** 적혀 있다(전수 주입 45분 · `dist/` 필요 2건). ⚠️ **못 잡는 것**: 훅이 실제로 설치됐는지는 레포가 못 본다(`.git/hooks` 는 클론에 안 딸려 온다) — 원격 세션은 세션마다 `bash scripts/install-git-hooks.sh` 를 다시 돌릴 것. 우회 `SKIP_PREPUSH_GATE=1`. 🩸 **그리고 이 처방을 올린 커밋 자신이 곧바로 같은 클래스를 밟았다** — `verify.yml` 에 스텝을 더하며 들여쓰기를 깨뜨렸는데(스텝 하나가 2칸 + `continue-on-error` 중복) 결과가 **CI 실패가 아니라 '잡 0개로 즉시 종료'** 였다. PR 화면엔 Verify 가 **뜨지도 않은 채** `Cloudflare Pages ✅` 하나만 남아 훑어보면 통과처럼 보인다(이 레포가 반복해 당한 **조용한 부재**). 게이트도 통과시켰다 — 목록을 **줄 단위 정규식**으로 뽑아 깨진 YAML 에서도 그럴듯한 94개가 나왔기 때문이다. ⇒ **R4** 모든 워크플로가 YAML 로 파싱되는가(깨지면 빨간불이 아니라 부재) · **R5** 정규식이 줍는 이름을 파서가 전부 strict/warn 으로 분류하는가(파서 눈먼 자리 차단) 추가, 그리고 **판정 기준을 정규식 → 파싱된 구조로** 옮겼다. 그 교체가 즉시 실물 오류를 드러냈다: `check-bundle-size.mjs` 는 CI 에서 `continue-on-error: true`(경고)인데 3줄 창 밖이라 정규식이 **차단으로 오해**해 게이트가 붙들고 있었다 — 반대 방향이면 CI 가 막는 가드가 조용히 샌다. 주입 6건 전부 빨간불 확인 |
| **주입 앵커가 낡음**(가드가 지키는 척) | `check-stale-mutation-anchors.mjs`(**전수, ~1초**) | `verify.yml` (strict) | 2026-09-26 — `check-guard-mutations` 는 두 가지를 잡는다: ⓐ **헛도는 가드**(결함을 심었는데 통과) ⓑ **낡은 지도**(`find` 가 소스에 없다 = 코드가 옮겨졌다). ⓐ는 테스트를 실제로 돌려야 해서 전수 실행이 길고, 그래서 PR 은 `--changed` 로 좁혀 돈다(📏 **실측 2026-09-26 job 108341703677 — 주입 1,963건: 그 스텝 하나가 77.4분 = Verify 전체 91.0분의 85%.** 2026-09-08 의 37분 24초는 주입이 더 적던 때의 값이고, 매니페스트가 자라면 이 값도 같이 자란다). **그런데 ⓑ는 테스트가 필요 없다** — 문자열이 파일에 있는지 보면 끝이다. 그걸 그 긴 스텝에 묶어 둔 탓에 **`--changed` 범위 밖이면 PR 을 통과하고 main 에서야 빨간불**이 났다. 그날 실제로 두 건: `pinReturn` 도입으로 `{tool === 'pin' && <PinSheet …}` 앵커가 사라진 것(PR CI 한 바퀴를 태우고서야 드러났다)과, 그걸 고치는 사이 등록이 마이로 옮겨가며 `enterSeat('/seller/meal-voucher/new')` 가 사라진 것. ⇒ ⓑ만 떼어 **전수·무테스트**로 검사한다 — 같은 CI 실행에서 **0초**(1,963건 · 637파일). 77분 안에 섞여 있던 판정 하나가 떼어 내니 무료가 됐다. 분할 매니페스트(`scripts/mutations/*.mjs`)와 런너 안 **인라인 배열**을 둘 다 읽는다 — 인라인을 빠뜨리면 950건이 통째로 눈먼다. ⚠️ **못 잡는 것**: ⓐ는 여전히 `check-guard-mutations` 의 몫이다(앵커는 멀쩡한데 아무것도 안 지키는 경우는 결함을 심어 봐야 드러난다). 🧭 앵커가 낡으면 **지우지 말고 불변식으로 재조준**할 것 — 지키려던 것은 대개 그대로 살아 있다. 예외 없음(앵커가 없으면 그 주입은 아무것도 검증하지 않는다). 🔴 **함께 드러난 비용 구조(2026-09-26, 미해결)**: `guard-mutations-scope` 는 `scripts/**` 가 바뀌면 전수로 폴백하는데, 주입 매니페스트가 바로 `scripts/mutations/` 에 있다 — **새 가드를 만들 때마다(= 주입을 추가할 때마다) 그 PR 은 77분짜리 전수를 태운다.** 레포 룰이 "새 가드를 만들면 주입을 한 줄 추가" 를 요구하므로 이 비용은 규칙을 지킬수록 커진다. 🔬 **기제(실측으로 정정)**: 커밋 단위가 아니라 **브랜치 단위**다 — `changedScope` 는 `merge-base(origin/main, HEAD)..HEAD` 를 보므로, 브랜치 이력 **어딘가에** `scripts/**`·`verify.yml` 이 한 번이라도 들어가면 **그 뒤 모든 푸시가 전수를 태운다**(실측: 인계 `.md` 하나만 바꾼 커밋도 45.1분). ⇒ "주석·문서만 바뀐 경우 제외" 는 해결책이 **아니다**. 고치려면 ① 폴백 조건에서 `scripts/mutations/**` 를 떼거나(대신 `--changed` 가 새 주입만 골라 돌게) ② 전수를 야간(`guard-mutations-full.yml`)에만 두거나 ③ 폴백 판정을 **이번 푸시가 바꾼 범위**로 좁힌다(그 경우 앞 커밋에서 바꾼 가드가 뒤 푸시에서 검증 안 되는 구멍이 생긴다 — 교환을 함께 판단할 것). **CI 정책 변경이라 이 세션이 임의로 바꾸지 않았다.** |
| **가드가 실패할 수 없음**(헛도는 검사) | - | PR=`check-guard-mutations.mjs --changed`(`verify.yml`) · **전수=`guard-mutations-full.yml`(main push + 야간)** | 2026-08-02 — 이 레포의 반복 사고는 "검사가 실패한다"가 아니라 **"검사가 실패할 수 없다"** 이고, **하루에 세 번** 났다: ① `배열을 reverse 해도 같은가` — 15개 배열의 reverse 는 i→14−i 라 **홀짝이 보존**돼 2조 분할에 영향 0(정렬을 통째로 지워도 초록) ② `phase 이름이 조를 흔든다` — 이웃이 `?` 앞에서 이미 갈려 **실제로는 안 일어나는 일**이었다 ③ `한 회차가 예산을 넘지 않는다` — 픽스처에 **일 1회 레인이 하나도 없어** `always` 가 늘 빈 배열(라이브에선 예산 8 에 12개가 떠 3개가 CPU 한도로 잘리는데도 초록). **셋 다 손으로 주입해 보고서야** 알았다 — 손으로 하면 다음 세션은 안 한다. 주입 매니페스트(현재 9건)를 두고 CI 가 대신 깨뜨린다. 두 모드를 잡는다: **헛도는 가드**(결함을 심었는데 통과) · **낡은 지도**(`find` 가 소스에 없음 = 코드 이동). 🔑 **새 가드를 만들면 주입을 한 줄 추가할 것 — 자리는 `scripts/mutations/<도메인>.mjs`** (2026-09-08 ⑤). 인라인 배열(`check-guard-mutations.mjs` 안 950건)은 **그대로 두고 읽는 곳만 늘렸다** — 지금 열려 있는 다른 세션 브랜치들이 그 배열에 덧붙이고 있어 통째로 옮기면 전부 깨진다. 새 주입만 분할 파일로 가고 옛 배열은 자연히 빈다. 파일은 `export default [ … ]` 하나, **이름은 전체에서 유일**(`--only` 가 부분일치라 중복이면 무엇이 돌았는지 모른다 — 러너가 멈춘다). 분할 파일도 융합-키 무결성 검사를 **똑같이** 받는다. ⏱️ **2026-09-08 — PR 은 좁게, 전수는 따로**: 실측으로 이 검사가 Verify 48분 29초 중 **37분 24초(77%)** 였고, 그 길이가 만든 2차 피해가 더 컸다 — CI 가 도는 동안 main 이 움직이고 거의 모든 PR 이 이 매니페스트를 건드리니 **머지마다 충돌**했다(그날 4번 중 3번이 405). ⇒ PR 은 `--changed` 로 자기가 건드린 주입만(소비자 페이지 1개 = 958건 중 **8건**), **전수는 `guard-mutations-full.yml`(main push + 야간)** 이 돈다. 🔴 **둘은 짝이다** — `verify.yml` 이 좁혀 도는데 그 워크플로를 지우면 전수가 **어디서도 안 돌고 에러도 안 난다**(2026-09-08 이전에는 전수가 `verify.yml` 한 곳에서만 돌았다). 판정은 순수 모듈 `scripts/guard-mutations-scope.mjs`(테스트가 문자열이 아니라 **동작을 잰다**), 짝은 `guard-mutations-scope.test.ts` 가 고정. 애매하면 전수로 폴백한다(base 못 구함 · `scripts/**` · `package.json` · `verify.yml` · 테스트 헬퍼 변경 · 바뀐 파일 0개). ⚠️ 그리고 **주입이 실제로 적용됐는지부터 확인하라** — 같은 날 존재하지 않는 경로에 마커를 심어 "검증됐다"고 착각할 뻔했다(추적 안 되는 파일이라 스캔에서 빠져 초록불) |
...
| **다크에서 안 보이는 글자**(밝은 면 위 밝은 글자) | - | `check-dark-contrast.mjs` (`dark-contrast.yml` — 브라우저 워크플로 · **PR 게이트 아님**) + audit-gate | 2026-09-03 대표 신고 *"글자가 또 하얘 · 이런 경우 지금 많은 것 같은데 전수조사 필요해"* — 지도 검색창(`/map?q=부산`)에 친 글자가 **흰 배경 위 흰색**이었다(실측 대비 **1.1:1**, 글자 rgb(243,244,246)/배경 rgb(255,255,255)). 원인은 오타가 아니라 **구조**다: 전역 `.dark input:not(...)`(특이도 **0,5,1**)이 `text-gray-900`(**0,1,0**)을 **언제나** 이긴다 — 코드는 맞게 썼는데 CSS 가 조용히 뒤집는다. 그리고 지도에 달린 `light-fixed` 는 **주석**이라 `check-theme-consistency` 를 면제해 줄 뿐 **런타임엔 아무 일도 안 한다** ⇒ 가드는 초록인데 화면은 안 보이는 "조용한 부재". 🔴 **grep 으로는 구조적으로 못 잡는다** — 같은 className 에 밝은/어두운 분기가 함께 있어(`panel ? A : B`) 정적 검사가 실제로 **0건**을 냈다(다른 분기의 `dark:text-` 때문에 통과). 특이도 싸움의 승자는 브라우저만 아니까 **17개 소비자 경로를 다크로 렌더해 글자색 vs 실제 뒤 배경색의 WCAG 대비를 잰다**(투명하면 조상을 타고 올라가고, 사진/그라디언트 위는 계산 밖이라 건너뛴다). 입력요소는 **값을 채워** 재고(빈 칸이면 placeholder 만 잰다), 시트/모달은 **열어서** 잰다(닫힌 화면만 보면 0건이 거짓 안심이 된다). 양방향을 본다 — 밝은 위 밝음 + 어두운 위 어두움(후자로 `SiteFooter` 저작권 `text-gray-400 dark:text-gray-600` = 2.4:1 을 잡았다). **측정 텍스트가 200개 미만이면 통과가 아니라 실패**(렌더가 깨졌는데 findings 0 으로 초록 뜨는 것 차단 — 이 레포가 반복해 당한 헛도는 가드). 장식(빈 별점·구분자)은 `scripts/dark-contrast-baseline.json` 의 `allow` 에 등록, **읽는 글자는 등록 금지 — 그건 고쳐야 하는 결함이다**. ⚠️ 못 보는 것: 사진 위 글자 · 호버/포커스 상태 · 경로 목록 밖 화면(= 경로 목록이 곧 범위다). ⚠️ **PR 게이트로 승격 금지** — `render-smoke.yml`·`live-contracts.yml` 과 같은 판단이다(브라우저 검사는 느리고 환경에 민감해서 간헐 실패가 머지를 막으면 결국 가드를 꺼 버리게 된다). 브라우저를 못 띄우면 **크게 소리내고 건너뛴다** — 조용한 통과는 "돌고 있다"는 착각을 만든다 |
| 깨진 이미지 아이콘 노출(cfImage `<img>` 무방비) | `check-image-fallback.mjs` (warn) | `verify.yml` (strict) + audit-gate | 2026-08-31 — 대표 *"유어샵 나머지"* 를 파다 실측: `cfImage()` 로 감싼 `<img>` **92개 중 47개에 `onError` 가 없었다.** 리사이저가 원본을 못 가져오거나(핫링크 차단·외부 CDN 5xx) 원본이 404 면 그 자리에 **깨진 이미지 아이콘**이 그대로 뜬다 — 에러 로그도, 배포 실패도, 사용자 신고도 없다(사진 한 장이 안 뜬 걸 누가 신고하겠나). SSOT `cfImageOnError(el, 원본URL)` 는 [원본 1회 재시도 → 그래도 죽으면 숨김 → 부모의 카테고리색/스켈레톤이 보임] 으로 처리한다. 절반이 빠진 건 취향이 아니라 **배선 누락**이고, 배선은 눈에 안 보여 계속 새로 빠진다 → 래칫(`scripts/image-fallback-baseline.json`, 2026-08-31 2차로 **baseline 0**). 일괄 배선: `node scripts/codemods/adopt-image-fallback.mjs`. ⚠️ **못 잡는 것**: CSS `background-image`(onError 개념 자체가 없다) · 빈 핸들러 · cfImage 를 안 거치는 raw `<img>`. 예외 `image-fallback-ok` |

...
| **주입 앵커가 낡음**(가드가 지키는 척) | `check-stale-mutation-anchors.mjs`(**전수, ~1초**) | `verify.yml` (strict) | 2026-09-26 — `check-guard-mutations` 는 두 가지를 잡는다: ⓐ **헛도는 가드**(결함을 심었는데 통과) ⓑ **낡은 지도**(`find` 가 소스에 없다 = 코드가 옮겨졌다). ⓐ는 테스트를 실제로 돌려야 해서 전수 **37분**이고, 그래서 PR 은 `--changed` 로 좁혀 돈다. **그런데 ⓑ는 테스트가 필요 없다** — 문자열이 파일에 있는지 보면 끝이다. 그걸 37분짜리에 묶어 둔 탓에 **`--changed` 범위 밖이면 PR 을 통과하고 main 에서야 빨간불**이 났다. 그날 실제로 두 건: `pinReturn` 도입으로 `{tool === 'pin' && <PinSheet …}` 앵커가 사라진 것(PR CI 한 바퀴를 태우고서야 드러났다)과, 그걸 고치는 사이 등록이 마이로 옮겨가며 `enterSeat('/seller/meal-voucher/new')` 가 사라진 것. ⇒ ⓑ만 떼어 **전수·무테스트**로 검사한다(실측 1,962건 · 636파일 · 1초). 분할 매니페스트(`scripts/mutations/*.mjs`)와 런너 안 **인라인 배열**을 둘 다 읽는다 — 인라인을 빠뜨리면 950건이 통째로 눈먼다. ⚠️ **못 잡는 것**: ⓐ는 여전히 `check-guard-mutations` 의 몫이다(앵커는 멀쩡한데 아무것도 안 지키는 경우는 결함을 심어 봐야 드러난다). 🧭 앵커가 낡으면 **지우지 말고 불변식으로 재조준**할 것 — 지키려던 것은 대개 그대로 살아 있다. 예외 없음(앵커가 없으면 그 주입은 아무것도 검증하지 않는다) |

...
### 변경 audit log
- 2026-08-12 `[UNLOCK]` `PaymentSuccessPage.tsx` **몰 손님용 "가게로 돌아가기" 배너 렌더 1줄 추가** (대표 승인 — *"유어딜이랑 공동구매랑 완전 별개, 분리라니까?"* → 범위 설명 후 *"허가해줄게"*). **배경**: 🏪 공구 서비스(운영자 몰) 분리가 **장바구니 앞까지**만 돼 있었다. 손님이 담는 순간부터 결제·완료·주문내역이 전부 유어딜 화면이라, 운영자가 카톡으로 데려온 손님이 결제 순간 남의 간판 아래로 넘어간다. 게다가 이 화면의 셀러 전환 넛지(*"내 쇼핑몰에서도 팔 수 있어요"*)가 **몰 손님에게도** 떠서 유어딜이 운영자 손님에게 셀러 전환을 권하고 있었다 — 브랜딩이 아니라 **고객 가로채기**. **수정(잠금 파일은 렌더 1줄 + import 1줄만)**: 셀러 전환 넛지 다음에 `<MallOriginBanner className="mt-4 sm:mt-5" />`(신규 `components/mall/MallOriginBanner.tsx`) 추가 — 그 컴포넌트가 **스스로** 세션 흔적(`shared/mall/origin.ts`)을 보고 몰 이름·로고를 조회해 간판과 되돌아갈 문을 그린다. **🔴 잠금 파일에서 몰을 판정하지 않는다**(`readMallOrigin`/`isFromMallSession`/`mall_id` 참조 0 — 테스트가 고정). 판정을 여기 심으면 다음 세션이 잠금 파일에 조건을 계속 얹게 되고 승인 절차가 형해화된다. **흔적 없으면 조회조차 안 하고 `null`** → **본진 손님 화면 byte-불변**. 조회 실패도 `null`(잘못된 가게 이름이 결제 화면에 뜨는 것이 아무것도 없는 것보다 나쁘다). **⚠️ Toss confirm/금액검증(`serverTotal!==parsedAmount`)/`TossPaymentObject` 표시(receipt.url·cashReceipt·easyPay·card·approvedAt)/pendingBookings/autoReturn/셀러 전환 넛지 렌더 조건 전부 byte-불변.** 동반(비잠금): 넛지가 몰 세션이면 스스로 미노출 · `CartPage` "쇼핑 계속하기" → `ContinueShoppingLink`(가게로) · `CheckoutPage` 동일 배너. 검증: tsc 0 · build 0 · `mall-surface-boundary` 49 pass · 되돌려-검증 5건(배너 제거 ×2 · 흔적 무시 · 실패 시 렌더 · **잠금 파일 직접 판정**) 전부 빨강 확인 후 복원 · file-size(두 파일 600 미만 유지) · theme GREEN. ⚠️ **staging 결제 성공 1회 권장**(몰 흔적 없는 본진 결제에서 화면이 이전과 동일한지 — 배너가 안 떠야 정상). 롤백: 렌더 1줄 + import 1줄 제거.
